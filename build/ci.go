...
func doInstall(cmdline []string) {
	...
	packages := flag.Args()
	if len(packages) == 0 {
		packages = build.FindMainPackages("./programs/cmd")
	}
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/upcx/internalversion.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/upcx/internalversion.gitDate="+env.Date)
	}
