...
#include <upcx/producer_schedule.hpp>

namespace upcxbios {

   using upcx::action_wrapper;
   using upcx::check;
   using upcx::checksum256;
   using upcx::ignore;
   using upcx::name;
   using upcx::permission_level;
   ...
   struct block_header {
      ...
      std::optional<upcx::producer_schedule>   new_producers;
   ...
   /**
    * The `upcx.bios` is the first sample of system contract provided by `block.one` through the UPCX platform. It is a minimalist system contract because it only supplies the actions that are absolutely critical to bootstrap a chain and nothing more. This allows for a chain agnostic approach to bootstrapping a chain.
    *
    * Just like in the `upcx.system` sample contract implementation, there are a few actions which are not implemented at the contract level (`newaccount`, `updateauth`, `deleteauth`, `linkauth`, `unlinkauth`, `canceldelay`, `onerror`, `setabi`, `setcode`), they are just declared in the contract so they will show in the contract's ABI and users will be able to push those actions to the chain via the account holding the `upcx.system` contract, but the implementation is at the UPCX core level. They are referred to as UPCX native actions.
    */
   class [[upcx::contract("upcx.bios")]] bios : public upcx::contract {
      public:
          ...
          * This is useful because when doing authorization checks, the UPCX based blockchain starts with the
          * action needed to be authorized (and the contract belonging to), and looks up which permission
          * is needed to pass authorization validation. If a link is set, that permission is used for authoraization
          * validation otherwise then active is the default, with the exception of `upcx.any`.
          * `upcx.any` is an implicit permission which exists on every account; you can link actions to `upcx.any`
          ...
          */
         [[upcx::action]]
         void setprods( const std::vector<upcx::producer_authority>& schedule );
          ...
          */
         [[upcx::action]]
         void setparams( const upcx::blockchain_parameters& params );
          ...
          */
         [[upcx::action]]
         void setkvparams( const upcx::kv_parameters& params );
          ...
          */
         [[upcx::action]]
         void activate( const upcx::checksum256& feature_digest );
          ...
          */
         [[upcx::action]]
         void reqactivated( const upcx::checksum256& feature_digest );

         struct [[upcx::table]] abi_hash {
            name              owner;
            checksum256       hash;
         ...
         typedef upcx::multi_index< "abihash"_n, abi_hash > abi_hash_table;
