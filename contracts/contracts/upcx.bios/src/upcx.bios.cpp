#include <upcx.bios/upcx.bios.hpp>
...
void bios::setabi( name account, const std::vector<char>& abi ) {
   ...
   if( itr == table.end() ) {
      table.emplace( account, [&]( auto& row ) {
         row.owner = account;
         row.hash  = upcx::sha256(const_cast<char*>(abi.data()), abi.size());
      });
   } else {
      table.modify( itr, upcx::same_payer, [&]( auto& row ) {
         row.hash = upcx::sha256(const_cast<char*>(abi.data()), abi.size());
      });
   }
}
...
void bios::setprods( const std::vector<upcx::producer_authority>& schedule ) {
   require_auth( get_self() );
   set_proposed_producers( schedule );
}

void bios::setparams( const upcx::blockchain_parameters& params ) {
   require_auth( get_self() );
...
void bios::setkvparams( const upcx::kv_parameters& params ) {
   require_auth( get_self() );
...
void bios::activate( const upcx::checksum256& feature_digest ) {
   require_auth( get_self() );
   preactivate_feature( feature_digest );
}

void bios::reqactivated( const upcx::checksum256& feature_digest ) {
