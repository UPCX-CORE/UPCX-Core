...
1. export PATH=$UPCX_2_1_0/build/bin:$PATH
1. nodupcx -e -p upcxio --plugin upcxio::producer*plugin --plugin upcxio::producer_api_plugin --plugin upcxio::chain_api_plugin --plugin upcxio::http_plugin --plugin upcxio::history_plugin --plugin upcxio::history_api_plugin --filter-on=* --access-control-allow-origin=\_ --contracts-console --http-validate-host=false --verbose-http-errors --max-transaction-time=1000 --backing-store chainbase --data-dir=datadir
...
1. make
1. clupcx create account upcxio jane UPCX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
1. clupcx set contract jane $UPCXIO_CDT_1_8_0/examples/kv_map/build/kv_map -p jane@active
