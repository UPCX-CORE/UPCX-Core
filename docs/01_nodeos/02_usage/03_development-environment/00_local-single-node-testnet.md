...
```sh
nodeos -e -p upcxio --plugin upcxio::chain_api_plugin --plugin upcxio::history_api_plugin
```

[[info | Nodeos Minimal Options]]
| A minimal `nodeos` instance setup for block production requires both `chain_api_plugin` and `history_api_plugin` with the `-e` option (enable stale production) and `-p upcxio` option (producer name `upcxio`). Alternatively, you can also setup and specify your own account as the producer name.
...
- Mac OS: `~/Library/Application\ Support/upcxio/nodeos/config`
- Linux: `~/.local/share/upcxio/nodeos/config`
...
- Mac OS: `~/Library/Application\ Support/upcxio/nodeos/data`
- Linux: `~/.local/share/upcxio/nodeos/data`
