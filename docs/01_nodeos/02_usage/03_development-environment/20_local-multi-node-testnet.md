...
- It is assumed that `nodeos`, `cleos`, and `keosd` are accessible through the path. If you built UPCXIO using shell scripts, make sure to run the [Install Script](../../../00_install/01_build-from-source/01_shell-scripts/03_install-upcxio-binaries.md).
...
```sh
nodeos --enable-stale-production --producer-name upcxio --plugin upcxio::chain_api_plugin --plugin upcxio::net_api_plugin
```
...
```sh
cleos --wallet-url http://127.0.0.1:8899 set contract upcxio build/contracts/upcxio.bios
```
...
```sh
cleos --wallet-url http://127.0.0.1:8899 create account upcxio inita UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```
...
```sh
nodeos --producer-name inita --plugin upcxio::chain_api_plugin --plugin upcxio::net_api_plugin --http-server-address 127.0.0.1:8889 --p2p-listen-endpoint 127.0.0.1:9877 --p2p-peer-address 127.0.0.1:9876 --config-dir node2 --data-dir node2 --private-key [\"UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\",\"5JgbL2ZnoEAhTudReWH1RnMuQS6DBeLZt4ucV6t8aymVEuYg7sr\"]
```
...
```sh
cleos --wallet-url http://127.0.0.1:8899 push action upcxio setprods "{ \"schedule\": [{\"producer_name\": \"inita\",\"block_signing_key\": \"UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\"}]}" -p upcxio@active
```
