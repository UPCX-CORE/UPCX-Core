...
```shell
nodeos -e -p upcxio --plugin upcxio::producer_plugin --plugin upcxio::producer_api_plugin --plugin upcxio::chain_api_plugin --backing-store=’rocksdb’ --persistent-storage-num-threads=’2’ --persistent-storage-max-num-files=’2’ --persistent-storage-write-buffer-size-mb=’128’  --plugin upcxio::http_plugin
```

To use `chainbase` for state storage:

```shell
nodeos -e -p upcxio --plugin upcxio::producer_plugin --plugin upcxio::producer_api_plugin --plugin upcxio::chain_api_plugin --backing-store=’chainbase’ --plugin upcxio::http_plugin
```

or

```shell
nodeos -e -p upcxio --plugin upcxio::producer_plugin --plugin upcxio::producer_api_plugin --plugin upcxio::chain_api_plugin --plugin upcxio::http_plugin
```
