...
Version 2.1 or above of the UPCX development environment.
...
```console
Config Options for upcx::chain_plugin:
...
```shell
nodupcx -e -p upcx --plugin upcx::producer_plugin --plugin upcx::producer_api_plugin --plugin upcx::chain_api_plugin --backing-store=’rocksdb’ --persistent-storage-num-threads=’2’ --persistent-storage-max-num-files=’2’ --persistent-storage-write-buffer-size-mb=’128’  --plugin upcx::http_plugin
```

To use `chainbase` for state storage:

```shell
nodupcx -e -p upcx --plugin upcx::producer_plugin --plugin upcx::producer_api_plugin --plugin upcx::chain_api_plugin --backing-store=’chainbase’ --plugin upcx::http_plugin
```

or

```shell
nodupcx -e -p upcx --plugin upcx::producer_plugin --plugin upcx::producer_api_plugin --plugin upcx::chain_api_plugin --plugin upcx::http_plugin
```
