...
```sh
# command-line
nodeos ... --plugin upcxio::trace_api_plugin [options]
```
...
```sh
# command-line
nodeos ... --plugin upcxio::chain_plugin [options]  \
           --plugin upcxio::http_plugin [options]
```

## Configuration Example

Here is a `nodeos` configuration example for the `trace_api_plugin` when tracing some UPCXIO reference contracts:
