...
For example, the CLI option `--plugin upcx::chain_api_plugin` can also be set by adding `plugin = upcx::chain_api_plugin` in `config.ini`.
...
nodupcx \
  -e -p upcx \
  --data-dir /users/mydir/upcx/data     \
  --config-dir /users/mydir/upcx/config \
...
- identifying itself as block producer "upcx" (`-p`)
