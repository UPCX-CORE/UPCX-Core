...
```sh
nodupcx -e -p upcxio --plugin upcxio::chain_api_plugin --plugin upcxio::history_api_plugin
```

[[info | Nodupcx Minimal Options]]
| A minimal `nodupcx` instance setup for block production requires both `chain_api_plugin` and `history_api_plugin` with the `-e` option (enable stale production) and `-p upcxio` option (producer name `upcxio`). Alternatively, you can also setup and specify your own account as the producer name.
...
...
upcxio generated block 046b9984... #101527 @ 2018-04-01T14:24:58.000 with 0 trxs
upcxio generated block 5e527ee2... #101528 @ 2018-04-01T14:24:58.500 with 0 trxs
...
```
...
# config.ini:
    ...
    producer-name = upcxio
    # Load the block producer plugin, so you can produce blocks
    plugin = upcxio::producer_plugin
    # As well as API and HTTP plugins
    plugin = upcxio::chain_api_plugin
...
- Mac OS: `~/Library/Application\ Support/upcxio/nodupcx/data`
- Linux: `~/.local/share/upcxio/nodupcx/data`
