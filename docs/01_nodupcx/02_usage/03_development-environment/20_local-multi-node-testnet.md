...
3. [Loading the UPCXIO Key](#3-loading-the-upcxio-key)
...
```sh
nodupcx --enable-stale-production --producer-name upcxio --plugin upcxio::chain_api_plugin --plugin upcxio::net_api_plugin
```
...
The following commands assume that you are running this tutorial from the `upcx\build` directory, from which you ran `./upcxio_build.sh` to build the UPCXIO binaries.

To start additional nodes, you must first load the `upcxio.bios` contract. This contract enables you to have direct control over the resource allocation of other accounts and to access other privileged API calls. Return to the second terminal window and run the following command to load the contract:

```sh
clupcx --wallet-url http://127.0.0.1:8899 set contract upcxio build/contracts/upcxio.bios
```
...
```sh
clupcx --wallet-url http://127.0.0.1:8899 create account upcxio inita UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```

```console
executed transaction: d1ea511977803d2d88f46deb554f5b6cce355b9cc3174bec0da45fc16fe9d5f3  352 bytes  102400 cycles
#         upcxio <= upcxio::newaccount            {"creator":"upcxio","name":"inita","owner":{"threshold":1,"keys":[{"key":"UPCX6hMjoWRF2L8x9YpeqtUEcsDK...
```
...
```sh
nodupcx --producer-name inita --plugin upcxio::chain_api_plugin --plugin upcxio::net_api_plugin --http-server-address 127.0.0.1:8889 --p2p-listen-endpoint 127.0.0.1:9877 --p2p-peer-address 127.0.0.1:9876 --config-dir node2 --data-dir node2 --private-key [\"UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\",\"5JgbL2ZnoEAhTudReWH1RnMuQS6DBeLZt4ucV6t8aymVEuYg7sr\"]
```
...
2396002ms thread-0   producer_plugin.cpp:244       block_production_loo ] Not producing block because it isn't my turn, its upcxio
```
...
```sh
clupcx --wallet-url http://127.0.0.1:8899 push action upcxio setprods "{ \"schedule\": [{\"producer_name\": \"inita\",\"block_signing_key\": \"UPCX6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\"}]}" -p upcxio@active
```

```console
executed transaction: 2cff4d96814752aefaf9908a7650e867dab74af02253ae7d34672abb9c58235a  272 bytes  105472 cycles
#         upcxio <= upcxio::setprods              {"version":1,"producers":[{"producer_name":"inita","block_signing_key":"UPCX6hMjoWRF2L8x9YpeqtUEcsDKA...
```
