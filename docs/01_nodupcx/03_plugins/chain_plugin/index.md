...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an UPCXIO node.
...
Config Options for upcxio::chain_plugin:
  ...
  --wasm-runtime runtime (=upcx-vm-jit)  Override default WASM runtime (
                                        "upcx-vm-jit", "upcx-vm")
                                        "upcx-vm-jit" : A WebAssembly runtime
                                        that compiles WebAssembly code to
                                        native x86 code prior to execution.
                                        "upcx-vm" : A WebAssembly interpreter.
  ...
  --telemetry-service-name arg (=nodupcx)
