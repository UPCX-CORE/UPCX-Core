...
The `http_plugin` is a core plugin supported by both `nodupcx` and `kupcxd`. The plugin is required to enable any RPC API functionality provided by a `nodupcx` or `kupcxd` instance.
...
```sh
# command-line
nodupcx ... --plugin upcxio::http_plugin [options]
 (or)
kupcxd ... --plugin upcxio::http_plugin [options]
```
