---
content_title: Nodupcx Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodupcx`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodupcx`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | Nodupcx is modular]]
| Plugins add incremental functionality to `nodupcx`. Unlike runtime plugins, `nodupcx` plugins are built at compile-time.
