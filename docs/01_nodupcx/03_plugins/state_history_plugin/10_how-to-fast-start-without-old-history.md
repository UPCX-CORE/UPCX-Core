...
- Make sure [UPCXIO is installed](../../../00_install/index.md).
...
3. Start `nodupcx` with the `--snapshot` option, and the options listed in the [`state_history_plugin`](#index.md).
...
6. Do not stop `nodupcx` until it has received at least 1 block from the network, or it won't be able to restart.

## Remarks

If `nodupcx` fails to receive blocks from the network, then try the above using `net_api_plugin`. Use `clupcx net disconnect` and `clupcx net connect` to reconnect nodes which timed out.
