...
The `state_history_plugin` is useful for capturing historical data about the blockchain state. The plugin receives blockchain data from other connected nodes and caches the data into files. The plugin listens on a socket for applications to connect and sends blockchain data back based on the plugin options specified when starting `nodupcx`.

## Usage

```console
# config.ini
plugin = upcx::state_history_plugin
[options]
```

```sh
# command-line
nodupcx ... --plugin upcx::state_history_plugin [operations] [options]
```

## Operations

These can only be specified from the `nodupcx` command-line:

```console
Command Line Options for upcx::state_history_plugin:
...
```console
Config Options for upcx::state_history_plugin:
  ...
  --state-history-archive-dir arg (="archive")
                                        ...
                                        they won't be accessed by nodupcx
                                        anymore.
...
- [Source code](https://github.com/UPCX/history-tools/)
- [Documentation](https://upcx.github.io/history-tools/)
...
```console
# config.ini
plugin = upcx::chain_plugin --disable-replay-opts
```

```sh
# command-line
nodupcx ... --plugin upcx::chain_plugin --disable-replay-opts
```
