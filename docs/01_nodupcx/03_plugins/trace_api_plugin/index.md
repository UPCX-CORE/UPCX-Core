...
```console
# config.ini
plugin = upcxio::trace_api_plugin
[options]
```

```sh
# command-line
nodupcx ... --plugin upcxio::trace_api_plugin [options]
```
...
```console
Config Options for upcxio::trace_api_plugin:
...
```sh
# command-line
nodupcx ... --plugin upcxio::chain_plugin [options]  \
           --plugin upcxio::http_plugin [options]
```

## Configuration Example

Here is a `nodupcx` configuration example for the `trace_api_plugin` when tracing some UPCXIO reference contracts:

```sh
nodupcx --data-dir data_dir --config-dir config_dir --trace-dir traces_dir
--plugin upcxio::trace_api_plugin
--trace-rpc-abi=upcxio=abis/upcxio.abi
--trace-rpc-abi=upcxio.token=abis/upcxio.token.abi
--trace-rpc-abi=upcxio.msig=abis/upcxio.msig.abi
--trace-rpc-abi=upcxio.wrap=abis/upcxio.wrap.abi
```
...
The `trace-dir` option defines the directory on the filesystem where the trace log files are stored by the `trace_api_plugin`. These files are stable once the LIB block has progressed past a given slice and then can be deleted at any time to reclaim filesystem space. The deployed UPCXIO system will tolerate any out-of-process management system that removes some or all of these files in this directory regardless of what data they represent, or whether there is a running `nodupcx` instance accessing them or not. Data which would nominally be available, but is no longer so due to manual maintenance, will result in a HTTP 404 response from the appropriate API endpoint(s).
