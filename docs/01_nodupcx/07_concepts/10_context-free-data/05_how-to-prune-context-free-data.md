...
This how-to procedure showcases the steps to prune context-free data (CFD) from a transaction. The process involves launching the [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) utility with the `--prune-transactions` option, the transaction ID(s) that contain(s) the context-free data, and additional options as specified below.

[[caution | Data Pruning on Public Chains]]
| Pruning transaction data is not suitable for public UPCXIO blockchains, unless previously agreed upon through UPCXIO consensus by a supermajority of producers. Even if a producing node on a public UPCXIO network prunes context-free data from a transaction, only their node would be affected. The integrity of the blockchain would not be compromised.
...
- Review the [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) command-line utility reference.
...
4. Launch the [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) utility as follows:

   `upcxio-blocklog [--blocks-dir <blocks_dir>] [--state-history-dir <state_hist_dir>] --prune-transactions --block-num <block_num> --transaction <trx_id> [--transaction <trx_id2> ...]`

   If the operation is _successful_:

   - The [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) utility terminates silently with a zero error code (no error).
   ...
   - The [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) utility outputs an error to `stderr` and terminates with a non-zero error code (indicating an error).

## Notes

Some additional considerations are in order:

- You can pass multiple transactions to [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) if they are within the same block.
- You can use [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) to display the block that contains the pruned transactions.
...
4. Launch the [`upcxio-blocklog`](../../../10_utilities/upcxio-blocklog.md) utility as follows:
   ...
   `upcxio-blocklog --blocks-dir <blocks_dir> --state-history-dir <state_hist_dir> --prune-transactions --block-num 119 --transaction 1b9a9c53f9b692d3382bcc19c0c21eb22207e2f51a30fe88dabbb45376b6ff23`
