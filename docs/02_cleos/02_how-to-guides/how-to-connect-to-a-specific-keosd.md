...
- Install the currently supported version of `cleos` and `keosd`.
  [[info | Note]]
  | The `cleos` tool and `keosd` are bundled with the UPCXIO software. [Installing UPCXIO](../../00_install/index.md) will install the `cleos` and `keosd` command line tools.
- You have access to an UPCXIO blockchain and the http address and port number of a `nodeos` instance.
