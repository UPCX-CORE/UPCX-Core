...
This guide provides instructions on how to connect to specifc UPCXIO blockchain when using `cleos`. `Cleos` can connect to a specific node by using the `--url` optional argument, followed by the http address and port number.
...
- You have access to an UPCXIO blockchain and the http afddress and port number of a `nodeos` instance.
