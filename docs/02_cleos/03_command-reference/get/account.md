...
- Install the currently supported version of `cleos.`
  [[info | Note]]
  | The `cleos` tool is bundled with the UPCXIO software. [Installing UPCXIO](../../00_install/index.md) will install the `cleos` and `keosd` command line tools.
- You have access to an UPCXIO blockchain.

## Examples

1. Get formatted data for user **upcxio**

```shell
cleos get account upcxio
```

**Where**
`upcxio` = The name of the account.
...
2. Get JSON data for user **upcxio**

```sh
cleos get account upcxio --json
```
