...
Simply output the hash of upcx.token contract

```sh
clupcx get code upcx.token
```
...
```sh
clupcx get code upcx.token -a upcx.token.abi
```
...
Retrieve and save wast code for upcx.token contract

```sh
clupcx get code upcx.token -c upcx.token.wast
```

```console
code hash: f675e7aeffbf562c033acfaf33eadff255dacb90d002db51c7ad7cbf057eb791
saving wast to upcx.token.wast
```
