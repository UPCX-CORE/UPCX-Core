...
- Install the currently supported version of `clupcx` and `kupcxd`.
  [[info | Note]]
  | The `clupcx` tool and `kupcxd` are bundled with the UPCX software. [Installing UPCX](../../00_install/index.md) will install the `clupcx` and `kupcxd` command line tools.
- You have access to an UPCX blockchain and the http address and port number of a `nodupcx` instance.
