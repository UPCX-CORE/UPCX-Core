...
The example uses `clupcx` to link a custom permission _customp_ in the account _alice_ to a _hi_ action deployed to the _scontract_ account so that the _alice_ account's `active` permission and _customp_ permission are authorized to call the _hi_ \_action.
...
- You have an UPCXIO account and access to the account's `active` private key.
- You have created a custom permission. See [clupcx set account permission](../03_command-reference/set/set-account-permission.md).

## Command Reference

See the following reference guides for command line usage and related options:

- [clupcx set action permission](../03_command-reference/set/set-action-permission.md) command
...
```shell
clupcx set action permission alice scontract hi customp -p alice@active
```
...
#         upcxio <= upcxio::linkauth              {"account":"alice","code":"scontract","type":"hi","requirement":"customp"}
```
