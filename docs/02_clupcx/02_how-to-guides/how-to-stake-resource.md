...
This how-to guide provides instructions on how to stake resources, NET and/or CPU, for your account using the `clupcx` CLI tool.
...
- Understand the following:
  ...
  - What [NET bandwidth](https://developers.upcx.io/manuals/upcxio.contracts/v1.9/key-concepts/net) is.
  - What [CPU bandwidth](https://developers.upcx.io/manuals/upcxio.contracts/v1.9/key-concepts/cpu) is.
...
```sh
clupcx system delegatebw alice bob "0.01 SYS" "0 SYS"
```
...
#         upcxio <= upcxio::delegatebw            {"from":"bob","receiver":"alice","stake_net_quantity":"0.01 SYS","stake_cpu_quanti...
#   upcxio.token <= upcxio.token::transfer        {"from":"bob","to":"upcxio.stake","quantity":"0.01 UPCX","memo":"stake bandwidth"}
#  alice <= upcxio.token::transfer        {"from":"bob","to":"upcxio.stake","quantity":"0.01 SYS","memo":"stake bandwidth"}
#   upcxio.stake <= upcxio.token::transfer        {"from":"bob","to":"upcxio.stake","quantity":"0.01 SYS","memo":"stake bandwidth"}
```
...
```sh
clupcx system delegatebw alice bob "0 SYS" "0.01 SYS"
```
...
```console
executed transaction: 5487afafd67bf459a20fcc2dbc5d0c2f0d1f10e33123eaaa07088046fd18e3ae  192 bytes  503 us
#         upcxio <= upcxio::delegatebw            {"from":"bob","receiver":"alice","stake_net_quantity":"0.0000 SYS","stake_cpu_quanti...
#   upcxio.token <= upcxio.token::transfer        {"from":"bob","to":"upcxio.stake","quantity":"0.01 UPCX","memo":"stake bandwidth"}
#  alice <= upcxio.token::transfer        {"from":"bob","to":"upcxio.stake","quantity":"0.01 SYS","memo":"stake bandwidth"}
#   upcxio.stake <= upcxio.token::transfer        {"from":"bob","to":"upcxio.stake","quantity":"0.01 SYS","memo":"stake bandwidth"}
```
...
```sh
clupcx system delegatebw alice bob "0.01 SYS" "0.02 SYS"
```
