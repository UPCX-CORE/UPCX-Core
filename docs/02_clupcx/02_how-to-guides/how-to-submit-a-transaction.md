...
This how-to guide provides instructions on how to submit, or push, a transaction using the `clupcx` CLI tool.
...
[[info]]
| Be aware that if a clear text `data` field is used, `clupcx` needs to fetch the smart contract's ABI using `nodupcx` API. This operation has an overall performance overhead for both `clupcx` and `nodupcx`. On the other hand if hex data is used in the `data` field then the ABI fetching is not executed and thus the total time to send and execute the transaction is faster.
