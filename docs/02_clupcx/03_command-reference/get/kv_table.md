...
Point query to return the row that matches upcxio name key `boba` from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -i boba contr_acct kvtable primarykey -b
```

```json
{
  "rows": ["000000000000600e3d010000000000000004626f6261000000"],
  "more": false,
  "next_key": ""
}
```
...
```sh
clupcx get kv_table --encode-type dec -i 1 contr_acct kvtable foo
```

```json
{
  "rows": ["000000000000600e3d010000000000000004626f6261"],
  "more": false,
  "next_key": ""
}
```

Range query to return all rows starting from upcxio name key `bobd` up to `bobh` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -L bobd -U bobh contr_acct kvtable primarykey -b
```
...
Range query to return all rows (in reverse order) starting from upcxio name key `bobh` down to `bobd` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -L bobd -U bobh contr_acct kvtable primarykey -b -r
```
...
Range query to return all rows starting from upcxio name key `bobg` up to the last row key from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -L bobg contr_acct kvtable primarykey -b
```
...
Range query to return all rows (in reverse order) starting from the last row key down to upcxio name key `bobg` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -L bobg contr_acct kvtable primarykey -b -r
```
...
Range query to return all rows starting from the first row key up to upcxio name key `bobe` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -U bobe contr_acct kvtable primarykey -b
```
...
Range query to return all rows (in reverse order) starting from upcxio name key `bobe` down to the first row key from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -U bobe contr_acct kvtable primarykey -b -r
```
...
Range query to return all rows (in reverse order, limit results to 2 rows) starting from upcxio name key `bobe` down to the first row key from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:

```sh
clupcx get kv_table --encode-type name -U bobe contr_acct kvtable primarykey -b -r -l 2
```
...
```sh
clupcx get kv_table --encode-type bytes -U 3D0E800000000000 contr_acct kvtable primarykey -b -r -l 2
```
...
```sh
clupcx get kv_table --encode-type bytes -U 3D0E600000000000 contr_acct kvtable primarykey -b -r -l 2
```
...
```sh
clupcx get kv_table --encode-type dec -L 0 -U 3 contr_acct kvtable foo -b
```
...
```sh
clupcx get kv_table --encode-type dec -L 6 contr_acct kvtable foo -b
```
...
```sh
clupcx get kv_table --encode-type dec -L 6 contr_acct kvtable foo -b -l 2
```
...
```sh
clupcx get kv_table --encode-type bytes -L 0000000000000008 contr_acct kvtable foo -b -l 2
```
...
```sh
clupcx get kv_table --encode-type bytes -L 000000000000000A contr_acct kvtable foo -b -l 2
```

```json
{
  "rows": ["000000000000f00e3d0a0000000000000004626f626a"],
  "more": false,
  "next_key": ""
}
```
...
```sh
clupcx get kv_table --encode-type bytes -L 2 -U 4 contr_acct kvtable foo -b -r
```
...
```sh
clupcx get kv_table --encode-type dec -L 2 -U 4 contr_acct kvtable foo -b -r
```
...
```sh
clupcx get kv_table --encode-type bytes -L 0000000000000002 -U 0000000000000004 contr_acct kvtable foo -b -r
```
...
```sh
clupcx get kv_table --encode-type string -L boba -U bobe contr_acct kvtable foo -b
```
...
```sh
clupcx get kv_table --encode-type string -L boba -U bobe contr_acct kvtable bar -b
```
...
```sh
clupcx get kv_table --encode-type string -L boba -U bobe contr_acct kvtable bar -b -r
```
...
```sh
clupcx get kv_table --encode-type string -L boba -U bobe contr_acct kvtable bar -b -r -l 2
```
...
```sh
clupcx get kv_table --encode-type bytes -L 626F62610000 -U 626F62630000 contr_acct kvtable bar -b -r -l 2
```
