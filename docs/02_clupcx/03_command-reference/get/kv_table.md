...
Point query to return the row that matches upcx name key `boba` from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows starting from upcx name key `bobd` up to `bobh` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows (in reverse order) starting from upcx name key `bobh` down to `bobd` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows starting from upcx name key `bobg` up to the last row key from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows (in reverse order) starting from the last row key down to upcx name key `bobg` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows starting from the first row key up to upcx name key `bobe` (exclusive) from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows (in reverse order) starting from upcx name key `bobe` down to the first row key from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
...
Range query to return all rows (in reverse order, limit results to 2 rows) starting from upcx name key `bobe` down to the first row key from kv_table named `kvtable` owned by `contr_acct` account using kv_table index `primarykey`:
