...
The following information shows the different positionals and options you can use with the `clupcx net peers` command:
...
- Install the currently supported version of `clupcx`.
  [[info | Note]]
  | `clupcx` is bundled with the UPCXIO software. [Installing UPCXIO](../../../00_install/index.md) will also install the `clupcx` and `kupcxd` command line tools.
...
The following examples demonstrate how to use the `clupcx net peers` command:

- List the status of all peer connections for a local node listening at http address `http://127.0.0.1:8001`:

```sh
clupcx -u http://127.0.0.1:8001 net peers
```
...
[{
    ...
    "last_handshake": {
      ...
      "key": "UPCX1111111111111111111111111111111114T1Anm",
...
**Note:** The `last_handshake` field contains the chain state of each connected peer as of the last handshake message with the node. For more information read the [Handshake Message](https://developers.upcx.io/welcome/latest/protocol/network_peer_protocol#421-handshake-message) in the _Network Peer Protocol_ document.
