...
```sh
clupcx net status [OPTIONS] host
```
...
The following information shows the different positionals and options you can use with the `clupcx net status` command:
...
The following examples demonstrate how to use the `clupcx net status` command:
...
```sh
clupcx -u http://127.0.0.1:8002 net status localhost:9001
```
