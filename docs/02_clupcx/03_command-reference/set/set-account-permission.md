...
clupcx set account permission [OPTIONS] account permission [authority] [parent]
...
The following information shows the different positionals and options you can use with the `clupcx set account` command:
...
- `--add-code` [code] Add 'upcxio.code' permission to specified permission authority
- `--remove-code` [code] Remove 'upcxio.code' permission from specified permission authority
...
- Install the currently supported version of `clupcx`.
  [[info | Note]]
  | `Clupcx` is bundled with the UPCXIO software. [Installing UPCXIO](../../../00_install/index.md) will also install the `clupcx` and `kupcxd` comand line tools.
- You have access to an UPCXIO blockchain.
...
2. Add the `upcxio.code` permission to the contract account `active` permission to enable calling inline actions by the contract account's `active` permission:

```shell
clupcx set account permission alice active --add-code -p alice@active
```

**Where**

- `alice` = The name of the account to add `upcxio.code`.
- `active`= The name of the permission to add `upcxio.code`.
- `--add-code` = Tells the command to add `upcxio.code`.
...
```shell
clupcx set account permission alice customp UPCX58wmANoBtT7RdPgMRCGDb37tcCQswfwVpj6NzC55D247tTMU9D active -p alice@active
```
...
#         upcxio <= upcxio::updateauth            {"account":"alice","permission":"customp","parent":"active","auth":{"threshold":1,"keys":[{"key":"UPCX...```
