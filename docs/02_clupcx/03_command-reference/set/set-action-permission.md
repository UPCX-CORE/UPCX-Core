...
clupcx set action permission [OPTIONS] account code type requirement
...
The following information shows the different positionals and options you can use with the `clupcx set action permission` command:
...
```shell
clupcx set action permission bob scontract1 hi customp1 -p bob@active
```
...
#         upcxio <= upcxio::linkauth              {"account":"bob","code":"scontract1","type":"hi","requirement":"customp1"}
```
...
```shell
clupcx set action permission bob scontract1 bye customp2 -p bob@active
```
...
```shell
executed transaction: 4eb4cf3aea232d46e0e949bc273c3f0575be5bdba7b61851ab51d927cf74a838  128 bytes  141 us
#         upcxio <= upcxio::linkauth              {"account":"bob","code":"scontract1","type":"bye","requirement":"customp2"}
```

3. To remove the customp1 permission from the _hi_ action:

```shell
clupcx set action permission bob scontract1 hi NULL -p bob@active
```
...
#         upcxio <= upcxio::unlinkauth            {"account":"bob","code":"scontract1","type":"hi"}
```
