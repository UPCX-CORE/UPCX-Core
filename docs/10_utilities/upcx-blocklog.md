---
content_title: upcx-blocklog
link_text: upcx-blocklog
---

`upcx-blocklog` is a command-line interface (CLI) utility that allows node operators to perform low-level tasks on the block logs created by a `nodupcx` instance. `upcx-blocklog` can perform one of the following operations:
...
`upcx-blocklog` supports the following options:
...
When `upcx-blocklog` is launched, the utility attempts to perform the specified operation, then yields the following possible outcomes:
