...
const { JsSignatureProvider } = require("../../dist/upcxjs-jssig");
const fetch = require("node-fetch");
const { TextEncoder, TextDecoder } = require("util");

const privateKey = "5JuH9fCXmU3xbj8nRmhPZaVrxxXrdPaRmZLW1cznNTmTQR2Kg5Z"; // replace with "bob" account private key
/* new accounts for testing can be created by unlocking a clupcx wallet then calling:
 * 1) clupcx create key --to-console (copy this privateKey & publicKey)
 * 2) clupcx wallet import
 * 3) clupcx create account bob publicKey
 * 4) clupcx create account alice publicKey
 */
...
const transactWithConfig = async () =>
  await api.transact(
    {
      actions: [
        {
...
const transactWithoutConfig = async () => {
  ...
  const blockInfo = await rpc.get_block(
    transactionResponse.processed.block_num - 3
  );
...
const transactWithoutBroadcast = async () =>
  await api.transact(
    {
      actions: [
        {
