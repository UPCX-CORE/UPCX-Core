...
#include <upcxio/chain_conversions.hpp>
#include <upcxio/from_bin.hpp>
#include <upcxio/from_json.hpp>
#include <upcxio/reflection.hpp>
#include <upcxio/to_bin.hpp>
#include <upcxio/to_json.hpp>
#include <upcxio/abi.hpp>
#include <upcxio/operators.hpp>
#include <upcxio/bytes.hpp>
#include <upcxio/crypto.hpp>
#include <upcxio/symbol.hpp>
#include <upcxio/asset.hpp>
#include <upcxio/time.hpp>
#include <upcxio/fixed_bytes.hpp>
#include <upcxio/float.hpp>
#include <upcxio/varint.hpp>
...
struct jvalue_to_bin_state {
    ...
    bool get_bool() const {
      ...
      upcxio::check(b, upcxio::convert_json_error(upcxio::from_json_error::expected_bool));
      return *b;
    }

    std::string_view get_string() const {
        auto* s = std::get_if<std::string>(&received_value->value);
        upcxio::check(s, upcxio::convert_json_error(upcxio::from_json_error::expected_string));
        return *s;
    }
    void get_null() {
       upcxio::check(std::holds_alternative<std::nullptr_t>(received_value->value),
              upcxio::convert_json_error(upcxio::from_json_error::expected_null));
    }
    bool get_null_pred() {
...
struct json_to_bin_state : upcxio::json_token_stream {
    ...
    explicit json_to_bin_state(char* in, upcxio::vector_stream& out)
      : upcxio::json_token_stream(in), writer(out) {}
};

struct bin_to_json_state {
    upcxio::input_stream& bin;
    upcxio::vector_stream& writer;
    std::vector<bin_to_json_stack_entry> stack{};
    bool skipped_extension = false;

    bin_to_json_state(upcxio::input_stream& bin, upcxio::vector_stream& writer)
...
void json_to_bin(bytes*, State& state, bool, const abi_type*, bool start) {
    ...
    upcxio::check( !(s.size() & 1), upcxio::convert_json_error(upcxio::from_json_error::expected_hex_string) );
    upcxio::varuint32_to_bin(s.size() / 2, state.writer);
    // FIXME: Add a function to encode a hex string to a stream
    upcxio::check(upcxio::unhex(std::back_inserter(state.writer.data), s.begin(), s.end()),
        upcxio::convert_json_error(upcxio::from_json_error::expected_hex_string));
}
...
using upcxio::float128;
using upcxio::checksum160;
...
using upcxio::public_key;
using upcxio::private_key;
using upcxio::signature;
using upcxio::name;

using upcxio::varuint32;
using upcxio::varint32;

using upcxio::time_point;
using upcxio::time_point_sec;
using upcxio::block_timestamp;
using upcxio::symbol_code;
using upcxio::symbol;
using upcxio::asset;
using upcxio::extended_asset;
...
void from_json(int128& obj, S& stream) {
    ...
    if (s.size() && s[0] == '-') {
        ...
        negate(obj.data);
        upcxio::check(is_negative(obj.data), upcxio::convert_json_error(upcxio::from_json_error::number_out_of_range));
    } else {
...
inline void json_to_jvalue(jvalue& value, std::string_view json, F&& f) {
    ...
    upcxio::check(reader.Parse<rapidjson::kParseValidateEncodingFlag | rapidjson::kParseIterativeFlag |
...
inline void json_to_bin(pseudo_object*, jvalue_to_bin_state& state, bool allow_extensions,
                                       const abi_type* type, bool start) {
    if (start) {
       upcxio::check(!(!state.received_value || !std::holds_alternative<jobject>(state.received_value->value)),
            upcxio::convert_json_error(upcxio::from_json_error::expected_start_object));
    ...
    const std::vector<upcxio::abi_field>& fields = stack_entry.type->as_struct()->fields;
    ...
    if (it == obj.end()) {
        ...
        upcxio::check(false, upcxio::convert_json_error(upcxio::from_json_error::expected_field));
    }
    upcxio::check(!state.skipped_extension,
        upcxio::convert_json_error(upcxio::from_json_error::unexpected_field));
...
inline void json_to_bin(pseudo_array*, jvalue_to_bin_state& state, bool, const abi_type* type,
                                       bool start) {
    if (start) {
       upcxio::check(!(!state.received_value || !std::holds_alternative<jarray>(state.received_value->value)),
            upcxio::convert_json_error(upcxio::from_json_error::expected_start_array));
        if (trace_jvalue_to_bin)
            printf("%*s[ %d elements\n", int(state.stack.size() * 4), "",
                   int(std::get<jarray>(state.received_value->value).size()));
        upcxio::varuint32_to_bin(std::get<jarray>(state.received_value->value).size(), state.writer);
...
inline void json_to_bin(pseudo_variant*, jvalue_to_bin_state& state, bool allow_extensions,
    ...
    if (start) {
       upcxio::check(!(!state.received_value || !std::holds_alternative<jarray>(state.received_value->value)),
            upcxio::convert_json_error(upcxio::from_json_error::expected_variant));
        auto& arr = std::get<jarray>(state.received_value->value);
        upcxio::check(arr.size() == 2,
            upcxio::convert_json_error(upcxio::from_json_error::expected_variant));
        upcxio::check(std::holds_alternative<std::string>(arr[0].value),
            upcxio::convert_json_error(upcxio::from_json_error::expected_variant));
    ...
    if (stack_entry.position == 0) {
        auto& typeName = std::get<std::string>(arr[0].value);
        const std::vector<upcxio::abi_field>& fields = *stack_entry.type->as_variant();
        auto it = std::find_if(fields.begin(), fields.end(),
                               [&](auto& field) { return field.name == typeName; });
        upcxio::check(it != fields.end(),
            upcxio::convert_json_error(upcxio::from_json_error::invalid_type_for_variant));
        upcxio::varuint32_to_bin(it - fields.begin(), state.writer);
        state.received_value = &arr[++stack_entry.position];
...
inline void json_to_bin(std::vector<char>& bin, const abi_type* type, std::string_view json, F&& f) {
    ...
    upcxio::vector_stream out(out_buf);
    ...
    while(!state.stack.empty()) {
        ...
        upcxio::check(state.stack.size() <= max_stack_size,
            upcxio::convert_abi_error(upcxio::abi_error::recursion_limit_reached));
        type->ser->json_to_bin(state, entry.allow_extensions, type, false);
    }
    upcxio::check(state.complete(),
        upcxio::convert_json_error(upcxio::from_json_error::expected_end));

    size_t pos = 0;
    for (auto& insertion : state.size_insertions) {
        bin.insert(bin.end(), out_buf.begin() + pos, out_buf.begin() + insertion.position);
        upcxio::push_varuint32(bin, insertion.size);
...
inline void json_to_bin(pseudo_object*, json_to_bin_state& state, bool allow_extensions,
    ...
    auto& stack_entry = state.stack.back();
    const std::vector<upcxio::abi_field>& fields = type->as_struct()->fields;
    ...
    if (key) {
       upcxio::check(!(++stack_entry.position >= (ptrdiff_t)fields.size() || state.skipped_extension),
             upcxio::convert_json_error(upcxio::from_json_error::unexpected_field));
        auto& field = fields[stack_entry.position];
        if (*key != field.name) {
            stack_entry.position = -1;
...
inline void json_to_bin(pseudo_variant*, json_to_bin_state& state, bool allow_extensions,
    ...
    ++stack_entry.position;
    if (state.get_end_array_pred()) {
       upcxio::check(stack_entry.position == 2,
            upcxio::convert_json_error(upcxio::from_json_error::expected_variant));
        if (trace_json_to_bin)
            printf("%*s]\n", int((state.stack.size() - 1) * 4), "");
        state.stack.pop_back();
    ...
    if (stack_entry.position == 0) {
        ...
        upcxio::varuint32_to_bin(stack_entry.variant_type_index, state.writer);
       ...
       upcxio::check(false, upcxio::convert_json_error(upcxio::from_json_error::expected_variant));
    }
}
...
template<typename F>
inline void bin_to_json(upcxio::input_stream& bin, const abi_type* type, std::string& dest, F&& f) {
    // FIXME: Write directly to the string instead of creating an additional buffer
    std::vector<char> buffer;
    upcxio::vector_stream writer{buffer};
...
inline void bin_to_json(pseudo_variant*, bin_to_json_state& state, bool allow_extensions,
    ...
    if (++stack_entry.position == 0) {
        ...
        const std::vector<upcxio::abi_field>& fields = *stack_entry.type->as_variant();
        upcxio::check(index < fields.size(), upcxio::convert_stream_error(upcxio::stream_error::bad_variant_index));
        auto& f = fields[index];
