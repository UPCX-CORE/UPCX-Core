...
inline void decimal_to_binary(std::array<uint8_t, size>& result,
    ...
    for (auto& src_digit : s) {
       upcxio::check(!(src_digit < '0' || src_digit > '9'),
            upcxio::convert_json_error(upcxio::from_json_error::expected_int));
        ...
        upcxio::check(!carry,
              upcxio::convert_json_error(upcxio::from_json_error::number_out_of_range));
    }
}
