...
#include <array>
#include <cstdint>
#include "../include/upcxio/crypto.hpp"
#include "../include/upcxio/from_bin.hpp"
#include "../include/upcxio/from_json.hpp"
#include "../include/upcxio/to_bin.hpp"
#include "../include/upcxio/to_json.hpp"
#include <string>
#include <string_view>
...
void base58_to_binary(Container& result, std::string_view s) {
    ...
    for (auto& src_digit : s) {
        ...
        check(carry >= 0,
            ::upcxio::convert_json_error(::upcxio::from_json_error::expected_key));
...
std::array<unsigned char, 20> digest_suffix_ripemd160(const Container&... data) {
    ...
    abiupcx_ripemd160::ripemd160_state self;
    abiupcx_ripemd160::ripemd160_init(&self);
    (abiupcx_ripemd160::ripemd160_update(&self, data.data(), data.size()), ...);
    check( abiupcx_ripemd160::ripemd160_digest(&self, digest.data()),
        convert_json_error(upcxio::from_json_error::invalid_signature) );
    return digest;
}
...
Key string_to_key(std::string_view s, key_type type, std::string_view suffix) {
    ...
    check(whole.size() > 5,
        convert_json_error(upcxio::from_json_error::expected_key));
...
std::string upcxio::public_key_to_string(const public_key& key) {
    if (key.index() == key_type::k1) {
       ...
       check(false, convert_json_error(upcxio::from_json_error::expected_public_key));
       __builtin_unreachable();
    }
}

public_key upcxio::public_key_from_string(std::string_view s) {
    public_key result;
    if (s.substr(0, 3) == "UPCX") {
...
std::string upcxio::private_key_to_string(const private_key& private_key) {
...
private_key upcxio::private_key_from_string(std::string_view s) {
...
std::string upcxio::signature_to_string(const upcxio::signature& signature) {
    if (signature.index() == key_type::k1)
       ...
       check(false, convert_json_error(upcxio::from_json_error::expected_signature));
       __builtin_unreachable();
    }
}

signature upcxio::signature_from_string(std::string_view s) {
