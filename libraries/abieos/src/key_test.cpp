...
void test_key(const T& x, const T& y) {
   auto keyx = upcxio::convert_to_key(x);
   auto keyy = upcxio::convert_to_key(y);
...
void test_compare() {
   ...
   test_key(public_key(std::in_place_index<0>, upcxio::ecc_public_key{1}), public_key(std::in_place_index<1>));
   test_key(public_key(upcxio::webauthn_public_key{{}, upcxio::webauthn_public_key::user_presence_t::USER_PRESENCE_NONE, "b"}),
            public_key(upcxio::webauthn_public_key{{}, upcxio::webauthn_public_key::user_presence_t::USER_PRESENCE_PRESENT, "a"}));
