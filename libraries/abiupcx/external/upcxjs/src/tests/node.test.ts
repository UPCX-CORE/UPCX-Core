...
describe("Node JS environment", () => {
  ...
  it("transacts with manually configured TAPOS fields", async () => {
    if (process.env.NODUPCX_VER && process.env.NODUPCX_VER === "release/2.0.x")
      return;
  ...
  it("transacts without broadcasting, returning signatures and packed transaction", async () => {
    ...
    expect(Object.keys(transactionSignatures)).toContain(
  ...
  it("confirms an action's return value can be verified", async () => {
    ...
    expect(
      transactionResponse.processed.action_traces[0].return_value_data
    ).toEqual(expectedValue);
  });

  it("transacts with resource payer", async () => {
    if (
      process.env.NODUPCX_VER &&
      (process.env.NODUPCX_VER === "release/2.0.x" ||
        process.env.NODUPCX_VER === "release/2.1.x")
    )
      return;
  ...
  it("confirms the return value of the read-only query", async () => {
    ...
    expect(
      transactionResponse.result.action_traces[0].return_value_data
    ).toEqual(expectedValue);
  });
  ...
  it("returns failure trace for failed transaction", async () => {
    ...
    try {
      ...
      expect(e.details.stack[0].format).toEqual(
  ...
  it("throws appropriate error message without configuration object or TAPOS in place", async () => {
    try {
      ...
      if (
        e.message !== "Required configuration or TAPOS fields are not present"
      ) {
        failedAsPlanned = false;
      }
    }
