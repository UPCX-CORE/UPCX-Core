...
import * as ser from "../upcxjs-serialize";
...
import { Transaction, TransactResult } from "../upcxjs-api-interfaces";
...
describe("Chain API Plugin Endpoints", () => {
  ...
  it("validates return type of abi_json_to_bin", async () => {
    const result: AbiJsonToBinResult = await rpc.abi_json_to_bin(
      "returnvalue",
      "sum",
      [5, 5]
    );
  ...
  it("validates return type of get_accounts_by_authorizers", async () => {
    const result: GetAccountsByAuthorizersResult =
      await rpc.get_accounts_by_authorizers(
        [
  ...
  it("validates return type of get_activated_protocol_features", async () => {
    const result: GetActivatedProtocolFeaturesResult =
      await rpc.get_activated_protocol_features({});
  ...
  it("validates return type of get_block_header_state", async () => {
    ...
    const result: GetBlockHeaderStateResult = await rpc.get_block_header_state(
      info.head_block_id
    );
  ...
  it("validates return type of get_block_info", async () => {
    const info: GetInfoResult = await rpc.get_info();
    const result: GetBlockInfoResult = await rpc.get_block_info(
  ...
  it("validates return type of get_block", async () => {
    const info: GetInfoResult = await rpc.get_info();
    const result: GetBlockResult = await rpc.get_block(
      info.last_irreversible_block_num
    );
  ...
  it("validates return type of get_scheduled_transactions", async () => {
    const result: GetScheduledTransactionsResult =
      await rpc.get_scheduled_transactions();
  ...
  it("validates return type of get_required_keys", async () => {
    ...
    const result: string[] = await rpc.getRequiredKeys({
      transaction,
      availableKeys,
    });
    result.forEach((element: any) => {
  ...
  it("validates return type of push_ro_transaction", async () => {
    ...
    const result: ReadOnlyTransactResult = await rpc.push_ro_transaction(
      transaction
    );
  ...
  it("validates return type of push_transactions", async () => {
    ...
    const result: TransactResult[] = await rpc.push_transactions([
      transactionA,
      transactionB,
    ]);
    const transactResult = {
...
describe("Trace API Plugin Endpoints", () => {
  it("validates return type of get_block", async () => {
    const info: GetInfoResult = await rpc.get_info();
    const result: TraceApiGetBlockResult = await rpc.trace_get_block(
      info.last_irreversible_block_num
    );
