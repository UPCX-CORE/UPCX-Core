...
import { Api } from "../upcxjs-api";
import { JsonRpc } from "../upcxjs-jsonrpc";
import { JsSignatureProvider } from "../upcxjs-jssig";
import * as path from "path";
import * as fs from "fs";
import { Action } from "../upcxjs-serialize";
import { base64ToBinary } from "../upcxjs-numeric";
...
const serializedTx = [
  ...
  195, 84, 203, 0, 0, 0, 0, 168, 237, 50, 50, 0, 48, 21, 164, 25, 83, 149, 177,
  202, 0, 0, 0, 87, 45, 60, 205, 205, 1, 0, 128, 138, 81, 125, 195, 84, 203, 0,
  0, 0, 0, 168, 237, 50, 50, 0, 0,
];
...
describe("upcxjs-api", () => {
  ...
  it("getAbi returns an abi", async () => {
    const response = await api.getAbi("testupcxtoken");
  ...
  it("getContract returns a contract", async () => {
    const response = await api.getContract("testupcxtoken");
  ...
  it("rawAbiToJson returns correct Json from raw Abi", async () => {
    const expected = await api.getAbi("testupcxtoken");
    const response = await rpc.getRawAbi("testupcxtoken");
  ...
  it("jsonToRawAbi returns correct raw Abi from Json", async () => {
    ...
    const jsonAbi = await api.getAbi("testupcxtoken");
  ...
  it("confirms jsonToRawAbi and rawAbiToJson are reciprocal", async () => {
    const expectedJsonAbi = await api.getAbi("testupcxtoken");
    const response = await rpc.getRawAbi("testupcxtoken");
  ...
  describe("Api shorthand design (JsonAbi)", () => {
    ...
    it("confirms the transaction extension serialization is reciprocal", async () => {
      ...
      const serialized = [
        [1, "0000000080ABBCA90010000000000000FA000000000000000000000000000000"],
      ];
      const deserialized = {
        resource_payer: {
          payer: "payer",
      ...
      const serializedTransactionExtensions =
        api.serializeTransactionExtensions(resourcePayerTrx);
      expect(serializedTransactionExtensions).toEqual(serialized);
      const deserializedTransactionExtensions =
        api.deserializeTransactionExtensions(serialized);
