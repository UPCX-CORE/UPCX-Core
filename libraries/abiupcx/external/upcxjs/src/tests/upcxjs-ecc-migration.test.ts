...
import { ecc as eccMigration } from "../upcxjs-ecc-migration";

import { PrivateKey } from "../upcxjs-key-conversions";
...
describe("ecc Migration", () => {
  ...
  it("verifies `sign`, `recover`, and `verify` functions are consistent between ecc objects", () => {
    ...
    const eccMigrationSig = eccMigration.sign(
      dataAsString,
      privateKeys[0],
      "utf8"
    );
    ...
    const eccMigrationKPub = eccMigration.recover(
      eccMigrationSig,
      dataAsString,
      "utf8"
    );
  ...
  it("verifies `signHash`, `recoverHash`, and `sha256` functions are consistent between ecc objects", () => {
    ...
    const eccMigrationKPub = eccMigration.recoverHash(
      eccSig,
      eccMigrationHash,
      "utf8"
    );
