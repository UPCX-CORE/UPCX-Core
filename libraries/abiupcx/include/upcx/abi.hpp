...
constexpr inline std::string_view convert_abi_error(upcx::abi_error e) {
   switch (e) {
...
UPCX_REFLECT(type_def, new_type_name, type);
...
UPCX_REFLECT(field_def, name, type);
...
UPCX_REFLECT(struct_def, name, base, fields);
...
UPCX_REFLECT(action_def, name, type, ricardian_contract);
...
UPCX_REFLECT(table_def, name, index_type, key_names, key_types, type);
...
UPCX_REFLECT(clause_pair, id, body);
...
UPCX_REFLECT(error_message, error_code, error_msg);
...
UPCX_REFLECT(variant_def, name, types);
...
UPCX_REFLECT(action_result_def, name, result_type);
...
UPCX_REFLECT(primary_key_index_def, name, type);

struct secondary_index_def {
   std::string type;
};

UPCX_REFLECT(secondary_index_def, type);

struct kv_table_entry_def {
   std::string                                type;
   primary_key_index_def                      primary_index;
   std::map<upcx::name, secondary_index_def> secondary_indices;
};

UPCX_REFLECT(kv_table_entry_def, type, primary_index, secondary_indices);
...
struct abi_def {
   ...
   might_not_exist<std::map<upcx::name, kv_table_entry_def>> kv_tables{};
};

UPCX_REFLECT(abi_def, version, types, structs, actions, tables, ricardian_clauses, error_messages, abi_extensions,
...
struct abi {
   std::map<upcx::name, std::string> action_types;
   std::map<upcx::name, std::string> table_types;
   std::map<upcx::name, std::string> kv_tables;
   std::map<std::string, abi_type>    abi_types;
   std::map<upcx::name, std::string> action_result_types;
