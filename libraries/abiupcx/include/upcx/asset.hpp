...
struct asset {
   ...
   asset(int64_t a, class symbol s) : amount(a), symbol{ s } {
      upcx::check(is_amount_within_range(), "magnitude of asset amount must be less than 2^62");
      upcx::check(symbol.is_valid(), "invalid symbol name");
   }
   ...
   asset& operator-=(const asset& a) {
      upcx::check(a.symbol == symbol, "attempt to subtract asset with different symbol");
      amount -= a.amount;
      upcx::check(-max_amount <= amount, "subtraction underflow");
      upcx::check(amount <= max_amount, "subtraction overflow");
      return *this;
   }
    ...
    */
   asset& operator+=(const asset& a) {
      upcx::check(a.symbol == symbol, "attempt to add asset with different symbol");
      amount += a.amount;
      upcx::check(-max_amount <= amount, "addition underflow");
      upcx::check(amount <= max_amount, "addition overflow");
      return *this;
   }
   ...
   asset& operator*=(int64_t a) {
      ...
      upcx::check(tmp <= max_amount, "multiplication overflow");
      upcx::check(tmp >= -max_amount, "multiplication underflow");
      amount = (int64_t)tmp;
      return *this;
   }
#endif
    ...
    */
   asset& operator/=(int64_t a) {
      upcx::check(a != 0, "divide by zero");
      upcx::check(!(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow");
      amount /= a;
      return *this;
   }
   ...
   friend int64_t operator/(const asset& a, const asset& b) {
      upcx::check(b.amount != 0, "divide by zero");
      upcx::check(a.symbol == b.symbol, "comparison of assets with different symbols is not allowed");
...
UPCX_REFLECT(asset, amount, symbol);

template <typename S>
inline void from_string(asset& result, S& stream) {
   int64_t  amount;
   uint64_t sym;
   check(upcx::string_to_asset(amount, sym, stream.pos, stream.end, true),
      convert_stream_error(upcx::stream_error::invalid_asset_format));
...
void from_json(asset& obj, S& stream) {
   ...
   check(string_to_asset(obj.amount, obj.symbol.value, s.data(), s.data() + s.size()),
      convert_json_error(upcx::from_json_error::expected_symbol_code));
}
...
struct extended_asset {
   ...
   friend extended_asset operator-(const extended_asset& a, const extended_asset& b) {
      upcx::check(a.contract == b.contract, "type mismatch");
...
UPCX_REFLECT(extended_asset, quantity, contract);
} // namespace upcx
