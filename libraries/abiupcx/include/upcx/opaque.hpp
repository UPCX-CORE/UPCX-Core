...
class opaque_base {
 ...
 public:
    ...
    * @pre !this->empty()
    */
   [[deprecated("Use unpack() free function instead.")]] void unpack(T& obj) { upcx::from_bin(obj, bin); }
   ...
   template <typename S>
   void from(S& stream) {
      upcx::from_bin(this->bin, stream);
   }

   template <typename S>
   void to_bin(S& stream) const {
      upcx::to_bin(this->bin, stream);
   }
