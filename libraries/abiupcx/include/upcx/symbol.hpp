...
class symbol_code {
 public:
   ...
   constexpr explicit symbol_code(std::string_view str) : value(0) {
      if (str.size() > 7) {
         upcx::check(false, "string is too long to be a valid symbol_code");
      }
      for (auto itr = str.rbegin(); itr != str.rend(); ++itr) {
         if (*itr < 'A' || *itr > 'Z') {
            upcx::check(false, "only uppercase letters allowed in symbol_code string");
         }
         value <<= 8;
         value |= *itr;
      }
...
UPCX_REFLECT(symbol_code, value);
UPCX_COMPARE(symbol_code);
...
void from_json(symbol& obj, S& stream) {
   ...
   check(string_to_symbol(obj.value, s.data(), s.data() + s.size()),
      convert_json_error(upcx::from_json_error::expected_symbol));
}
...
UPCX_REFLECT(extended_symbol, sym, contract);
UPCX_COMPARE(extended_symbol);
} // namespace upcx
