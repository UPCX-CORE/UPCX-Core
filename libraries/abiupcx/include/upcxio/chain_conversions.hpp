...
struct month_day {
   inline month_day( upcxio::month m, upcxio::day d ) : m(m), d(d) {}
...
constexpr inline uint64_t char_to_name_digit_strict() {
   static_assert(is_valid_char(C), "character is not for an upcxio name");
...
constexpr inline uint64_t string_to_name_strict_impl() {
   if constexpr (N == 12)
      static_assert((char_to_name_digit_strict<C>() & 0xf) == char_to_name_digit_strict<C>(),
            "upcxio name 13th character cannot be a letter after j");
...
constexpr inline uint64_t string_to_name_strict() {
   static_assert(sizeof...(Str) <= 13, "upcxio name string is too long");
...
[[nodiscard]] inline bool string_to_symbol(uint64_t& result, uint8_t precision, const char*& pos, const char* end,
                                           bool require_end) {
   if (!upcxio::string_to_symbol_code(result, pos, end, require_end))
      return false;
...
inline std::string symbol_to_string(uint64_t v) {
   return std::to_string(v & 0xff) + "," + upcxio::symbol_code_to_string(v >> 8);
}
...
[[nodiscard]] inline bool string_to_asset(int64_t& amount, uint64_t& symbol, const char*& s, const char* end,
   ...
   amount = uamount;
   uint64_t code;
   if (!upcxio::string_to_symbol_code(code, s, end, expect_end))
      return false;
...
inline std::string asset_to_string(int64_t amount, uint64_t symbol) {
   ...
   return result + ' ' + upcxio::symbol_code_to_string(symbol >> 8);
}
