...
struct name {
   ...
   explicit    operator std::string() const { return upcxio::name_to_string(value); }
   ...
   static constexpr uint8_t char_to_value(char c) {
      if (c == '.')
         ...
         upcxio::check(false, "character is not in allowed character set for names");
...
void to_json(const name& obj, S& stream) {
   to_json(upcxio::name_to_string(obj.value), stream);
}
