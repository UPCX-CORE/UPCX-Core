...
struct abi_serializer_impl : abi_serializer {
    void json_to_bin(::abiupcx::jvalue_to_bin_state& state, bool allow_extensions, const abi_type* type,
                             bool start) const override {
        return ::abiupcx::json_to_bin((T*)nullptr, state, allow_extensions, type, start);
    }
    void json_to_bin(::abiupcx::json_to_bin_state& state, bool allow_extensions, const abi_type* type,
                             bool start) const override {
        return ::abiupcx::json_to_bin((T*)nullptr, state, allow_extensions, type, start);
    }
    void bin_to_json(::abiupcx::bin_to_json_state& state, bool allow_extensions, const abi_type* type,
                             bool start) const override {
        return ::abiupcx::bin_to_json((T*)nullptr, state, allow_extensions, type, start);
    }
};
...
abi_type* get_type(std::map<std::string, abi_type>& abi_types,
   ...
   upcxio::check(depth < 32,
        upcxio::convert_abi_error(abi_error::recursion_limit_reached));
    auto it = abi_types.find(name);
    if (it == abi_types.end()) {
        if (ends_with(name, "?")) {
            auto base = get_type(abi_types, name.substr(0, name.size() - 1), depth + 1);
            upcxio::check(!holds_any_alternative<abi_type::optional, abi_type::array, abi_type::extension>(base->_data),
                  upcxio::convert_abi_error(abi_error::invalid_nesting));
            auto [iter, success] = abi_types.try_emplace(name, name, abi_type::optional{base}, &abi_serializer_for< ::abiupcx::pseudo_optional>);
            return &iter->second;
        } else if (ends_with(name, "[]")) {
            auto element = get_type(abi_types, name.substr(0, name.size() - 2), depth + 1);
            upcxio::check(!holds_any_alternative<abi_type::optional, abi_type::array, abi_type::extension>(element->_data),
                  upcxio::convert_abi_error(abi_error::invalid_nesting));
            auto [iter, success] = abi_types.try_emplace(name, name, abi_type::array{element}, &abi_serializer_for< ::abiupcx::pseudo_array>);
            return &iter->second;
        } else if (ends_with(name, "$")) {
            auto base = get_type(abi_types, name.substr(0, name.size() - 1), depth + 1);
            upcxio::check(!std::holds_alternative<abi_type::extension>(base->_data),
                  upcxio::convert_abi_error(abi_error::invalid_nesting));
            auto [iter, success] = abi_types.try_emplace(name, name, abi_type::extension{base}, &abi_serializer_for< ::abiupcx::pseudo_extension>);
            return &iter->second;
        } else
           upcxio::check(false, upcxio::convert_abi_error(abi_error::unknown_type));
    }
   ...
   upcxio::check(depth < 32,
    ...
    if (!type->base.empty()) {
        ...
        if(auto* b = std::get_if<abi_type::struct_>(&base->_data)) {
           ...
           upcxio::check(false, upcxio::convert_abi_error(abi_error::base_not_a_struct));
        }
    }
...
abi_type::alias resolve(std::map<std::string, abi_type>& abi_types, const abi_type::alias_def* type, int depth) {
    ...
    upcxio::check(!std::holds_alternative<abi_type::extension>(t->_data),
        upcxio::convert_abi_error(abi_error::extension_typedef));
...
const abi_type* upcxio::abi::get_type(const std::string& name) {
   return ::get_type(abi_types, name, 0);
}

void upcxio::convert(const abi_def& abi, upcxio::abi& c) {
    ...
    });
    {
        c.abi_types.try_emplace("extended_asset", "extended_asset",
                                ...
                                &abi_serializer_for<::abiupcx::pseudo_object>);
    }

    for (auto& t : abi.types) {
       upcxio::check(!t.new_type_name.empty(),
            upcxio::convert_abi_error(abi_error::missing_name));
        auto [_, inserted] = c.abi_types.try_emplace(t.new_type_name, t.new_type_name, &t.type, nullptr);
        upcxio::check(inserted,
            upcxio::convert_abi_error(abi_error::redefined_type));
    }
    for (auto& s : abi.structs) {
       upcxio::check(!s.name.empty(),
            upcxio::convert_abi_error(abi_error::missing_name));
        auto [it, inserted] = c.abi_types.try_emplace(s.name, s.name, &s, &abi_serializer_for<::abiupcx::pseudo_object>);
        upcxio::check(inserted,
            upcxio::convert_abi_error(abi_error::redefined_type));
    }
    for (auto& v : abi.variants.value) {
       upcxio::check(!v.name.empty(),
            upcxio::convert_abi_error(abi_error::missing_name));
        auto [it, inserted] = c.abi_types.try_emplace(v.name, v.name, &v, &abi_serializer_for<::abiupcx::pseudo_variant>);
        upcxio::check(inserted,
            upcxio::convert_abi_error(abi_error::redefined_type));
    }
    ...
    for (const auto& [key, val] : abi.kv_tables.value) {
        ...
        upcxio::vector_stream strm(bytes);
        to_json(val, strm);
...
void to_abi_def(abi_def& def, const std::string& name, const T*) {
   upcxio::check(false, upcxio::convert_abi_error(upcxio::abi_error::bad_abi));
}
...
void upcxio::convert(const upcxio::abi& abi, upcxio::abi_def& def) {
   def.version = "upcxio::abi/1.0";
   for(auto& [name, type] : abi.abi_types) {
      std::visit([&name = type.name, &def](const auto& t){ return to_abi_def(def, name, t); }, type._data);
   }
}

const abi_serializer* const upcxio::object_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_object>;
const abi_serializer* const upcxio::variant_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_variant>;
const abi_serializer* const upcxio::array_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_array>;
const abi_serializer* const upcxio::extension_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_extension>;
const abi_serializer* const upcxio::optional_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_optional>;

std::vector<char> upcxio::abi_type::json_to_bin_reorderable(std::string_view json, std::function<void()> f) const {
   abiupcx::jvalue tmp;
   abiupcx::json_to_jvalue(tmp, json, f);
   std::vector<char> result;
   abiupcx::json_to_bin(result, this, tmp, f);
   return result;
}

std::vector<char> upcxio::abi_type::json_to_bin(std::string_view json, std::function<void()> f) const {
   std::vector<char> result;
   abiupcx::json_to_bin(result, this, json, f);
   return result;
}

std::string upcxio::abi_type::bin_to_json(input_stream& bin, std::function<void()> f) const {
   std::string result;
   abiupcx::bin_to_json(bin, this, result, f);
   return result;
}
