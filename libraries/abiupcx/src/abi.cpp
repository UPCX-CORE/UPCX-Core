...
abi_type* get_type(std::map<std::string, abi_type>& abi_types,
   ...
   upcx::check(depth < 32,
        upcx::convert_abi_error(abi_error::recursion_limit_reached));
    auto it = abi_types.find(name);
    if (it == abi_types.end()) {
        if (ends_with(name, "?")) {
            auto base = get_type(abi_types, name.substr(0, name.size() - 1), depth + 1);
            upcx::check(!holds_any_alternative<abi_type::optional, abi_type::array, abi_type::extension>(base->_data),
                  upcx::convert_abi_error(abi_error::invalid_nesting));
   ...
   upcx::check(depth < 32,
    ...
    if (!type->base.empty()) {
        ...
        if(auto* b = std::get_if<abi_type::struct_>(&base->_data)) {
           ...
           upcx::check(false, upcx::convert_abi_error(abi_error::base_not_a_struct));
        }
    }
...
abi_type::alias resolve(std::map<std::string, abi_type>& abi_types, const abi_type::alias_def* type, int depth) {
    ...
    upcx::check(!std::holds_alternative<abi_type::extension>(t->_data),
        upcx::convert_abi_error(abi_error::extension_typedef));
...
const abi_type* upcx::abi::get_type(const std::string& name) {
   return ::get_type(abi_types, name, 0);
}

void upcx::convert(const abi_def& abi, upcx::abi& c) {
    ...
    for (auto& t : abi.types) {
       upcx::check(!t.new_type_name.empty(),
            upcx::convert_abi_error(abi_error::missing_name));
        auto [_, inserted] = c.abi_types.try_emplace(t.new_type_name, t.new_type_name, &t.type, nullptr);
        upcx::check(inserted,
            upcx::convert_abi_error(abi_error::redefined_type));
    }
    for (auto& s : abi.structs) {
       upcx::check(!s.name.empty(),
            upcx::convert_abi_error(abi_error::missing_name));
...
void to_abi_def(abi_def& def, const std::string& name, const T*) {
   upcx::check(false, upcx::convert_abi_error(upcx::abi_error::bad_abi));
}
...
void upcx::convert(const upcx::abi& abi, upcx::abi_def& def) {
   def.version = "upcx::abi/1.0";
   for(auto& [name, type] : abi.abi_types) {
      std::visit([&name = type.name, &def](const auto& t){ return to_abi_def(def, name, t); }, type._data);
   }
}

const abi_serializer* const upcx::object_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_object>;
const abi_serializer* const upcx::variant_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_variant>;
const abi_serializer* const upcx::array_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_array>;
const abi_serializer* const upcx::extension_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_extension>;
const abi_serializer* const upcx::optional_abi_serializer = &abi_serializer_for< ::abiupcx::pseudo_optional>;

std::vector<char> upcx::abi_type::json_to_bin_reorderable(std::string_view json, std::function<void()> f) const {
   abiupcx::jvalue tmp;
...
std::vector<char> upcx::abi_type::json_to_bin(std::string_view json, std::function<void()> f) const {
   std::vector<char> result;
   abiupcx::json_to_bin(result, this, json, f);
   return result;
}

std::string upcx::abi_type::bin_to_json(input_stream& bin, std::function<void()> f) const {
   std::string result;
