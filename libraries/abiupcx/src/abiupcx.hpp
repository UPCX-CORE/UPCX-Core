...
struct jvalue_to_bin_state {
    ...
    bool get_bool() const {
      ...
      upcx::check(b, upcx::convert_json_error(upcx::from_json_error::expected_bool));
      return *b;
    }

    std::string_view get_string() const {
        auto* s = std::get_if<std::string>(&received_value->value);
        upcx::check(s, upcx::convert_json_error(upcx::from_json_error::expected_string));
        return *s;
    }
    void get_null() {
       upcx::check(std::holds_alternative<std::nullptr_t>(received_value->value),
              upcx::convert_json_error(upcx::from_json_error::expected_null));
    }
    bool get_null_pred() {
...
struct json_to_bin_state : upcx::json_token_stream {
    ...
    explicit json_to_bin_state(char* in, upcx::vector_stream& out)
      : upcx::json_token_stream(in), writer(out) {}
};
...
struct bin_to_json_state {
    ...
    bin_to_json_state(upcx::input_stream& bin, upcx::vector_stream& writer)
...
void json_to_bin(bytes*, State& state, bool, const abi_type*, bool start) {
    ...
    upcx::check( !(s.size() & 1), upcx::convert_json_error(upcx::from_json_error::expected_hex_string) );
    upcx::varuint32_to_bin(s.size() / 2, state.writer);
    // FIXME: Add a function to encode a hex string to a stream
    upcx::check(upcx::unhex(std::back_inserter(state.writer.data), s.begin(), s.end()),
        upcx::convert_json_error(upcx::from_json_error::expected_hex_string));
}
...
UPCX_REFLECT(uint128, data)

struct int128 {
    std::array<uint8_t, 16> data = {};
};
UPCX_REFLECT(int128, data)
...
void from_json(int128& obj, S& stream) {
    ...
    if (s.size() && s[0] == '-') {
        ...
        negate(obj.data);
        upcx::check(is_negative(obj.data), upcx::convert_json_error(upcx::from_json_error::number_out_of_range));
    } else {
...
inline void json_to_jvalue(jvalue& value, std::string_view json, F&& f) {
    ...
    upcx::check(reader.Parse<rapidjson::kParseValidateEncodingFlag | rapidjson::kParseIterativeFlag |
        rapidjson::kParseNumbersAsStringsFlag>(ss, state),
        upcx::convert_json_error(upcx::from_json_error::unspecific_syntax_error));
}
...
inline void json_to_bin(pseudo_object*, jvalue_to_bin_state& state, bool allow_extensions,
                                       const abi_type* type, bool start) {
    if (start) {
       upcx::check(!(!state.received_value || !std::holds_alternative<jobject>(state.received_value->value)),
            upcx::convert_json_error(upcx::from_json_error::expected_start_object));
    ...
    const std::vector<upcx::abi_field>& fields = stack_entry.type->as_struct()->fields;
    ...
    if (it == obj.end()) {
        ...
        upcx::check(false, upcx::convert_json_error(upcx::from_json_error::expected_field));
    }
    upcx::check(!state.skipped_extension,
        upcx::convert_json_error(upcx::from_json_error::unexpected_field));
...
inline void json_to_bin(pseudo_array*, jvalue_to_bin_state& state, bool, const abi_type* type,
                                       bool start) {
    if (start) {
       upcx::check(!(!state.received_value || !std::holds_alternative<jarray>(state.received_value->value)),
            upcx::convert_json_error(upcx::from_json_error::expected_start_array));
        if (trace_jvalue_to_bin)
            printf("%*s[ %d elements\n", int(state.stack.size() * 4), "",
                   int(std::get<jarray>(state.received_value->value).size()));
        upcx::varuint32_to_bin(std::get<jarray>(state.received_value->value).size(), state.writer);
...
inline void json_to_bin(pseudo_variant*, jvalue_to_bin_state& state, bool allow_extensions,
    ...
    if (start) {
       upcx::check(!(!state.received_value || !std::holds_alternative<jarray>(state.received_value->value)),
            upcx::convert_json_error(upcx::from_json_error::expected_variant));
        auto& arr = std::get<jarray>(state.received_value->value);
        upcx::check(arr.size() == 2,
            upcx::convert_json_error(upcx::from_json_error::expected_variant));
        upcx::check(std::holds_alternative<std::string>(arr[0].value),
            upcx::convert_json_error(upcx::from_json_error::expected_variant));
    ...
    if (stack_entry.position == 0) {
        auto& typeName = std::get<std::string>(arr[0].value);
        const std::vector<upcx::abi_field>& fields = *stack_entry.type->as_variant();
        auto it = std::find_if(fields.begin(), fields.end(),
                               [&](auto& field) { return field.name == typeName; });
        upcx::check(it != fields.end(),
            upcx::convert_json_error(upcx::from_json_error::invalid_type_for_variant));
        upcx::varuint32_to_bin(it - fields.begin(), state.writer);
        state.received_value = &arr[++stack_entry.position];
...
inline void json_to_bin(std::vector<char>& bin, const abi_type* type, std::string_view json, F&& f) {
    ...
    upcx::vector_stream out(out_buf);
    ...
    while(!state.stack.empty()) {
        ...
        upcx::check(state.stack.size() <= max_stack_size,
            upcx::convert_abi_error(upcx::abi_error::recursion_limit_reached));
        type->ser->json_to_bin(state, entry.allow_extensions, type, false);
    }
    upcx::check(state.complete(),
        upcx::convert_json_error(upcx::from_json_error::expected_end));

    size_t pos = 0;
    for (auto& insertion : state.size_insertions) {
        bin.insert(bin.end(), out_buf.begin() + pos, out_buf.begin() + insertion.position);
        upcx::push_varuint32(bin, insertion.size);
...
inline void json_to_bin(pseudo_object*, json_to_bin_state& state, bool allow_extensions,
    ...
    auto& stack_entry = state.stack.back();
    const std::vector<upcx::abi_field>& fields = type->as_struct()->fields;
    ...
    if (key) {
       upcx::check(!(++stack_entry.position >= (ptrdiff_t)fields.size() || state.skipped_extension),
             upcx::convert_json_error(upcx::from_json_error::unexpected_field));
        auto& field = fields[stack_entry.position];
        if (*key != field.name) {
            stack_entry.position = -1;
...
inline void json_to_bin(pseudo_variant*, json_to_bin_state& state, bool allow_extensions,
    ...
    ++stack_entry.position;
    if (state.get_end_array_pred()) {
       upcx::check(stack_entry.position == 2,
            upcx::convert_json_error(upcx::from_json_error::expected_variant));
        if (trace_json_to_bin)
            printf("%*s]\n", int((state.stack.size() - 1) * 4), "");
        state.stack.pop_back();
    ...
    if (stack_entry.position == 0) {
        ...
        upcx::varuint32_to_bin(stack_entry.variant_type_index, state.writer);
       ...
       upcx::check(false, upcx::convert_json_error(upcx::from_json_error::expected_variant));
    }
}
...
template<typename F>
inline void bin_to_json(upcx::input_stream& bin, const abi_type* type, std::string& dest, F&& f) {
    // FIXME: Write directly to the string instead of creating an additional buffer
    std::vector<char> buffer;
    upcx::vector_stream writer{buffer};
...
inline void bin_to_json(pseudo_variant*, bin_to_json_state& state, bool allow_extensions,
    ...
    if (++stack_entry.position == 0) {
        ...
        const std::vector<upcx::abi_field>& fields = *stack_entry.type->as_variant();
        upcx::check(index < fields.size(), upcx::convert_stream_error(upcx::stream_error::bad_variant_index));
        auto& f = fields[index];
