...
#include <array>
#include <cstdint>
#include "../include/upcx/crypto.hpp"
#include "../include/upcx/from_bin.hpp"
#include "../include/upcx/from_json.hpp"
#include "../include/upcx/to_bin.hpp"
#include "../include/upcx/to_json.hpp"
#include <string>
#include <string_view>
...
void base58_to_binary(Container& result, std::string_view s) {
    ...
    for (auto& src_digit : s) {
        ...
        check(carry >= 0,
            ::upcx::convert_json_error(::upcx::from_json_error::expected_key));
...
std::array<unsigned char, 20> digest_suffix_ripemd160(const Container&... data) {
    ...
    check( abiupcx_ripemd160::ripemd160_digest(&self, digest.data()),
        convert_json_error(upcx::from_json_error::invalid_signature) );
    return digest;
}
...
Key string_to_key(std::string_view s, key_type type, std::string_view suffix) {
    ...
    check(whole.size() > 5,
        convert_json_error(upcx::from_json_error::expected_key));
...
std::string upcx::public_key_to_string(const public_key& key) {
    if (key.index() == key_type::k1) {
       ...
       check(false, convert_json_error(upcx::from_json_error::expected_public_key));
       __builtin_unreachable();
    }
}

public_key upcx::public_key_from_string(std::string_view s) {
    public_key result;
...
std::string upcx::private_key_to_string(const private_key& private_key) {
...
private_key upcx::private_key_from_string(std::string_view s) {
...
std::string upcx::signature_to_string(const upcx::signature& signature) {
    if (signature.index() == key_type::k1)
       ...
       check(false, convert_json_error(upcx::from_json_error::expected_signature));
       __builtin_unreachable();
    }
}

signature upcx::signature_from_string(std::string_view s) {
