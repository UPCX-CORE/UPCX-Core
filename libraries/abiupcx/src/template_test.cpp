...
std::vector<char> test_serialize(const T& value, F&& f) {
   ...
   upcxio::vector_stream vecstream(buf1);
   upcxio::size_stream szstream;
   f(value, vecstream);
   f(value, szstream);
   CHECK(szstream.size == vecstream.data.size());
   std::vector<char> buf2(szstream.size);
   upcxio::fixed_buf_stream fxstream(buf2.data(), buf2.size());
   f(value, fxstream);
   CHECK(buf1 == buf2);
   return buf1;
}

upcxio::abi round_trip_abi(const upcxio::abi& src) {
   upcxio::abi_def def;
   convert(src, def);
   upcxio::abi result;
   convert(def, result);
...
template<typename T>
void test(const T& value, upcxio::abi& abi1, upcxio::abi& abi2) {
   ...
   {
      T bin_value;
      upcxio::input_stream bin_stream(bin);
      from_bin(bin_value, bin_stream);
      CHECK(bin_value == value);
      T json_value;
      std::string mutable_json(json.data(), json.size());
      upcxio::json_token_stream json_stream(mutable_json.data());
      from_json(json_value, json_stream);
      CHECK(json_value == value);
   }

   for(upcxio::abi* abi : {&abi1, &abi2})
   {
      // Get the ABI
      using upcxio::get_type_name;
      const upcxio::abi_type* type = abi->get_type(get_type_name((T*)nullptr));
...
template<typename T>
void test_int(upcxio::abi& abi1, upcxio::abi& abi2) {
...
int main() {
   ...
   upcxio::abi_def def = upcxio::from_json<upcxio::abi_def>(stream);
   upcxio::abi abi;
   convert(def, abi);
   abi.add_type<struct_type>();
   upcxio::abi new_abi(round_trip_abi(abi));
   ...
   test(upcxio::name("upcxio"), abi, new_abi);
   test(upcxio::name(), abi, new_abi);
