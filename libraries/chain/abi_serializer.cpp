...
namespace upcx { namespace chain {
   ...
   void abi_serializer::set_abi(const abi_def& abi, const yield_function_t& yield) {
      ...
      UPCX_ASSERT(starts_with(abi.version, "upcx::abi/1."), unsupported_abi_version_exception, "ABI has an unsupported version");
   ...
   bool abi_serializer::_is_type(const std::string_view& rtype, impl::abi_traverse_context& ctx )const {
      ...
      if( upcx::chain::is_string_valid_name(type) ) {
