...
#include <upcx/chain/authority_checker.hpp>
...
#include <upcx/chain/database_utils.hpp>
...
namespace upcx { namespace chain {
                                                                                  ...
                                                                                  )const
   {
      ...
      try {
         ...
         if( *linked_permission == config::upcx_any_name )
            return std::optional<permission_name>();
                                                             ...
                                                             )const
   {
      ...
      if( *unlinked_permission_name == config::upcx_any_name )
         return;
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is upcx.any
