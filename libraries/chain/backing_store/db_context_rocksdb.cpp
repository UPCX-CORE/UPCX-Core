...
#include <upcxio/chain/backing_store/db_key_value_any_lookup.hpp>
#include <upcxio/chain/backing_store/db_key_value_sec_lookup.hpp>
...
namespace upcxio { namespace chain { namespace backing_store {
   ...
   public:
      ...
      using session_variant_type = upcxio::session::session_variant<session_type::parent_type, session_type>;
      using shared_bytes = upcxio::session::shared_bytes;
      ...
      using uint128_t = upcxio::chain::uint128_t;
      using key256_t = upcxio::chain::key256_t;
   ...
   int32_t db_context_rocksdb::db_store_i64(uint64_t scope, uint64_t table, account_name payer, uint64_t id, const char* value , size_t value_size) {
      ...
      UPCX_ASSERT( !old_key_value.value, db_rocksdb_invalid_operation_exception, "db_store_i64 called with pre-existing key");
   ...
   void db_context_rocksdb::db_update_i64(int32_t itr, account_name payer, const char* value , size_t value_size) {
      ...
      UPCX_ASSERT( table_store.contract == receiver, table_access_violation, "db access violation" );
      const auto old_key_value = get_primary_key_value(receiver, table_store.scope, table_store.table, key_store.primary);

      UPCX_ASSERT( old_key_value.value, db_rocksdb_invalid_operation_exception,
   ...
   int32_t db_context_rocksdb::db_next_i64(int32_t itr, uint64_t& primary) {
      ...
      UPCX_ASSERT( exact.valid, db_rocksdb_invalid_operation_exception,
   ...
   int32_t db_context_rocksdb::db_previous_i64(int32_t itr, uint64_t& primary) {
      if( itr < primary_iter_store.invalid_iterator() ) { // is end iterator
         ...
         UPCX_ASSERT( table_store, invalid_table_iterator, "not a valid end iterator" );
   ...
   upcxio::chain::key256_t convert(const uint128_t* data) {
      upcxio::chain::key256_t secondary;
      std::memcpy(secondary.data(), data, sizeof(secondary));
      return secondary;
   }

   void convert_back(uint128_t* data, const upcxio::chain::key256_t& secondary) {
   ...
   int32_t db_context_rocksdb::find_and_store_primary_key(const session_variant_type::iterator& session_iter,
      ...
      UPCX_ASSERT( db_key_value_format::get_primary_key((*session_iter).first, type_prefix, found_key), db_rocksdb_invalid_operation_exception,
   ...
   int32_t db_context_rocksdb::find_i64(name code, name scope, name table, uint64_t id, comp comparison) {
      ...
      if (!primary_lookup.match(primary_and_prefix_keys.full_key, (*session_iter).first)) {
         ...
         UPCX_ASSERT( is_in_table(session_iter), db_rocksdb_invalid_operation_exception,
      ...
      if (!primary_key) {
         ...
         UPCX_ASSERT( valid, db_rocksdb_invalid_operation_exception,
...
}}} // namespace upcxio::chain::backing_store
