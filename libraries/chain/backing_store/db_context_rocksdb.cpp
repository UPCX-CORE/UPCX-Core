...
#include <upcx/chain/backing_store/db_key_value_any_lookup.hpp>
#include <upcx/chain/backing_store/db_key_value_sec_lookup.hpp>
...
namespace upcx { namespace chain { namespace backing_store {
   ...
   public:
      ...
      using session_variant_type = upcx::session::session_variant<session_type::parent_type, session_type>;
      using shared_bytes = upcx::session::shared_bytes;
      ...
      using uint128_t = upcx::chain::uint128_t;
      using key256_t = upcx::chain::key256_t;
   ...
   upcx::chain::key256_t convert(const uint128_t* data) {
      upcx::chain::key256_t secondary;
      std::memcpy(secondary.data(), data, sizeof(secondary));
      return secondary;
   }

   void convert_back(uint128_t* data, const upcx::chain::key256_t& secondary) {
