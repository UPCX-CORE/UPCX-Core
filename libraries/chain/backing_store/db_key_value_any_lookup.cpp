...
namespace upcxio { namespace chain { namespace backing_store {
   ...
   upcxio::session::shared_bytes make_useless_value() {
      const char null = '\0';
      return upcxio::session::shared_bytes {&null, 1};
   }

   const upcxio::session::shared_bytes db_key_value_any_lookup::useless_value = make_useless_value();
   ...
   void db_key_value_any_lookup::remove_table_if_empty(const shared_bytes& key) {
      ...
      UPCX_ASSERT( session_itr != current_session.end(), db_rocksdb_invalid_operation_exception,
      ...
      UPCX_ASSERT( match_prefix(entire_table_prefix_key, key_value.first), db_rocksdb_invalid_operation_exception,
                  "invariant failure in remove_table_if_empty, iter store found and removed, but no table entry was found");
