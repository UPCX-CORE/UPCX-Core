...
namespace upcxio { namespace chain { namespace backing_store { namespace db_key_value_format {
   namespace detail {
      constexpr std::size_t key_size(key_type kt) {
         switch(kt) {
            ...
            case key_type::sec_i128:
               return sizeof(upcxio::chain::uint128_t);
      ...
      intermittent_decomposed_prefix_values extract_from_composite_key_prefix(b1::chain_kv::bytes::const_iterator begin, b1::chain_kv::bytes::const_iterator key_end) {
         ...
         UPCX_ASSERT(b1::chain_kv::extract_key(key_loc, key_end, scope_name), bad_composite_key_exception,
                    "DB intrinsic key-value store composite key is malformed, it does not contain a scope");
         uint64_t table_name = 0;
         UPCX_ASSERT(b1::chain_kv::extract_key(key_loc, key_end, table_name), bad_composite_key_exception,
      ...
      intermittent_decomposed_values extract_from_composite_key(b1::chain_kv::bytes::const_iterator begin, b1::chain_kv::bytes::const_iterator key_end) {
         ...
         UPCX_ASSERT(key_loc != key_end, bad_composite_key_exception,
      ...
      constexpr uint64_t db_type_and_code_size = detail::prefix_size<upcxio::session::shared_bytes>() - detail::prefix_size<b1::chain_kv::bytes>(); // 1 (db type) + 8 (contract)
   ...
   upcxio::session::shared_bytes create_table_key(const upcxio::session::shared_bytes& prefix_key) {
      const std::size_t full_prefix_size = db_key_value_format::detail::prefix_size<upcxio::session::shared_bytes>();
      UPCX_ASSERT(prefix_key.size() >= full_prefix_size, bad_composite_key_exception,
      ...
      return detail::assemble<2, upcxio::session::shared_bytes>({rocksdb::Slice{prefix_key.data(), full_prefix_size},
   ...
   bool get_trailing_primary_key(const rocksdb::Slice& full_key, const rocksdb::Slice& secondary_key_prefix, uint64_t& primary_key) {
      ...
      UPCX_ASSERT(full_key.size() == sec_prefix_size + sizeof(primary_key), bad_composite_key_exception,
      ...
      UPCX_ASSERT(b1::chain_kv::extract_key(composite_loc, composite_primary_key.cend(), primary_key), bad_composite_key_exception,
   ...
   key_type extract_primary_to_sec_key_type(const b1::chain_kv::bytes& composite_key) {
      ...
      UPCX_ASSERT(kt == key_type::primary_to_sec, bad_composite_key_exception,
   ...
   rocksdb::Slice extract_legacy_slice(const upcxio::session::shared_bytes& full_key) {
   ...
   upcxio::session::shared_bytes create_full_key(const b1::chain_kv::bytes& composite_key, name code) {
      ...
      auto ret = upcxio::session::make_shared_bytes<std::string_view, 3>({std::string_view{&db_type_prefix, 1},
   ...
   upcxio::session::shared_bytes create_full_key_prefix(const upcxio::session::shared_bytes& full_key, end_of_prefix prefix_end) {
      ...
      const std::size_t full_prefix_size = db_key_value_format::detail::prefix_size<upcxio::session::shared_bytes>() + extension_size;
      UPCX_ASSERT( full_prefix_size < full_key.size(), db_rocksdb_invalid_operation_exception,
      ...
      return upcxio::session::shared_bytes(full_key.data(), full_prefix_size);
   }

   full_key_data parse_full_key(const upcxio::session::shared_bytes& full_key) {
      static const char db_type_prefix = make_rocksdb_contract_db_prefix();
      UPCX_ASSERT( full_key.size() >= 1, db_rocksdb_invalid_operation_exception,
      ...
      UPCX_ASSERT( full_key.size() >= db_type_and_contract_size, db_rocksdb_invalid_operation_exception,
      ...
      uint64_t contract;
      UPCX_ASSERT(b1::chain_kv::extract_key(composite_loc, composite_contract_key.cend(), contract), bad_composite_key_exception,
                 "DB intrinsic key-value store invariant has changed, extract_key should only fail if the string size is"
                 " less than the sizeof(uint64_t), which was verified that it was not");
      UPCX_ASSERT( composite_loc == composite_contract_key.cend(), db_rocksdb_invalid_operation_exception,
   ...
   upcxio::session::shared_bytes create_full_prefix_key(name code, name scope, name table, std::optional<key_type> kt) {
      bytes composite_key = kt ? create_prefix_type_key(scope, table, *kt) : create_prefix_key(scope, table);
      return create_full_key(composite_key, code);
   }
}}}} // namespace upcxio::chain::backing_store::db_key_value_format
