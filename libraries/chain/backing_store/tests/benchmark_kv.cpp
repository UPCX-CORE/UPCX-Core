...
void benchmark(const cmd_args& args) {
   if (args.backing_store == "rocksdb") {
      ...
      auto rocks_session = upcx::session::make_session(make_rocks_db("kvrdb-tmp"), max_rocks_iterators);
      auto session = upcx::session::session<decltype(rocks_session)>{rocks_session};
