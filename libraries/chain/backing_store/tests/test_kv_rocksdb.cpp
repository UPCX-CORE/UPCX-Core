...
#include <upcxio/chain/backing_store/kv_context_rocksdb.hpp>
#include <upcxio/chain/backing_store/chain_kv_payer.hpp>
...
struct kv_rocksdb_fixture {
   ...
   class mock_session {
    public:
      class iterator final {
       public:
         ...
         iterator                                                              operator--(int) { return *this; }
         std::pair<upcxio::session::shared_bytes, std::optional<upcxio::session::shared_bytes>> operator*() const {
         ...
         upcxio::session::shared_bytes key() { return m_fixture->mock_get_kv().first; }
       private:
    ...
    public:
      ...
      std::optional<upcxio::session::shared_bytes> read(const upcxio::session::shared_bytes& key) const {
         return m_fixture->mock_get(key);
      }
      void write(const upcxio::session::shared_bytes& key, const upcxio::session::shared_bytes& value) {}
      bool contains(const upcxio::session::shared_bytes& key) const { return false; }
      void erase(const upcxio::session::shared_bytes& key) {}

      iterator begin() { return iterator{ *m_fixture, false }; }
      iterator end() { return iterator{ *m_fixture, true }; }
      iterator lower_bound(const upcxio::session::shared_bytes& key) { 
   ...
   std::function<int64_t(uint64_t contract, const char* key, uint32_t key_size, const char* value, uint32_t value_size,
                         account_name payer)>
                                                                                                mock_kv_set;
   std::function<std::optional<upcxio::session::shared_bytes>(const upcxio::session::shared_bytes&)>             mock_get;
   std::function<std::pair<upcxio::session::shared_bytes, std::optional<upcxio::session::shared_bytes>> const()> mock_get_kv;
   std::function<bool()>                                                                        mock_is_end = [](){return false;};
   std::function<bool()>                                                                        mock_is_valid;
   std::function<bool()>                                                                        mock_is_erased;
   std::function<upcxio::session::shared_bytes()>                                                mock_get_key_prefix;
   ...
   void check_test_kv_it_status(it_state state, it_pos pos, kv_it_stat expected_status) {
      mock_get_kv = []() -> std::pair<upcxio::session::shared_bytes, std::optional<upcxio::session::shared_bytes>> {
         static const auto expected_key = prefix + std::string{"key"};
         static const auto key = upcxio::chain::make_prefix_key(contract, expected_key.c_str(), expected_key.size());
         return std::pair{ key, upcxio::session::shared_bytes{"payernamValue", 13} };
      };
      mock_get_key_prefix = [&]() { return upcxio::chain::make_prefix_key(contract, prefix.c_str(), prefix.size()); };
   ...
   void check_kv_it_move(it_dir dir, it_state state, it_pos pos, kv_it_stat expected_status) {
      ...
      mock_get_kv = []() -> std::pair<upcxio::session::shared_bytes, std::optional<upcxio::session::shared_bytes>> {
         return std::pair{ upcxio::session::shared_bytes("keykeykeykeykey", 15),
                           std::optional<upcxio::session::shared_bytes>{upcxio::session::shared_bytes("payernamMyvalue", 15)} };
      };
      mock_get_key_prefix = [&]() { return upcxio::chain::make_prefix_key(contract, prefix.c_str(), it_key_size); };
   ...
   void check_kv_it_key(it_state state, it_key_existing key_existing, kv_it_stat expected_status) {
      ...
      auto               sample_key_value = std::pair{ upcxio::session::shared_bytes(key.c_str(), key.size()),
                                                       std::optional<upcxio::session::shared_bytes>{upcxio::session::shared_bytes("valuevaluevalue", 15)} };
      mock_get_kv                         = [key_existing, sample_key_value]() -> std::pair<upcxio::session::shared_bytes, std::optional<upcxio::session::shared_bytes>> {
         if (key_existing == it_key_existing::YES) {
            return sample_key_value;
         } else {
            return {upcxio::session::shared_bytes{}, std::optional<upcxio::session::shared_bytes>{}};
         }
      };
      mock_get_key_prefix = [&]() { return upcxio::chain::make_prefix_key(contract, prefix.c_str(), it_key_size); };
   ...
   void check_kv_it_value(it_state state, it_key_existing key_existing, kv_it_stat expected_status) {
      ...
      auto               sample_key_value = std::pair{ upcxio::session::shared_bytes("keykeykeykeykey", 15),
                                                        std::optional<upcxio::session::shared_bytes>{upcxio::session::shared_bytes(value.c_str(), value.size())} };
      mock_get_kv                         = [&]() -> std::pair<upcxio::session::shared_bytes, std::optional<upcxio::session::shared_bytes>> {
         if (key_existing == it_key_existing::YES) {
            return sample_key_value;
         } else {
   ...
   void check_kv_it_compare(it_state state, it_exception_expected exception_expected, uint64_t rhs_contract) {
      mock_is_erased = [state]() -> bool { return state == it_state::ERASED; };
      mock_get_kv = []() {
         return std::pair{ upcxio::session::shared_bytes{}, std::optional<upcxio::session::shared_bytes>{} };
      };
      ...
      uint32_t size          = 10;
      mock_get_key_prefix = [&]() { return upcxio::chain::make_prefix_key(rhs_contract, prefix.c_str(), size); };
...
BOOST_FIXTURE_TEST_CASE(test_kv_erase, kv_rocksdb_fixture) {
   ...
   mock_get      = [v](const upcxio::session::shared_bytes&) -> std::optional<upcxio::session::shared_bytes> {
      return upcxio::session::shared_bytes(v.data(), v.size());
   };
...
BOOST_FIXTURE_TEST_CASE(test_kv_erase_key_not_exist, kv_rocksdb_fixture) {
   mock_get = [](const upcxio::session::shared_bytes&) -> std::optional<upcxio::session::shared_bytes> {
      return std::optional<upcxio::session::shared_bytes>{};
   };
...
BOOST_FIXTURE_TEST_CASE(test_kv_set_new_val, kv_rocksdb_fixture) {
   mock_get = [](const upcxio::session::shared_bytes&) -> std::optional<upcxio::session::shared_bytes> {
      return std::optional<upcxio::session::shared_bytes>{}; // Not found means we need to create new key
   };
...
BOOST_FIXTURE_TEST_CASE(test_iter_creation_key_size_too_big, kv_rocksdb_fixture) {
   mock_get_key_prefix = [&]() { return upcxio::chain::make_prefix_key(contract, prefix.c_str(), max_key_size); };
