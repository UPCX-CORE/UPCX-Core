...
namespace upcxio { namespace chain {
   void additional_block_signatures_extension::reflector_init() {
      ...
      UPCX_ASSERT( signatures.size() > 0, ill_formed_additional_block_signatures_extension,
      ...
      for( const auto& s : signatures ) {
         ...
         UPCX_ASSERT( res.second, ill_formed_additional_block_signatures_extension,
   ...
   static flat_multimap<uint16_t, block_extension> validate_and_extract_block_extensions(const extensions_type& block_extensions) {
      ...
      for( size_t i = 0; i < block_extensions.size(); ++i ) {
         ...
         UPCX_ASSERT( id >= id_type_lower_bound, invalid_block_extension,
         ...
         UPCX_ASSERT( match, invalid_block_extension,
                     "Block extension with id type ${id} is not supported",
                     ("id", id)
         );

         if( match->enforce_unique ) {
            UPCX_ASSERT( i == 0 || id > id_type_lower_bound, invalid_block_header_extension,
