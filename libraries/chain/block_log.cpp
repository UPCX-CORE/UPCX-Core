...
#include <upcxio/chain/log_data_base.hpp>
#include <upcxio/chain/log_index.hpp>
#include <upcxio/chain/log_catalog.hpp>
#include <upcxio/chain/block_log_config.hpp>
...
namespace upcxio { namespace chain {
   ...
   struct block_log_preamble {
      ...
      void read_from(fc::datastream<const char*>& ds, const fc::path& log_path) {
        ds.read((char*)&version, sizeof(version));
         UPCX_ASSERT(version > 0, block_log_exception, "Block log was not setup properly");
         UPCX_ASSERT(
         ...
         if (block_log::contains_genesis_state(version, first_block_num)) {
            ...
            UPCX_THROW(block_log_exception,
   ...
   namespace {
      ...
      log_entry_v4::metadata_type unpack(Stream& ds, signed_block& block){
         ...
         UPCX_ASSERT(compression < static_cast<uint8_t>(packed_transaction::cf_compression_type::COMPRESSION_TYPE_COUNT), block_log_exception, 
                  "Unknown compression_type");
         meta.compression = static_cast<packed_transaction::cf_compression_type>(compression);
         UPCX_ASSERT(meta.compression == packed_transaction::cf_compression_type::none, block_log_exception,
         ...
         UPCX_ASSERT(bytes_to_skip >= 0, block_log_exception,
   ...
   std::unique_ptr<signed_block> read_block(Stream&& ds, uint32_t version, uint32_t expect_block_num = 0) {
      ...
      if (expect_block_num != 0) {
         UPCX_ASSERT(!!block && block->block_num() == expect_block_num, block_log_exception,
   ...
   block_id_type read_block_id(Stream&& ds, uint32_t version, uint32_t expect_block_num) {
      if (version >= pruned_transaction_version) {
         ...
         fc::raw::unpack(ds, compression);
         UPCX_ASSERT(compression == static_cast<uint8_t>(packed_transaction::cf_compression_type::none),
                     block_log_exception, "Only \"none\" compression type is supported.");
      }
      block_header bh;
      fc::raw::unpack(ds, bh);

      UPCX_ASSERT(bh.block_num() == expect_block_num, block_log_exception,
   ...
   public:
     ...
     fc::datastream<const char*> open(const fc::path& path, mapmode mode = mapmode::readonly) {
      ...
      uint32_t block_num_at(uint64_t position) const {
         ...
         UPCX_ASSERT(position <= size(), block_log_exception, "Invalid block position ${position}", ("position", position));
      ...
      void light_validate_block_entry_at(uint64_t pos, uint32_t expected_block_num) const {
         ...
         UPCX_ASSERT(actual_block_num == expected_block_num, block_log_exception,
         ...
         if (version() >= pruned_transaction_version) {
            ...
            UPCX_ASSERT(pos == entry_position, block_log_exception, 
      ...
      full_validate_block_entry(fc::datastream<const char*>& ds, uint32_t previous_block_num, const block_id_type& previous_block_id, log_entry& entry) {
         ...
         UPCX_ASSERT(pos == tmp_pos, block_log_exception, "the block position for block ${num} at the end of a block entry is incorrect", ("num", block_num));
   ...
   using block_log_index = upcxio::chain::log_index<block_log_exception>;
   ...
   struct reverse_block_position_iterator {
      ...
      reverse_block_position_iterator& operator++() {
         UPCX_ASSERT(current_position > begin_position && current_position < data.size(), block_log_exception,
   ...
   void block_log_data::construct_index(const fc::path& index_file_path) {
      ...
      UPCX_ASSERT(blocks_found == num_blocks, block_log_exception,
   ...
   struct block_log_verifier {
      ...
      void verify(const block_log_data& log, const boost::filesystem::path& log_path) {
         if (chain_id.empty()) {
            ...
            UPCX_ASSERT(chain_id == log.chain_id(), block_log_exception,
                       "block log file ${path} has a different chain id", ("path", log_path.generic_string()));
         }
      }
   };
   using block_log_catalog = upcxio::chain::log_catalog<block_log_data, block_log_index, block_log_verifier>;
   ...
   : stride( config.stride )
   {
      ...
      if (log_size) {
         ...
         UPCX_ASSERT(catalog.verifier.chain_id.empty() || catalog.verifier.chain_id == preamble.chain_id(), block_log_exception,
   ...
   std::vector<char> create_block_buffer( const signed_block& b, uint32_t version, packed_transaction::cf_compression_type segment_compression ) {
      ...
      if (version >= pruned_transaction_version)  {
         ...
         UPCX_ASSERT(block_ptr, block_log_append_fail, "Unable to convert block to legacy format");
         UPCX_ASSERT(segment_compression == packed_transaction::cf_compression_type::none, block_log_append_fail,
   ...
   uint64_t detail::block_log_impl::append(const signed_block_ptr& b, packed_transaction::cf_compression_type segment_compression) {
      try {
         UPCX_ASSERT( genesis_written_to_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );

         block_file.seek_end(0);
         index_file.seek_end(0);
         UPCX_ASSERT(index_file.tellp() == sizeof(uint64_t) * (b->block_num() - preamble.first_block_num),
   ...
   void block_log::reset(const chain_id_type& chain_id, uint32_t first_block_num) {
      UPCX_ASSERT(first_block_num > 1, block_log_exception,
                 "Block log version ${ver} needs to be created with a genesis state if starting from block number 1.");

      UPCX_ASSERT(my->catalog.verifier.chain_id.empty() || chain_id == my->catalog.verifier.chain_id, block_log_exception,
   ...
   fc::path block_log::repair_log(const fc::path& data_dir, uint32_t truncate_at_block, const char* reversible_block_dir_name) {
      ...
      UPCX_ASSERT(fc::is_directory(data_dir) && fc::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
      ...
      UPCX_ASSERT(!fc::exists(backup_dir), block_log_backup_dir_exist,
   ...
   size_t prune_trxs(fc::datastream<char*> strm, uint32_t block_num, std::vector<transaction_id_type>& ids, uint32_t version) {
      ...
      UPCX_ASSERT(version >= pruned_transaction_version, block_log_exception,
      ...
      UPCX_ASSERT(entry.block.block_num() == block_num, block_log_exception,
                     "Wrong block was read from block log.");
   ...
   size_t block_log::prune_transactions(uint32_t block_num, std::vector<transaction_id_type>& ids) {
      ...
      UPCX_ASSERT(pos != npos, block_log_exception, "Specified block_num ${block_num} does not exist in block log.",
                 ("block_num", block_num));
   ...
   bool block_log::trim_blocklog_front(const fc::path& block_dir, const fc::path& temp_dir, uint32_t truncate_at_block) {
      UPCX_ASSERT( block_dir != temp_dir, block_log_exception, "block_dir and temp_dir need to be different directories" );
