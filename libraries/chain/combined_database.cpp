...
#include <upcxio/chain/backing_store/kv_context_chainbase.hpp>
...
#include <upcxio/chain/backing_store/db_context.hpp>
#include <upcxio/chain/backing_store/db_key_value_format.hpp>

namespace upcxio { namespace chain {
   combined_session::combined_session(chainbase::database& cb_database, upcxio::session::undo_stack<rocks_db_type>* undo_stack)
   ...
   void add_kv_table_to_snapshot(const snapshot_writer_ptr& snapshot, const chainbase::database& db, const kv_undo_stack_ptr& kv_undo_stack) {
      snapshot->write_section<kv_object>([&db,&kv_undo_stack](auto& section) {
         if (kv_undo_stack && db.get<kv_db_config_object>().backing_store == backing_store_type::ROCKSDB) {
            ...
            const auto begin_key = upcxio::session::shared_bytes(&backing_store::rocksdb_contract_kv_prefix, 1);
   ...
   void read_kv_table_from_snapshot(const snapshot_reader_ptr& snapshot, chainbase::database& db,
      ...
      if (backing_store == backing_store_type::ROCKSDB) {
         auto key_values = std::vector<std::pair<upcxio::session::shared_bytes, upcxio::session::shared_bytes>>{};
         ...
         snapshot->read_section<kv_object>([&key_values, &db, &kv_database](auto& section) {
            ...
            while (more) {
               ...
               auto full_key =
                     upcxio::session::make_shared_bytes<std::string_view, 3>({prefix_key,
   ...
   combined_database::combined_database(chainbase::database& chain_db,
       ...
       : backing_store(backing_store_type::ROCKSDB), db(chain_db), kv_database{ [&]() {
            ...
            return std::make_unique<rocks_db_type>(upcxio::session::make_session(std::move(rdb), 1024));
         }() },
         kv_undo_stack(std::make_unique<upcxio::session::undo_stack<rocks_db_type>>(*kv_database, cfg.state_dir)),
   ...
   void combined_database::check_backing_store_setting(bool clean_startup) {
      if (backing_store != db.get<kv_db_config_object>().backing_store) {   
         UPCX_ASSERT(clean_startup, database_move_kv_disk_exception,
   ...
   std::unique_ptr<kv_context> combined_database::create_kv_context(name receiver, kv_resource_manager resource_manager,
      ...
      UPCX_ASSERT(false, action_validate_exception, "Unknown backing store.");
   }
   ...
   void combined_database::add_to_snapshot(
         const upcxio::chain::snapshot_writer_ptr& snapshot, const upcxio::chain::block_state& head,
         const upcxio::chain::authorization_manager&                    authorization,
         const upcxio::chain::resource_limits::resource_limits_manager& resource_limits) const {
      ...
      upcxio::chain::controller_index_set::walk_indices([this, &snapshot](auto utils) {
   ...
   void combined_database::read_from_snapshot(const snapshot_reader_ptr& snapshot,
                                              uint32_t blog_start,
                                              uint32_t blog_end,
                                              upcxio::chain::authorization_manager& authorization,
                                              upcxio::chain::resource_limits::resource_limits_manager& resource_limits,
                                              upcxio::chain::fork_database& fork_db, upcxio::chain::block_state_ptr& head,
                                              uint32_t&                          snapshot_head_block,
                                              const upcxio::chain::chain_id_type& chain_id) {
      ...
      { /// load and upgrade the block header state
         ...
         UPCX_ASSERT(blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end, block_log_exception,
      ...
      controller_index_set::walk_indices([this, &snapshot, &header](auto utils) {
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version) {
               ...
               UPCX_ASSERT(genesis, snapshot_exception,
      ...
      UPCX_ASSERT(gpo.chain_id == chain_id, chain_id_type_exception,
   ...
   void combined_database::add_contract_tables_to_snapshot(const snapshot_writer_ptr& snapshot) const {
      snapshot->write_section("contract_tables", [this](auto& section) {
         if (kv_undo_stack && db.get<kv_db_config_object>().backing_store == backing_store_type::ROCKSDB) {
            ...
            const auto begin_key = upcxio::session::shared_bytes(&backing_store::rocksdb_contract_db_prefix, 1);
            const auto end_key = begin_key.next();
            backing_store::walk_rocksdb_entries_with_prefix(kv_undo_stack, begin_key, end_key, writer);
         }
         else {
   ...
   void rocksdb_read_contract_tables_from_snapshot(rocks_db_type& kv_database, chainbase::database& db,
      ...
      std::vector<std::pair<upcxio::session::shared_bytes, upcxio::session::shared_bytes>> batch;
      bool                more     = !section.empty();
      ...
      while (more) {
         ...
         auto write_secondary_index = [&put, &read_row](auto index) {
            ...
            static const upcxio::session::shared_bytes  empty_payload;
            unsigned_int       size;
            read_row(size);
   ...
   std::optional<upcxio::chain::genesis_state> extract_legacy_genesis_state(snapshot_reader& snapshot,
                                                                           uint32_t         version) {
      std::optional<upcxio::chain::genesis_state> genesis;
      using v2 = legacy::snapshot_global_property_object_v2;

      if (std::clamp(version, v2::minimum_version, v2::maximum_version) == version) {
         genesis.emplace();
         snapshot.read_section<upcxio::chain::genesis_state>(
