...
#include <upcx/chain/protocol_feature_manager.hpp>
#include <upcx/chain/authorization_manager.hpp>
#include <upcx/chain/resource_limits.hpp>
#include <upcx/chain/thread_utils.hpp>
#include <upcx/chain/platform_timer.hpp>
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( upcx, upcx, newaccount );
   SET_APP_HANDLER( upcx, upcx, setcode );
   SET_APP_HANDLER( upcx, upcx, setabi );
   SET_APP_HANDLER( upcx, upcx, updateauth );
   SET_APP_HANDLER( upcx, upcx, deleteauth );
   SET_APP_HANDLER( upcx, upcx, linkauth );
   SET_APP_HANDLER( upcx, upcx, unlinkauth );
/*
   SET_APP_HANDLER( upcx, upcx, postrecovery );
   SET_APP_HANDLER( upcx, upcx, passrecovery );
   SET_APP_HANDLER( upcx, upcx, vetorecovery );
*/

   SET_APP_HANDLER( upcx, upcx, canceldelay );
   }
   ...
   void create_native_account( const fc::time_point& initial_timestamp, account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            // The initial upcx ABI value affects consensus; see  https://github.com/UPCX/upcx/issues/7794
            // TODO: This doesn't charge RAM; a fix requires a consensus upgrade.
            a.abi.assign(upcx_abi_bin, sizeof(upcx_abi_bin));
         }
      });
...
upcx::chain::combined_database& controller::kv_db() const { return my->kv_db; }
