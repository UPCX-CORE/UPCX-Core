...
namespace chain { namespace backing_store { namespace db_key_value_format {
   using key256_t = std::array<upcx::chain::uint128_t, 2>;
   ...
   namespace detail {
      ...
      template<>
      struct determine_sec_type<upcx::chain::uint128_t> { static constexpr key_type kt = key_type::sec_i128; };
      ...
      template<>
      struct associated<upcx::session::shared_bytes> {
         using stored_key = upcx::session::shared_bytes;
      };
      ...
      constexpr std::size_t prefix_size() {
         ...
         if constexpr (std::is_same_v<CharKey, upcx::session::shared_bytes>) {
      ...
      constexpr void consistent_keys() {
         if constexpr (std::is_same_v<CharKey1, CharKey2>) {
            ...
            static_assert(!std::is_same_v<CharKey1, upcx::session::shared_bytes>);
            static_assert(!std::is_same_v<CharKey2, upcx::session::shared_bytes>);
         }
      }
   ...
   upcx::session::shared_bytes create_table_key(const upcx::session::shared_bytes& prefix_key);
   ...
   b1::chain_kv::bytes extract_legacy_key(const upcx::session::shared_bytes& complete_key);

   rocksdb::Slice extract_legacy_slice(const upcx::session::shared_bytes& complete_key);

   upcx::session::shared_bytes create_full_key(const b1::chain_kv::bytes& ck, name code);
   enum class end_of_prefix { pre_type, at_type, at_prim_to_sec_type, at_prim_to_sec_primary_key };
   upcx::session::shared_bytes create_full_key_prefix(const upcx::session::shared_bytes& ck, end_of_prefix prefix_end);
   ...
   full_key_data parse_full_key(const upcx::session::shared_bytes& full_key);
   ...
   upcx::session::shared_bytes create_full_primary_key(name code, name scope, name table, uint64_t primary_key);
   upcx::session::shared_bytes create_full_prefix_key(name code, name scope, name table, std::optional<key_type> kt = std::optional<key_type>{});

   template<typename Key>
   upcx::session::shared_bytes create_full_secondary_key(name code, name scope, name table, const Key& sec_key, uint64_t primary_key) {
      bytes composite_key = create_secondary_key(scope, table, sec_key, primary_key);
      return create_full_key(composite_key, code);
   }

   template<typename Key>
   upcx::session::shared_bytes create_full_prefix_secondary_key(name code, name scope, name table, const Key& sec_key) {
      bytes composite_key = create_prefix_secondary_key(scope, table, sec_key);
      return create_full_key(composite_key, code);
   }

}}}} // ns upcx::chain::backing_store::db_key_value_format
