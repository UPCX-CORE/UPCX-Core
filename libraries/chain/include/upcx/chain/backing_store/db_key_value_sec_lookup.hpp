...
namespace upcx { namespace chain { namespace backing_store {
   ...
   struct secondary_helper<uint64_t> {
      ...
      upcx::session::shared_bytes value(const uint64_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
      void extract(const upcx::session::shared_bytes& payload, uint64_t& sec_key, name& payer) {
   ...
   template<>
   struct secondary_helper<upcx::chain::uint128_t> {
      const char* desc() { return "idx128"; }
      upcx::session::shared_bytes value(const upcx::chain::uint128_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
      void extract(const upcx::session::shared_bytes& payload, upcx::chain::uint128_t& sec_key, name& payer) {
         payer_payload pp(payload);
         payer = pp.payer;
         UPCX_ASSERT( pp.value_size == 0, db_rocksdb_invalid_operation_exception,
                     "Payload should not have anything more than the payer");
   ...
   template<>
   struct secondary_helper<upcx::chain::key256_t> {
      const char* desc() { return "idx256"; }
      upcx::session::shared_bytes value(const upcx::chain::key256_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
      void extract(const upcx::session::shared_bytes& payload, upcx::chain::key256_t& sec_key, name& payer) {
         payer_payload pp(payload);
         payer = pp.payer;
         UPCX_ASSERT( pp.value_size == 0, db_rocksdb_invalid_operation_exception,
                     "Payload should not have anything more than the payer");
   ...
   struct secondary_helper<float64_t> {
      ...
      upcx::session::shared_bytes value(const float64_t& sec_key, name payer) {
      ...
      void extract(const upcx::session::shared_bytes& payload, float64_t& sec_key, name& payer) {
         payer_payload pp(payload);
   ...
   struct secondary_helper<float128_t> {
      ...
      upcx::session::shared_bytes value(const float128_t& sec_key, name payer) {
         constexpr static auto value_size = sizeof(sec_key);
      ...
      void extract(const upcx::session::shared_bytes& payload, float128_t& sec_key, name& payer) {
         payer_payload pp(payload);
         UPCX_ASSERT( pp.value_size == sizeof(sec_key), db_rocksdb_invalid_operation_exception,
