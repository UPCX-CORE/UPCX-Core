...
#include <upcx/chain/backing_store.hpp>
...
#include <upcx/chain/database_header_object.hpp>
#include <upcx/chain/generated_transaction_object.hpp>
#include <upcx/chain/genesis_intrinsics.hpp>
...
namespace upcx { namespace chain {
   ...
   class combined_session {
    public:
      ...
      combined_session(chainbase::database& cb_database, upcx::session::undo_stack<rocks_db_type>* undo_stack);
    ...
    private:
      ...
      upcx::session::undo_stack<rocks_db_type>*     kv_undo_stack = nullptr;
   };
   ...
   class combined_database {
    public:
      ...
      void add_to_snapshot(const upcx::chain::snapshot_writer_ptr& snapshot, const upcx::chain::block_state& head,
      ...
      void read_from_snapshot(const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end,
                              ...
                              uint32_t& snapshot_head_block, const upcx::chain::chain_id_type& chain_id);
   ...
   std::optional<upcx::chain::genesis_state> extract_legacy_genesis_state(snapshot_reader& snapshot, uint32_t version);
