...
namespace upcx { namespace chain { namespace config {
...
const static name system_account_name    { "upcx"_n };
const static name null_account_name      { "upcx.null"_n };
const static name producers_account_name { "upcx.prods"_n };
...
const static name upcx_auth_scope       { "upcx.auth"_n };
const static name upcx_all_scope        { "upcx.all"_n };

const static name active_name     { "active"_n };
const static name owner_name      { "owner"_n };
const static name upcx_any_name  { "upcx.any"_n };
const static name upcx_code_name { "upcx.code"_n };
const static name upcx_token_name { "upcx.token"_n };
const static name upcx_swap_name { "upcx.swap"_n };
const static name upcx_fee_name { "upcx.fee"_n };
...
const static uint32_t   default_target_block_net_usage_pct           = 100 * percent_1; /// we target 1000 TPS
...
const static upcx::chain::wasm_interface::vm_type default_wasm_runtime = upcx::chain::wasm_interface::vm_type::upcx_vm_jit;
const static uint32_t   default_abi_serializer_max_time_us = 1500*1000; ///< default deadline for abi serialization methods
...
} } } // namespace upcx::chain::config

constexpr uint64_t UPCX_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / upcx::chain::config::percent_100;
}

template<typename Number>
Number UPCX_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + upcx::chain::config::percent_100 - upcx::chain::config::percent_1)  / upcx::chain::config::percent_100;
}
