...
using namespace fc;
using namespace upcx::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
      ...
      wasm_interface_impl(wasm_interface::vm_type vm, bool upcxvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const upcxvmoc::config& upcxvmoc_config) : db(d), wasm_runtime_time(vm) {
         ...
         if(vm == wasm_interface::vm_type::upcx_vm)
            runtime_interface = std::make_unique<webassembly::upcx_vm_runtime::upcx_vm_runtime<upcx::vm::interpreter>>();
#endif
#ifdef UPCX_UPCX_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::upcx_vm_jit)
            runtime_interface = std::make_unique<webassembly::upcx_vm_runtime::upcx_vm_runtime<upcx::vm::jit>>();
#endif
#ifdef UPCX_UPCX_VM_OC_RUNTIME_ENABLED
