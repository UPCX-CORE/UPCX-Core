...
namespace upcx { namespace chain { namespace webassembly {
   namespace detail {
      ...
      struct is_whitelisted_type<vm::span<T>> {
         ...
         // If we wish to expand to general span<T> in the future, changes are needed in UPCX VM
   ...
   UPCX_VM_PRECONDITION(core_precondition,
         UPCX_VM_INVOKE_ON_ALL(([&](auto&& arg, auto&&... rest) {
            ...
            if constexpr (is_span_type_v<arg_t> || vm::is_argument_proxy_type_v<arg_t>) {
               upcx::vm::invoke_on<false, upcx::vm::invoke_on_all_t>([&arg](auto&& narg, auto&&... nrest) {
                  using nested_arg_t = std::decay_t<decltype(narg)>;
                  if constexpr (upcx::vm::is_span_type_v<nested_arg_t> || vm::is_argument_proxy_type_v<nested_arg_t>)
