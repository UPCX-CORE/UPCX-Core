...
#include <upcx/chain/webassembly/upcx-vm-oc/upcx-vm-oc.hpp>
...
using namespace upcx::chain::upcxvmoc;

class upcxvmoc_instantiated_module;

class upcxvmoc_runtime : public upcx::chain::wasm_runtime_interface {
   public:
...
struct upcx_vm_oc_execution_interface {
   ...
   upcx::vm::native_value* os;
};

struct upcx_vm_oc_type_converter : public upcx::vm::type_converter<webassembly::interface, upcx_vm_oc_execution_interface> {
   using base_type = upcx::vm::type_converter<webassembly::interface, upcx_vm_oc_execution_interface>;
   ...
   UPCX_VM_FROM_WASM(memcpy_params, (vm::wasm_ptr_t dst, vm::wasm_ptr_t src, vm::wasm_size_t size)) {
   ...
   UPCX_VM_FROM_WASM(memcmp_params, (vm::wasm_ptr_t lhs, vm::wasm_ptr_t rhs, vm::wasm_size_t size)) {
     auto l = array_ptr_impl<char>(lhs, size);
     auto r = array_ptr_impl<char>(rhs, size);
     return { l, r, size };
   }

   UPCX_VM_FROM_WASM(memset_params, (vm::wasm_ptr_t dst, int32_t val, vm::wasm_size_t size)) {
     auto d = array_ptr_impl<char>(dst, size);
     array_ptr_impl<char>(dst, 1);
     return { d, val, size };
   }
   ...
   auto from_wasm(vm::wasm_ptr_t ptr, vm::tag<T> = {}) const
      ...
      if constexpr(T::is_legacy()) {
         UPCX_ASSERT(ptr != 0, wasm_execution_error, "references cannot be created for null pointers");
      }
      void* p = array_ptr_impl<typename T::pointee_type>(ptr, 1);
      return {p};
   }

   UPCX_VM_FROM_WASM(null_terminated_ptr, (vm::wasm_ptr_t ptr)) {
...
auto fn(A... a) {
   try {
      ...
      return result_resolver{tc}, upcx::vm::invoke_with_host<F, Preconditions, native_args>(tc, &host, std::make_index_sequence<sizeof...(A)>());
   }
   catch(...) {
...
void register_upcxvm_oc(Name n) {
   ...
   if(n == BOOST_HANA_STRING("env.upcx_exit")) return;
...
} } } }// upcx::chain::webassembly::upcxvmoc
