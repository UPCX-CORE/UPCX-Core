...
#include <upcx/chain/webassembly/upcx-vm-oc/gs_seg_helpers.h>
...
class memory {
      ...
      static constexpr uint64_t mutable_global_size               = 8u  * upcx::chain::wasm_constraints::maximum_mutable_globals/4u;
      static constexpr uint64_t table_size                        = 16u * upcx::chain::wasm_constraints::maximum_table_elements;
      static constexpr size_t   wcb_allowance                     = 512u;
      static_assert(sizeof(control_block) <= wcb_allowance, "UPCX VM OC memory doesn't set aside enough memory for control block");
...
#define OFFSET_OF_CONTROL_BLOCK_MEMBER(M) (-(int)upcx::chain::upcxvmoc::memory::cb_offset + (int)offsetof(upcx::chain::upcxvmoc::control_block, M))
#define OFFSET_OF_FIRST_INTRINSIC ((int)-upcx::chain::upcxvmoc::memory::first_intrinsic_offset)
