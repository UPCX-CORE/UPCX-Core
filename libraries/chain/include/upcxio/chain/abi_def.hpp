...
abi_def upcxio_contract_abi(const abi_def& upcxio_system_abi);
vector<type_def> common_type_defs();

extern unsigned char upcxio_abi_bin[2132];
...
ST& operator << (ST& s, const upcxio::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
ST& operator >> (ST& s, upcxio::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const upcxio::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, upcxio::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}


template<typename ST, typename T>
ST& operator << (ST& s, const upcxio::chain::kv_tables_as_object<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
ST& operator >> (ST& s, upcxio::chain::kv_tables_as_object<T>& v) {
   if( s.remaining() )
      raw::unpack(s, v.value);
   return s;
}
...
template<typename T>
void to_variant(const upcxio::chain::kv_tables_as_object<T>& o, fc::variant& v) {
...
void from_variant(const fc::variant& v, upcxio::chain::kv_tables_as_object<T>& o) {
    ...
    for( const auto& table_it : tables ) {
        ...
        upcxio::chain::kv_table_def kv_tbl_def;
        ...
        if( const auto st_it = table_obj.find("secondary_indices"); st_it != table_obj.end() ) {
            ...
            for( const auto& sidx_it : sec_indices_obj ) {
                upcxio::chain::secondary_index_def idx_def;
                from_variant(sidx_it.value(), idx_def);
                kv_tbl_def.secondary_indices[upcxio::chain::index_name(sidx_it.key())] = idx_def;
            }
        }
        kv_tables[upcxio::chain::name(table_it.key())] = kv_tbl_def;
    }
}
} // namespace fc

FC_REFLECT( upcxio::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( upcxio::chain::field_def                        , (name)(type) )
FC_REFLECT( upcxio::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( upcxio::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( upcxio::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( upcxio::chain::primary_index_def                , (name)(type) )
FC_REFLECT( upcxio::chain::secondary_index_def              , (type) )
FC_REFLECT( upcxio::chain::kv_table_def                     , (type)(primary_index)(secondary_indices) )
FC_REFLECT( upcxio::chain::clause_pair                      , (id)(body) )
FC_REFLECT( upcxio::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( upcxio::chain::variant_def                      , (name)(types) )
FC_REFLECT( upcxio::chain::action_result_def                , (name)(result_type) )
FC_REFLECT( upcxio::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
