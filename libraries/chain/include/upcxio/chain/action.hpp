...
namespace upcxio { namespace chain {
    ...
    *  In the context of upcxio, every action is dispatched to the handler defined
   ...
   struct action : public action_base {
      ...
      template<typename T>
      T data_as()const {
         UPCX_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         UPCX_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( upcxio::chain::permission_level, (actor)(permission) )
FC_REFLECT( upcxio::chain::action_base, (account)(name)(authorization) )
FC_REFLECT_DERIVED( upcxio::chain::action, (upcxio::chain::action_base), (data) )
