...
#include <upcxio/chain/backing_store/db_chainbase_iter_store.hpp>
#include <upcxio/chain/backing_store/db_secondary_key_helper.hpp>
...
//               context.require_write_lock( scope );
            ...
            void remove( int iterator ) {
               ...
               UPCX_ASSERT( table_obj.code == context.receiver, table_access_violation, "db access violation" );
...
# warning look into if we can make any of the db_** methods and idx***'s methods const and provide a const interface
   ...
   public:
      ...
      kv_context& kv_get_backing_store() {
         UPCX_ASSERT( kv_backing_store, action_validate_exception, "KV APIs cannot access state (null backing_store)" );
