...
#include <upcxio/chain/symbol.hpp>
...
struct asset : fc::reflect_init
{
   ...
   explicit asset(share_type a = 0, symbol id = symbol(CORE_SYMBOL)) :amount(a), sym(id) {
      UPCX_ASSERT( is_amount_within_range(), asset_type_exception, "magnitude of asset amount must be less than 2^62" );
      UPCX_ASSERT( sym.valid(), asset_type_exception, "invalid symbol" );
   }
   ...
   {
      UPCX_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      amount += o.amount;
      return *this;
   }
   ...
   asset& operator -= (const asset& o)
   {
      UPCX_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      amount -= o.amount;
      return *this;
   }
   ...
   friend bool operator < (const asset& a, const asset& b)
   {
      UPCX_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "logical operation between two different asset is not allowed");
   ...
   friend asset operator - (const asset& a, const asset& b) {
      UPCX_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      return asset(a.amount - b.amount, a.get_symbol());
   }

   friend asset operator + (const asset& a, const asset& b) {
      UPCX_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      return asset(a.amount + b.amount, a.get_symbol());
   }
...
namespace fc {
inline void to_variant(const upcxio::chain::asset& var, fc::variant& vo) { vo = var.to_string(); }
inline void from_variant(const fc::variant& var, upcxio::chain::asset& vo) {
   vo = upcxio::chain::asset::from_string(var.get_string());
}
}

namespace fc {
inline void from_variant(const fc::variant& var, upcxio::chain::extended_asset& vo) {
   if( var.is_array() ) {
...
FC_REFLECT(upcxio::chain::asset, (amount)(sym))
FC_REFLECT(upcxio::chain::extended_asset, (quantity)(contract) )
