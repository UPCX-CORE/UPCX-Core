...
namespace upcxio { namespace chain { namespace backing_store {
   ...
   inline static uint32_t actual_value_size(const uint32_t raw_value_size) {
      UPCX_ASSERT(raw_value_size >= payer_in_value_size, kv_rocksdb_bad_value_size_exception , "The size of value returned from RocksDB is less than payer's size");
   ...
   struct payer_payload {
      ...
      upcxio::session::shared_bytes as_payload() const {
         char payer_buf[payer_in_value_size];
         memcpy(payer_buf, &payer, payer_in_value_size);
         return upcxio::session::make_shared_bytes<std::string_view, 2>({std::string_view{payer_buf,
                                                                                         payer_in_value_size},
