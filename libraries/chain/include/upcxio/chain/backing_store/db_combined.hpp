...
using rocks_db_type = upcxio::session::session<upcxio::session::rocksdb_t>;
using session_type = upcxio::session::session<rocks_db_type>;
using kv_undo_stack_ptr = std::unique_ptr<upcxio::session::undo_stack<rocks_db_type>>;
...
template <typename F>
bool read_rocksdb_entry(const upcxio::session::shared_bytes& actual_db_kv_key,
                        const upcxio::session::shared_bytes& value,
                        F& function) {
   uint64_t    contract;
   constexpr std::size_t type_size = 1;
   std::size_t key_prefix_size = type_size + sizeof(contract);
   UPCX_ASSERT(actual_db_kv_key.size() >= key_prefix_size, database_exception, "Unexpected key in rocksdb");
...
public:
   ...
   void extract_primary_index(b1::chain_kv::bytes::const_iterator remaining,
      ...
      if (!is_table_only()) {
         uint64_t primary_key;
         UPCX_ASSERT(b1::chain_kv::extract_key(remaining, key_end, primary_key), bad_composite_key_exception,
   ...
   void extract_secondary_index(b1::chain_kv::bytes::const_iterator remaining,
      ...
      if (!is_table_only()) {
         ...
         uint64_t primary_key;
         UPCX_ASSERT(b1::chain_kv::extract_key(remaining, key_end, secondary_key), bad_composite_key_exception,
                    "DB intrinsic key-value store composite key is malformed");
         UPCX_ASSERT(b1::chain_kv::extract_key(remaining, key_end, primary_key), bad_composite_key_exception,
                    "DB intrinsic key-value store composite key is malformed");
         backing_store::payer_payload pp(value, value_size);
...
namespace detail {
   ...
   class iterator_pair {
   public:
      iterator_pair(const upcxio::session::shared_bytes& begin_key,
                    const upcxio::session::shared_bytes& end_key,
                    bool is_reverse,
                    kv_undo_stack_ptr::element_type::variant_type& session) : is_reverse_(is_reverse) {
         UPCX_ASSERT(begin_key < end_key, database_exception, "Invalid iterator_pair request: begin_key was greater than or equal to end_key.");
         if (is_reverse_) {
            ...
            if (current_ == session.end() || (*current_).first > end_key) {
               --current_;
               UPCX_ASSERT(current_ != session.end(), database_exception, "iterator_pair: failed to find lower bound of end_key");
            }
...
void walk_rocksdb_entries_with_prefix(const kv_undo_stack_ptr& kv_undo_stack,
                                      const upcxio::session::shared_bytes& begin_key,
                                      const upcxio::session::shared_bytes& end_key,
                                      F& function) {
...
bool process_rocksdb_entry(Session& session,
                           const upcxio::session::shared_bytes& key,
                           Receiver& receiver) {
   if (!key) {
      return false;
   }
...
const char* contract_table_type() {
   ...
   else if constexpr (std::is_same_v<Object, secondary_index_view<upcxio::chain::uint128_t>>) {
      return "secondary_index_view<uint128_t>";
   }
   else if constexpr (std::is_same_v<Object, secondary_index_view<upcxio::chain::key256_t>>) {
...
FC_REFLECT(upcxio::chain::backing_store::table_id_object_view, (code)(scope)(table)(payer)(count) )
FC_REFLECT(upcxio::chain::backing_store::primary_index_view, (primary_key)(payer)(value) )
REFLECT_SECONDARY(upcxio::chain::backing_store::secondary_index_view<uint64_t>)
REFLECT_SECONDARY(upcxio::chain::backing_store::secondary_index_view<upcxio::chain::uint128_t>)
REFLECT_SECONDARY(upcxio::chain::backing_store::secondary_index_view<upcxio::chain::key256_t>)
REFLECT_SECONDARY(upcxio::chain::backing_store::secondary_index_view<float64_t>)
REFLECT_SECONDARY(upcxio::chain::backing_store::secondary_index_view<float128_t>)

namespace fc {
   inline
   void to_variant( const upcxio::chain::backing_store::blob& b, variant& v ) {
      v = variant(base64_encode(b.str_data.data(), b.str_data.size()));
   }

   inline
   void from_variant( const variant& v, upcxio::chain::backing_store::blob& b ) {
