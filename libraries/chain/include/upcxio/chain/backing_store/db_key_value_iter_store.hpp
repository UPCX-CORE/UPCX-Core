...
class db_key_value_iter_store {
   public:
      ...
      int get_end_iterator_by_table( const unique_table& tobj )const {
         auto itr = _table_cache.find(tobj);
         UPCX_ASSERT( itr != _table_cache.end(), table_not_in_cache, "an invariant was broken, table should be in cache" );
         return itr->second;
      }

      const unique_table* find_table_by_end_iterator( int ei )const {
         UPCX_ASSERT( ei < invalid_iterator(), invalid_table_iterator, "not an end iterator" );
      ...
      const unique_table& get_table( const secondary_obj_type& obj )const {
         UPCX_ASSERT( obj.table_ei < invalid_iterator(), invalid_table_iterator, "not an end iterator" );
         auto indx = end_iterator_to_index(obj.table_ei);
         UPCX_ASSERT( indx < _end_iterator_to_table.size(), table_not_in_cache,
      ...
      const secondary_obj_type& get( int iterator ) const {
         ...
         UPCX_ASSERT( result, table_operation_not_permitted, "dereference of deleted object" );
         return *result;
      }
      ...
      int add( const secondary_obj_type& obj ) {
         ...
         UPCX_ASSERT( indx < _end_iterator_to_table.size(), invalid_table_iterator, "an invariant was broken, table should be in cache" );
   ...
   private:
      void validate_object_iterator(int iterator, const char* explanation_for_no_end_iterators) const {
         UPCX_ASSERT( iterator != invalid_iterator(), invalid_table_iterator, "invalid iterator" );
         UPCX_ASSERT( iterator >= 0, table_operation_not_permitted, explanation_for_no_end_iterators );
         UPCX_ASSERT( (size_t)iterator < _iterator_to_object.size(), invalid_table_iterator, "iterator out of range" );
      }
