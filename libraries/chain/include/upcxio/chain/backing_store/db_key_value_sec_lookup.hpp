...
namespace upcxio { namespace chain { namespace backing_store {
   ...
   struct secondary_helper<uint64_t> {
      ...
      upcxio::session::shared_bytes value(const uint64_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
      void extract(const upcxio::session::shared_bytes& payload, uint64_t& sec_key, name& payer) {
         payer_payload pp(payload);
         payer = pp.payer;
         UPCX_ASSERT( pp.value_size == 0, db_rocksdb_invalid_operation_exception,
   ...
   template<>
   struct secondary_helper<upcxio::chain::uint128_t> {
      const char* desc() { return "idx128"; }
      upcxio::session::shared_bytes value(const upcxio::chain::uint128_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
      void extract(const upcxio::session::shared_bytes& payload, upcxio::chain::uint128_t& sec_key, name& payer) {
         payer_payload pp(payload);
         payer = pp.payer;
         UPCX_ASSERT( pp.value_size == 0, db_rocksdb_invalid_operation_exception,
                     "Payload should not have anything more than the payer");
   ...
   template<>
   struct secondary_helper<upcxio::chain::key256_t> {
      const char* desc() { return "idx256"; }
      upcxio::session::shared_bytes value(const upcxio::chain::key256_t& sec_key, name payer) {
         return payer_payload(payer, nullptr, 0).as_payload();
      }
      void extract(const upcxio::session::shared_bytes& payload, upcxio::chain::key256_t& sec_key, name& payer) {
         payer_payload pp(payload);
         payer = pp.payer;
         UPCX_ASSERT( pp.value_size == 0, db_rocksdb_invalid_operation_exception,
                     "Payload should not have anything more than the payer");
   ...
   struct secondary_helper<float64_t> {
      ...
      upcxio::session::shared_bytes value(const float64_t& sec_key, name payer) {
      ...
      void extract(const upcxio::session::shared_bytes& payload, float64_t& sec_key, name& payer) {
         payer_payload pp(payload);
         UPCX_ASSERT( pp.value_size == sizeof(sec_key), db_rocksdb_invalid_operation_exception,
   ...
   struct secondary_helper<float128_t> {
      ...
      upcxio::session::shared_bytes value(const float128_t& sec_key, name payer) {
         constexpr static auto value_size = sizeof(sec_key);
      ...
      void extract(const upcxio::session::shared_bytes& payload, float128_t& sec_key, name& payer) {
         payer_payload pp(payload);
         UPCX_ASSERT( pp.value_size == sizeof(sec_key), db_rocksdb_invalid_operation_exception,
   ...
   public:
      ...
      int store( name scope, name table, const account_name& payer,
         ...
         UPCX_ASSERT( !old_value, db_rocksdb_invalid_operation_exception, "db_${d}_store called with pre-existing key", ("d", helper.desc()));
         ...
         UPCX_ASSERT( !match_prefix(secondary_key.prefix_primary_to_sec_key, session_iter), db_rocksdb_invalid_operation_exception,
      ...
      void remove( int iterator ) {
         ...
         UPCX_ASSERT( table.contract == parent.receiver, table_access_violation, "db access violation" );
         ...
         UPCX_ASSERT( old_value, db_rocksdb_invalid_operation_exception,
                     "invariant failure in db_${d}_remove, iter store found to update but nothing in database", ("d", helper.desc()));

         auto session_iter = current_session.lower_bound(secondary_key.prefix_primary_to_sec_key);
         UPCX_ASSERT( match_prefix(secondary_key.full_primary_to_sec_key, session_iter), db_rocksdb_invalid_operation_exception,
      ...
      void update( int iterator, account_name payer, const SecondaryKey& secondary ) {
         ...
         UPCX_ASSERT( primary_sec_uesless_value, db_rocksdb_invalid_operation_exception,
...
#warning currently using useless_value to distinguish between no key and no value, eventually will just store empty value
      ...
      int find_secondary( name code, name scope, name table, const SecondaryKey& secondary, uint64_t& primary ) {
         ...
         UPCX_ASSERT( valid_key, db_rocksdb_invalid_operation_exception,
      ...
      int next_secondary( int iterator, uint64_t& primary ) {
         ...
         auto session_iter = current_session.lower_bound(secondary_key.full_key);
         UPCX_ASSERT( match(secondary_key.full_key, session_iter), db_rocksdb_invalid_operation_exception,
      ...
      int previous_secondary( int iterator, uint64_t& primary ) {
         if( iterator < iter_store.invalid_iterator() ) {
            ...
            UPCX_ASSERT( table, invalid_table_iterator, "not a valid end iterator" );
         ...
         UPCX_ASSERT( found_keys, db_rocksdb_invalid_operation_exception,
      ...
      int find_primary( name code, name scope, name table, SecondaryKey& secondary, uint64_t primary ) {
         ...
         UPCX_ASSERT( old_value->data() != nullptr, db_rocksdb_invalid_operation_exception,
                     "invariant failure in db_${d}_previous_secondary, empty value", ("d", helper.desc()));
         UPCX_ASSERT( old_value->size(), db_rocksdb_invalid_operation_exception,
      ...
      int next_primary( int iterator, uint64_t& primary ) {
         ...
         UPCX_ASSERT( match(key, session_iter), db_rocksdb_invalid_operation_exception,
   ...
   private:
      ...
      std::optional<found_keys> find_matching_sec_prim(const std::pair<shared_bytes, std::optional<shared_bytes> >& found_kv,
         ...
         UPCX_ASSERT( found_kv.second, db_rocksdb_invalid_operation_exception,
      ...
      std::optional<found_keys> find_matching_prim_sec(const std::pair<shared_bytes, std::optional<shared_bytes>>& found_kv,
         ...
         UPCX_ASSERT( match(found_kv.first, session_iter), db_rocksdb_invalid_operation_exception,
         ...
         UPCX_ASSERT( (*session_iter)->second, db_rocksdb_invalid_operation_exception,
