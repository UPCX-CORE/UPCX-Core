...
namespace upcxio { namespace chain {
   ...
   struct kv_iterator_chainbase : kv_iterator {
      ...
      int32_t kv_it_compare(const kv_iterator& rhs) override {
         UPCX_ASSERT(rhs.is_kv_chainbase_context_iterator(), kv_bad_iter, "Incompatible key-value iterators");
         auto& r = static_cast<const kv_iterator_chainbase&>(rhs);
         
         UPCX_ASSERT(contract == r.contract, kv_bad_iter, "Incompatible key-value iterators");
         UPCX_ASSERT(!current || !tracker.is_removed(*current), kv_bad_iter, "Iterator to erased element");
         UPCX_ASSERT(!r.current || !tracker.is_removed(*r.current), kv_bad_iter, "Iterator to erased element");
         if (!r.current) {
            if (!current)
               return 0;
            else {
      ...
      int32_t kv_it_key_compare(const char* key, uint32_t size) override {
         if (!current)
            return 1;
         UPCX_ASSERT(!tracker.is_removed(*current), kv_bad_iter, "Iterator to erased element");
   ...
   struct kv_context_chainbase : kv_context {
      ...
      int64_t kv_erase(uint64_t contract, const char* key, uint32_t key_size) override {
         UPCX_ASSERT(name{ contract } == receiver, table_operation_not_permitted, "Can not write to this key");
