...
namespace upcxio { namespace chain {
   ...
   static inline uint32_t actual_key_size(const uint32_t raw_key_size) {
      ...
      UPCX_ASSERT(raw_key_size >= prefix_size, kv_rocksdb_bad_value_size_exception,
   ...
   // to another payer
   inline static upcxio::session::shared_bytes build_value(const char* value, uint32_t value_size, const account_name& payer) {
      const uint32_t final_value_size = backing_store::payer_in_value_size + value_size;
      auto result = upcxio::session::shared_bytes(final_value_size);
   ...
   static inline upcxio::session::shared_bytes make_prefix_key(uint64_t contract, const char* user_prefix, uint32_t user_prefix_size) {
      static auto rocks_prefix = make_rocksdb_contract_kv_prefix();

      auto result = upcxio::session::shared_bytes(rocks_prefix.size() + sizeof(contract) + user_prefix_size);
   ...
   static inline upcxio::session::shared_bytes
   make_composite_key(uint64_t contract, const char* user_prefix, uint32_t user_prefix_size, const char* key, uint32_t key_size) {
      static auto rocks_prefix = make_rocksdb_contract_kv_prefix(); 

      auto result = upcxio::session::shared_bytes(rocks_prefix.size() + sizeof(contract) + user_prefix_size + key_size);
      std::memcpy(result.data(), rocks_prefix.data(), rocks_prefix.size());
      b1::chain_kv::insert_key(result, rocks_prefix.size(), contract);
      std::memcpy(result.data() + rocks_prefix.size() + sizeof(contract), user_prefix, user_prefix_size);
   ...
   static inline upcxio::session::shared_bytes make_composite_key(const upcxio::session::shared_bytes& prefix,
                                                                 const char* key, uint32_t key_size) {
      auto result = upcxio::session::shared_bytes(prefix.size() + key_size);
      std::memcpy(result.data(), prefix.data(), prefix.size());
      std::memcpy(result.data() + prefix.size(), key, key_size);
      return result;
   }

   static inline int32_t compare_bytes(const upcxio::session::shared_bytes& left, const upcxio::session::shared_bytes& right) {
      if (left < right) {
          return -1;
      }
   ...
   struct kv_iterator_rocksdb : kv_iterator {
      ...
      upcxio::session::shared_bytes    kv_prefix; // Format: [contract, prefix]
      ...
      int32_t kv_it_compare(const kv_iterator& rhs) override {
         UPCX_ASSERT(rhs.is_kv_rocksdb_context_iterator(), kv_bad_iter, "Incompatible key-value iterators");
         auto& r = const_cast<kv_iterator_rocksdb&>(static_cast<const kv_iterator_rocksdb&>(rhs));
         UPCX_ASSERT(kv_session == r.kv_session && kv_prefix == r.kv_prefix, kv_bad_iter,
                    "Incompatible key-value iterators");
         UPCX_ASSERT(!kv_current.deleted(), kv_bad_iter, "Iterator to erased element");
         UPCX_ASSERT(!r.kv_current.deleted(), kv_bad_iter, "Iterator to erased element");
         try {
            try {
      ...
      kv_it_stat kv_it_key(uint32_t offset, char* dest, uint32_t size, uint32_t& actual_size) override {
         ...
         auto key = upcxio::session::shared_bytes{};
         try {
            try {
               if (kv_it_status() == kv_it_stat::iterator_ok) {
                  key = kv_current.key();
      ...
      kv_it_stat kv_it_value(uint32_t offset, char* dest, uint32_t size, uint32_t& actual_size) override {
         ...
         auto kv = std::pair{ upcxio::session::shared_bytes{}, std::optional<upcxio::session::shared_bytes>{} };
         try {
            try {
               if (kv_it_status() == kv_it_stat::iterator_ok) {
                 kv = *kv_current;
   ...
   struct kv_context_rocksdb : kv_context {
      ...
      upcxio::session::shared_bytes                current_key;
      std::optional<upcxio::session::shared_bytes> current_value;
      ...
      int64_t kv_erase(uint64_t contract, const char* key, uint32_t key_size) override {
         ...
         UPCX_ASSERT(contract_name == receiver, table_operation_not_permitted, "Can not write to this key");
         
         current_value.reset();
         current_key                = upcxio::session::shared_bytes{};
         auto old_value             = std::optional<upcxio::session::shared_bytes>{};
      ...
      int64_t kv_set(uint64_t contract, const char* key, uint32_t key_size, const char* value, uint32_t value_size,
         ...
         const name contract_name {contract};
         UPCX_ASSERT(contract_name == receiver, table_operation_not_permitted, "Can not write to this key");
         UPCX_ASSERT(key_size <= limits.max_key_size, kv_limit_exceeded, "Key too large");
         UPCX_ASSERT(value_size <= limits.max_value_size, kv_limit_exceeded, "Value too large");
      ...
      std::unique_ptr<kv_iterator> kv_it_create(uint64_t contract, const char* prefix, uint32_t size) override {
         UPCX_ASSERT(num_iterators < limits.max_iterators, kv_bad_iter, "Too many iterators");
         UPCX_ASSERT(size <= limits.max_key_size, kv_bad_iter, "Prefix too large");
         try {
            try {
