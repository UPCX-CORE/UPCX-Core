...
#include <upcxio/chain/chain_config_helper.hpp>
...
FC_REFLECT(upcxio::chain::chain_config_v0,
...
FC_REFLECT_DERIVED(upcxio::chain::chain_config_v1, (upcxio::chain::chain_config_v0), 
...
inline DataStream &operator<<(DataStream &s, const upcxio::chain::data_entry<upcxio::chain::chain_config_v0, upcxio::chain::config_entry_validator> &entry){
   ...
   UPCX_ASSERT(entry.is_allowed(), unsupported_feature, "config id ${id} is no allowed", ("id", entry.id));
 ...
 */
template <typename DataStream>
inline DataStream &operator<<(DataStream &s, const upcxio::chain::data_entry<upcxio::chain::chain_config_v1, upcxio::chain::config_entry_validator> &entry){
   using namespace upcxio::chain;
   ...
   //When the protocol feature is not activated, the old version of nodupcx that doesn't know about 
   //the entry MUST behave the same as the new version of nodupcx that does.
 ...
 */
template <typename DataStream>
inline DataStream &operator>>(DataStream &s, upcxio::chain::data_entry<upcxio::chain::chain_config_v0, upcxio::chain::config_entry_validator> &entry){
   using namespace upcxio::chain;

   UPCX_ASSERT(entry.is_allowed(), upcxio::chain::unsupported_feature, "config id ${id} is no allowed", ("id", entry.id));
   ...
   switch (entry.id){
      ...
      break;
      default:
      FC_THROW_EXCEPTION(upcxio::chain::config_parse_error, "DataStream& operator<<: no such id: ${id}", ("id", entry.id));
   }
 ...
 */
template <typename DataStream>
inline DataStream &operator>>(DataStream &s, upcxio::chain::data_entry<upcxio::chain::chain_config_v1, upcxio::chain::config_entry_validator> &entry){
   using namespace upcxio::chain;
   ...
   switch (entry.id){
      ...
      break;
      default:
      upcxio::chain::data_entry<chain_config_v0, config_entry_validator> base_entry(entry);
...
inline DataStream& operator<<( DataStream& s, const upcxio::chain::data_range<T, upcxio::chain::config_entry_validator>& selection ) {
   using namespace upcxio::chain;
   ...
   for (auto uid : selection.ids){
      uint32_t id = uid;
      UPCX_ASSERT(id < visited.size(), config_parse_error, "provided id ${id} should be less than ${size}", ("id", id)("size", visited.size()));
      UPCX_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
      visited[id] = true;
 ...
 */
template<typename DataStream, typename T>
inline DataStream& operator>>( DataStream& s, upcxio::chain::data_range<T, upcxio::chain::config_entry_validator>& selection ) {
   using namespace upcxio::chain;
   ...
   for (uint32_t i = 0; i < length; ++i) {
      ...
      UPCX_ASSERT(id.value < visited.size(), config_parse_error, "provided id ${id} should be less than ${size}", ("id", id)("size", visited.size()));
      UPCX_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
      visited[id] = true;
