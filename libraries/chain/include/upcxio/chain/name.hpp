...
namespace fc {
  class variant;
  void to_variant(const upcxio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, upcxio::chain::name& check);
} // fc
...
namespace upcxio::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, upcxio::chain::name& check);
...
namespace std {
   template<> struct hash<upcxio::chain::name> : private hash<uint64_t> {
      typedef upcxio::chain::name argument_type;
...
FC_REFLECT( upcxio::chain::name, (value) )
