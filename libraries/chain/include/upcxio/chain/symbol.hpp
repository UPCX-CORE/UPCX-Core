...
#include <upcxio/chain/core_symbol.hpp>
#include <string>
#include <functional>
...
namespace upcxio {
   namespace chain {
      ...
      /**
         ...
         from_string constructs a symbol from an input a string of the form "4,UPCX"
...
#define SY(P,X) ::upcxio::chain::string_to_symbol_c(P,#X)
      ...
      static uint64_t string_to_symbol(uint8_t precision, const char* str) {
         try {
            ...
            for (uint32_t i = 0; i < len; ++i) {
               ...
               UPCX_ASSERT (str[i] >= 'A' && str[i] <= 'Z', symbol_type_exception, "invalid character in symbol name");
      ...
      class symbol : fc::reflect_init {
         public:
            ...
            explicit symbol(uint8_t p, const char* s): m_value(string_to_symbol(p, s)) {
               UPCX_ASSERT(valid(), symbol_type_exception, "invalid symbol: ${s}", ("s",s));
            }
            explicit symbol(uint64_t v = CORE_SYMBOL): m_value(v) {
               UPCX_ASSERT(valid(), symbol_type_exception, "invalid symbol: ${name}", ("name",name()));
            }
            static symbol from_string(const string& from)
            {
               try {
                  string s = fc::trim(from);
                  UPCX_ASSERT(!s.empty(), symbol_type_exception, "creating symbol from empty string");
                  auto comma_pos = s.find(',');
                  UPCX_ASSERT(comma_pos != string::npos, symbol_type_exception, "missing comma in symbol");
                  auto prec_part = s.substr(0, comma_pos);
                  uint8_t p = fc::to_int64(prec_part);
                  string name_part = s.substr(comma_pos + 1);
                  UPCX_ASSERT( p <= max_precision, symbol_type_exception, "precision ${p} should be <= 18", ("p", p));
            ...
            {
               UPCX_ASSERT( decimals() <= max_precision, symbol_type_exception, "precision ${p} should be <= 18", ("p", decimals()) );
               uint64_t p10 = 1;
               uint64_t p = decimals();
               while( p > 0  ) {
                  p10 *= 10; --p;
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = upcxio::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
            ...
            void reflector_init()const {
               UPCX_ASSERT( decimals() <= max_precision, symbol_type_exception, "precision ${p} should be <= 18", ("p", decimals()) );
               UPCX_ASSERT( valid_name(name()), symbol_type_exception, "invalid symbol: ${name}", ("name",name()));
            }
...
namespace fc {
   inline void to_variant(const upcxio::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, upcxio::chain::symbol& vo) {
      vo = upcxio::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const upcxio::chain::symbol_code& var, fc::variant& vo) {
      vo = upcxio::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, upcxio::chain::symbol_code& vo) {
      vo = upcxio::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(upcxio::chain::symbol_code, (value))
FC_REFLECT(upcxio::chain::symbol, (m_value))
FC_REFLECT(upcxio::chain::extended_symbol, (sym)(contract))
