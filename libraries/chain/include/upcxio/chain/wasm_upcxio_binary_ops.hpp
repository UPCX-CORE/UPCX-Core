...
namespace upcxio { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return upcxio::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       upcxio::chain::wasm_ops::to_string(f); }
};
template <>
...
template <class Op_Types>
struct UPCXIO_OperatorDecoderStream
{
   UPCXIO_OperatorDecoderStream(const std::vector<U8>& codeBytes)
   ...
   instr* decodeOp() {
      UPCX_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      ...
      #define VISIT_OPCODE(opcode,name,nameString,Imm,...) \
         case IR::Opcode::name: \
         { \
            UPCX_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_exception, ""); \
...
template <class Op_Types>
const std::vector<instr*>* UPCXIO_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace upcxio, chain, wasm_ops

FC_REFLECT_TEMPLATE( (typename T), upcxio::chain::wasm_ops::block< T >, (code)(rt) )
FC_REFLECT( upcxio::chain::wasm_ops::memarg, (a)(o) )
FC_REFLECT( upcxio::chain::wasm_ops::blocktype, (result) )
FC_REFLECT( upcxio::chain::wasm_ops::memoryoptype, (end) )
