...
Removes the restriction on upcxio::linkauth for non-native actions named one of the five special action names:
...
Forward upcxio::setcode actions to the WebAssembly code deployed on the upcxio account.
*/
            {}
         } )
...
*/
   ...
   const char* builtin_protocol_feature_codename( builtin_protocol_feature_t codename ) {
      ...
      UPCX_ASSERT( itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
   ...
   ,_type( feature_type )
   {
      switch( feature_type ) {
         ...
         default:
         {
            UPCX_THROW( protocol_feature_validation_exception,
   ...
   {
      UPCX_ASSERT( builtin_feature, protocol_feature_exception, "not a builtin protocol feature" );
   ...
   const protocol_feature& protocol_feature_set::get_protocol_feature( const digest_type& feature_digest )const {
      ...
      UPCX_ASSERT( itr != _recognized_protocol_features.end(), protocol_feature_exception,
   ...
   const protocol_feature& protocol_feature_set::add_feature( const builtin_protocol_feature& f ) {
      ...
      UPCX_ASSERT( builtin_itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
      ...
      if( indx < _recognized_builtin_protocol_features.size() ) {
         UPCX_ASSERT( _recognized_builtin_protocol_features[indx] == _recognized_protocol_features.end(),
      ...
      if( expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size() ) {
         ...
         for( const auto& builtin_codename : missing_builtins ) {
            ...
            UPCX_ASSERT( itr != builtin_protocol_feature_codenames.end(),
                        protocol_feature_exception,
                        "Unexpected error"
            );
      ...
      UPCX_ASSERT( res.second, protocol_feature_exception,
   ...
   void protocol_feature_manager::init( chainbase::database& db ) {
      UPCX_ASSERT( !is_initialized(), protocol_feature_exception, "cannot initialize protocol_feature_manager twice" );
   ...
   const protocol_feature* protocol_feature_manager::const_iterator::get_pointer()const {
      //UPCX_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot dereference singular iterator" );
      //UPCX_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot dereference end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator++() {
      UPCX_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot increment singular iterator" );
      UPCX_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot increment end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator--() {
      UPCX_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot decrement singular iterator" );
      if( _index == end_index ) {
         UPCX_ASSERT( _pfm->_activated_protocol_features.size() > 0,
                     protocol_feature_iterator_exception,
   ...
   {
      UPCX_ASSERT( is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized" );

      auto itr = _protocol_feature_set.find( feature_digest );

      UPCX_ASSERT( itr != _protocol_feature_set.end(), protocol_feature_exception,
                  "unrecognized protocol feature digest: ${digest}", ("digest", feature_digest) );

      if( _activated_protocol_features.size() > 0 ) {
         const auto& last = _activated_protocol_features.back();
         UPCX_ASSERT( last.activation_block_num <= current_block_num,
                     protocol_feature_exception,
      ...
      UPCX_ASSERT( itr->builtin_feature,
                  protocol_feature_exception,
      ...
      UPCX_ASSERT( indx < _builtin_protocol_features.size(), protocol_feature_exception,
      ...
      UPCX_ASSERT( _builtin_protocol_features[indx].activation_block_num == builtin_protocol_feature_entry::not_active,
                  protocol_feature_exception,
