...
void variant_snapshot_reader::validate() const {
   UPCX_ASSERT(snapshot.is_object(), snapshot_validation_exception,
         "Variant snapshot is not an object");
   const fc::variant_object& o = snapshot.get_object();

   UPCX_ASSERT(o.contains("version"), snapshot_validation_exception,
         "Variant snapshot has no version");

   const auto& version = o["version"];
   UPCX_ASSERT(version.is_integer(), snapshot_validation_exception,
         "Variant snapshot version is not an integer");

   UPCX_ASSERT(version.as_uint64() == (uint64_t)current_snapshot_version, snapshot_validation_exception,
   ...
   UPCX_ASSERT(o.contains("sections"), snapshot_validation_exception,
         "Variant snapshot has no sections");

   const auto& sections = o["sections"];
   UPCX_ASSERT(sections.is_array(), snapshot_validation_exception, "Variant snapshot sections is not an array");

   const auto& section_array = sections.get_array();
   for( const auto& section: section_array ) {
      UPCX_ASSERT(section.is_object(), snapshot_validation_exception, "Variant snapshot section is not an object");

      const auto& so = section.get_object();
      UPCX_ASSERT(so.contains("name"), snapshot_validation_exception,
            "Variant snapshot section has no name");

      UPCX_ASSERT(so["name"].is_string(), snapshot_validation_exception,
                 "Variant snapshot section name is not a string");

      UPCX_ASSERT(so.contains("rows"), snapshot_validation_exception,
                 "Variant snapshot section has no rows");

      UPCX_ASSERT(so["rows"].is_array(), snapshot_validation_exception,
...
void variant_snapshot_reader::set_section( const string& section_name ) {
   ...
   UPCX_THROW(snapshot_exception, "Variant snapshot has no section named ${n}", ("n", section_name));
}
...
{
   UPCX_ASSERT(section_pos == std::streampos(-1), snapshot_exception, "Attempting to write a new section without closing the previous section");
...
void istream_snapshot_reader::validate() const {
   ...
   try {
      ...
      UPCX_ASSERT(actual_totem == expected_totem, snapshot_exception,
      ...
      UPCX_ASSERT(actual_version == expected_version, snapshot_exception,
...
void istream_snapshot_reader::set_section( const string& section_name ) {
   ...
   UPCX_THROW(snapshot_exception, "Binary snapshot has no section named ${n}", ("n", section_name));
}
