...
void deferred_transaction_generation_context::reflector_init() {
      ...
      UPCX_ASSERT( sender != account_name(), ill_formed_deferred_transaction_generation_context,
...
void transaction_header::validate()const {
   UPCX_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      UPCX_ASSERT( now < deadline, tx_cpu_usage_exceeded, "transaction signature verification executed for too long ${time}us",
                  ("time", now - start)("now", now)("deadline", deadline)("start", start) );
      auto[ itr, successful_insertion ] = recovered_pub_keys.emplace( sig, digest );
      UPCX_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
flat_multimap<uint16_t, transaction_extension> transaction::validate_and_extract_extensions()const {
   ...
   for( size_t i = 0; i < transaction_extensions.size(); ++i ) {
      ...
      UPCX_ASSERT( id >= id_type_lower_bound, invalid_transaction_extension,
      ...
      UPCX_ASSERT( match, invalid_transaction_extension,
                  "Transaction extension with id type ${id} is not supported",
                  ("id", id)
      );

      if( match->enforce_unique ) {
         UPCX_ASSERT( i == 0 || id > id_type_lower_bound, invalid_transaction_extension,
...
uint32_t packed_transaction_v0::get_unprunable_size()const {
   ...
   UPCX_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      UPCX_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
void packed_transaction_v0::reflector_init()
{
   ...
   UPCX_ASSERT( unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction_v0 already unpacked" );
   local_unpack_transaction({});
   local_unpack_context_free_data();
}
...
static transaction unpack_transaction(const bytes& packed_trx, packed_transaction_v0::compression_type compression) {
   try {
      switch( compression ) {
         ...
         default:
            UPCX_THROW( unknown_transaction_compression, "Unknown transaction compression algorithm" );
      }
...
{
   UPCX_ASSERT(unpacked_trx.context_free_data.empty(), tx_decompression_error, "packed_transaction.context_free_data not empty");
...
digest_type  packed_transaction::prunable_data_type::partial::prunable_digest() const {
   UPCX_THROW(tx_prune_exception, "unimplemented");
}
...
static std::size_t padded_pack_size(const packed_transaction::prunable_data_type::none& obj, packed_transaction::prunable_data_type::compression_type) {
   ...
   UPCX_ASSERT(result == digest_pack_size, packed_transaction_type_exception, "Unexpected size of packed digest");
   return result;
}
...
packed_transaction::packed_transaction(const packed_transaction_v0& other, bool legacy)
 : compression(other.compression),
   ...
   trx_id(other.id())
{
   UPCX_ASSERT( legacy, transaction_exception, "Full type of prunable_data_type is not supported" );
   estimated_size = calculate_estimated_size();
}
...
packed_transaction_v0_ptr packed_transaction::to_packed_transaction_v0() const {
   ...
   UPCX_ASSERT( std::holds_alternative<packed_transaction::prunable_data_type::full_legacy>(prunable_data.prunable_data), transaction_exception, "Failed to get full_legacy variant in to_packed_transaction_v0" );
...
static uint32_t get_prunable_size_impl(const T&) {
   UPCX_THROW(tx_prune_exception, "unknown size: prunable data has been pruned.");
}
...
void packed_transaction::reflector_init()
{
   ...
   static_assert(fc::raw::has_feature_reflector_init_on_unpacked_reflected_types,
                 "FC unpack needs to call reflector_init otherwise unpacked_trx will not be initialized");
   UPCX_ASSERT( unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction already unpacked" );
