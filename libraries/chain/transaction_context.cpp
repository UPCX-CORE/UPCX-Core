...
namespace upcx { namespace chain {
   ...
   void transaction_context::disallow_transaction_extensions( const char* error_msg )const {
      if( control.is_producing_block() ) {
         UPCX_THROW( subjective_block_production_exception, error_msg );
      } else {
         UPCX_THROW( disallowed_transaction_extensions_bad_block_exception, error_msg );
      }
   }

   void transaction_context::init( uint64_t initial_net_usage )
   {
      UPCX_ASSERT( !is_initialized, transaction_exception, "cannot initialize twice" );
      ...
      if( !explicit_billed_cpu_time ) {
         ...
         if( validate_account_cpu_limit > 0 )
            validate_account_cpu_limit -= UPCX_PERCENT( validate_account_cpu_limit, 10 * config::percent_1 );
   ...
   void transaction_context::exec() {
      UPCX_ASSERT( is_initialized, transaction_exception, "must first initialize" );
   ...
   void transaction_context::check_net_usage()const {
      if( BOOST_UNLIKELY(net_usage > eager_net_limit) ) {
         if ( net_limit_due_to_block ) {
            UPCX_THROW( block_net_usage_exceeded,
            ...
            UPCX_THROW( greylist_net_usage_exceeded,
                        "greylisted transaction net usage is too high: ${net_usage} > ${net_limit}",
                        ("net_usage", net_usage)("net_limit", eager_net_limit) );
         } else {
            UPCX_THROW( tx_net_usage_exceeded,
   ...
   void transaction_context::checktime()const {
      ...
      if( explicit_billed_cpu_time || deadline_exception_code == deadline_exception::code_value ) {
         UPCX_THROW( deadline_exception, "deadline exceeded ${billing_timer}us",
                     ("billing_timer", now - pseudo_start)("now", now)("deadline", _deadline)("start", start) );
      } else if( deadline_exception_code == block_cpu_usage_exceeded::code_value ) {
         UPCX_THROW( block_cpu_usage_exceeded,
   ...
   void transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, int64_t account_cpu_limit, bool check_minimum )const {
      if (!control.skip_trx_checks()) {
         if( check_minimum ) {
            const auto& cfg = control.get_global_properties().configuration;
            UPCX_ASSERT( billed_us >= cfg.min_transaction_cpu_usage, transaction_exception,
   ...
   void transaction_context::validate_account_cpu_usage( int64_t billed_us, int64_t account_cpu_limit )const {
      if( (billed_us > 0) && !control.skip_trx_checks() ) {
         ...
         if( !cpu_limited_by_account && (billing_timer_exception_code == block_cpu_usage_exceeded::code_value) ) {
            UPCX_ASSERT( billed_us <= objective_duration_limit.count(),
            ...
            if( cpu_limit_due_to_greylist && cpu_limited_by_account ) {
               UPCX_ASSERT( billed_us <= account_cpu_limit,
               ...
               UPCX_ASSERT( billed_us <= cpu_limit,
   ...
   void transaction_context::validate_account_cpu_usage_estimate( int64_t prev_billed_us, int64_t account_cpu_limit )const {
      ...
      if( (prev_billed_us >= 0) && !control.skip_trx_checks() ) {
         ...
         if( !cpu_limited_by_account && (billing_timer_exception_code == block_cpu_usage_exceeded::code_value) ) {
            UPCX_ASSERT( prev_billed_us < objective_duration_limit.count(),
                        block_cpu_usage_exceeded,
            ...
            if( cpu_limit_due_to_greylist && cpu_limited_by_account ) {
               UPCX_ASSERT( prev_billed_us < account_cpu_limit,
                           greylist_cpu_usage_exceeded,
               ...
               // exceeds trx.max_cpu_usage_ms or cfg.max_transaction_cpu_usage if objective_duration_limit is greater
               const int64_t cpu_limit = (cpu_limited_by_account ? account_cpu_limit : objective_duration_limit.count());
               UPCX_ASSERT( prev_billed_us < cpu_limit,
                           tx_cpu_usage_exceeded,
   ...
   std::tuple<int64_t, int64_t, bool, bool> transaction_context::max_bandwidth_billed_accounts_can_pay( bool force_elastic_limits ) const{
      ...
      UPCX_ASSERT( (!force_elastic_limits && control.is_producing_block()) || (!greylisted_cpu && !greylisted_net),
   ...
   action_trace& transaction_context::get_action_trace( uint32_t action_ordinal ) {
      UPCX_ASSERT( 0 < action_ordinal && action_ordinal <= trace->action_traces.size() ,
   ...
   void transaction_context::record_transaction( const transaction_id_type& id, fc::time_point_sec expire ) {
      try {
         ...
         throw;
          ...
          UPCX_ASSERT( false, tx_duplicate,
   ...
   void transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
      ...
      if( !trx.context_free_actions.empty() && !control.skip_trx_checks() ) {
         for( const auto& a : trx.context_free_actions ) {
            ...
            UPCX_ASSERT( code != nullptr, transaction_exception,
                        "action's code account '${account}' does not exist", ("account", a.account) );
            UPCX_ASSERT( a.authorization.size() == 0, transaction_exception,
      ...
      for( const auto& a : trx.actions ) {
         ...
         for( const auto& auth : a.authorization ) {
            ...
            UPCX_ASSERT( actor  != nullptr, transaction_exception,
                        "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
            UPCX_ASSERT( auth_manager.find_permission(auth) != nullptr, transaction_exception,
      ...
      UPCX_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
   ...
   bool transaction_context::check_free_action(const action& act)const {
      if (act.authorization[0].actor == config::system_account_name 
      || act.authorization[0].actor == config::upcx_token_name 
   ...
   void transaction_context::verifyTransactionFee()const {
      uint64_t ram_usage = 0; // Used for calculating RAM Usage
      uint64_t fee = 0;
      const transaction& trx = packed_trx.get_transaction();
      auto& am = control.get_mutable_authorization_manager();
      for( const auto& act : trx.actions ) {
         if (!check_free_action(act)) {
            ram_usage += fc::raw::pack_size(act.data);
         }

         if (act.name == "fee"_n) {
            auto feeData = act.data_as<chain::fee>();
            fee += feeData.amount;
         }
      }

      UPCX_ASSERT( fee >= ram_usage, transaction_exception,
         "Transaction Fee is not enough! Required:${required}, Current:${current}", ("required", ram_usage)("current", fee));
   }
