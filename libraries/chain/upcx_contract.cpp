...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::upcx_code_name ) // virtual upcx.code permission does not really exist but is allowed
         continue;
 ...
 */
void apply_upcx_newaccount(apply_context& context) {
   auto create = context.get_action().data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::upcx_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      UPCX_ASSERT( name_str.find( "upcx." ) != 0, action_validate_exception,
...
void apply_upcx_setcode(apply_context& context) {
...
void apply_upcx_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_upcx_updateauth(apply_context& context) {
   ...
   UPCX_ASSERT( update.permission.to_string().find( "upcx." ) != 0, action_validate_exception,
               "Permission names that start with 'upcx.' are reserved" );
...
void apply_upcx_deleteauth(apply_context& context) {
//   context.require_write_lock( config::upcx_auth_scope );
...
void apply_upcx_linkauth(apply_context& context) {
//   context.require_write_lock( config::upcx_auth_scope );
   ...
   try {
      ...
      if( requirement.requirement != config::upcx_any_name ) {
...
void apply_upcx_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::upcx_auth_scope );
...
void apply_upcx_canceldelay(apply_context& context) {
