...
namespace upcxio { namespace chain { namespace webassembly {
   ...
   void interface::abort() const {
      UPCX_ASSERT( false, abort_called, "abort() called" );
   }

   void interface::upcxio_assert( bool condition, null_terminated_ptr msg ) const {
      if( BOOST_UNLIKELY( !condition ) ) {
         const size_t sz = strnlen( msg.data(), max_assert_message );
         std::string message( msg.data(), sz );
         UPCX_THROW( upcxio_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void interface::upcxio_assert_message( bool condition, legacy_span<const char> msg ) const {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void interface::upcxio_assert_code( bool condition, uint64_t error_code ) const {
      if( BOOST_UNLIKELY( !condition ) ) {
         if( error_code >= static_cast<uint64_t>(system_error_code::generic_system_error) ) {
            restricted_error_code_exception e( FC_LOG_MESSAGE(
                                                   error,
                                                   "upcxio_assert_code called with reserved error code: ${error_code}",
                                                   ("error_code", error_code)
            ) );
            e.error_code = static_cast<uint64_t>(system_error_code::contract_restricted_error_code);
            throw e;
         } else {
            upcxio_assert_code_exception e( FC_LOG_MESSAGE(
                                             error,
   ...
   void interface::upcxio_exit( int32_t code ) const {
