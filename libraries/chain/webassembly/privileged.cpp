...
namespace upcxio { namespace chain { namespace webassembly {
   ...
   void interface::activate_feature( int64_t feature_name ) const {
      UPCX_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
   }
   ...
   void interface::set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight ) {
      UPCX_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
      UPCX_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
      UPCX_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
   ...
   void interface::set_resource_limit( account_name account, name resource, int64_t limit ) {
      UPCX_ASSERT(limit >= -1, wasm_execution_error, "invalid value for ${resource} resource limit expected [-1,INT64_MAX]", ("resource", resource));
      ...
      if( resource == string_to_name("ram") ) {
         ...
         UPCX_THROW(wasm_execution_error, "unknown resource ${resource}", ("resource", resource));
      }
   }
   ...
   int64_t set_proposed_producers_common( apply_context& context, vector<producer_authority> && producers, bool validate_keys ) {
      UPCX_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
      UPCX_ASSERT( producers.size() > 0
      ...
      for (const auto& p: producers) {
         UPCX_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
         std::visit([&p, num_supported_key_types, validate_keys](const auto& a) {
            ...
            for (const auto& kw: a.keys ) {
               UPCX_ASSERT( kw.key.which() < num_supported_key_types, unactivated_key_type,
                           "Unactivated key type used in proposed producer schedule");

               if( validate_keys ) {
                  UPCX_ASSERT( kw.key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
               }
            ...
            UPCX_ASSERT( a.keys.size() == unique_keys.size(), wasm_execution_error, "producer schedule includes a duplicated key for ${account}", ("account", p.producer_name));
            UPCX_ASSERT( a.threshold > 0, wasm_execution_error, "producer schedule includes an authority with a threshold of 0 for ${account}", ("account", p.producer_name));
            UPCX_ASSERT( sum_weights >= a.threshold, wasm_execution_error, "producer schedule includes an unsatisfiable authority for ${account}", ("account", p.producer_name));
         }, p.authority);

         unique_producers.insert(p.producer_name);
      }
      UPCX_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
   ...
   void interface::set_wasm_parameters_packed( span<const char> packed_parameters ) {
      ...
      UPCX_ASSERT(version == 0, wasm_config_unknown_version, "set_wasm_parameters_packed: Unknown version: ${version}", ("version", version));
   ...
   int64_t interface::set_proposed_producers_ex( uint64_t packed_producer_format, legacy_span<const char> packed_producer_schedule) {
      if (packed_producer_format == 0) {
         ...
         UPCX_THROW(wasm_execution_error, "Producer schedule is in an unknown format!");
      }
   }
   ...
   uint32_t interface::get_parameters_packed( span<const char> packed_parameter_ids, span<char> packed_parameters) const{
      ...
      UPCX_ASSERT(size <= packed_parameters.size(),
   ...
   void interface::set_kv_parameters_packed( span<const char> packed_kv_parameters ) {
      ...
      fc::raw::unpack(ds, version);
      UPCX_ASSERT(version == 0, kv_unknown_parameters_version, "set_kv_parameters_packed: Unknown version: ${version}", ("version", version));
      fc::raw::unpack(ds, cfg);
      context.db.modify( context.control.get_global_properties(),
         [&]( auto& gprops ) {
