      ...
      if(_outstanding_compiles_and_poison[result.code] == false) {
         std::visit(overloaded {
            ...
            [&](const compilation_result_unknownfailure&) {
               wlog("code ${c} failed to tier-up with UPCX VM OC", ("c", result.code.code_id));
   ...
   if(_outstanding_compiles_and_poison.size()) {
      ...
      while(count_processed && _queued_compiles.size()) {
         ...
         if(codeobject) {
            ...
            FC_ASSERT(write_message_with_fds(_compile_monitor_write_socket, compile_wasm_message{ *nextup }, fds_to_pass), "UPCX VM failed to communicate to OOP manager");
            --count_processed;
         }
...
code_cache_sync::~code_cache_sync() {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   if(success)
      elog("unexpected response from UPCX VM OC compile monitor during shutdown");
}
