#include <upcx/chain/webassembly/upcx-vm-oc/executor.hpp>
#include <upcx/chain/webassembly/upcx-vm-oc/code_cache.hpp>
#include <upcx/chain/webassembly/upcx-vm-oc/memory.hpp>
#include <upcx/chain/webassembly/upcx-vm-oc/intrinsic_mapping.hpp>
...
#error UPCX VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, UPCXVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, UPCXVMOC_EXIT_SEGV);
...
//This is effectively overriding the upcx_exit intrinsic in wasm_interface
static void upcx_exit(int32_t code) {
   siglongjmp(*upcx_vm_oc_get_jmp_buf(), UPCXVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
static intrinsic upcx_exit_intrinsic("env.upcx_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&upcx_exit,
  std::integral_constant<std::size_t, find_intrinsic_index("env.upcx_exit")>::value
);
...
#define DEFINE_UPCXVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function UPCXVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. UPCX VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, memory& mem, apply_context& context) {
   ...
   uint64_t max_call_depth = upcx::chain::wasm_constraints::maximum_call_depth+1;
   uint64_t max_pages = upcx::chain::wasm_constraints::maximum_linear_memory/upcx::chain::wasm_constraints::wasm_page_size;
   ...
   UPCX_ASSERT(code.starting_memory_pages <= (int)max_pages, wasm_execution_error, "Initial memory out of range");
   ...
   if(code.starting_memory_pages > 0 ) {
      ...
      if(initial_page_offset < static_cast<uint64_t>(code.starting_memory_pages)) {
         mprotect(mem.full_page_memory_base() + initial_page_offset * upcx::chain::wasm_constraints::wasm_page_size,
                  (code.starting_memory_pages - initial_page_offset) * upcx::chain::wasm_constraints::wasm_page_size, PROT_READ | PROT_WRITE);
      }
   ...
   auto cleanup = fc::make_scoped_exit([cb, &tt=context.trx_context.transaction_timer, &mem=mem](){
      ...
      if(cb->current_linear_memory_pages > base_pages) {
         mprotect(mem.full_page_memory_base() + base_pages * upcx::chain::wasm_constraints::wasm_page_size,
                  (cb->current_linear_memory_pages - base_pages) * upcx::chain::wasm_constraints::wasm_page_size, PROT_NONE);
      }
   });
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      case 0:
         ...
         break;
      //case 1: clean upcx_exit
      case UPCXVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case UPCXVMOC_EXIT_SEGV:
         UPCX_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case UPCXVMOC_EXIT_EXCEPTION: //exception
