#include <upcxio/chain/webassembly/upcx-vm-oc/executor.hpp>
...
static intrinsic upcxio_exit_intrinsic("env.upcxio_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&upcxio_exit,
  std::integral_constant<std::size_t, find_intrinsic_index("env.upcxio_exit")>::value
);
...
void executor::execute(const code_descriptor& code, memory& mem, apply_context& context) {
   ...
   uint64_t max_call_depth = upcxio::chain::wasm_constraints::maximum_call_depth+1;
   uint64_t max_pages = upcxio::chain::wasm_constraints::maximum_linear_memory/upcxio::chain::wasm_constraints::wasm_page_size;
   ...
   if(code.starting_memory_pages > 0 ) {
      ...
      if(initial_page_offset < static_cast<uint64_t>(code.starting_memory_pages)) {
         mprotect(mem.full_page_memory_base() + initial_page_offset * upcxio::chain::wasm_constraints::wasm_page_size,
                  (code.starting_memory_pages - initial_page_offset) * upcxio::chain::wasm_constraints::wasm_page_size, PROT_READ | PROT_WRITE);
      }
   ...
   auto cleanup = fc::make_scoped_exit([cb, &tt=context.trx_context.transaction_timer, &mem=mem](){
      ...
      if(cb->current_linear_memory_pages > base_pages) {
         mprotect(mem.full_page_memory_base() + base_pages * upcxio::chain::wasm_constraints::wasm_page_size,
                  (cb->current_linear_memory_pages - base_pages) * upcxio::chain::wasm_constraints::wasm_page_size, PROT_NONE);
      }
   });
