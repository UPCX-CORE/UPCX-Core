...
namespace detail {
   ...
   template <typename T, std::size_t N>
   auto insert_key(upcxio::session::shared_bytes& dest, size_t index, T value) {
...
template <typename T>
auto insert_key(upcxio::session::shared_bytes& dest, size_t index, T value) -> std::enable_if_t<std::is_unsigned_v<T>, void> {
   detail::insert_key<T, 1>(dest, index, value);
}

template <typename T, std::size_t N>
auto insert_key(upcxio::session::shared_bytes& dest, size_t index, std::array<T, N> value) -> std::enable_if_t<std::is_unsigned_v<T>, void> {
   detail::insert_key<T*, N>(dest, index, value.data());
}

inline void insert_key(upcxio::session::shared_bytes& dest, size_t index, float64_t value) {
   auto float_key = detail::float_to_key<uint64_t>(value);
   detail::insert_key<uint64_t, 1>(dest, index, float_key);
}

inline void insert_key(upcxio::session::shared_bytes& dest, size_t index, float128_t value) {
   auto float_key = detail::float_to_key<detail::uint128_t>(value);
   // underlying storage is implemented as uint64_t[2], but it is laid out like it is uint128_t
...
// https://github.com/UPCXIO/spec-repo/blob/master/esr_key_value_database_intrinsics.md
