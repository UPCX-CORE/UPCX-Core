...
inline shared_bytes shared_bytes::next() const {
   ...
   EOS_ASSERT(!buffer.empty(), upcxio::chain::chain_exception, "shared_bytes::next() result buffer is empty");
...
inline shared_bytes shared_bytes::truncate_key(const shared_bytes &key) {
   EOS_ASSERT(!key.empty(), upcxio::chain::chain_exception, "chain_plugin::truncate_key() invalid key parameter: empty");
