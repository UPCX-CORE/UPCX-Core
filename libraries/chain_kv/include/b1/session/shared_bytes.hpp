...
shared_bytes::shared_bytes(const T* data, size_t size)
    : m_size{ size * sizeof(T) }, m_offset{ upcx::session::details::aligned_size(m_size) - m_size }, m_data{ [&]() {
         if (!data || size == 0) {
...
inline shared_bytes::shared_bytes(size_t size)
    : m_size{ size }, m_offset{ upcx::session::details::aligned_size(m_size) - m_size }, m_data{ [&]() {
         if (size == 0) {
            return std::shared_ptr<char>{};
         }
...
inline shared_bytes shared_bytes::next() const {
   ...
   UPCX_ASSERT(!buffer.empty(), upcx::chain::chain_exception, "shared_bytes::next() result buffer is empty");
   return upcx::session::shared_bytes(buffer.data(), buffer.size());
}

inline size_t            shared_bytes::size() const { return m_size; }
inline size_t            shared_bytes::aligned_size() const { return upcx::session::details::aligned_size(m_size); }
...
inline shared_bytes shared_bytes::truncate_key(const shared_bytes &key) {
   UPCX_ASSERT(!key.empty(), upcx::chain::chain_exception, "chain_plugin::truncate_key() invalid key parameter: empty");
...
template <>
struct less<upcx::session::shared_bytes> {
   bool operator()(const upcx::session::shared_bytes& lhs, const upcx::session::shared_bytes& rhs) const {
      return lhs < rhs;
   };
};

template <>
struct greater<upcx::session::shared_bytes> {
   bool operator()(const upcx::session::shared_bytes& lhs, const upcx::session::shared_bytes& rhs) const {
      return lhs > rhs;
   };
};
...
template <>
struct hash<upcx::session::shared_bytes> {
   size_t operator()(const upcx::session::shared_bytes& b) const {
      if (b.size() == 0) {
         return 0;
      }
...
template <>
struct equal_to<upcx::session::shared_bytes> {
   bool operator()(const upcx::session::shared_bytes& lhs, const upcx::session::shared_bytes& rhs) const {
      return lhs == rhs;
   }
};
