...
using namespace upcx::session;
using namespace upcx::session_tests;
...
void perform_session_level_test(const std::string& dbpath, bool always_undo = false) {
   ...
   auto root_session  = upcx::session_tests::make_session(dbpath);
   using session_type = upcx::session::session<decltype(root_session)>;
...
BOOST_AUTO_TEST_CASE(session_create_test) {
   {
      auto session1 = upcx::session_tests::make_session("/tmp/session18");
      make_data_store(session1, char_key_values, string_t{});
      verify_equal(session1, char_key_values, string_t{});
   }
   {
      auto session2 = upcx::session_tests::make_session("/tmp/session19");
...
BOOST_AUTO_TEST_CASE(session_rwd_test) {
   {
      auto session1 = upcx::session_tests::make_session("/tmp/session20");
      make_data_store(session1, char_key_values, string_t{});
      for (const auto& kv : char_batch_values) { verify_rwd(session1, kv.first, kv.second); }
   }
   {
      auto session2 = upcx::session_tests::make_session("/tmp/session21");
      make_data_store(session2, int_key_values, int_t{});
...
BOOST_AUTO_TEST_CASE(session_rwd_batch_test) {
   {
      auto session1 = upcx::session_tests::make_session("/tmp/session1");
      make_data_store(session1, char_key_values, string_t{});
      verify_rwd_batch(session1, char_batch_values);
   }
   {
      auto session2 = upcx::session_tests::make_session("/tmp/session2");
      make_data_store(session2, int_key_values, int_t{});
...
BOOST_AUTO_TEST_CASE(session_rw_ds_test) {
   {
      auto session1 = upcx::session_tests::make_session("/tmp/session3");
      auto session2 = upcx::session_tests::make_session("/tmp/session4");
      make_data_store(session1, char_key_values, string_t{});
      verify_read_from_datastore(session1, session2);
   }
   {
      auto session3 = upcx::session_tests::make_session("/tmp/session5");
      auto session4 = upcx::session_tests::make_session("/tmp/session6");
...
BOOST_AUTO_TEST_CASE(session_iterator_test) {
   {
      auto session1 = upcx::session_tests::make_session("/tmp/session7");
      make_data_store(session1, char_key_values, string_t{});
      verify_iterators(session1, string_t{});
   }
   {
      auto session2 = upcx::session_tests::make_session("/tmp/session8");
      make_data_store(session2, char_key_values, string_t{});
      verify_iterators<decltype(session2)>(session2, string_t{});
   }
   {
      auto session3 = upcx::session_tests::make_session("/tmp/session9");
      make_data_store(session3, int_key_values, int_t{});
      verify_iterators(session3, int_t{});
   }
   {
      auto session4 = upcx::session_tests::make_session("/tmp/session10");
...
BOOST_AUTO_TEST_CASE(session_iterator_key_order_test) {
   {
      auto session1 = upcx::session_tests::make_session("/tmp/session11");
      make_data_store(session1, char_key_values, string_t{});
      verify_session_key_order(session1);
   }
   {
      auto session2 = upcx::session_tests::make_session("/tmp/session12");
      make_data_store(session2, char_key_values, string_t{});
      verify_session_key_order<decltype(session2)>(session2);
   }
   {
      auto session3 = upcx::session_tests::make_session("/tmp/session13");
      make_data_store(session3, int_key_values, int_t{});
      verify_session_key_order(session3);
   }
   {
      auto session4 = upcx::session_tests::make_session("/tmp/session14");
      make_data_store(session4, int_key_values, int_t{});
...
BOOST_AUTO_TEST_CASE(session_level_test_undo_sometimes) {
   upcx::session_tests::perform_session_level_test("/tmp/session22");
}

BOOST_AUTO_TEST_CASE(session_level_test_undo_always) {
   upcx::session_tests::perform_session_level_test("/tmp/session23", true);
}

BOOST_AUTO_TEST_CASE(session_level_test_attach_detach) {
   size_t key_count      = 10;
   auto   root_session   = upcx::session_tests::make_session("/tmp/session15");
   using session_type    = upcx::session::session<decltype(root_session)>;
...
BOOST_AUTO_TEST_CASE(session_overwrite_key_in_child) {
   auto verify_key_value = [](auto& ds, uint16_t key, uint16_t expected_value) {
      auto key_       = upcx::session::shared_bytes(&key, 1);
      auto value      = upcx::session::shared_bytes(&expected_value, 1);
   ...
   auto root_session  = upcx::session_tests::make_session("/tmp/session16");
   using session_type = upcx::session::session<decltype(root_session)>;
...
BOOST_AUTO_TEST_CASE(session_delete_key_in_child) {
   ...
   auto verify_keys_exist = [](auto& ds, const auto& key_values) {
      for (const auto& key_value : key_values) {
         auto key   = upcx::session::shared_bytes(&key_value.first, 1);
         auto value = upcx::session::shared_bytes(&key_value.second, 1);
   ...
   auto root_session  = upcx::session_tests::make_session("/tmp/session17");
   using session_type = upcx::session::session<decltype(root_session)>;
   auto root_session_kvs =
         std::unordered_map<uint16_t, uint16_t>{ { 0, 10 }, { 1, 9 }, { 2, 8 }, { 3, 7 }, { 4, 6 }, { 5, 5 },
                                                 { 6, 4 },  { 7, 3 }, { 8, 2 }, { 9, 1 }, { 10, 0 } };
...
//     auto cache_ds = upcx::session::make_cache(memory_allocator);
//     auto rocksdb = make_rocks_db("testdb");
//     auto rocks_ds = upcx::session::make_rocks_data_store(std::move(rocksdb), std::move(memory_allocator));

//     auto root_session = upcx::session::make_session(rocks_ds, cache_ds);
...
//     auto block_session = upcx::session::make_session(root_session);
//     write(block_session, block_session_kvs);

//     auto transaction_session_kvs = generate_kvs(5000);
//     auto transaction_session = upcx::session::make_session(block_session);
//     write(transaction_session, transaction_session_kvs);
