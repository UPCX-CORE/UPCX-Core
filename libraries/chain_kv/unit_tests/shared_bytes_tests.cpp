...
BOOST_AUTO_TEST_CASE(next_test) {
   ...
   BOOST_CHECK_THROW(empty.next().size(), upcxio::chain::chain_exception);
   ...
   BOOST_CHECK_THROW(single_last.next().size(), upcxio::chain::chain_exception);

   char double_last_value[2] = {static_cast<char>(0xFF), static_cast<char>(0xFF)};
   auto double_last = shared_bytes(double_last_value, 2);
   BOOST_CHECK_THROW(double_last.next().size(), upcxio::chain::chain_exception);
...
BOOST_AUTO_TEST_CASE(truncate_key_test) {
   ...
   static constexpr auto* empty_value  = "";
   auto empty = shared_bytes(empty_value, 0);
   BOOST_CHECK_THROW(shared_bytes::truncate_key(empty), upcxio::chain::chain_exception);
}
