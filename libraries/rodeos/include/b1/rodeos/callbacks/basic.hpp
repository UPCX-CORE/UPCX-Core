...
struct context_free_system_callbacks {
   ...
   void upcxio_assert(bool condition, null_terminated_ptr msg) {
      if (!condition)
         throw assert_exception(std::string(msg.data(), msg.size()));
   }

   void upcxio_assert_message(bool condition, legacy_span<const char> msg) {
      if (!condition)
         throw assert_exception(std::string(msg.data(), msg.size()));
   }
   ...
   static void register_callbacks() {
      ...
      Rft::template add<&Derived::upcxio_assert>("env", "upcxio_assert");
      Rft::template add<&Derived::upcxio_assert_message>("env", "upcxio_assert_message");
   }
};

template <typename Backend>
struct data_state {
   upcxio::input_stream input_data;
...
struct data_callbacks {
   ...
   uint32_t get_input_data(upcxio::vm::span<char> dest) {
   ...
   void set_output_data(upcxio::vm::span<const char> data) {
