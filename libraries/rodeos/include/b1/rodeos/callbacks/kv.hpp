...
inline constexpr upcxio::name kvram_db_id{ "upcxio.kvram" };
inline constexpr upcxio::name kvdisk_db_id{ "upcxio.kvdisk" };
inline constexpr upcxio::name state_db_id{ "upcxio.state" };
...
struct kv_iterator_rocksdb {
   ...
   int32_t kv_it_compare(const kv_iterator_rocksdb& rhs) {
      upcxio::check(rhs.is_kv_rocksdb_context_iterator(), "Incompatible key-value iterators");
      auto& r = static_cast<const kv_iterator_rocksdb&>(rhs);
      upcxio::check(&view == &r.view && contract == r.contract, "Incompatible key-value iterators");
      upcxio::check(!kv_it.is_erased(), "Iterator to erased element");
      upcxio::check(!r.kv_it.is_erased(), "Iterator to erased element");
...
struct kv_context_rocksdb {
   ...
   kv_context_rocksdb(chain_kv::database& database, chain_kv::write_session& write_session,
                      std::vector<char> contract_kv_prefix, upcxio::name database_id, upcxio::name receiver,
                      const kv_database_config& limits)
   ...
   int64_t kv_erase(uint64_t contract, const char* key, uint32_t key_size) {
      upcxio::check(enable_write && (bypass_receiver_check || upcxio::name{ contract } == receiver),
   ...
   int64_t kv_set(uint64_t contract, const char* key, uint32_t key_size, const char* value, uint32_t value_size) {
      upcxio::check(enable_write && (bypass_receiver_check || upcxio::name{ contract } == receiver),
                   "Can not write to this key");
      upcxio::check(key_size <= limits.max_key_size, "Key too large");
      upcxio::check(value_size <= limits.max_value_size, "Value too large");
      temp_data_buffer = nullptr;
   ...
   std::unique_ptr<kv_iterator_rocksdb> kv_it_create(uint64_t contract, const char* prefix, uint32_t size) {
      upcxio::check(num_iterators < limits.max_iterators, "Too many iterators");
...
struct db_view_state {
   ...
   db_view_state(upcxio::name receiver, chain_kv::database& database, chain_kv::write_session& write_session,
   ...
   void reset() {
      upcxio::check(kv_iterators.size() == kv_destroyed_iterators.size() + 1, "iterators are still alive");
...
struct db_callbacks {
   ...
   int64_t kv_erase(uint64_t db, uint64_t contract, upcxio::vm::span<const char> key) {
      return kv_get_db(db).kv_erase(contract, key.data(), key.size());
   }

   int64_t kv_set(uint64_t db, uint64_t contract, upcxio::vm::span<const char> key, upcxio::vm::span<const char> value) {
      return kv_get_db(db).kv_set(contract, key.data(), key.size(), value.data(), value.size());
   }

   bool kv_get(uint64_t db, uint64_t contract, upcxio::vm::span<const char> key, uint32_t* value_size) {
      return kv_get_db(db).kv_get(contract, key.data(), key.size(), *value_size);
   }

   uint32_t kv_get_data(uint64_t db, uint32_t offset, upcxio::vm::span<char> data) {
      return kv_get_db(db).kv_get_data(offset, data.data(), data.size());
   }

   uint32_t kv_it_create(uint64_t db, uint64_t contract, upcxio::vm::span<const char> prefix) {
      ...
      if (!derived().get_db_view_state().kv_destroyed_iterators.empty()) {
         ...
         upcxio::check(derived().get_db_view_state().kv_iterators.size() <= 0xFFFFFFFFu, "Too many iterators");
   ...
   int32_t kv_it_key_compare(uint32_t itr, upcxio::vm::span<const char> key) {
      kv_check_iterator(itr);
   ...
   int32_t kv_it_lower_bound(uint32_t itr, upcxio::vm::span<const char> key, uint32_t* found_key_size,
   ...
   int32_t kv_it_key(uint32_t itr, uint32_t offset, upcxio::vm::span<char> dest, uint32_t* actual_size) {
      kv_check_iterator(itr);
      return static_cast<int32_t>(
            derived().get_db_view_state().kv_iterators[itr]->kv_it_key(offset, dest.data(), dest.size(), *actual_size));
   }

   int32_t kv_it_value(uint32_t itr, uint32_t offset, upcxio::vm::span<char> dest, uint32_t* actual_size) {
      kv_check_iterator(itr);
   ...
   void kv_check_iterator(uint32_t itr) {
      upcxio::check(itr < derived().get_db_view_state().kv_iterators.size() &&
