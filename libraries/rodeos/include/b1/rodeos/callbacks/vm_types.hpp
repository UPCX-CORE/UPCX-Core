...
using legacy_ptr = upcxio::vm::argument_proxy<T*, Align>;

template <typename T, std::size_t Align = alignof(T)>
using legacy_span = upcxio::vm::argument_proxy<upcxio::vm::span<T>, Align>;

struct null_terminated_ptr : upcxio::vm::span<const char> {
   using base_type = upcxio::vm::span<const char>;
...
template <typename Host, typename Execution_Interface = upcxio::vm::execution_interface>
struct type_converter : upcxio::vm::type_converter<Host, Execution_Interface> {
   using base_type = upcxio::vm::type_converter<Host, Execution_Interface>;
   ...
   template <typename T>
   auto from_wasm(void* ptr) const -> std::enable_if_t<std::is_pointer_v<T>, upcxio::vm::argument_proxy<T>> {
...
using registered_host_functions =
      upcxio::vm::registered_host_functions<Cls, upcxio::vm::execution_interface,
                                           type_converter<Cls, upcxio::vm::execution_interface>>;
