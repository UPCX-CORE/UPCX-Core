...
namespace detail {
   ...
   static void deserialize(V& value, void* buffer, size_t size) {
      ...
      ds >> idx;
      upcxio::check(idx==unsigned_int(0), "there was an error deserializing this value.");
      ds >> value;
   }
...
inline key_type make_prefix(upcxio::name table_name, upcxio::name index_name, uint8_t status = 1) {
...
static constexpr upcxio::name kv_ram = "upcxio.kvram"_n;
static constexpr upcxio::name kv_disk = "upcxio.kvdisk"_n;
...
class kv_table {
   ...
   public:
      ...
      T value() const {
         ...
         upcxio::check(itr_stat != status::iterator_end, "Cannot read end iterator");
         ...
         upcxio::check(static_cast<status>(stat) == status::iterator_ok, "Error reading value");
         ...
         if (!is_primary) {
            ...
            upcxio::check(success, "failure getting primary key in `value()`");
      ...
      key_type key() const {
         ...
         upcxio::check(static_cast<status>(stat) == status::iterator_ok, "Error getting key");

         return {(char*)buffer, actual_value_size};
      }

      iterator& operator++() {
         upcxio::check(itr_stat != status::iterator_end, "cannot increment end iterator");
      ...
      iterator& operator--() {
         ...
         upcxio::check(itr_stat != status::iterator_end, "decremented past the beginning");
         return *this;
      }
   ...
   public:
      upcxio::name index_name;
      upcxio::name table_name;
      upcxio::name contract_name;
   ...
   protected:
      ...
      kv_index(upcxio::name index_name, KF&& kf) : index_name{index_name} {
...
public:
   ...
   public:
      ...
      template <typename KF>
      index(upcxio::name name, KF&& kf) : kv_index{name, kf} {
      ...
      T operator[](const key_type& key) const {
         auto opt = get(key);
         upcxio::check(opt, __FILE__ ":" + std::to_string(__LINE__) + " Key not found in `[]`");
         return *opt;
      }
      ...
      std::optional<T> get(const key_type& k ) const {
         ...
         if (!is_primary) {
            ...
            upcxio::check(success, "failure getting primary key");
   ...
   void put(const T& value) {
      ...
      for (const auto& idx : secondary_indices) {
         ...
         if (!primary_key_found) {
            upcxio::check(!sec_found, "Attempted to store an existing secondary index.");
            ...
            if (sec_found) {
               ...
               upcxio::check(copy_size == tbl_key.size() && res == 0, "Attempted to update an existing secondary index.");
...
protected:
   ...
   void init(upcxio::name contract, upcxio::name table, upcxio::name db, PrimaryIndex& prim_index, SecondaryIndices&... indices) {
...
private:
   ...
   upcxio::name primary_index_name;
...
template <typename T, upcxio::name::raw SingletonName, upcxio::name::raw DbName = "upcxio.kvram"_n>
class kv_singleton {
   kv_environment environment;
...
public:
   ...
   explicit kv_singleton(upcxio::name contract_name, bool write_on_destruct) : contract_name{contract_name}, write_on_destruct{write_on_destruct} {
      key = make_prefix();
   }

   kv_singleton(upcxio::name contract_name, kv_environment environment, bool write_on_destruct) : environment{environment}, contract_name{contract_name}, write_on_destruct{write_on_destruct} {}
   ...
   const T& get() {
      ...
      if (!ste.is_cached) {
         ...
         upcxio::check(success, "tried to get a singleton that does not exist");
