...
void rodeos_db_snapshot::write_block_info(uint32_t block_num, const upcxio::checksum256& id,
...
void rodeos_db_snapshot::write_deltas(uint32_t block_num, upcxio::opaque<std::vector<ship_protocol::table_delta>> deltas, std::function<bool()> shutdown) {
   db_view_state view_state{ state_account, *db, *write_session, partition->contract_kv_prefix };
...
rodeos_filter::rodeos_filter(upcxio::name name, const std::string& wasm_filename) : name{ name } {
