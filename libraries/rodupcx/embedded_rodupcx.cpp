...
void with_result(const char* data, uint64_t size, F f) {
   upcx::input_stream          bin{ data, data + size };
   upcx::ship_protocol::result result;
   from_bin(result, bin);
   auto* result_v0 = std::get_if<upcx::ship_protocol::get_blocks_result_v0>(&result);
   if (result_v0)
      return f(*result_v0);

   auto* result_v1 = std::get_if<upcx::ship_protocol::get_blocks_result_v1>(&result);
   if (result_v1)
      return f(*result_v1);
...
extern "C" rodupcx_filter* rodupcx_create_filter(rodupcx_error* error, uint64_t name, const char* wasm_filename) {
   return handle_exceptions(error, nullptr, [&]() -> rodupcx_filter* { //
      return std::make_unique<rodupcx_filter>(upcx::name{ name }, wasm_filename).release();
   });
}
...
rodupcx_bool rodupcx_query_transaction(rodupcx_error* error, rodupcx_query_handler* handler, rodupcx_db_snapshot* snapshot,
   ...
   return handle_exceptions(error, false, [&]() {
      ...
      *result_size = 0;
      ...
      upcx::input_stream            s{ data, size };
      auto trx = upcx::from_bin<upcx::ship_protocol::packed_transaction>(s);

      auto                                    thread_state = handler->state_cache.get_state();
      upcx::ship_protocol::transaction_trace tt;
      if (snapshot->snap) {
      ...
      *result = (char*)malloc(ss.size);
      ...
      upcx::fixed_buf_stream fbs(*result, ss.size);
      to_bin(tt, fbs);
      if (fbs.pos != fbs.end) {
         upcx::check(false, upcx::convert_stream_error(upcx::stream_error::underrun));
      }
      *result_size = ss.size;
