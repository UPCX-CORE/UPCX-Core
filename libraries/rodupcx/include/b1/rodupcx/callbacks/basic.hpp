...
struct context_free_system_callbacks {
   ...
   void upcx_assert(bool condition, null_terminated_ptr msg) {
      if (!condition)
         throw assert_exception(std::string(msg.data(), msg.size()));
   }

   void upcx_assert_message(bool condition, legacy_span<const char> msg) {
      if (!condition)
         throw assert_exception(std::string(msg.data(), msg.size()));
   }
   ...
   static void register_callbacks() {
      ...
      Rft::template add<&Derived::upcx_assert>("env", "upcx_assert");
      Rft::template add<&Derived::upcx_assert_message>("env", "upcx_assert_message");
   }
};

template <typename Backend>
struct data_state {
   upcx::input_stream input_data;
...
struct data_callbacks {
   ...
   uint32_t get_input_data(upcx::vm::span<char> dest) {
   ...
   void set_output_data(upcx::vm::span<const char> data) {
