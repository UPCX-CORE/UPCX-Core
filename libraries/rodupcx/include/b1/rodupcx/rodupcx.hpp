...
struct rodupcx_db_snapshot {
   ...
   void start_block(const upcxio::ship_protocol::get_blocks_result_base& result);
   void end_block(const upcxio::ship_protocol::get_blocks_result_base& result, bool force_write);
   void check_write(const upcxio::ship_protocol::get_blocks_result_base& result);
   void write_block_info(const upcxio::ship_protocol::get_blocks_result_v0& result);
   void write_block_info(const upcxio::ship_protocol::get_blocks_result_v1& result);
   void write_deltas(const upcxio::ship_protocol::get_blocks_result_v0& result, std::function<bool()> shutdown);
   void write_deltas(const upcxio::ship_protocol::get_blocks_result_v1& result, std::function<bool()> shutdown);

 private:
   void write_block_info(uint32_t block_num, const upcxio::checksum256& id,
                         const upcxio::ship_protocol::signed_block_header& block);
   void write_deltas(uint32_t block_num, upcxio::opaque<std::vector<upcxio::ship_protocol::table_delta>> deltas, std::function<bool()> shutdown);
...
struct rodupcx_filter {
   ...
   rodupcx_filter(upcxio::name name, const std::string& wasm_filename);

   void process(rodupcx_db_snapshot& snapshot, const upcxio::ship_protocol::get_blocks_result_base& result,
                upcxio::input_stream bin, const std::function<void(const char* data, uint64_t size)>& push_data);
};
