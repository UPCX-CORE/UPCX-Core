...
#define UPCX_CDT_GET_RETURN_T(value_class, index_name) std::decay_t<decltype(std::invoke(&value_class::index_name, std::declval<const value_class*>()))>
...
#define KV_NAMED_INDEX(index_name, member_name)                                                                        \
   index<UPCX_CDT_GET_RETURN_T(value_type, member_name)> member_name{upcx::name{index_name}, &value_type::member_name};
...
namespace detail {
   ...
   static void deserialize(V& value, void* buffer, size_t size) {
      ...
      ds >> idx;
      upcx::check(idx==unsigned_int(0), "there was an error deserializing this value.");
      ds >> value;
   }
...
inline key_type make_prefix(upcx::name table_name, upcx::name index_name, uint8_t status = 1) {
...
#if UPCX_CDT_DOXYGEN
/**
 * @brief A function for converting types to the appropriate binary representation for the UPCX Key Value database.
...
static constexpr upcx::name kv_ram = "upcx.kvram"_n;
static constexpr upcx::name kv_disk = "upcx.kvdisk"_n;
 ...
 * @ingroup contracts
 *
 * @brief Defines an UPCX Key Value Table
 * @details UPCX Key Value API provides a C++ interface to the UPCX Key Value database.
...
class kv_table {
   ...
   public:
      ...
      T value() const {
         ...
         upcx::check(itr_stat != status::iterator_end, "Cannot read end iterator");
         ...
         upcx::check(static_cast<status>(stat) == status::iterator_ok, "Error reading value");
         ...
         if (!is_primary) {
            ...
            upcx::check(success, "failure getting primary key in `value()`");
      ...
      key_type key() const {
         ...
         upcx::check(static_cast<status>(stat) == status::iterator_ok, "Error getting key");

         return {(char*)buffer, actual_value_size};
      }

      iterator& operator++() {
         upcx::check(itr_stat != status::iterator_end, "cannot increment end iterator");
      ...
      iterator& operator--() {
         ...
         upcx::check(itr_stat != status::iterator_end, "decremented past the beginning");
         return *this;
      }
   ...
   public:
      upcx::name index_name;
      upcx::name table_name;
      upcx::name contract_name;
   ...
   protected:
      ...
      kv_index(upcx::name index_name, KF&& kf) : index_name{index_name} {
...
public:
    ...
    * @ingroup keyvalue
    *
    * @brief Defines an index on an UPCX Key Value Table
   ...
   public:
      ...
      template <typename KF>
      index(upcx::name name, KF&& kf) : kv_index{name, kf} {
      ...
      T operator[](const key_type& key) const {
         auto opt = get(key);
         upcx::check(opt, __FILE__ ":" + std::to_string(__LINE__) + " Key not found in `[]`");
         return *opt;
      }
      ...
      std::optional<T> get(const key_type& k ) const {
         ...
         if (!is_primary) {
            ...
            upcx::check(success, "failure getting primary key");
   ...
   void put(const T& value) {
      ...
      for (const auto& idx : secondary_indices) {
         ...
         if (!primary_key_found) {
            upcx::check(!sec_found, "Attempted to store an existing secondary index.");
            ...
            if (sec_found) {
               ...
               upcx::check(copy_size == tbl_key.size() && res == 0, "Attempted to update an existing secondary index.");
...
protected:
   ...
   void init(upcx::name contract, upcx::name table, upcx::name db, PrimaryIndex& prim_index, SecondaryIndices&... indices) {
...
template <typename T, upcx::name::raw SingletonName, upcx::name::raw DbName = "upcx.kvram"_n>
class kv_singleton {
   kv_environment environment;
...
public:
   ...
   explicit kv_singleton(upcx::name contract_name, bool write_on_destruct) : contract_name{contract_name}, write_on_destruct{write_on_destruct} {
      key = make_prefix();
   }

   kv_singleton(upcx::name contract_name, kv_environment environment, bool write_on_destruct) : environment{environment}, contract_name{contract_name}, write_on_destruct{write_on_destruct} {}
   ...
   const T& get() {
      ...
      if (!ste.is_cached) {
         ...
         upcx::check(success, "tried to get a singleton that does not exist");
