...
void rodupcx_db_snapshot::start_block(const get_blocks_result_base& result) {
   ...
   if (head_id != upcxio::checksum256{} && (!result.prev_block || result.prev_block->block_id != head_id))
...
void rodupcx_db_snapshot::write_block_info(uint32_t block_num, const upcxio::checksum256& id,
...
namespace {
   std::string to_string( const upcxio::checksum256& cs ) {
...
void rodupcx_db_snapshot::write_block_info(const ship_protocol::get_blocks_result_v0& result) {
   ...
   upcxio::input_stream bin       = *result.block;
...
void rodupcx_db_snapshot::write_block_info(const ship_protocol::get_blocks_result_v1& result) {
   ...
   fc_add_tag( blk_span, "block_id", to_string( result.this_block->block_id ) );
   fc_add_tag( blk_span, "block_num", block_num );
   fc_add_tag( blk_span, "block_time", upcxio::microseconds_to_str( header.timestamp.to_time_point().elapsed.count() ) );

   write_block_info(block_num, result.this_block->block_id, header);
}

void rodupcx_db_snapshot::write_deltas(uint32_t block_num, upcxio::opaque<std::vector<ship_protocol::table_delta>> deltas, std::function<bool()> shutdown) {
   db_view_state view_state{ state_account, *db, *write_session, partition->contract_kv_prefix };
...
void rodupcx_db_snapshot::write_deltas(const ship_protocol::get_blocks_result_v0& result,
   ...
   uint32_t            block_num = result.this_block->block_num;
   write_deltas(block_num, upcxio::opaque<std::vector<ship_protocol::table_delta>>(*result.deltas), shutdown);
}
...
rodupcx_filter::rodupcx_filter(upcxio::name name, const std::string& wasm_filename) : name{ name } {
