...
#include <upcxio/vm/watchdog.hpp>
#include <fc/log/logger.hpp>
#include <fc/scoped_exit.hpp>
#include <mutex>

using namespace std::literals;
namespace ship_protocol = upcxio::ship_protocol;
...
using upcxio::ship_protocol::action_receipt_v0;
using upcxio::ship_protocol::action_trace_v1;
using upcxio::ship_protocol::transaction_trace_v0;
...
void to_json(const std::pair<uint16_t, std::vector<char>>&, S& stream) {
   upcxio::check(false, upcxio::convert_stream_error(stream_error::bad_variant_index));
}
...
struct wasm_ql_backend_options {
   ...
   // static constexpr std::uint32_t upcxio_max_nested_structures   = 1023;
...
using backend_t = upcxio::vm::backend<rhf_t, upcxio::vm::jit, wasm_ql_backend_options>;
...
struct backend_entry {
   upcxio::name                name; // only for wasms loaded from disk
   upcxio::checksum256         hash; // only for wasms loaded from chain
...
using backend_container = multi_index_container<
      ...
      indexed_by<sequenced<tag<by_age>>, //
                 ordered_non_unique<tag<by_name>, member<backend_entry, upcxio::name, &backend_entry::name>>,
                 ordered_non_unique<tag<by_hash>, member<backend_entry, upcxio::checksum256, &backend_entry::hash>>>>;
...
class backend_cache {
 ...
 public:
   ...
   std::optional<backend_entry> get(upcxio::name name) {
   ...
   std::optional<backend_entry> get(const upcxio::checksum256& hash) {
      std::optional<backend_entry> result;
      std::lock_guard<std::mutex>  lock{ mutex };
...
std::optional<std::vector<uint8_t>> read_code(wasm_ql::thread_state& thread_state, upcxio::name account) {
...
std::optional<upcxio::checksum256> get_contract_hash(db_view_state& db_view_state, upcxio::name account) {
   std::optional<upcxio::checksum256> result;
   auto                              meta = get_state_row<ship_protocol::account_metadata>(
         db_view_state.kv_state.view,
         std::make_tuple(upcxio::name{ "account.meta" }, upcxio::name{ "primary" }, account));
   if (!meta)
      return result;
...
std::optional<std::vector<uint8_t>> read_contract(db_view_state& db_view_state, const upcxio::checksum256& hash,
                                                  upcxio::name account) {
   std::optional<std::vector<uint8_t>> result;
   auto                                code_row = get_state_row<ship_protocol::code>(
         db_view_state.kv_state.view,
         std::make_tuple(upcxio::name{ "code" }, upcxio::name{ "primary" }, uint8_t(0), uint8_t(0), hash));
   if (!code_row)
      return result;
   ...
   ilog("compiling ${h}: ${a}", ("h", upcxio::convert_to_json(hash))("a", (std::string)account));
   return result;
}
...
void run_action(wasm_ql::thread_state& thread_state, const std::vector<char>& contract_kv_prefix,
   ...
   if (std::chrono::steady_clock::now() >= stop_time)
      throw upcxio::vm::timeout_exception("execution timed out");
   ...
   std::optional<upcxio::checksum256> hash;
   if (!entry && !code) {
   ...
   try {
      upcxio::vm::watchdog wd{ stop_time - std::chrono::steady_clock::now() };
...

   {
      ...
      if (table.primary_index.begin() != table.primary_index.end()) {
         ...
         if (auto* obj = std::get_if<ship_protocol::global_property_v1>(&record)) {
            found = true;
            result += ",\"chain_id\":" + upcxio::convert_to_json(obj->chain_id);
         }
      }
      if (!found)
      ...
      if (sing.exists()) {
         std::visit(
               [&](auto& obj) {
                  ...
                  result += ",\"head_block_id\":" + upcxio::convert_to_json(obj.head_id);
                  result += ",\"last_irreversible_block_num\":\"" + std::to_string(obj.irreversible) + "\"";
                  result += ",\"last_irreversible_block_id\":" + upcxio::convert_to_json(obj.irreversible_id);
               },
               sing.get());
      } else
         throw std::runtime_error("No fill_status records found; is filler running?");
...
const std::vector<char>& query_get_block(wasm_ql::thread_state&   thread_state,
   ...
   upcxio::json_token_stream stream{ s.data() };
   try {
   ...
   upcxio::json_token_stream bn_stream{ bn_json.data() };
   ...
   if (params.block_num_or_id.size() == 64) {
      ...
      info = get_state_row_secondary<block_info>(db_view_state.kv_state.view,
                                                 std::make_tuple(upcxio::name{ "block.info" }, upcxio::name{ "id" }, id));
   } else {
      uint32_t num;
      try {
      ...
      info = get_state_row<block_info>(db_view_state.kv_state.view,
                                       std::make_tuple(upcxio::name{ "block.info" }, upcxio::name{ "primary" }, num));
   }
   ...
   if (info) {
      ...
      std::string result = "{";
      result += "\"block_num\":" + upcxio::convert_to_json(obj.num);
      result += ",\"id\":" + upcxio::convert_to_json(obj.id);
      result += ",\"timestamp\":" + upcxio::convert_to_json(obj.timestamp);
      result += ",\"producer\":" + upcxio::convert_to_json(obj.producer);
      result += ",\"confirmed\":" + upcxio::convert_to_json(obj.confirmed);
      result += ",\"previous\":" + upcxio::convert_to_json(obj.previous);
      result += ",\"transaction_mroot\":" + upcxio::convert_to_json(obj.transaction_mroot);
      result += ",\"action_mroot\":" + upcxio::convert_to_json(obj.action_mroot);
      result += ",\"schedule_version\":" + upcxio::convert_to_json(obj.schedule_version);
      result += ",\"producer_signature\":" + upcxio::convert_to_json(obj.producer_signature);
      result += ",\"ref_block_prefix\":" + upcxio::convert_to_json(ref_block_prefix);
      result += "}";
...
const std::vector<char>& query_get_abi(wasm_ql::thread_state& thread_state, const std::vector<char>& contract_kv_prefix,
   ...
   auto acc = get_state_row<ship_protocol::account>(
         db_view_state.kv_state.view,
         std::make_tuple(upcxio::name{ "account" }, upcxio::name{ "primary" }, params.account_name));
   if (!acc)
   ...
   if (acc0.abi.pos != acc0.abi.end) {
      result.abi.emplace();
      upcxio::from_bin(*result.abi, acc0.abi);
   }

   // todo: avoid the extra copy
   auto json = upcxio::convert_to_json(result);
...
struct send_transaction_params {
   ...
   upcxio::bytes                  packed_context_free_data = {};
...
const std::vector<char>& query_send_transaction(wasm_ql::thread_state&   thread_state,
   ...
   // todo: avoid the extra copy
   auto json = upcxio::convert_to_json(results);
   thread_state.action_return_value.assign(json.begin(), json.end());
   return thread_state.action_return_value;
...
transaction_trace_v0 query_send_transaction(wasm_ql::thread_state&                   thread_state,       //
                                            ...
                                            std::vector<std::vector<char>>&          memory,             //
                                            bool                                     return_trace_on_except) {
   upcxio::input_stream        s{ trx.packed_trx };
   ship_protocol::transaction unpacked;
   try {
      upcxio::from_bin(unpacked, s);
