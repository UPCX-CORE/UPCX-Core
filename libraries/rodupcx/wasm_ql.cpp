...
using upcx::ship_protocol::action_receipt_v0;
using upcx::ship_protocol::action_trace_v1;
using upcx::ship_protocol::transaction_trace_v0;

namespace upcx {

// todo: abiupcx support for pair. Used by extensions_type.
template <typename S>
void to_json(const std::pair<uint16_t, std::vector<char>>&, S& stream) {
   upcx::check(false, upcx::convert_stream_error(stream_error::bad_variant_index));
}
...
// todo: restore max_function_section_elements to 1023 and use nodupcx's hard fork
struct wasm_ql_backend_options {
   ...
   // static constexpr std::uint32_t upcx_max_nested_structures   = 1023;
...
using backend_t = upcx::vm::backend<rhf_t, upcx::vm::jit, wasm_ql_backend_options>;
...
struct callbacks : action_callbacks<callbacks>,
   ...
   rodupcx::chaindb_state& chaindb_state;
   rodupcx::db_view_state& db_view_state;

   callbacks(wasm_ql::thread_state& thread_state, rodupcx::chaindb_state& chaindb_state,
             rodupcx::db_view_state& db_view_state)
...
struct backend_entry {
   upcx::name                name; // only for wasms loaded from disk
   upcx::checksum256         hash; // only for wasms loaded from chain
...
using backend_container = multi_index_container<
      ...
      indexed_by<sequenced<tag<by_age>>, //
                 ordered_non_unique<tag<by_name>, member<backend_entry, upcx::name, &backend_entry::name>>,
                 ordered_non_unique<tag<by_hash>, member<backend_entry, upcx::checksum256, &backend_entry::hash>>>>;
...
class backend_cache {
 ...
 public:
   ...
   std::optional<backend_entry> get(upcx::name name) {
   ...
   std::optional<backend_entry> get(const upcx::checksum256& hash) {
      std::optional<backend_entry> result;
      std::lock_guard<std::mutex>  lock{ mutex };
...
std::optional<std::vector<uint8_t>> read_code(wasm_ql::thread_state& thread_state, upcx::name account) {
...
std::optional<upcx::checksum256> get_contract_hash(db_view_state& db_view_state, upcx::name account) {
   std::optional<upcx::checksum256> result;
   auto                              meta = get_state_row<ship_protocol::account_metadata>(
         db_view_state.kv_state.view,
         std::make_tuple(upcx::name{ "account.meta" }, upcx::name{ "primary" }, account));
   if (!meta)
      return result;
...
std::optional<std::vector<uint8_t>> read_contract(db_view_state& db_view_state, const upcx::checksum256& hash,
                                                  upcx::name account) {
   std::optional<std::vector<uint8_t>> result;
   auto                                code_row = get_state_row<ship_protocol::code>(
         db_view_state.kv_state.view,
         std::make_tuple(upcx::name{ "code" }, upcx::name{ "primary" }, uint8_t(0), uint8_t(0), hash));
   if (!code_row)
      return result;
   ...
   ilog("compiling ${h}: ${a}", ("h", upcx::convert_to_json(hash))("a", (std::string)account));
   return result;
}
...
void run_action(wasm_ql::thread_state& thread_state, const std::vector<char>& contract_kv_prefix,
   ...
   if (std::chrono::steady_clock::now() >= stop_time)
      throw upcx::vm::timeout_exception("execution timed out");
   ...
   std::optional<upcx::checksum256> hash;
   if (!entry && !code) {
   ...
   try {
      upcx::vm::watchdog wd{ stop_time - std::chrono::steady_clock::now() };
...

   {
      ...
      if (table.primary_index.begin() != table.primary_index.end()) {
         ...
         if (auto* obj = std::get_if<ship_protocol::global_property_v1>(&record)) {
            found = true;
            result += ",\"chain_id\":" + upcx::convert_to_json(obj->chain_id);
         }
      }
      if (!found)
      ...
      if (sing.exists()) {
         std::visit(
               [&](auto& obj) {
                  ...
                  result += ",\"head_block_id\":" + upcx::convert_to_json(obj.head_id);
                  result += ",\"last_irreversible_block_num\":\"" + std::to_string(obj.irreversible) + "\"";
                  result += ",\"last_irreversible_block_id\":" + upcx::convert_to_json(obj.irreversible_id);
               },
               sing.get());
      } else
         throw std::runtime_error("No fill_status records found; is filler running?");
...
UPCX_REFLECT(get_block_params, block_num_or_id)
...
const std::vector<char>& query_get_block(wasm_ql::thread_state&   thread_state,
   ...
   upcx::json_token_stream stream{ s.data() };
   try {
   ...
   upcx::json_token_stream bn_stream{ bn_json.data() };
   ...
   if (params.block_num_or_id.size() == 64) {
      ...
      info = get_state_row_secondary<block_info>(db_view_state.kv_state.view,
                                                 std::make_tuple(upcx::name{ "block.info" }, upcx::name{ "id" }, id));
   } else {
      uint32_t num;
      try {
      ...
      info = get_state_row<block_info>(db_view_state.kv_state.view,
                                       std::make_tuple(upcx::name{ "block.info" }, upcx::name{ "primary" }, num));
   }
   ...
   if (info) {
      ...
      std::string result = "{";
      result += "\"block_num\":" + upcx::convert_to_json(obj.num);
      result += ",\"id\":" + upcx::convert_to_json(obj.id);
      result += ",\"timestamp\":" + upcx::convert_to_json(obj.timestamp);
      result += ",\"producer\":" + upcx::convert_to_json(obj.producer);
      result += ",\"confirmed\":" + upcx::convert_to_json(obj.confirmed);
      result += ",\"previous\":" + upcx::convert_to_json(obj.previous);
      result += ",\"transaction_mroot\":" + upcx::convert_to_json(obj.transaction_mroot);
      result += ",\"action_mroot\":" + upcx::convert_to_json(obj.action_mroot);
      result += ",\"schedule_version\":" + upcx::convert_to_json(obj.schedule_version);
      result += ",\"producer_signature\":" + upcx::convert_to_json(obj.producer_signature);
      result += ",\"ref_block_prefix\":" + upcx::convert_to_json(ref_block_prefix);
      result += "}";
...
struct get_abi_params {
   upcx::name account_name = {};
};

UPCX_REFLECT(get_abi_params, account_name)

struct get_abi_result {
   upcx::name                   account_name;
   std::optional<upcx::abi_def> abi;
};

UPCX_REFLECT(get_abi_result, account_name, abi)
...
const std::vector<char>& query_get_abi(wasm_ql::thread_state& thread_state, const std::vector<char>& contract_kv_prefix,
   ...
   auto acc = get_state_row<ship_protocol::account>(
         db_view_state.kv_state.view,
         std::make_tuple(upcx::name{ "account" }, upcx::name{ "primary" }, params.account_name));
   if (!acc)
   ...
   if (acc0.abi.pos != acc0.abi.end) {
      result.abi.emplace();
      upcx::from_bin(*result.abi, acc0.abi);
   }

   // todo: avoid the extra copy
   auto json = upcx::convert_to_json(result);
...
UPCX_REFLECT(extension_hex_data, type, data)

UPCX_REFLECT(action_no_data, account, name, authorization)
...
UPCX_REFLECT(transaction_for_get_keys, base ship_protocol::transaction_header, context_free_actions, actions,
              transaction_extensions)

struct get_required_keys_params {
   transaction_for_get_keys       transaction    = {};
   std::vector<upcx::public_key> available_keys = {};
};

UPCX_REFLECT(get_required_keys_params, transaction, available_keys)

struct get_required_keys_result {
   std::vector<upcx::public_key> required_keys = {};
};

UPCX_REFLECT(get_required_keys_result, required_keys)
...
struct send_transaction_params {
   ...
   upcx::bytes                  packed_context_free_data = {};
   upcx::bytes                  packed_trx               = {};
};

UPCX_REFLECT(send_transaction_params, signatures, compression, packed_context_free_data, packed_trx)

struct send_transaction_results {
   upcx::checksum256   transaction_id; // todo: redundant with processed.id
   transaction_trace_v0 processed;
};

UPCX_REFLECT(send_transaction_results, transaction_id, processed)
...
const std::vector<char>& query_send_transaction(wasm_ql::thread_state&   thread_state,
   ...
   // todo: avoid the extra copy
   auto json = upcx::convert_to_json(results);
   thread_state.action_return_value.assign(json.begin(), json.end());
   return thread_state.action_return_value;
...
transaction_trace_v0 query_send_transaction(wasm_ql::thread_state&                   thread_state,       //
                                            ...
                                            std::vector<std::vector<char>>&          memory,             //
                                            bool                                     return_trace_on_except) {
   upcx::input_stream        s{ trx.packed_trx };
   ship_protocol::transaction unpacked;
   try {
      upcx::from_bin(unpacked, s);
