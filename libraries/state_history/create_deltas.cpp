...
#include <upcxio/state_history/rocksdb_receiver.hpp>
...
std::vector<table_delta> create_deltas_rocksdb(const chainbase::database& db, const upcxio::chain::kv_undo_stack_ptr &kv_undo_stack, bool full_snapshot) {
   std::vector<table_delta> deltas;
   ...
   if(full_snapshot) {
      ...
      auto begin_key = upcxio::session::shared_bytes(&chain::backing_store::rocksdb_contract_kv_prefix, 1);
      ...
      begin_key = upcxio::session::shared_bytes(&chain::backing_store::rocksdb_contract_db_prefix, 1);
      end_key = begin_key.next();
      chain::backing_store::walk_rocksdb_entries_with_prefix(kv_undo_stack, begin_key, end_key, writer);
   } else {
      auto* session = std::visit(upcxio::session::overloaded{
        [](upcxio::chain::kv_undo_stack_ptr::element_type::session_type* session){
          return session;
        }, [](auto*){
          UPCX_ASSERT(false, upcxio::chain::chain_exception, "undo_stack is empty");
          static upcxio::chain::kv_undo_stack_ptr::element_type::session_type* invalid = nullptr;
          return invalid;
