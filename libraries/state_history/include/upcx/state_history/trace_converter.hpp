...
struct ondisk_prunable_full_legacy {
   ...
   upcx::chain::digest_type   digest;
   upcx::chain::digest_type   prunable_digest() const { return digest; }
};
...
void for_each_packed_transaction(const upcx::state_history::augmented_transaction_trace& obj, const Lambda& lambda) {
   auto& trace = *obj.trace;
   if (trace.failed_dtrx_trace) {
      for_each_packed_transaction(
          upcx::state_history::augmented_transaction_trace{trace.failed_dtrx_trace, obj.packed_trx}, lambda);
   }
...
template <typename Lambda>
void for_each_packed_transaction(const std::vector<upcx::state_history::augmented_transaction_trace>& traces,
                                 const Lambda&                                                         lambda) {
...
FC_REFLECT(upcx::state_history::trace_converter::ondisk_prunable_full_legacy,
