...
using upcxio::chain::overloaded;
using upcxio::chain::packed_transaction;
using upcxio::chain::state_history_exception;
...
struct ondisk_prunable_full_legacy {
   ...
   upcxio::chain::digest_type   digest;
   upcxio::chain::digest_type   prunable_digest() const { return digest; }
};
...
void for_each_packed_transaction(const upcxio::state_history::augmented_transaction_trace& obj, const Lambda& lambda) {
   auto& trace = *obj.trace;
   if (trace.failed_dtrx_trace) {
      for_each_packed_transaction(
          upcxio::state_history::augmented_transaction_trace{trace.failed_dtrx_trace, obj.packed_trx}, lambda);
   }
...
template <typename Lambda>
void for_each_packed_transaction(const std::vector<upcxio::state_history::augmented_transaction_trace>& traces,
                                 const Lambda&                                                         lambda) {
...
void pack(OSTREAM&& strm, const T& obj, compression_type compression) {
   ...
   case compression_type::none:
      ...
      break;
   default:
      UPCX_ASSERT(false, state_history_exception, "unsupported compression type");
   }
}
...
FC_REFLECT(upcxio::state_history::trace_converter::ondisk_prunable_full_legacy,
