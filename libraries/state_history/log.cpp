#include <upcxio/state_history/compression.hpp>
...
uint64_t state_history_log_data::payload_size_at(uint64_t pos) const {
   UPCX_ASSERT(file.size() >= pos + sizeof(state_history_log_header), chain::state_history_exception,
   ...
   UPCX_ASSERT(is_ship(header.magic) && is_ship_supported_version(header.magic), chain::state_history_exception,
              "corrupt ${name}: invalid header for entry at position ${pos}", ("name", filename)("pos", pos));

   UPCX_ASSERT(file.size() >= pos + sizeof(state_history_log_header) + header.payload_size,
...
void state_history_log::read_header(state_history_log_header& header, bool assert_version) {
   ...
   fc::raw::unpack(ds, header);
   UPCX_ASSERT(!ds.remaining(), chain::state_history_exception, "state_history_log_header_serial_size mismatch");
...
void state_history_log::get_entry_header(state_history_log::block_num_type block_num,
                                         state_history_log_header&         header) {
   UPCX_ASSERT(block_num >= _begin_block && block_num < _end_block, chain::state_history_exception,
...
void state_history_log::recover_blocks(uint64_t size) {
   ...
   read_log.flush();
   UPCX_ASSERT(get_last_block(pos), chain::state_history_exception, "recover ${name}.log failed", ("name", name));
}
...
void state_history_log::open_log(bfs::path log_filename) {
   ...
   if (size >= state_history_log_header_serial_size) {
      ...
      read_log.seek(0);
      read_header(header, false);
      UPCX_ASSERT(is_ship(header.magic) && is_ship_supported_version(header.magic) &&
      ...
      UPCX_ASSERT(!size, chain::state_history_exception, "corrupt ${name}.log (5)", ("name", name));
...
state_history_log::write_entry_header(const state_history_log_header& header, const chain::block_id_type& prev_id) {
   ...
   UPCX_ASSERT(_begin_block == _end_block || block_num <= _end_block, chain::state_history_exception,
              "missed a block in ${name}.log", ("name", name));

   if (_begin_block != _end_block && block_num > _begin_block) {
      if (block_num == _end_block) {
         UPCX_ASSERT(prev_id == last_block_id, chain::state_history_exception, "missed a fork change in ${name}.log",
                    ("name", name));
      } else {
         state_history_log_header prev;
         get_entry_header(block_num - 1, prev);
         UPCX_ASSERT(prev_id == prev.block_id, chain::state_history_exception, "missed a fork change in ${name}.log",
                    ("name", name));
      }
   }
...
void state_history_traces_log::prune_transactions(state_history_log::block_num_type        block_num,
   ...
   if (ds.remaining()) {
      UPCX_ASSERT(version > 0, chain::state_history_exception,
   ...
   state_history_log_header header;
   get_entry_header(block_num, header);
   UPCX_ASSERT(get_ship_version(header.magic) > 0, chain::state_history_exception,
              "The trace log version 0 does not support transaction pruning.");
