...
#define CORE_SYM  ( ::upcxio::chain::string_to_symbol_c( CORE_SYM_PRECISION, CORE_SYM_NAME ) )

struct core_sym {
   static inline upcxio::chain::asset from_string(const std::string& s) {
      return upcxio::chain::asset::from_string(s + " " CORE_SYM_NAME);
   }
};
...
#define MAKE_READ_WASM_ABI(CN,C, D)                                                         \
   static std::vector<uint8_t> CN ## _wasm() {                                              \
      std::string s;                                                                        \
      fc::read_file_contents("/Volumes/work/krozy/02_UPCX/02_DevSrc/UPCX-Core/unittests/" #D "/" #C "/" #C ".wasm", s); \
      return std::vector<uint8_t>(s.begin(), s.end());                                      \
   }                                                                                        \
   static std::vector<char> CN ## _abi() {                                                  \
      std::string s;                                                                        \
      fc::read_file_contents("/Volumes/work/krozy/02_UPCX/02_DevSrc/UPCX-Core/unittests/" #D "/" #C "/" #C ".abi", s);  \
...
namespace upcxio {
   namespace testing {
      struct contracts {
         // Contracts in `upcx/unittests/contracts' directory
         MAKE_READ_WASM_ABI(upcxio_bios,   upcxio.bios, contracts)
         MAKE_READ_WASM_ABI(upcxio_msig,   upcxio.msig, contracts)
         MAKE_READ_WASM_ABI(upcxio_system, upcxio.system, contracts)
         MAKE_READ_WASM_ABI(upcxio_token,  upcxio.token,  contracts)
         MAKE_READ_WASM_ABI(upcxio_wrap,   upcxio.wrap,   contracts)

         MAKE_READ_WASM_ABI(before_producer_authority_upcxio_bios,   upcxio.bios, contracts/old_versions/v1.7.0-develop-preactivate_feature)
         MAKE_READ_WASM_ABI(before_preactivate_upcxio_bios,          upcxio.bios, contracts/old_versions/v1.6.0-rc3)

         // Contracts in `upcx/unittests/unittests/test-contracts' directory
