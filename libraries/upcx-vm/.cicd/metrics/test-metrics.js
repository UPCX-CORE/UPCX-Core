...
async function download(url) {
  ...
  if (isNullOrEmpty(body)) {
    console.log(
      `ERROR: URL returned nothing! URL: ${url.replace(
...
async function getBuild(pipeline, buildNumber) {
  ...
  const httpResponse = await fetch(
    `https://api.buildkite.com/v2/organizations/UPCX/pipelines/${pipeline}/builds/${buildNumber}${buildkiteAccessToken}`
  );
...
function getLineNumber(text, key, startIndex) {
  ...
  const begin =
    isNullOrEmpty(startIndex) || !Number.isInteger(startIndex) || startIndex < 1
      ? 0
      : startIndex;
  let found = false;
  let lineNumber = 0;
  ...
  text.split("\n").some((line) => {
    if (
      lineNumber >= begin &&
      ((regex && key.test(line)) || (!regex && line.includes(key)))
    ) {
      found = true;
...
function getOS(environment) {
  ...
  if (
    ...
    isNullOrEmpty(environment.BUILDKITE_LABEL)
  ) {
    console.log(
  ...
  if (/aws.*2/.test(label) || /amazon.*2/.test(label)) return "Amazon Linux 2";
  if (/centos(?!.*[89])/.test(label)) return "CentOS 7";
  if (/fedora(?!.*2[89])/.test(label) && /fedora(?!.*3\d)/.test(label))
    return "Fedora 27";
  if (/high.*sierra/.test(label)) return "High Sierra";
  if (/mojave/.test(label)) return "Mojave";
  if (/ubuntu.*16.*04/.test(label) || /ubuntu.*16(?!.*10)/.test(label))
    return "Ubuntu 16.04";
  if (/ubuntu.*18.*04/.test(label) || /ubuntu.*18(?!.*10)/.test(label))
    return "Ubuntu 18.04";
  if (/docker/.test(label)) return "Docker";
  return "Unknown";
}
...
async function getXML(job) {
  ...
  const testResultsArtifact = JSON.parse(artifacts).filter(
    (artifact) => artifact.filename === xmlFilename
  );
  ...
  await XML.parseString(rawXML, xmlOptions, (err, result) => {
    xmlTestResults = result;
    xmlError = err;
  });
  if (isNullOrEmpty(xmlError)) return xmlTestResults;
  console.log(
...
function isNullOrEmpty(str) {
  return (
    str === null || str === undefined || str.length === 0 || /^\s*$/.test(str)
  );
}
...
function parseLog(logText) {
  ...
  return resultLines.map((line) => {
    const y = line
      .trim()
      .split(/test\s+#\d+/)
      .pop(); // remove everything before the test declaration
    ...
    let testResult;
    if (rawResult.includes("failed")) testResult = "Failed";
    else if (rawResult.includes("passed")) testResult = "Passed";
    else testResult = "Exception";
...
function parseXunit(xUnit) {
  ...
  return xUnit.site.testing.test.map((test) => {
    ...
    const testTime = test.results.namedmeasurement.filter((x) =>
      /execution\s+time/.test(x.name.toLowerCase())
    )[0].value;
    let testResult;
    if (test.status.includes("failed")) testResult = "Failed";
    else if (test.status.includes("passed")) testResult = "Passed";
    else testResult = "Exception";
...
function sanitize(text) {
  if (debug)
    console.log(`sanitize(text) where text.length = ${text.length} bytes`); // DEBUG
  const chunkSize = 131072; // process text in 128 kB chunks
  if (text.length > chunkSize)
    return sanitize(text.slice(0, chunkSize)).concat(
      sanitize(text.slice(chunkSize))
    );
  return text
    .replace(/(?!\n)\r(?!\n)/g, "\n")
...
function testDiagnostics(test, logText) {
  ...
  // isolate the error message after exception
    ...
    output.errorMsg = `fc::${
      errorLine.split("::")[1].replace(/['",]/g, "").split(" ")[0]
    ...
    !isNullOrEmpty(testLog.filter((line) => /boost.+exception/.test(line)))
  ) {
    // boost exception
    [errorLine] = testLog.filter((line) => /boost.+exception/.test(line));
    output.errorMsg = `boost: ${
      errorLine.replace(/[()]/g, "").split(/: (.+)/)[1]
    }`; // capturing parenthesis, split only at first ' :'
    output.stackTrace = testLog
      .filter((line) => /thread-\d+/.test(line))[0]
      .split("thread-")[1]
      .replace(/^\d+/, "")
      .trim()
      .replace(/[[]\d+m$/, ""); // get the bottom of the stack trace
  } else if (
    /unit[-_. ]+test/.test(test.testName) ||
    /plugin[-_. ]+test/.test(test.testName)
  ) {
    // unit test, application exception
    if (
      !isNullOrEmpty(testLog.filter((line) => line.includes("exception: ")))
    ) {
...
async function testMetrics(buildkiteObject) {
  if (!isNullOrEmpty(buildkiteObject.type)) {
    // input is a Buildkite job object
    const job = buildkiteObject;
    console.log(
      `Processing test metrics for "${job.name}"${
        inBuildkite ? "" : ` at ${job.web_url}`
      }...`
    );
    if (isNullOrEmpty(job.exit_status)) {
      console.log(
        `${inBuildkite ? "+++ :warning: " : ""}WARNING: "${
          job.name
        }" was skipped!`
      );
      return null;
    ...
    try {
      ...
      if (isNullOrEmpty(testResults)) testResults = parseLog(logText);
    }
    // get test metrics
    const env = await getEnvironment(job);
    env.BUILDKITE_REPO = env.BUILDKITE_REPO.replace(
      new RegExp("^git@github.com:(UPCX/)?"),
      ""
    ...
    testResults.forEach((result) => {
      ...
      const test = {
        ...
        agentRole:
          ...
          env.BUILDKITE_AGENT_META_DATA_ROLE,
    ...
    return metrics;
  } else if (!isNullOrEmpty(buildkiteObject.number)) {
    // input is a Buildkite build object
    const build = buildkiteObject;
    console.log(
      `Processing test metrics for ${build.pipeline.slug} build ${
        build.number
      }${inBuildkite ? "" : ` at ${build.web_url}`}...`
    );
    let metrics = [],
      promises = [];
    ...
    build.jobs
      .filter(
        (job) =>
          job.type === "script" &&
          /test/.test(job.name.toLowerCase()) &&
          !/test metrics/.test(job.name.toLowerCase())
      )
      .forEach((job) => {
        promises.push(
          testMetrics(job)
            ...
            .catch((error) => {
              console.log(
                `${
                  inBuildkite ? "+++ :no_entry: " : ""
                }ERROR: Failed to process test metrics for "${
                  job.name
                }"! Link: ${job.web_url}`
              );
              console.log(JSON.stringify(error));
    ...
    console.log(
      `${
        inBuildkite ? "+++ :no_entry: " : ""
      }ERROR: Buildkite object not recognized or not a test step!`
    );
...
async function main() {
  ...
  console.log(
    `${
      inBuildkite ? "+++ :evergreen_tree: " : ""
    }Getting information from enviroment...`
  );
  ...
  if (
    isNullOrEmpty(buildNumber) ||
    isNullOrEmpty(pipeline) ||
    isNullOrEmpty(process.env.BUILDKITE_API_KEY)
  ) {
    console.log(
      `${inBuildkite ? "+++ :no_entry: " : ""}ERROR: Missing required inputs!`
    );
    if (isNullOrEmpty(process.env.BUILDKITE_API_KEY))
      console.log(
        "- Buildkite API key, as BUILDKITE_API_KEY environment variable"
      );
    if (isNullOrEmpty(buildNumber))
      console.log("- Build Number, as BUILDKITE_BUILD_NUMBER or argument 1");
    if (isNullOrEmpty(pipeline))
      console.log("- Pipeline Slug, as BUILDKITE_PIPELINE_SLUG or argument 2");
    errorCount = -1;
  } else {
    console.log(
  ...
  if (errorCount === 0)
    ...
    console.log(
      `Please send automation a link to this job${
        isNullOrEmpty(build) ? "." : `: ${build.web_url}`
      }`
    );
