...
#include <upcx/vm/execution_interface.hpp>
#include <upcx/vm/host_function.hpp>
#include <upcx/vm/opcodes.hpp>
#include <upcx/vm/signals.hpp>
...
namespace upcx { namespace vm {
   ...
   namespace detail {
      ...
      struct type_converter<std::nullptr_t> {
         using type = upcx::vm::type_converter<std::nullptr_t, upcx::vm::execution_interface>;
      };
...
#define CREATE_LABEL(NAME, CODE)                                                                                  \
      ev_label_##NAME : visitor(ev_variant->template get<upcx::vm::UPCX_VM_OPCODE_T(NAME)>());                    \
