...
#include <upcx/vm/function_traits.hpp>
...
#define UPCX_VM_FROM_WASM_ADD_TAG(...) (__VA_ARGS__, ::upcx::vm::tag<T> = {})
...
#define UPCX_VM_INVOKE_ON(TYPE, CONDITION) \
   upcx::vm::invoke_on<false, TYPE>(CONDITION, args...);

#define UPCX_VM_INVOKE_ON_ALL(CONDITION) \
   upcx::vm::invoke_on<false, upcx::vm::invoke_on_all_t>(CONDITION, args...);

#define UPCX_VM_INVOKE_ONCE(CONDITION) \
   upcx::vm::invoke_on<true, upcx::vm::invoke_on_all_t>(CONDITION, args...);
   ...
   struct registered_host_functions {
      ...
      static void resolve(Module& mod) {
         ...
         for (std::size_t i = 0; i < mod.imports.size(); i++) {
            ...
            printf("%s", mod_name.c_str());
            UPCX_VM_ASSERT(current_mappings.named_mapping.count({ mod_name, fn_name }), wasm_link_exception, fn_name.c_str());
