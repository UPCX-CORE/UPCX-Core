...
#include <upcx/vm/constants.hpp>
#include <upcx/vm/exceptions.hpp>
#include <upcx/vm/leb128.hpp>
#include <upcx/vm/options.hpp>
#include <upcx/vm/sections.hpp>
#include <upcx/vm/types.hpp>
#include <upcx/vm/utils.hpp>
#include <upcx/vm/vector.hpp>
...
#define MAX_ELEMENTS(name, default_)\
   ...
   // Matches the behavior of upcx::chain::wasm_validations::nested_validator
   template<typename Options, typename Enable = void>
   struct upcx_max_nested_structures_checker {
      void on_control(const Options&) {}
      void on_end(const Options&) {}
   };
   template<typename Options>
   struct upcx_max_nested_structures_checker<Options, std::void_t<decltype(std::declval<Options>().upcx_max_nested_structures)>> {
      void on_control(const Options& options) {
         ++_count;
         UPCX_VM_ASSERT(_count <= options.upcx_max_nested_structures, wasm_parse_exception, "Nested depth exceeded");
      }
      void on_end(const Options& options) {
         if(_count == 0) ++_count;
         else --_count;
      }
      std::decay_t<decltype(std::declval<Options>().upcx_max_nested_structures)> _count = 0;
   };
...
#undef BINOP
    ...
    private:
      ...
      detail::upcx_max_nested_structures_checker<Options> _nested_checker;
   };
}} // namespace upcx::vm
