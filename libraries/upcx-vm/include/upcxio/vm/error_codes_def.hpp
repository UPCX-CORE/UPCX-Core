...
#define UPCX_VM_OPEN_NAMESPACE namespace upcxio::vm {
#define UPCX_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "upcx-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "upcx-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "upcx-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "upcx-vm auxiliary errors")
...
#define _UPCX_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define UPCX_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \
