...
#define UPCX_VM_HAS_MEMBER(ARG, NAME)                           \
   upcxio::vm::detail::overloaded {                             \
        [](auto&& f, std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>> && \
                        upcxio::vm::detail::pass_type<          \
                            decltype(&std::decay_t<decltype(f)>::type::NAME)>(), int> = 0) constexpr { \
                            return true;                       \
       }, [](...) constexpr { return false; }                  \
    }(upcxio::vm::detail::wrapper_t<decltype(ARG)>{})

#define UPCX_VM_HAS_MEMBER_TY(TY, NAME)                         \
   upcxio::vm::detail::overloaded {                             \
    ...
    }(upcxio::vm::detail::wrapper_t<TY>{})

#define UPCX_VM_HAS_TEMPLATE_MEMBER(ARG, NAME)                  \
   upcxio::vm::detail::overloaded {                             \
        [&](auto&& f, std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>> && \
                        upcxio::vm::detail::pass_type<          \
...
#define UPCX_VM_HAS_TEMPLATE_MEMBER_TY(TY, NAME)                \
   upcxio::vm::detail::overloaded {                             \
        [](auto&& f, std::enable_if_t<std::is_class_v<TY> &&   \
                        upcxio::vm::detail::pass_type<          \
                            decltype(&std::decay_t<decltype(f)>::type::template NAME)>(), int> = 0) constexpr { \
...
#define AUTO_PARAM_WORKAROUND(X) upcxio::vm::detail::make_dependent<decltype(X)>(X)
...
namespace upcxio { namespace vm {
   ...
   template <auto FN>
   inline constexpr static bool is_callable_v = UPCX_VM_HAS_MEMBER(AUTO_PARAM_WORKAROUND(FN), operator());

   template <typename F>
   constexpr bool is_callable(F&& fn) { return UPCX_VM_HAS_MEMBER(fn, operator()); }
