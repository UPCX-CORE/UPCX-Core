...
TEST_CASE("Testing signals", "[invoke_with_signal_handler]") {
   bool okay = false;
   try {
      upcx::vm::invoke_with_signal_handler([]() {
         std::raise(SIGSEGV);
      }, [](int sig) {
...
TEST_CASE("Testing throw", "[signal_handler_throw]") {
   CHECK_THROWS_AS(upcx::vm::invoke_with_signal_handler([](){
      upcx::vm::throw_<upcx::vm::wasm_exit_exception>( "Exiting" );
   }, [](int){}), upcx::vm::wasm_exit_exception);
}
...
TEST_CASE("Test signal handler forwarding", "[signal_handler_forward]") {
   ...
   auto guard = upcx::vm::scope_guard{[]{
      std::signal(SIGSEGV, SIG_DFL);
      std::signal(SIGBUS, SIG_DFL);
      std::signal(SIGFPE, SIG_DFL);
      upcx::vm::setup_signal_handler_impl(); // This is normally only called once
   }};
   {
      std::signal(SIGSEGV, &handle_signal);
      std::signal(SIGBUS, &handle_signal);
      std::signal(SIGFPE, &handle_signal);
      upcx::vm::setup_signal_handler_impl();
      sig_handled = 0;
      std::raise(SIGSEGV);
