...
#include <upcxio/vm/error_codes.hpp>
...
struct example_host_methods {
   ...
   void upcxio_assert(bool test, const char* msg) {
      if (!test) {
...
/**
 * Simple implementation of an interpreter using upcx-vm.
 */
int main(int argc, char** argv) {
   ...
   using rhf_t     = upcxio::vm::registered_host_functions<example_host_methods, execution_interface, cnv>;
   using backend_t = upcxio::vm::backend<rhf_t>;

   // register print_num
   rhf_t::add<&example_host_methods::print_num>("env", "print_num");
   // register upcxio_assert
   rhf_t::add<&example_host_methods::upcxio_assert>("env", "upcxio_assert");
   // register print_name
   ...
   } catch (...) { std::cerr << "upcx-vm interpreter error\n"; }
   return 0;
}
