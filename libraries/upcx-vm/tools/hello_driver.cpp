...
#include <upcx/vm/error_codes.hpp>
...
struct example_host_methods {
   ...
   void upcx_assert(bool test, const char* msg) {
      if (!test) {
...
int main(int argc, char** argv) {
   ...
   using rhf_t     = upcx::vm::registered_host_functions<example_host_methods, execution_interface, cnv>;
   using backend_t = upcx::vm::backend<rhf_t>;

   // register print_num
   rhf_t::add<&example_host_methods::print_num>("env", "print_num");
   // register upcx_assert
   rhf_t::add<&example_host_methods::upcx_assert>("env", "upcx_assert");
   // register print_name
