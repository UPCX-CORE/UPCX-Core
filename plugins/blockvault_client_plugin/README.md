...
Block Vault is a component in an UPCX network architecture which provides a replicated durable storage with strong consistency guarantees for all the input required by a redundant cluster of nodupcx nodes to achieve the guarantees outlined in BlockVault: Failover for Nodupcx reproduced here:
...
```
nodupcx --plugin upcx::producer_plugin --producer-name myproducera --plugin upcx::blockvault_client_plugin --block-vault-backend postgresql://user:password@mycompany.com
```
...
nodupcx --plugin upcx::producer_plugin --producer-name myproducera --plugin upcx::blockvault_client_plugin --block-vault-backend postgresql://user@mycompany.com
```
