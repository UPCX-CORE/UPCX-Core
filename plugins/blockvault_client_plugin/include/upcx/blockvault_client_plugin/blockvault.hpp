...
using watermark_t = std::pair<uint32_t, upcx::chain::block_timestamp_type>;

class sync_callback {
 public:
   virtual void on_snapshot(const char* snapshot_filename)     = 0;
   virtual void on_block(upcx::chain::signed_block_ptr block) = 0;
};
...
class block_vault_interface {
 public:
   ...
   virtual void async_propose_constructed_block(uint32_t lib,
                                                upcx::chain::signed_block_ptr block,
   ...
   /// \param handler The callback function to inform caller whether the block has accepted the Block Vault or not.
   ///
   virtual void async_append_external_block(uint32_t lib, upcx::chain::signed_block_ptr block,
                                            std::function<void(bool)> handler) = 0;
   ...
   ///
   virtual void sync(const upcx::chain::block_id_type* block_id, sync_callback& callback) = 0;
};
