...
using watermark_t = std::pair<uint32_t, upcxio::chain::block_timestamp_type>;

class sync_callback {
 public:
   virtual void on_snapshot(const char* snapshot_filename)     = 0;
   virtual void on_block(upcxio::chain::signed_block_ptr block) = 0;
};
...
class block_vault_interface {
 public:
   ...
   ///
   virtual void sync(const upcxio::chain::block_id_type* block_id, sync_callback& callback) = 0;
};
