...
namespace {
   ...
   bool is_onblock(const chain::transaction_trace_ptr& p) {
      ...
      if (act.account != upcxio::chain::config::system_account_name || act.name != "onblock"_n ||
          act.authorization.size() != 1)
         return false;
      const auto& auth = act.authorization[0];
      return auth.actor == upcxio::chain::config::system_account_name &&
             auth.permission == upcxio::chain::config::active_name;
   }
   ...
   struct account_query_db_impl {
      ...
      void build_account_query_map() {
         ...
         for (uint32_t block_num = lib_num + 1; block_num <= head_num; block_num++) {
            ...
            UPCX_ASSERT(block_p, chain::plugin_exception, "cannot fetch reversible block ${block_num}, required for account_db initialization", ("block_num", block_num));
      ...
      uint32_t last_updated_time_to_height( const fc::time_point& last_updated) {
         ...
         if (last_updated > lib_time) {
            ...
            UPCX_ASSERT(iter != time_to_block_num.end(), chain::plugin_exception, "invalid block time encountered in on-chain accounts ${time}", ("time", last_updated));
      ...
      void commit_block(const chain::block_state_ptr& bsp ) {
         ...
         if (!updated.empty() || !deleted.empty() || rollback_required) {
            ...
            for (const auto& up: updated) {
               ...
               UPCX_ASSERT(source_itr != permission_by_owner.end(), chain::plugin_exception, "chain data is missing");
