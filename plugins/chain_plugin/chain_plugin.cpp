...
#include <upcx/chain_plugin/blockvault_sync_strategy.hpp>
#include <upcx/chain/fork_database.hpp>
#include <upcx/chain/block_log.hpp>
...
std::ostream& operator<<(std::ostream& osm, upcx::chain::db_read_mode m) {
   if ( m == upcx::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == upcx::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == upcx::chain::db_read_mode::READ_ONLY ) { // deprecated
      osm << "read-only";
   } else if ( m == upcx::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              upcx::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(upcx::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(upcx::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(upcx::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(upcx::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, upcx::chain::validation_mode m) {
   if ( m == upcx::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == upcx::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              upcx::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(upcx::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(upcx::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, upcx::chain::backing_store_type b) {
   if ( b == upcx::chain::backing_store_type::CHAINBASE ) {
      osm << "chainbase";
   } else if ( b == upcx::chain::backing_store_type::ROCKSDB ) {
      osm << "rocksdb";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              upcx::chain::backing_store_type* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "chainbase" ) {
     v = boost::any(upcx::chain::backing_store_type::CHAINBASE);
  } else if ( s == "rocksdb" ) {
     v = boost::any(upcx::chain::backing_store_type::ROCKSDB);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace upcx::chain::config;
using namespace upcx::chain::plugin_interface;
...
chain_plugin::chain_plugin()
   ...
   app().register_config_type<upcx::chain::db_read_mode>();
   app().register_config_type<upcx::chain::validation_mode>();
   app().register_config_type<upcx::chain::backing_store_type>();
   app().register_config_type<chainbase::pinnable_mapped_file::map_mode>();
   app().register_config_type<upcx::chain::wasm_interface::vm_type>();
}
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   std::string delim;
#ifdef UPCX_UPCX_VM_JIT_RUNTIME_ENABLED
   ...
   std::string default_wasm_runtime_str= upcx::chain::wasm_interface::vm_type_string(upcx::chain::config::default_wasm_runtime);
   ...
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<upcx::chain::wasm_interface::vm_type>()->value_name("runtime")->notifier([](const auto& vm){
#ifndef UPCX_UPCX_VM_OC_DEVELOPER
...
#endif
         })->default_value(upcx::chain::config::default_wasm_runtime, default_wasm_runtime_str), wasm_runtime_opt.c_str()
         )
         ...
         ("backing-store", boost::program_options::value<upcx::chain::backing_store_type>()->default_value(upcx::chain::backing_store_type::CHAINBASE),
         ...
         ("read-mode", boost::program_options::value<upcx::chain::db_read_mode>()->default_value(upcx::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<upcx::chain::validation_mode>()->default_value(upcx::chain::validation_mode::FULL),
          "Chain validation mode (\"full\" or \"light\").\n"
          "In \"full\" mode all incoming blocks will be fully validated.\n"
          "In \"light\" mode all incoming blocks headers will be fully validated; transactions in those validated blocks will be trusted \n")
         ("disable-ram-billing-notify-checks", bpo::bool_switch()->default_value(false),
          "Disable the check which subjectively fails a transaction if a contract bills more RAM to another account within the context of a notification handler (i.e. when the receiver is not the code of the action).")
#ifdef UPCX_DEVELOPER
...
if( options.count(op_name) ) { \
   ...
   for( const auto& v : ops ) { \
      container.emplace( upcx::chain::name( v ) ); \
   } \
}
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if UPCX_ROOT_KEY is bad
      } catch ( const std::exception& ) {
         elog( "UPCX_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::upcx_root_key));
         throw;
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         upcx::chain::combined_database::destroy( my->chain_config->state_dir );
...
uint64_t convert_to_type(const upcx::name &n, const string &desc) {
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,UPCX
      try {
         auto symb = upcx::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }

   try {
      return ( upcx::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
...
read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = upcx::chain_apis::get_abi( db, p.code );
...
struct kv_table_rows_context {
   std::unique_ptr<upcx::chain::kv_context>  kv_context;
   ...
   kv_table_rows_context(const controller& db, const read_only::get_kv_table_rows_params& param,
       ...
       , abi(upcx::chain_apis::get_abi(db, param.code))
...
read_only::get_table_by_scope_result read_only::get_table_by_scope( const read_only::get_table_by_scope_params& p )const {
   ...
   if (db_backing_store == upcx::chain::backing_store_type::CHAINBASE) {
      ...
      if (reverse) {
         lower = upcx::session::shared_bytes::truncate_key(lower);
      }
      ...
      upcx::chain::backing_store::walk_rocksdb_entries_with_prefix(kv_database.get_kv_undo_stack(), lower, upper, writer);
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( upcx::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const try {
   ...
   const abi_def abi = upcx::chain_apis::get_abi(db, config::system_account_name);
   ...
   if (db_backing_store == upcx::chain::backing_store_type::CHAINBASE) {
      ...
      upcx::chain::backing_store::walk_rocksdb_entries_with_prefix(kv_database.get_kv_undo_stack(), lower_key, upper_key, writer);
   };
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on upcx account (i.e. the system contract) and how it stores its data.
   get_primary_key<detail::ram_market_exchange_state_t>("upcx"_n, "upcx"_n, "rammarket"_n, upcx::chain::string_to_symbol_c(4,"RAMCORE"),
...
fc::variant read_only::get_primary_key(name code, name scope, name table, uint64_t primary_key, row_requirements require_table,
   ...
   const abi_def abi = upcx::chain_apis::get_abi(db, code);
   abi_serializer abis;
...
upcx::chain::backing_store_type read_only::get_backing_store() const {
   const auto& kv_database = db.kv_db();
...
FC_REFLECT( upcx::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
