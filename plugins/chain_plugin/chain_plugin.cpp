#include <upcxio/chain_plugin/chain_plugin.hpp>
#include <upcxio/chain_plugin/blockvault_sync_strategy.hpp>
#include <upcxio/chain/fork_database.hpp>
#include <upcxio/chain/block_log.hpp>
#include <upcxio/chain/exceptions.hpp>
#include <upcxio/chain/authorization_manager.hpp>
#include <upcxio/chain/code_object.hpp>
#include <upcxio/chain/config.hpp>
#include <upcxio/chain/wasm_interface.hpp>
#include <upcxio/chain/resource_limits.hpp>
#include <upcxio/chain/reversible_block_object.hpp>
#include <upcxio/chain/controller.hpp>
#include <upcxio/chain/generated_transaction_object.hpp>
#include <upcxio/chain/global_property_object.hpp>
#include <upcxio/chain/snapshot.hpp>
#include <upcxio/chain/combined_database.hpp>
#include <upcxio/chain/backing_store/kv_context.hpp>
#include <upcxio/to_key.hpp>

#include <upcxio/chain/upcxio_contract.hpp>
#include <upcxio/resource_monitor_plugin/resource_monitor_plugin.hpp>
#include <upcxio/blockvault_client_plugin/blockvault_client_plugin.hpp>
...
std::ostream& operator<<(std::ostream& osm, upcxio::chain::db_read_mode m) {
   if ( m == upcxio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == upcxio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == upcxio::chain::db_read_mode::READ_ONLY ) { // deprecated
      osm << "read-only";
   } else if ( m == upcxio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              upcxio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(upcxio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(upcxio::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(upcxio::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(upcxio::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, upcxio::chain::validation_mode m) {
   if ( m == upcxio::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == upcxio::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              upcxio::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(upcxio::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(upcxio::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, upcxio::chain::backing_store_type b) {
   if ( b == upcxio::chain::backing_store_type::CHAINBASE ) {
      osm << "chainbase";
   } else if ( b == upcxio::chain::backing_store_type::ROCKSDB ) {
      osm << "rocksdb";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              upcxio::chain::backing_store_type* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "chainbase" ) {
     v = boost::any(upcxio::chain::backing_store_type::CHAINBASE);
  } else if ( s == "rocksdb" ) {
     v = boost::any(upcxio::chain::backing_store_type::ROCKSDB);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace upcxio::chain;
using namespace upcxio::chain::config;
using namespace upcxio::chain::plugin_interface;
...
chain_plugin::chain_plugin()
   ...
   app().register_config_type<upcxio::chain::db_read_mode>();
   app().register_config_type<upcxio::chain::validation_mode>();
   app().register_config_type<upcxio::chain::backing_store_type>();
   app().register_config_type<chainbase::pinnable_mapped_file::map_mode>();
   app().register_config_type<upcxio::chain::wasm_interface::vm_type>();
}
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   wasm_runtime_opt += " \"upcx-vm-jit\"";
   wasm_runtime_desc += "\"upcx-vm-jit\" : A WebAssembly runtime that compiles WebAssembly code to native x86 code prior to execution.\n";
   delim = ", ";
#endif

#ifdef UPCXIO_UPCX_VM_RUNTIME_ENABLED
   wasm_runtime_opt += delim + "\"upcx-vm\"";
   wasm_runtime_desc += "\"upcx-vm\" : A WebAssembly interpreter.\n";
   delim = ", ";
#endif

#ifdef UPCXIO_UPCX_VM_OC_DEVELOPER
   wasm_runtime_opt += delim + "\"upcx-vm-oc\"";
   wasm_runtime_desc += "\"upcx-vm-oc\" : Unsupported. Instead, use one of the other runtimes along with the option enable-upcx-vm-oc.\n";
#endif
   wasm_runtime_opt += ")\n" + wasm_runtime_desc;

   std::string default_wasm_runtime_str= upcxio::chain::wasm_interface::vm_type_string(upcxio::chain::config::default_wasm_runtime);
   ...
   cfg.add_options()
...
#endif
         })->default_value(upcxio::chain::config::default_wasm_runtime, default_wasm_runtime_str), wasm_runtime_opt.c_str()
         )
         ...
         ("backing-store", boost::program_options::value<upcxio::chain::backing_store_type>()->default_value(upcxio::chain::backing_store_type::CHAINBASE),
         ...
         ("read-mode", boost::program_options::value<upcxio::chain::db_read_mode>()->default_value(upcxio::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<upcxio::chain::validation_mode>()->default_value(upcxio::chain::validation_mode::FULL),
          "Chain validation mode (\"full\" or \"light\").\n"
          "In \"full\" mode all incoming blocks will be fully validated.\n"
          "In \"light\" mode all incoming blocks headers will be fully validated; transactions in those validated blocks will be trusted \n")
         ("disable-ram-billing-notify-checks", bpo::bool_switch()->default_value(false),
          "Disable the check which subjectively fails a transaction if a contract bills more RAM to another account within the context of a notification handler (i.e. when the receiver is not the code of the action).")
#ifdef UPCXIO_DEVELOPER
...
if( options.count(op_name) ) { \
   ...
   for( const auto& v : ops ) { \
      container.emplace( upcxio::chain::name( v ) ); \
   } \
}
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      UPCX_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         UPCX_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      UPCX_ASSERT( !fc::exists( file_path ), plugin_exception,
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if UPCXIO_ROOT_KEY is bad
      } catch ( const std::exception& ) {
         elog( "UPCXIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::upcxio_root_key));
         throw;
      }
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            UPCX_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         UPCX_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            UPCX_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               UPCX_ASSERT( itr->second == item.second,
      ...
      if( options.count( "persistent-storage-num-threads" )) {
         ...
         UPCX_ASSERT( my->chain_config->persistent_storage_num_threads > 0, plugin_config_exception,
      ...
      if( options.count( "persistent-storage-max-num-files" )) {
         ...
         UPCX_ASSERT( my->chain_config->persistent_storage_max_num_files == -1 || my->chain_config->persistent_storage_max_num_files > 0, plugin_config_exception,
      ...
      if( options.count( "persistent-storage-write-buffer-size-mb" )) {
         ...
         UPCX_ASSERT( my->chain_config->persistent_storage_write_buffer_size > 0, plugin_config_exception,
      ...
      if( options.count( "persistent-storage-bytes-per-sync" )) {
         ...
         UPCX_ASSERT( my->chain_config->persistent_storage_bytes_per_sync > 0, plugin_config_exception,
      ...
      UPCX_ASSERT( my->chain_config->persistent_storage_mbytes_batch > 0, plugin_config_exception,
      ...
      if( options.count( "chain-threads" )) {
         ...
         UPCX_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      UPCX_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            UPCX_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
         ...
         UPCX_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         UPCX_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         upcxio::chain::combined_database::destroy( my->chain_config->state_dir );
         ...
         UPCX_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         UPCX_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         UPCX_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         UPCX_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               UPCX_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               UPCX_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            UPCX_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            if( block_log_genesis ) {
               UPCX_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  UPCX_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     UPCX_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               UPCX_ASSERT( !block_log_chain_id, plugin_config_exception,
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            ...
            UPCX_ASSERT( itr->second == id, checkpoint_exception,
...
{ try {
   ...
   try {
      ...
      if (nullptr != blockvault_instance) {
          upcxio::blockvault::blockvault_sync_strategy<chain_plugin_impl> bss(blockvault_instance, *my, shutdown, check_shutdown);
          bss.do_sync();
...
void chain_apis::read_write::validate() const {
   UPCX_ASSERT( api_accept_transactions, missing_chain_api_plugin_exception,
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      UPCX_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      UPCX_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         UPCX_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            UPCX_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   UPCX_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         UPCX_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const upcxio::name &n, const string &desc) {
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   try {
      return ( upcxio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      UPCX_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   UPCX_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      UPCX_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   UPCX_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   UPCX_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = upcxio::chain_apis::get_abi( db, p.code );
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      UPCX_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      UPCX_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      UPCX_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      UPCX_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
...
inline void key_convert_assert(bool condition) {
   // UPCX_ASSERT is avoided intentionally here because UPCX_ASSERT would create the fc::log_message object which is
...
struct kv_table_rows_context {
   std::unique_ptr<upcxio::chain::kv_context>  kv_context;
   ...
   kv_table_rows_context(const controller& db, const read_only::get_kv_table_rows_params& param,
       ...
       , abi(upcxio::chain_apis::get_abi(db, param.code))
       , shorten_abi_errors(shorten_error) {

      UPCX_ASSERT(p.limit > 0, chain::contract_table_query_exception, "invalid limit : ${n}", ("n", p.limit));
      ...
      if (table_it == abi.kv_tables.value.end()) {
         UPCX_ASSERT(false, chain::contract_table_query_exception, "Unknown kv_table: ${t}", ("t", tbl_name));
      }
      ...
      UPCX_ASSERT(is_primary_idx || is_sec_idx, chain::contract_table_query_exception, "Unknown kv index: ${t} ${i}",
...
struct kv_iterator_ex {
   ...
   kv_iterator_ex(const kv_table_rows_context& ctx, const std::vector<char>& full_key)
      ...
      UPCX_ASSERT(status != chain::kv_it_stat::iterator_erased, chain::contract_table_query_exception,
   ...
   std::vector<char> get_value() const {
      ...
      if (!context.is_primary_idx) {
         ...
         UPCX_ASSERT(success, chain::contract_table_query_exception, "invalid secondary index in ${t} ${i}",
...
read_only::get_table_rows_result read_only::get_kv_table_rows(const read_only::get_kv_table_rows_params& p) const {
   ...
   if (context.point_query()) {
      UPCX_ASSERT(p.lower_bound.empty() && p.upper_bound.empty(), chain::contract_table_query_exception,
...
read_only::get_table_by_scope_result read_only::get_table_by_scope( const read_only::get_table_by_scope_params& p )const {
   ...
   if (db_backing_store == upcxio::chain::backing_store_type::CHAINBASE) {
      ...
      using namespace upcxio::chain;
      UPCX_ASSERT(db_backing_store == backing_store_type::ROCKSDB,
      ...
      if (reverse) {
         lower = upcxio::session::shared_bytes::truncate_key(lower);
      }
      ...
      upcxio::chain::backing_store::walk_rocksdb_entries_with_prefix(kv_database.get_kv_undo_stack(), lower, upper, writer);
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, "accounts"_n, [&](const auto& obj){
      UPCX_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      UPCX_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( upcxio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, name(scope), "stat"_n, [&](const auto& obj){
      UPCX_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const try {
   ...
   const abi_def abi = upcxio::chain_apis::get_abi(db, config::system_account_name);
   ...
   UPCX_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   if (db_backing_store == upcxio::chain::backing_store_type::CHAINBASE) {
      ...
      UPCX_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      upcxio::chain::backing_store::walk_rocksdb_entries_with_prefix(kv_database.get_kv_undo_stack(), lower_key, upper_key, writer);
   };

   constexpr name global = "global"_n;
   const auto global_table_type = get_table_type(abi, global);
   UPCX_ASSERT(global_table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",global_table_type));
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  UPCX_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   UPCX_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
   ...
   if( block_num ) {
      ...
      } UPCX_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   UPCX_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_info(const read_only::get_block_info_params& params) const {
   ...
   UPCX_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   UPCX_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      } UPCX_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      UPCX_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   UPCX_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = "upcxio.token"_n;
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   UPCX_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer::create_yield_function( abi_serializer_max_time ) );
      auto action_type = abis.get_action_type(params.action);
      UPCX_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
      ...
      } UPCX_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      UPCX_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } UPCX_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
{
   UPCX_ASSERT(aqdb.has_value(), plugin_config_exception, "Account Queries being accessed when not enabled");
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on upcxio account (i.e. the system contract) and how it stores its data.
   get_primary_key<detail::ram_market_exchange_state_t>("upcxio"_n, "upcxio"_n, "rammarket"_n, upcxio::chain::string_to_symbol_c(4,"RAMCORE"),
...
fc::variant read_only::get_primary_key(name code, name scope, name table, uint64_t primary_key, row_requirements require_table,
   ...
   const abi_def abi = upcxio::chain_apis::get_abi(db, code);
   abi_serializer abis;
...
upcxio::chain::backing_store_type read_only::get_backing_store() const {
   const auto& kv_database = db.kv_db();
...
FC_REFLECT( upcxio::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
