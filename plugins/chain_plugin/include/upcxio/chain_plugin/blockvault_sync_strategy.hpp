#pragma once
#include <upcxio/blockvault_client_plugin/blockvault.hpp>
...
struct blockvault_sync_strategy : public sync_callback {
   blockvault_sync_strategy(block_vault_interface* blockvault, BP& blockchain_provider, std::function<void()> shutdown,
      ...
      UPCX_ASSERT(nullptr != blockvault, plugin_exception, "block_vault_interface cannot be null");
   }
   ...
   void on_snapshot(const char* snapshot_filename) override final {
      ...
      UPCX_ASSERT(!_received_snapshot, plugin_exception, "Received multiple snapshots from blockvault." );
   ...
   void on_block(upcxio::chain::signed_block_ptr block) override final {
      ...
      try {
         ...
         UPCX_ASSERT(rc, plugin_exception,
