...
using namespace upcxio::blockvault;
...
struct mock_blockvault_t : public block_vault_interface {
   upcxio::chain::block_id_type _previous_block_id{};
   bool                        _previous_block_id_sent = false;

   virtual void async_propose_constructed_block(uint32_t lib,
                                                upcxio::chain::signed_block_ptr block,
                                                std::function<void(bool)>      handler) override {}
   virtual void async_append_external_block(uint32_t lib, upcxio::chain::signed_block_ptr block,
                                            std::function<void(bool)> handler) override {}

   virtual bool propose_snapshot(watermark_t watermark, const char* snapshot_filename) override { return true; }
   virtual void sync(const upcxio::chain::block_id_type* previous_block_id, sync_callback& callback) override {
