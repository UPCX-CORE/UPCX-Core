...
#include <upcxio/http_plugin/local_endpoint.hpp>
#endif
      ...
      public:
         ...
         ssl_context_ptr on_tls_init() {
            ...
            try {
               ...
               if (!ecdh)
                  UPCX_THROW(chain::http_exception, "Failed to set NID_secp384r1");
               if(SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY*)ecdh) != 1)
                  UPCX_THROW(chain::http_exception, "Failed to set ECDH PFS");
               ...
               if(SSL_CTX_set_cipher_list(ctx->native_handle(), \
                  ...
                  UPCX_THROW(chain::http_exception, "Failed to set HTTPS cipher list");
