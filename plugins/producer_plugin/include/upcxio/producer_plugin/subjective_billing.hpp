...
private:
   uint32_t time_ordinal_for( const fc::time_point& t ) const {
      ...
      UPCX_ASSERT(ordinal <= std::numeric_limits<uint32_t>::max(), chain::tx_resource_exhaustion, "overflow of quantized time in subjective billing");
      return ordinal;
   }
   ...
   void remove_subjective_billing( const trx_cache_entry& entry, uint32_t time_ordinal ) {
      ...
      if( aitr != _account_subjective_bill_cache.end() ) {
         ...
         UPCX_ASSERT( aitr->second.pending_cpu_us >= 0, chain::tx_resource_exhaustion,
...
public:
   ...
   uint32_t get_subjective_bill( const account_name& first_auth, const fc::time_point& now ) const {
      ...
      if (sub_bill_info) {
         UPCX_ASSERT(sub_bill_info->pending_cpu_us >= in_block_pending_cpu_us, chain::tx_resource_exhaustion, "Logic error subjective billing ${a}", ("a", first_auth) );
