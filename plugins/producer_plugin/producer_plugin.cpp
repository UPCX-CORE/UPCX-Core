...
#include <upcx/producer_plugin/pending_snapshot.hpp>
#include <upcx/producer_plugin/subjective_billing.hpp>
...
#include <upcx/chain/transaction_object.hpp>
#include <upcx/chain/thread_utils.hpp>
#include <upcx/chain/unapplied_transaction_queue.hpp>
...
class block_only_sync : public blockvault::sync_callback {
 ...
 public:
   ...
   void on_block(upcx::chain::signed_block_ptr block) override;
};
...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      upcx::blockvault::block_vault_interface*                 blockvault = nullptr;
...
void new_chain_banner(const upcx::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
...
private:
   struct account_failure {
      ...
      void add( const account_name& n, int64_t exception_code ) {
         ...
         } else if( exception_code == upcx_assert_message_exception::code_value ||
                    exception_code == upcx_assert_code_exception::code_value ) {
            ex_flags = set_field( ex_flags, ex_fields::ex_upcx_assert_exception );
         } else {
      ...
      bool is_upcx_assert() const { return has_field( ex_flags, ex_fields::ex_upcx_assert_exception ); }
...
void block_only_sync::on_block(upcx::chain::signed_block_ptr block) {
   try {
