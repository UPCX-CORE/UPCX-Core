#include <upcxio/producer_plugin/producer_plugin.hpp>
#include <upcxio/producer_plugin/pending_snapshot.hpp>
#include <upcxio/producer_plugin/subjective_billing.hpp>
...
#include <upcxio/chain/transaction_object.hpp>
#include <upcxio/chain/thread_utils.hpp>
#include <upcxio/chain/unapplied_transaction_queue.hpp>
...
class block_only_sync : public blockvault::sync_callback {
 ...
 public:
   ...
   void on_block(upcxio::chain::signed_block_ptr block) override;
};
...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      upcxio::blockvault::block_vault_interface*                 blockvault = nullptr;
      ...
      bool on_incoming_block(const signed_block_ptr& block, const std::optional<block_id_type>& block_id) {
         ...
         UPCX_ASSERT( block->timestamp < (fc::time_point::now() + fc::seconds( 7 )), block_from_the_future,
...
void new_chain_banner(const upcxio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    UPCX_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
{ try {
   ...
   UPCX_ASSERT( my->chain_plug, plugin_config_exception, "chain_plugin not found" );
   my->_options = &options;
   ...
   UPCX_ASSERT( my->_produce_time_offset_us <= 0 && my->_produce_time_offset_us >= -config::block_interval_us, plugin_config_exception,
   ...
   UPCX_ASSERT( my->_last_block_time_offset_us <= 0 && my->_last_block_time_offset_us >= -config::block_interval_us, plugin_config_exception,
   ...
   UPCX_ASSERT( cpu_effort_pct >= 0 && cpu_effort_pct <= 100, plugin_config_exception,
               "cpu-effort-percent ${pct} must be 0 - 100", ("pct", cpu_effort_pct) );
      cpu_effort_pct *= config::percent_1;
   int32_t cpu_effort_offset_us =
         -UPCX_PERCENT( config::block_interval_us, chain::config::percent_100 - cpu_effort_pct );

   uint32_t last_block_cpu_effort_pct = options.at("last-block-cpu-effort-percent").as<uint32_t>();
   UPCX_ASSERT( last_block_cpu_effort_pct >= 0 && last_block_cpu_effort_pct <= 100, plugin_config_exception,
   ...
   int32_t last_block_cpu_effort_offset_us =
         -UPCX_PERCENT( config::block_interval_us, chain::config::percent_100 - last_block_cpu_effort_pct );
   ...
   UPCX_ASSERT( my->_max_block_cpu_usage_threshold_us < config::block_interval_us, plugin_config_exception,
   ...
   UPCX_ASSERT( max_incoming_transaction_queue_size > 0, plugin_config_exception,
   ...
   UPCX_ASSERT( thread_pool_size > 0, plugin_config_exception,
   ...
   if( options.count( "snapshots-dir" )) {
      ...
      UPCX_ASSERT( fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
...
{ try {
   ...
   chain::controller& chain = my->chain_plug->chain();
   UPCX_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   UPCX_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
              "node cannot have any producer-name configured because block production is not safe when validation_mode is not \"full\"" );

   UPCX_ASSERT( my->_producers.empty() || my->chain_plug->accept_transactions(), plugin_config_exception,
...
void producer_plugin::create_snapshot(producer_plugin::next_function<producer_plugin::snapshot_information> next) {
   ...
   if( chain.get_read_mode() == db_read_mode::IRREVERSIBLE ) {
      try {
         ...
         UPCX_ASSERT(!ec, snapshot_finalization_exception,
...
void producer_plugin::schedule_protocol_feature_activations( const scheduled_protocol_feature_activations& schedule ) {
   ...
   UPCX_ASSERT( set_of_features_to_activate.size() == schedule.protocol_features_to_activate.size(),
   ...
   for (auto &feature_digest : set_of_features_to_activate) {
      ...
      UPCX_ASSERT( !pf.preactivation_required, protocol_feature_exception,
...
private:
   struct account_failure {
      ...
      void add( const account_name& n, int64_t exception_code ) {
         ...
         } else if( exception_code == upcxio_assert_message_exception::code_value ||
                    exception_code == upcxio_assert_code_exception::code_value ) {
            ex_flags = set_field( ex_flags, ex_fields::ex_upcxio_assert_exception );
         } else {
      ...
      bool is_upcxio_assert() const { return has_field( ex_flags, ex_fields::ex_upcxio_assert_exception ); }
      ...
      while( itr != end_itr ) {
         ...
         try {
            ...
            if(!_subjective_billing.is_disabled() && prev_billed_cpu_time_us > 0 && !rl.is_unlimited_cpu( first_auth )) {
               auto prev_billed_plus100 = prev_billed_cpu_time_us + UPCX_PERCENT( prev_billed_cpu_time_us, 100 * config::percent_1 );
...
void producer_plugin_impl::schedule_production_loop() {
   ...
   if (result == start_block_result::failed) {
      ...
      UPCX_ASSERT( chain.is_building_block(), missing_pending_block_state, "speculating without pending_block_state" );
...
void producer_plugin_impl::schedule_maybe_produce_block( bool exhausted ) {
   ...
   if( !exhausted && deadline > fc::time_point::now() ) {
      ...
      UPCX_ASSERT( chain.is_building_block(), missing_pending_block_state,
      ...
      UPCX_ASSERT( chain.is_building_block(), missing_pending_block_state, "producing without pending_block_state" );
...
void block_only_sync::on_snapshot(const char*) {
   UPCX_THROW(producer_exception, "a snapshot");
}

void block_only_sync::on_block(upcxio::chain::signed_block_ptr block) {
   try {
...
void producer_plugin_impl::produce_block() {
   ...
   UPCX_ASSERT(_pending_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");
   chain::controller& chain = chain_plug->chain();
   UPCX_ASSERT(chain.is_building_block(), missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
   ...
   UPCX_ASSERT(relevant_providers.size() > 0, producer_priv_key_not_found, "Attempting to produce a block for which we don't have any relevant private keys");
   ...
   if ( blockvault != nullptr ) {
      ...
      if (!f.get()) {
         ...
         UPCX_ASSERT(false, block_validation_error, "Block rejected by block vault");
      }
