...
namespace {
    upcxio::producer_plugin::snapshot_information test_snap_info;
}

BOOST_AUTO_TEST_SUITE(snapshot_tests)

using next_t = upcxio::producer_plugin::next_function<upcxio::producer_plugin::snapshot_information>;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_snapshot_information, SNAPSHOT_SUITE, snapshot_suites) {
   ...
   auto final_path = upcxio::pending_snapshot::get_final_path(block2->previous, "../snapshots/");
   auto pending_path = upcxio::pending_snapshot::get_pending_path(block2->previous, "../snapshots/");

   write_snapshot( pending_path );
   next_t next;
   upcxio::pending_snapshot pending{ block2->previous, next, pending_path.generic_string(), final_path.generic_string(), nullptr };
