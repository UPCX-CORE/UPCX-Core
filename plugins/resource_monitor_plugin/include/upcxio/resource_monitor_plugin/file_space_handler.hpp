...
namespace upcxio::resource_monitor {
   ...
   public:
      ...
      void set_threshold(uint32_t new_threshold, uint32_t new_warning_threshold) {
         UPCX_ASSERT(new_warning_threshold < new_threshold, chain::plugin_config_exception,
         ...
         for (auto& fs: filesystems) {
            ...
            if ( info.available < fs.shutdown_available ) {
               ...
               if ( shutdown_on_exceeded) {
                  wlog("nodupcx will shutdown when space usage exceeds threshold ${threshold}%", ("threshold", shutdown_threshold));
               }
            }
         }
      ...
      void add_file_system(const bfs::path& path_name) {
         ...
         UPCX_ASSERT(status == 0, chain::plugin_config_exception,
         ...
         UPCX_ASSERT(!ec, chain::plugin_config_exception,
   ...
   void space_monitor_loop() {
      if ( is_threshold_exceeded() && shutdown_on_exceeded ) {
         wlog("Shutting down");
         appbase::app().quit(); // This will gracefully stop Nodupcx
         return;
      }
