/**
   ...
   belons to is running out of space, nodupcx will crash with SIGBUS as
...
#include <upcxio/resource_monitor_plugin/file_space_handler.hpp>
#include <upcxio/resource_monitor_plugin/system_file_space_provider.hpp>
...
using namespace upcxio::resource_monitor;
...
public:
   ...
   void set_program_options(options_description&, options_description& cfg) {
      cfg.add_options()
         ...
         ( "resource-monitor-not-shutdown-on-threshold-exceeded",
           "Used to indicate nodupcx will not shutdown when threshold is exceeded." )
   ...
   void plugin_initialize(const appbase::variables_map& options) {
      ...
      UPCX_ASSERT(interval >= monitor_interval_min && interval <= monitor_interval_max, chain::plugin_config_exception,
      ...
      UPCX_ASSERT(threshold >= space_threshold_min  && threshold <= space_threshold_max, chain::plugin_config_exception,
      ...
      UPCX_ASSERT(warning_interval >= warning_interval_min && warning_interval <= warning_interval_max, chain::plugin_config_exception,
