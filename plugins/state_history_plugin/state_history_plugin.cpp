...
#include <upcxio/state_history_plugin/state_history_plugin.hpp>
   ...
   void listen() {
      ...
      auto check_ec = [&](const char* what) {
         if (!ec)
            return;
         fc_elog(_log,"${w}: ${m}", ("w", what)("m", ec.message()));
         UPCX_ASSERT(false, plugin_exception, "unable to open listen socket");
      };
...
void state_history_plugin::plugin_initialize(const variables_map& options) {
   try {
      UPCX_ASSERT(options.at("disable-replay-opts").as<bool>(), plugin_exception,
      ...
      static upcxio::state_history_config config;
