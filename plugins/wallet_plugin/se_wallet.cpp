...
#include <upcxio/wallet_plugin/macos_user_auth.h>
...
se_wallet::se_wallet() : my(new detail::se_wallet_impl()) {
   if(!secure_enclave::application_signed()) {
      ...
      UPCX_THROW(secure_enclave_exception, "");
   }

   if(!secure_enclave::hardware_supports_secure_enclave())
      UPCX_THROW(secure_enclave_exception, "Secure Enclave not supported on this hardware");
}
...
void se_wallet::lock() {
   UPCX_ASSERT(!is_locked(), wallet_locked_exception, "You can not lock an already locked wallet");
   my->locked = true;
}
...
void se_wallet::unlock(string password) {
   ...
   macos_user_auth(detail::auth_callback, &prom, CFSTR("unlock your UPCXIO wallet"));
   if(!fut.get())
...
string se_wallet::create_key(string key_type) {
   UPCX_ASSERT(key_type.empty() || key_type == "R1", chain::unsupported_key_type_exception, "Secure Enclave wallet only supports R1 keys");
   return secure_enclave::create_key().public_key().to_string();
}

bool se_wallet::remove_key(string key) {
   UPCX_ASSERT(!is_locked(), wallet_locked_exception, "You can not remove a key from a locked wallet");

   auto se_keys = secure_enclave::get_all_keys();

   for(auto it = se_keys.begin(); it != se_keys.end(); ++it)
      if(it->public_key().to_string() == key) {
         upcxio::secure_enclave::delete_key(std::move(se_keys.extract(it).value()));
         return true;
      }
