/**
  ...
  have a local copy of `nodeos` running and configured to load the 'upcxio::chain_api_plugin'.
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      createAccount->callback([this] {
            ...
            if (!simple) {
               UPCXC_ASSERT( buy_ram_eos.size() || buy_ram_bytes_in_kbytes || buy_ram_bytes, "ERROR: One of --buy-ram, --buy-ram-kbytes or --buy-ram-bytes should have non-zero value" );
