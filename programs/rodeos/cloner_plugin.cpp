...
struct cloner_session : ship_client::connection_callbacks, std::enable_shared_from_this<cloner_session> {
   ...
   void connect(asio::io_context& ioc) {
      ...
      ilog("    chain:        ${a}", ("a", upcxio::convert_to_json(rodeos_snapshot->chain_id)));
      ilog("    head:         ${a} ${b}",
           ("a", rodeos_snapshot->head)("b", upcxio::convert_to_json(rodeos_snapshot->head_id)));
      ilog("    irreversible: ${a} ${b}",
           ("a", rodeos_snapshot->irreversible)(
                 "b", upcxio::convert_to_json(rodeos_snapshot->irreversible_id)));
   ...
   bool received(get_status_result_v0& status, upcxio::input_stream bin) override {
      ilog("nodeos has chain ${c}", ("c", upcxio::convert_to_json(status.chain_id)));
      if (rodeos_snapshot->chain_id == upcxio::checksum256{})
         rodeos_snapshot->chain_id = status.chain_id;
      if (rodeos_snapshot->chain_id != status.chain_id)
         throw std::runtime_error(
               "database is for chain " + upcxio::convert_to_json(rodeos_snapshot->chain_id) +
               " but nodeos has chain " + upcxio::convert_to_json(status.chain_id));
      ilog("request blocks");
