...
namespace ship = upcxio::ship_protocol;
...
struct connection_callbacks {
   ...
   virtual bool received(ship::get_status_result_v0& status, upcxio::input_stream bin) { return true; }
   virtual bool received(ship::get_blocks_result_v0& result, upcxio::input_stream bin) { return true; }
   virtual bool received(ship::get_blocks_result_v1& result, upcxio::input_stream bin) { return true; }
...
struct connection : std::enable_shared_from_this<connection> {
   ...
   using abi_type    = upcxio::abi_type;
   ...
   void receive_abi(const std::shared_ptr<flat_buffer>& p) {
      ...
      upcxio::json_token_stream stream{ json.data() };
   ...
   bool receive_result(const std::shared_ptr<flat_buffer>& p) {
      auto                data = p->data();
      upcxio::input_stream bin{ (const char*)data.data(), (const char*)data.data() + data.size() };
      auto                orig = bin;
      ship::result        result;
      from_bin(result, bin);
