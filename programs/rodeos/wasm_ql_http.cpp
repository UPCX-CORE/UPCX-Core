...
UPCX_REFLECT(error_info, code, name, what, details)
...
UPCX_REFLECT(error_results, code, message, error)
   ...
   try {
      if (req.target() == "/v1/chain/get_info") {
         ...
         return;
                 ...
                 "/v1/chain/get_block") { // todo: replace with /v1/chain/get_block_header. upgrade clupcx.
         ...
         state_cache.store_state(std::move(thread_state));
         return;
      } else if (req.target() == "/v1/chain/get_abi") { // todo: get_raw_abi. upgrade clupcx to use get_raw_abi.
         if (req.method() != http::verb::post)
            return send(
                  error(http::status::bad_request, "Unsupported HTTP-method for " + req.target().to_string() + "\n"));
         auto thread_state = state_cache.get_state();
         ...
         // or:   change nodupcx to not do abi deserialization if transaction extension present?
         if (req.method() != http::verb::post)
            return send(
                  error(http::status::bad_request, "Unsupported HTTP-method for " + req.target().to_string() + "\n"));
         auto thread_state = state_cache.get_state();
      ...
      try {
         ...
         return send(error(http::status::internal_server_error, upcx::convert_to_json(err),
                           "application/json"));
      } catch (...) { //
