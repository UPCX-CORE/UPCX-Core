...
using namespace upcxio::ship_protocol;
...
struct cloner_config : ship_client::connection_config {
   ...
   upcxio::name filter_name = {}; // todo: remove
...
struct cloner_session : ship_client::connection_callbacks, std::enable_shared_from_this<cloner_session> {
   ...
   void connect(asio::io_context& ioc) {
      ...
      ilog("    chain:        ${a}", ("a", upcxio::convert_to_json(rodupcx_snapshot->chain_id)));
      ilog("    head:         ${a} ${b}",
           ("a", rodupcx_snapshot->head)("b", upcxio::convert_to_json(rodupcx_snapshot->head_id)));
      ilog("    irreversible: ${a} ${b}",
           ("a", rodupcx_snapshot->irreversible)(
                 "b", upcxio::convert_to_json(rodupcx_snapshot->irreversible_id)));
   ...
   bool received(get_status_result_v0& status, upcxio::input_stream bin) override {
      ilog("nodupcx has chain ${c}", ("c", upcxio::convert_to_json(status.chain_id)));
      if (rodupcx_snapshot->chain_id == upcxio::checksum256{})
         rodupcx_snapshot->chain_id = status.chain_id;
      if (rodupcx_snapshot->chain_id != status.chain_id)
         throw std::runtime_error(
               "database is for chain " + upcxio::convert_to_json(rodupcx_snapshot->chain_id) +
               " but nodupcx has chain " + upcxio::convert_to_json(status.chain_id));
      ilog("request blocks");
      ...
      if (rodupcx_snapshot->head) {
         ...
         for (uint32_t i = rodupcx_snapshot->irreversible; i <= rodupcx_snapshot->head; ++i) {
            auto info = rodupcx::get_state_row<rodupcx::block_info>(
                  view_state.kv_state.view, std::make_tuple(upcxio::name{ "block.info" }, upcxio::name{ "primary" }, i));
            if (!info)
   ...
   bool process_received(Get_Blocks_Result& result, upcxio::input_stream bin) {
   ...
   bool received(get_blocks_result_v0& result, upcxio::input_stream bin) override {
      return process_received(result, bin);
   }

   bool received(get_blocks_result_v1& result, upcxio::input_stream bin) override {
      return process_received(result, bin);
   }
...
void cloner_plugin::plugin_initialize(const variables_map& options) {
   try {
      ...
      if (options.count("filter-name") && options.count("filter-wasm")) {
         my->config->filter_name = upcxio::name{ options["filter-name"].as<std::string>() };
