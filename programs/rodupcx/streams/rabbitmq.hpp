...
class rabbitmq : public stream_handler {
   ...
   std::vector<upcxio::name>             routes_;

 public:
   rabbitmq(boost::asio::io_service& io_service, std::vector<upcxio::name> routes, const AMQP::Address& address,
   ...
   const std::vector<upcxio::name>& get_routes() const override { return routes_; }

   void publish(const char* data, uint64_t data_size, const upcxio::name& routing_key) override {
