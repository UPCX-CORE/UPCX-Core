...
class rabbitmq : public stream_handler {
   ...
   std::vector<upcx::name>             routes_;

 public:
   rabbitmq(boost::asio::io_service& io_service, std::vector<upcx::name> routes, const AMQP::Address& address,
   ...
   const std::vector<upcx::name>& get_routes() const override { return routes_; }

   void publish(const char* data, uint64_t data_size, const upcx::name& routing_key) override {
