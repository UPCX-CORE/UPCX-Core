...
#include <net/if.h>
#include <upcx/chain/genesis_state.hpp>
...
using namespace upcx::launcher::config;
...
void
upcxd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string upcx_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), upcx_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string upcxd_extra_args;
   std::map<uint,string> specific_nodupcx_args;
   std::map<uint,string> specific_nodupcx_installation_paths;
   testnet_def network;
   string gelf_endpoint;
   ...
   upcx::chain::genesis_state genesis_from_file;

   void assign_name (upcxd_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (upcxd_def &node, host_def *host);
   void launch (upcxd_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, upcxd_def> find_node(uint16_t node_num);
   vector<pair<host_def, upcxd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    (node_executable_name, bpo::value<string>(&upcxd_extra_args), ("forward " + string(node_executable_name) + " command line argument(s) to each instance of " + string(node_executable_name) + ", enclose arg(s) in quotes").c_str())
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodupcx_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodupcx_installation_paths);
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &upcxd : binding.instances) {
          upcxd.host = binding.host_name;
          upcxd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(upcxd.p2p_port);

          aliases.push_back (upcxd.name);
        }
      }
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodupcx_args[num] = specific_args[i];
    }
  }

  char* erd_env_var = getenv ("UPCX_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable UPCX_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (upcxd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.upcx_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      upcxd_def upcxd;
      assign_name(upcxd, i == 0);
      aliases.push_back(upcxd.name);
      upcxd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(upcxd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->upcx_home =
          (local_id.contains (lhost->host_name) || servers.default_upcx_home.empty()) ?
          erd : servers.default_upcx_home;
        host_ndx++;
      } // ph_count == 0

      upcxd_def upcxd;
      assign_name(upcxd, do_bios);

      aliases.push_back(upcxd.name);
      upcxd.set_host (lhost.get(), do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(upcxd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
  }
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  upcxd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->upcx_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->upcx_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->upcx_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->upcx_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->upcx_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (node.producers.size()) {
    ...
    cfg << "plugin = upcx::producer_plugin\n";
  }
  cfg << "plugin = upcx::net_plugin\n";
  cfg << "plugin = upcx::chain_api_plugin\n"
...
void
launcher_def::prep_remote_config_dir (upcxd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->upcx_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->upcx_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->upcx_home;

  cmd = "cd " + host->upcx_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->upcx_home
...
void
launcher_def::launch (upcxd_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodupcx_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodupcx_installation_paths.count(node_num)) {
        install_path = specific_nodupcx_installation_paths[node_num] + "/";
     }
  }
  string upcxdcmd = install_path + "programs/nodupcx/" + string(node_executable_name) + " ";
  if (skip_transaction_signatures) {
    upcxdcmd += "--skip-transaction-signatures ";
  }
  if (!upcxd_extra_args.empty()) {
    upcxdcmd += upcxd_extra_args + " ";
  }
  if (instance.name != "bios" && !specific_nodupcx_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodupcx_args.count(node_num)) {
        upcxdcmd += specific_nodupcx_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    upcxdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  upcxdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  upcxdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    upcxdcmd += " --genesis-timestamp " + gts;
  }
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->upcx_home + "; nohup " + upcxdcmd + " > "
    ...
    string cmd = "cd " + host->upcx_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else if (!instance.node->dont_start) {
    cerr << "spawning child, " << upcxdcmd << endl;

    bp::child c(upcxdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << upcxdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
    ...
    cerr << "not spawning child, " << upcxdcmd << endl;
...
#if 0
void
launcher_def::kill_instance(upcxd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, upcxd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, upcxd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const upcxd_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/upcx-tn_bounce.sh " + upcxd_extra_args;
      if (node_num != "bios" && !specific_nodupcx_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodupcx_args.count(node_num_i)) {
            cmd += " " + specific_nodupcx_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "UPCX_HOME", host.upcx_home }, { "UPCX_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/upcx-tn_down.sh ";
      do_command(host, node.name,
                 { { "UPCX_HOME", host.upcx_home }, { "UPCX_NODE", node_num }, { "UPCX_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/upcx-tn_roll.sh ";
      do_command(*host, host_name, { { "UPCX_HOME", host->upcx_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the upcx-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the upcx-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the upcx-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << upcx::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (upcx_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_upcx_home) (ssh) )
