...
using upcx::check;
using upcx::convert_to_bin;
using upcx::chain::block_state_ptr;
using upcx::chain::builtin_protocol_feature_t;
using upcx::chain::digest_type;
using upcx::chain::kv_bad_db_id;
using upcx::chain::kv_bad_iter;
using upcx::chain::kv_context;
using upcx::chain::kvram_id;
using upcx::chain::protocol_feature_exception;
using upcx::chain::protocol_feature_set;
using upcx::chain::signed_transaction;
using upcx::chain::transaction_trace_ptr;
using upcx::chain::packed_transaction_ptr;
using upcx::state_history::block_position;
using upcx::state_history::create_deltas;
using upcx::state_history::get_blocks_result_v1;
using upcx::state_history::state_result;
using upcx::vm::span;

struct callbacks;
using rhf_t     = upcx::vm::registered_host_functions<callbacks>;
using backend_t = upcx::vm::backend<rhf_t, upcx::vm::jit>;
...
// Handle upcx version differences
namespace {
template <typename T>
auto to_uint64_t(T n) -> std::enable_if_t<std::is_same_v<T, upcx::chain::name>, decltype(n.value)> {
   return n.value;
}
template <typename T>
auto to_uint64_t(T n) -> std::enable_if_t<std::is_same_v<T, upcx::chain::name>, decltype(n.to_uint64_t())> {
   return n.to_uint64_t();
}

template <typename C, typename F0, typename F1, typename G>
auto do_startup(C&& control, F0&&, F1&& f1, G &&)
      -> std::enable_if_t<std::is_constructible_v<std::decay_t<decltype(*control)>, upcx::chain::controller::config,
...
struct __attribute__((__may_alias__)) xxx_transaction_checktime_timer {
   ...
   upcx::chain::platform_timer& _timer;
};
struct transaction_checktime_factory {
   upcx::chain::platform_timer              timer;
   std::atomic_bool                          expired;
   upcx::chain::transaction_checktime_timer get() {
      xxx_transaction_checktime_timer result{ expired, timer };
      return std::move(*reinterpret_cast<upcx::chain::transaction_checktime_timer*>(&result));
   }
};
}; // namespace

struct intrinsic_context {
   upcx::chain::controller&                          control;
   upcx::chain::packed_transaction                   trx;
   std::unique_ptr<upcx::chain::transaction_context> trx_ctx;
   std::unique_ptr<upcx::chain::apply_context>       apply_context;

   intrinsic_context(upcx::chain::controller& control) : control{ control } {
      static transaction_checktime_factory xxx_timer;

      upcx::chain::signed_transaction strx;
      strx.actions.emplace_back();
      strx.actions.back().account = upcx::chain::name{ "upcx.null" };
      strx.actions.back().authorization.push_back({ upcx::chain::name{ "upcx" }, upcx::chain::name{ "active" } });
      trx = upcx::chain::packed_transaction( std::move(strx), true );
      trx_ctx = std::make_unique<upcx::chain::transaction_context>(control, trx, xxx_timer.get(),
                                                                    fc::time_point::now());
      trx_ctx->init_for_implicit_trx(0);
      trx_ctx->exec();
      apply_context = std::make_unique<upcx::chain::apply_context>(control, *trx_ctx, 1, 0);
...
protocol_feature_set make_protocol_feature_set() {
   ...
   for (const auto& p : upcx::chain::builtin_protocol_feature_codenames) { add_builtins(p.first); }

   return pfs;
}

template <typename T>
using wasm_ptr = upcx::vm::argument_proxy<T*>;
...
struct test_chain {
   upcx::chain::private_key_type producer_key{ "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"s };

   fc::temp_directory                                dir;
   std::unique_ptr<upcx::chain::controller::config> cfg;
   std::unique_ptr<upcx::chain::controller>         control;
   std::optional<scoped_connection>                  applied_transaction_connection;
   std::optional<scoped_connection>                  accepted_block_connection;
   upcx::state_history::transaction_trace_cache     trace_cache;
   ...
   test_chain(const char* snapshot) {
      upcx::chain::genesis_state genesis;
      genesis.initial_timestamp = fc::time_point::from_iso_string("2020-01-01T00:00:00.000");
      cfg                       = std::make_unique<upcx::chain::controller::config>();
      cfg->blog.log_dir         = dir.path() / "blocks";
      cfg->state_dir            = dir.path() / "state";
      cfg->contracts_console    = true;
      cfg->wasm_runtime         = upcx::chain::wasm_interface::vm_type::upcx_vm_jit;

      std::optional<std::ifstream>                           snapshot_file;
      std::shared_ptr<upcx::chain::istream_snapshot_reader> snapshot_reader;
      if (snapshot && *snapshot) {
         std::optional<upcx::chain::chain_id_type> chain_id;
         {
            std::ifstream temp_file(snapshot, (std::ios::in | std::ios::binary));
            if (!temp_file.is_open())
               throw std::runtime_error("can not open " + std::string{ snapshot });
            upcx::chain::istream_snapshot_reader tmp_reader(temp_file);
            tmp_reader.validate();
            chain_id = upcx::chain::controller::extract_chain_id(tmp_reader);
         }
         snapshot_file.emplace(snapshot, std::ios::in | std::ios::binary);
         snapshot_reader = std::make_shared<upcx::chain::istream_snapshot_reader>(*snapshot_file);
         control         = std::make_unique<upcx::chain::controller>(*cfg, make_protocol_feature_set(), *chain_id);
      } else {
         control = std::make_unique<upcx::chain::controller>(*cfg, make_protocol_feature_set(),
      ...
      if (snapshot_reader) {
         ...
         control->start_block(control->head_block_time() + fc::microseconds(block_interval_us), 0,
                              { *control->get_protocol_feature_manager().get_builtin_digest(
                                    upcx::chain::builtin_protocol_feature_t::preactivate_feature) });
      }
   }
   ...
   void finish_block() {
      ...
      control->finalize_block([&](upcx::chain::digest_type d) { return std::vector{ producer_key.sign(d) }; });
...
upcx::checksum256 convert(const upcx::chain::checksum_type& obj) {
   std::array<uint8_t, 32> bytes;
   static_assert(bytes.size() == sizeof(obj));
   memcpy(bytes.data(), &obj, bytes.size());
   return upcx::checksum256(bytes);
}

chain_types::account_delta convert(const upcx::chain::account_delta& obj) {
...
chain_types::action_receipt_v0 convert(const upcx::chain::action_receipt& obj) {
   ...
   for (auto& auth : obj.auth_sequence)
      result.auth_sequence.push_back({ upcx::name{ to_uint64_t(auth.first) }, auth.second });
   result.code_sequence.value = obj.code_sequence.value;
   result.abi_sequence.value  = obj.abi_sequence.value;
   return result;
}

chain_types::action convert(const upcx::chain::action& obj) {
   ...
   for (auto& auth : obj.authorization)
      result.authorization.push_back(
            { upcx::name{ to_uint64_t(auth.actor) }, upcx::name{ to_uint64_t(auth.permission) } });
   result.data = { obj.data.data(), obj.data.data() + obj.data.size() };
   return result;
}

chain_types::action_trace_v1 convert(const upcx::chain::action_trace& obj) {
...
chain_types::transaction_trace_v0 convert(const upcx::chain::transaction_trace& obj) {
...
UPCX_REFLECT(contract_row, block_num, present, code, scope, table, primary_key, payer, value);
...
struct push_trx_args {
   upcx::chain::bytes                         transaction;
   std::vector<upcx::chain::bytes>            context_free_data;
   std::vector<upcx::chain::signature_type>   signatures;
   std::vector<upcx::chain::private_key_type> keys;
};
...
#define DB_WRAPPERS_ARRAY_SECONDARY(IDX, ARR_SIZE, ARR_ELEMENT_TYPE)                                                   \
   int db_##IDX##_find_secondary(uint64_t code, uint64_t scope, uint64_t table,                                        \
                                 upcx::chain::array_ptr<const ARR_ELEMENT_TYPE> data, uint32_t data_len,              \
                                 uint64_t& primary) {                                                                  \
      UPCX_ASSERT(data_len == ARR_SIZE, upcx::chain::db_api_exception,                                                 \
   ...
   int db_##IDX##_find_primary(uint64_t code, uint64_t scope, uint64_t table,                                          \
                               upcx::chain::array_ptr<ARR_ELEMENT_TYPE> data, uint32_t data_len, uint64_t primary) {  \
      UPCX_ASSERT(data_len == ARR_SIZE, upcx::chain::db_api_exception,                                                 \
                 "invalid size of secondary key array for " #IDX                                                       \
                 ": given ${given} bytes but expected ${expected} bytes",                                              \
                 ("given", data_len)("expected", ARR_SIZE));                                                           \
...
struct callbacks {
   ...
   void check_bounds(void* data, size_t size) {
      ...
      upcx::vm::ignore_unused_variable_warning(check);
   }
   ...
   template <typename T>
   T unpack(const upcx::input_stream& data) {
   ...
   char* alloc(uint32_t cb_alloc_data, uint32_t cb_alloc, uint32_t size) {
      ...
      auto result = state.backend.get_context().execute( //
            this, upcx::vm::jit_visitor(42), state.backend.get_module().tables[0].table[cb_alloc], cb_alloc_data,
            size);
      if (!result || !result->is_a<upcx::vm::i32_const_t>())
   ...
   void upcx_assert_message(bool condition, span<const char> msg) {
      if (!condition)
   ...
   void replace_producer_keys(uint32_t chain_index, span<const char> key) {
      ...
      auto  k     = unpack<upcx::chain::public_key_type>(key);
   ...
   void replace_account_keys(uint32_t chain_index, uint64_t account, uint64_t permission, span<const char> key) {
      auto& chain = assert_chain(chain_index);
      auto  k     = unpack<upcx::chain::public_key_type>(key);
      chain.control->replace_account_keys(upcx::chain::name{ account }, upcx::chain::name{ permission }, k);
   }
   ...
   void push_transaction(uint32_t chain_index, span<const char> args_packed, uint32_t cb_alloc_data,
      ...
      auto                             transaction = unpack<upcx::chain::transaction>(args.transaction);
      upcx::chain::signed_transaction signed_trx{ std::move(transaction), std::move(args.signatures),
      ...
      auto ptrx = std::make_shared<upcx::chain::packed_transaction>(
            std::move(signed_trx), true, upcx::chain::packed_transaction::compression_type::none);
      auto fut = upcx::chain::transaction_metadata::start_recover_keys(
   ...
   bool exec_deferred(uint32_t chain_index, uint32_t cb_alloc_data, uint32_t cb_alloc) {
      ...
      chain.start_if_needed();
      const auto& idx =
            chain.control->db().get_index<upcx::chain::generated_transaction_multi_index, upcx::chain::by_delay>();
      auto itr = idx.begin();
   ...
   void rodupcx_push_transaction(uint32_t rodupcx, span<const char> packed_args, uint32_t cb_alloc_data,
      ...
      upcx::chain::signed_transaction signed_trx{ std::move(transaction), std::move(args.signatures),
                                                   std::move(args.context_free_data) };
      for (auto& key : args.keys) signed_trx.sign(key, chain.control->get_chain_id());
      upcx::chain::packed_transaction ptrx{ std::move(signed_trx), true, upcx::chain::packed_transaction::compression_type::none };
      ...
      auto tt = upcx::convert_from_bin<upcx::ship_protocol::transaction_trace>(
                                   { result.data, result.data + result.size });
      auto& tt0 = std::get<upcx::ship_protocol::transaction_trace_v0>(tt);
      for (auto& at : tt0.action_traces) {
         auto& at1 = std::get<upcx::ship_protocol::action_trace_v1>(at);
...
void register_callbacks() {
   ...
   rhf_t::add<&callbacks::upcx_assert_message>("env", "upcx_assert_message");
...
static void run(const char* wasm, const std::vector<std::string>& args) {
   upcx::vm::wasm_allocator wa;
   auto                      code = upcx::vm::read_wasm(wasm);
   backend_t                 backend(code, nullptr);
   ::state                   state{ wasm, wa, backend, upcx::convert_to_bin(args) };
   callbacks                 cb{ state };
...
const char usage[] = "usage: upcx-tester [-h or --help] [-v or --verbose] file.wasm [args for wasm]\n";
