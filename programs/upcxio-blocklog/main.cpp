...
struct report_time {
    ...
    void report() {
        ...
        ilog("upcxio-blocklog - ${desc} took ${t} msec", ("desc", _desc)("t", duration));
    }
...
void blocklog::read_log() {
   ...
   UPCX_ASSERT( end, block_log_exception, "No blocks found in block log" );
   UPCX_ASSERT( end->block_num() > 1, block_log_exception, "Only one block found in block log" );
...
int prune_transactions(bfs::path block_dir, bfs::path state_history_dir, uint32_t block_num,
   ...
   using upcxio::state_history_traces_log;
...
int main(int argc, char** argv) {
   ...
   options_description cli ("upcxio-blocklog command line options");
   try {
      blocklog blog;
