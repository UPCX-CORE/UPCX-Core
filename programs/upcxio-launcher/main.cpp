...
using namespace upcxio::launcher::config;
...
struct server_name_def {
  ...
  string upcxio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), upcxio_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   upcxio::chain::genesis_state genesis_from_file;
  ...
  if (per_host == 0) {
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->upcxio_home =
          (local_id.contains (lhost->host_name) || servers.default_upcxio_home.empty()) ?
          erd : servers.default_upcxio_home;
        host_ndx++;
      } // ph_count == 0
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->upcxio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->upcxio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->upcxio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->upcxio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->upcxio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (node.producers.size()) {
    ...
    cfg << "plugin = upcxio::producer_plugin\n";
  }
  cfg << "plugin = upcxio::net_plugin\n";
  cfg << "plugin = upcxio::chain_api_plugin\n"
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      upcxio::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = network.nodes["bios"].keys[0].get_public_key().to_string();

   fc::json::from_file(genesis_path).as<upcxio::chain::genesis_state>(genesis_from_file);
...
launcher_def::prep_remote_config_dir (upcxd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->upcxio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->upcxio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->upcxio_home;

  cmd = "cd " + host->upcxio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->upcxio_home
...
launcher_def::launch (upcxd_def &instance, string &gts) {
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->upcxio_home + "; nohup " + upcxdcmd + " > "
    ...
    string cmd = "cd " + host->upcxio_home + "; kill -15 $(cat " + pidf.string() + ")";
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.upcxio_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/upcxio-tn_bounce.sh " + upcxd_extra_args;
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the upcxio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the upcxio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the upcxio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << upcxio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(upcxio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (upcxio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_upcxio_home) (ssh) )
