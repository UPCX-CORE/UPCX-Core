...
#include <upcxio/state_history/serialization.hpp>
#include <upcxio/state_history/trace_converter.hpp>
#include <upcxio/state_history/transaction_trace_cache.hpp>
...
using namespace upcxio::literals;
...
using upcxio::check;
using upcxio::convert_to_bin;
using upcxio::chain::block_state_ptr;
using upcxio::chain::builtin_protocol_feature_t;
using upcxio::chain::digest_type;
using upcxio::chain::kv_bad_db_id;
using upcxio::chain::kv_bad_iter;
using upcxio::chain::kv_context;
using upcxio::chain::kvram_id;
using upcxio::chain::protocol_feature_exception;
using upcxio::chain::protocol_feature_set;
using upcxio::chain::signed_transaction;
using upcxio::chain::transaction_trace_ptr;
using upcxio::chain::packed_transaction_ptr;
using upcxio::state_history::block_position;
using upcxio::state_history::create_deltas;
using upcxio::state_history::get_blocks_result_v1;
using upcxio::state_history::state_result;
using upcxio::vm::span;

struct callbacks;
using rhf_t     = upcxio::vm::registered_host_functions<callbacks>;
...
// Handle upcxio version differences
namespace {
template <typename T>
auto to_uint64_t(T n) -> std::enable_if_t<std::is_same_v<T, upcxio::chain::name>, decltype(n.value)> {
   return n.value;
}
template <typename T>
auto to_uint64_t(T n) -> std::enable_if_t<std::is_same_v<T, upcxio::chain::name>, decltype(n.to_uint64_t())> {
   return n.to_uint64_t();
}

template <typename C, typename F0, typename F1, typename G>
auto do_startup(C&& control, F0&&, F1&& f1, G &&)
      -> std::enable_if_t<std::is_constructible_v<std::decay_t<decltype(*control)>, upcxio::chain::controller::config,
...
struct __attribute__((__may_alias__)) xxx_transaction_checktime_timer {
   ...
   upcxio::chain::platform_timer& _timer;
};
struct transaction_checktime_factory {
   upcxio::chain::platform_timer              timer;
   std::atomic_bool                          expired;
   upcxio::chain::transaction_checktime_timer get() {
      xxx_transaction_checktime_timer result{ expired, timer };
      return std::move(*reinterpret_cast<upcxio::chain::transaction_checktime_timer*>(&result));
   }
};
}; // namespace

struct intrinsic_context {
   upcxio::chain::controller&                          control;
   upcxio::chain::packed_transaction                   trx;
   std::unique_ptr<upcxio::chain::transaction_context> trx_ctx;
   std::unique_ptr<upcxio::chain::apply_context>       apply_context;

   intrinsic_context(upcxio::chain::controller& control) : control{ control } {
      static transaction_checktime_factory xxx_timer;

      upcxio::chain::signed_transaction strx;
      strx.actions.emplace_back();
      strx.actions.back().account = upcxio::chain::name{ "upcxio.null" };
      strx.actions.back().authorization.push_back({ upcxio::chain::name{ "upcxio" }, upcxio::chain::name{ "active" } });
      trx = upcxio::chain::packed_transaction( std::move(strx), true );
      trx_ctx = std::make_unique<upcxio::chain::transaction_context>(control, trx, xxx_timer.get(),
                                                                    fc::time_point::now());
      trx_ctx->init_for_implicit_trx(0);
      trx_ctx->exec();
      apply_context = std::make_unique<upcxio::chain::apply_context>(control, *trx_ctx, 1, 0);
...
protocol_feature_set make_protocol_feature_set() {
   ...
   for (const auto& p : upcxio::chain::builtin_protocol_feature_codenames) { add_builtins(p.first); }

   return pfs;
}

template <typename T>
using wasm_ptr = upcxio::vm::argument_proxy<T*>;
...
struct test_chain {
   upcxio::chain::private_key_type producer_key{ "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"s };

   fc::temp_directory                                dir;
   std::unique_ptr<upcxio::chain::controller::config> cfg;
   std::unique_ptr<upcxio::chain::controller>         control;
   std::optional<scoped_connection>                  applied_transaction_connection;
   std::optional<scoped_connection>                  accepted_block_connection;
   upcxio::state_history::transaction_trace_cache     trace_cache;
   ...
   test_chain(const char* snapshot) {
      upcxio::chain::genesis_state genesis;
      genesis.initial_timestamp = fc::time_point::from_iso_string("2020-01-01T00:00:00.000");
      cfg                       = std::make_unique<upcxio::chain::controller::config>();
      ...
      if (snapshot && *snapshot) {
         std::optional<upcxio::chain::chain_id_type> chain_id;
         {
            std::ifstream temp_file(snapshot, (std::ios::in | std::ios::binary));
            if (!temp_file.is_open())
               throw std::runtime_error("can not open " + std::string{ snapshot });
            upcxio::chain::istream_snapshot_reader tmp_reader(temp_file);
            tmp_reader.validate();
            chain_id = upcxio::chain::controller::extract_chain_id(tmp_reader);
         }
         snapshot_file.emplace(snapshot, std::ios::in | std::ios::binary);
         snapshot_reader = std::make_shared<upcxio::chain::istream_snapshot_reader>(*snapshot_file);
         control         = std::make_unique<upcxio::chain::controller>(*cfg, make_protocol_feature_set(), *chain_id);
      } else {
         control = std::make_unique<upcxio::chain::controller>(*cfg, make_protocol_feature_set(),
      ...
      if (snapshot_reader) {
         ...
         control->start_block(control->head_block_time() + fc::microseconds(block_interval_us), 0,
                              { *control->get_protocol_feature_manager().get_builtin_digest(
                                    upcxio::chain::builtin_protocol_feature_t::preactivate_feature) });
      }
   }
   ...
   void finish_block() {
      ...
      control->finalize_block([&](upcxio::chain::digest_type d) { return std::vector{ producer_key.sign(d) }; });
...
upcxio::checksum256 convert(const upcxio::chain::checksum_type& obj) {
   std::array<uint8_t, 32> bytes;
   static_assert(bytes.size() == sizeof(obj));
   memcpy(bytes.data(), &obj, bytes.size());
   return upcxio::checksum256(bytes);
}

chain_types::account_delta convert(const upcxio::chain::account_delta& obj) {
...
chain_types::action_receipt_v0 convert(const upcxio::chain::action_receipt& obj) {
   ...
   for (auto& auth : obj.auth_sequence)
      result.auth_sequence.push_back({ upcxio::name{ to_uint64_t(auth.first) }, auth.second });
   result.code_sequence.value = obj.code_sequence.value;
   result.abi_sequence.value  = obj.abi_sequence.value;
   return result;
}

chain_types::action convert(const upcxio::chain::action& obj) {
   ...
   for (auto& auth : obj.authorization)
      result.authorization.push_back(
            { upcxio::name{ to_uint64_t(auth.actor) }, upcxio::name{ to_uint64_t(auth.permission) } });
   result.data = { obj.data.data(), obj.data.data() + obj.data.size() };
   return result;
}

chain_types::action_trace_v1 convert(const upcxio::chain::action_trace& obj) {
...
chain_types::transaction_trace_v0 convert(const upcxio::chain::transaction_trace& obj) {
...
struct push_trx_args {
   upcxio::chain::bytes                         transaction;
   std::vector<upcxio::chain::bytes>            context_free_data;
   std::vector<upcxio::chain::signature_type>   signatures;
   std::vector<upcxio::chain::private_key_type> keys;
};
...
struct callbacks {
   ...
   void check_bounds(void* data, size_t size) {
      ...
      upcxio::vm::ignore_unused_variable_warning(check);
   }
   ...
   template <typename T>
   T unpack(const upcxio::input_stream& data) {
   ...
   char* alloc(uint32_t cb_alloc_data, uint32_t cb_alloc, uint32_t size) {
      ...
      auto result = state.backend.get_context().execute( //
            this, upcxio::vm::jit_visitor(42), state.backend.get_module().tables[0].table[cb_alloc], cb_alloc_data,
            size);
      if (!result || !result->is_a<upcxio::vm::i32_const_t>())
   ...
   void upcxio_assert_message(bool condition, span<const char> msg) {
      if (!condition)
   ...
   void replace_producer_keys(uint32_t chain_index, span<const char> key) {
      ...
      auto  k     = unpack<upcxio::chain::public_key_type>(key);
   ...
   void replace_account_keys(uint32_t chain_index, uint64_t account, uint64_t permission, span<const char> key) {
      auto& chain = assert_chain(chain_index);
      auto  k     = unpack<upcxio::chain::public_key_type>(key);
      chain.control->replace_account_keys(upcxio::chain::name{ account }, upcxio::chain::name{ permission }, k);
   }
   ...
   void push_transaction(uint32_t chain_index, span<const char> args_packed, uint32_t cb_alloc_data,
      ...
      auto                             transaction = unpack<upcxio::chain::transaction>(args.transaction);
      upcxio::chain::signed_transaction signed_trx{ std::move(transaction), std::move(args.signatures),
      ...
      auto ptrx = std::make_shared<upcxio::chain::packed_transaction>(
            std::move(signed_trx), true, upcxio::chain::packed_transaction::compression_type::none);
      auto fut = upcxio::chain::transaction_metadata::start_recover_keys(
   ...
   bool exec_deferred(uint32_t chain_index, uint32_t cb_alloc_data, uint32_t cb_alloc) {
      ...
      chain.start_if_needed();
      const auto& idx =
            chain.control->db().get_index<upcxio::chain::generated_transaction_multi_index, upcxio::chain::by_delay>();
      auto itr = idx.begin();
   ...
   void rodupcx_push_transaction(uint32_t rodupcx, span<const char> packed_args, uint32_t cb_alloc_data,
      ...
      upcxio::chain::signed_transaction signed_trx{ std::move(transaction), std::move(args.signatures),
                                                   std::move(args.context_free_data) };
      for (auto& key : args.keys) signed_trx.sign(key, chain.control->get_chain_id());
      upcxio::chain::packed_transaction ptrx{ std::move(signed_trx), true, upcxio::chain::packed_transaction::compression_type::none };
...
void register_callbacks() {
   ...
   rhf_t::add<&callbacks::upcxio_assert_message>("env", "upcxio_assert_message");
...
static void run(const char* wasm, const std::vector<std::string>& args) {
   upcxio::vm::wasm_allocator wa;
   auto                      code = upcxio::vm::read_wasm(wasm);
   backend_t                 backend(code, nullptr);
   ::state                   state{ wasm, wa, backend, upcxio::convert_to_bin(args) };
   callbacks                 cb{ state };
...
const char usage[] = "usage: upcxio-tester [-h or --help] [-v or --verbose] file.wasm [args for wasm]\n";
