#!/bin/bash
#
# upcx-tn_roll is used to have all of the instances of the UPCX daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: upcx-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./upcx-tn_roll.sh is sufficient.
#

if [ -z "$UPCX_HOME" ]; then
    echo UPCX_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in upcxd upcxd nodupcx; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for upcxd or upcxd or nodupcx
    exit 1
fi
...
if [ $DD = "all" ]; then
    for UPCX_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $UPCX_HOME/scripts/upcx-tn_up.sh "$*"
    done
else
    bash $UPCX_HOME/scripts/upcx-tn_up.sh "$*"
fi
unset UPCX_RESTART_DATA_DIR
