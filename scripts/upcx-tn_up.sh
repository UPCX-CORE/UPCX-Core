#!/bin/bash
#
# upcx-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# upcx-tn_bounce.sh and upcx-tn_roll.sh scripts.
...
if [ "$PWD" != "$UPCX_HOME" ]; then
    echo $0 must only be run from $UPCX_HOME
    exit -1
fi
...
datadir=var/lib/node_$UPCX_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/upcx/node_$UPCX_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/upcx/node_$UPCX_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
