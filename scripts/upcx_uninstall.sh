...
function usage() {
  ...
  -i DIR      Directory where upcx is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the upcx data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export UPCX_INSTALL_DIR=${INSTALL_LOCATION:-$UPCX_INSTALL_DIR}

if [[ ! -d "${UPCX_INSTALL_DIR}" ]]; then
   echo "[UPCX installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${UPCX_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/UPCX/upcx/issues/6940
   echo "[UPCX installation found: ${UPCX_INSTALL_DIR}]" && INSTALL_PATHS+=("${UPCX_INSTALL_DIR}") # UPCX_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${UPCX_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/upcx")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/upcx")
            fi
            # Version < 1.8.0; Before we started using ~/upcx/1.8.x
            ...
            echo " - UPCX Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled UPCX Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing UPCX Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/upcx_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
