#!/bin/bash
#
# upcxio-tn_roll is used to have all of the instances of the UPCX daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: upcxio-tn_roll.sh [arglist]
...
prog=""
RD=""
for p in upcxd upcxiod nodupcx; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for upcxd or upcxiod or nodupcx
    exit 1
fi
