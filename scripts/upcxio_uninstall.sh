...
function usage() {
  ...
  -i DIR      Directory where upcxio is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the upcxio data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
if [[ ! -d "${EOSIO_INSTALL_DIR}" ]]; then
   ...
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/EOSIO/upcx/issues/6940
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/upcxio")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/upcxio")
            fi
            # Version < 1.8.0; Before we started using ~/upcxio/1.8.x
...
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/upcxio_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
