...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="upcx-ignition-wd/bootlog.txt"
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899
        ...
        self.upcxAccount=self.defProducerAccounts["upcx"]= Account("upcx")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        specificExtraNodupcxArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodupcx flags on launcher), example: { "5" : "--plugin upcx::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the upcx.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        ...
        if not self.walletd:
            nodupcxArgs += " --plugin upcx::wallet_api_plugin"
        if Utils.Debug:
            nodupcxArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodupcxArgs += " --plugin upcx::producer_api_plugin"
        ...
        for node, conf in manualProducerNodeConf.items():
            ...
            account = conf['key']
            arg = arg + " --plugin upcx::producer_plugin --signature-provider {}=KEY:{} ".format(account.ownerPublicKey, account.ownerPrivateKey)
        ...
        self.upcxAccount=self.defProducerAccounts["upcx"]
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.upcxAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse upcx private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by upcx-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/upcx.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/upcx.bios"
        ...
        upcxName="upcx"
        upcxKeys=producerKeys[upcxName]
        upcxAccount=Account(upcxName)
        upcxAccount.ownerPrivateKey=upcxKeys["private"]
        upcxAccount.ownerPublicKey=upcxKeys["public"]
        upcxAccount.activePrivateKey=upcxKeys["private"]
        upcxAccount.activePublicKey=upcxKeys["public"]
        producerKeys.pop(upcxName)

        if not self.walletMgr.importKey(upcxAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (upcxName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"upcx\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init upcx transfer")
            opts="--permission upcx@active"
            if name != "upcx":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from upcx.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True, manualProducerNodeConf=[]):
        ...
        trans=biosNode.publishContract(upcxAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, upcxAccount, 0)
            if trans is None:
        ...
        for node, conf in manualProducerNodeConf.items():
            ...
            for name in conf['names']:
                account.name = name
                trans=biosNode.createAccount(account, upcxAccount, 0)
                if trans is None:
                    Utils.Print("ERROR: Failed to create account %s" % (name))
                    return None
                Node.validateTransaction(trans)
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission upcx@active"
                    myTrans=biosNode.pushMessage("upcx", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("upcx", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but upcx).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "upcx"
        ...
        upcxTokenAccount=copy.deepcopy(upcxAccount)
        upcxTokenAccount.name="upcx.token"
        trans=biosNode.createAccount(upcxTokenAccount, upcxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxTokenAccount.name))
            return None

        upcxRamAccount=copy.deepcopy(upcxAccount)
        upcxRamAccount.name="upcx.ram"
        trans=biosNode.createAccount(upcxRamAccount, upcxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxRamAccount.name))
            return None

        upcxRamfeeAccount=copy.deepcopy(upcxAccount)
        upcxRamfeeAccount.name="upcx.ramfee"
        trans=biosNode.createAccount(upcxRamfeeAccount, upcxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxRamfeeAccount.name))
            return None

        upcxStakeAccount=copy.deepcopy(upcxAccount)
        upcxStakeAccount.name="upcx.stake"
        trans=biosNode.createAccount(upcxStakeAccount, upcxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(upcxTokenAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=upcxTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (upcxAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to upcx contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (upcxAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (upcxAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to upcx contract.")
            return None
        ...
        Utils.Print("Verify upcx issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountUpcxBalanceStr(upcxAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (upcxAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (upcxAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (upcxTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (upcxAccount.name)
            trans=biosNode.pushMessage(upcxAccount.name, action, data, opts)
