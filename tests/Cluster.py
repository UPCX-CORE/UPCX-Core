...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="upcxio-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kupcxd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899
        ...
        """Cluster container.
        walletd [True|False] Is wallet kupcxd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: upcx server host
        port: upcx server port
        walletHost: upcx wallet host
        ...
        self.upcxioAccount=self.defProducerAccounts["upcxio"]= Account("upcxio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodupcxArgs="", useBiosBootFile=True, specificExtraNodupcxArgs=None, onlySetProds=False,
        ...
        extraNodupcxArgs: string of arguments to pass through to each nodoes instance (via --nodupcx flag on launcher)
        ...
        specificExtraNodupcxArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodupcx flags on launcher), example: { "5" : "--plugin upcxio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the upcxio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodupcx default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodupcxArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodupcxArgs += " --plugin upcxio::wallet_api_plugin"
        if Utils.Debug:
            nodupcxArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodupcxArgs += " --plugin upcxio::producer_api_plugin"

        if len(manualProducerNodeConf) > 0 and specificExtraNodupcxArgs is None:
            specificExtraNodupcxArgs = {}

        for node, conf in manualProducerNodeConf.items():
            arg = specificExtraNodupcxArgs.get(node, "")
            account = conf['key']
            arg = arg + " --plugin upcxio::producer_plugin --signature-provider {}=KEY:{} ".format(account.ownerPublicKey, account.ownerPrivateKey)
            for name in conf['names']:
                arg = arg + "--producer-name {} ".format(name)
            specificExtraNodupcxArgs[node] = arg

        Utils.Print("specificExtraNodupcxArgs=", specificExtraNodupcxArgs)

        httpMaxResponseTimeSet = False
        if specificExtraNodupcxArgs is not None:
            assert(isinstance(specificExtraNodupcxArgs, dict))
            for nodeNum,arg in specificExtraNodupcxArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodupcx")
                if arg.find("--http-max-response-time-ms") != -1:
                    httpMaxResponseTimeSet = True
                cmdArr.append(arg)

        if not httpMaxResponseTimeSet and extraNodupcxArgs.find("--http-max-response-time-ms") == -1:
            extraNodupcxArgs+=" --http-max-response-time-ms 990000 "

        if extraNodupcxArgs is not None:
            assert(isinstance(extraNodupcxArgs, str))
            nodupcxArgs += extraNodupcxArgs

        if nodupcxArgs:
            cmdArr.append("--nodupcx")
            cmdArr.append(nodupcxArgs)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodupcx")
                cmdArr.append(path)
        ...
        self.upcxioAccount=self.defProducerAccounts["upcxio"]
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on upcx server port %d" % (
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.upcxioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse upcxio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by upcxio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/upcxio.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/upcxio.bios"
        ...
        upcxioName="upcxio"
        upcxioKeys=producerKeys[upcxioName]
        upcxioAccount=Account(upcxioName)
        upcxioAccount.ownerPrivateKey=upcxioKeys["private"]
        upcxioAccount.ownerPublicKey=upcxioKeys["public"]
        upcxioAccount.activePrivateKey=upcxioKeys["private"]
        upcxioAccount.activePublicKey=upcxioKeys["public"]
        producerKeys.pop(upcxioName)

        if not self.walletMgr.importKey(upcxioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (upcxioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="upcxio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"upcxio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init upcxio transfer")
            opts="--permission upcxio@active"
            if name != "upcxio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from upcxio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True, manualProducerNodeConf=[]):
        ...
        trans=biosNode.publishContract(upcxioAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, upcxioAccount, 0)
            if trans is None:
        ...
        for node, conf in manualProducerNodeConf.items():
            ...
            for name in conf['names']:
                account.name = name
                trans=biosNode.createAccount(account, upcxioAccount, 0)
                if trans is None:
                    Utils.Print("ERROR: Failed to create account %s" % (name))
                    return None
                Node.validateTransaction(trans)
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission upcxio@active"
                    myTrans=biosNode.pushMessage("upcxio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("upcxio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but upcxio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "upcxio"
        ...
        upcxioTokenAccount=copy.deepcopy(upcxioAccount)
        upcxioTokenAccount.name="upcxio.token"
        trans=biosNode.createAccount(upcxioTokenAccount, upcxioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxioTokenAccount.name))
            return None

        upcxioRamAccount=copy.deepcopy(upcxioAccount)
        upcxioRamAccount.name="upcxio.ram"
        trans=biosNode.createAccount(upcxioRamAccount, upcxioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxioRamAccount.name))
            return None

        upcxioRamfeeAccount=copy.deepcopy(upcxioAccount)
        upcxioRamfeeAccount.name="upcxio.ramfee"
        trans=biosNode.createAccount(upcxioRamfeeAccount, upcxioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxioRamfeeAccount.name))
            return None

        upcxioStakeAccount=copy.deepcopy(upcxioAccount)
        upcxioStakeAccount.name="upcxio.stake"
        trans=biosNode.createAccount(upcxioStakeAccount, upcxioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (upcxioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(upcxioTokenAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=upcxioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (upcxioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to upcxio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (upcxioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (upcxioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to upcxio contract.")
            return None
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (upcxioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (upcxioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (upcxioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (upcxioAccount.name)
            trans=biosNode.pushMessage(upcxioAccount.name, action, data, opts)
    ...
    # Kills a percentange of Upcx instances starting from the tail and update upcxInstanceInfos state
    ...
    def killall(self, kill=True, silent=True, allInstances=False):
        """Kill cluster nodupcx instances. allInstances will kill all nodupcx instances running on the system."""
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodupcx instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodupcx instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
