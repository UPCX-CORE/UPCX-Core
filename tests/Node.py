...
class Node(object):
    ...
    def getBlock(self, blockNumOrId, silentErrors=False, exitOnError=False):
        ...
        return self.processClupcxCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        for i in range(0,(int(60/timeout) - 1)):
            trans=self.processClupcxCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False, sign=False):
        ...
        trans=self.processClupcxCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getUpcxAccount(self, name, exitOnError=False, returnType=ReturnType.json):
        ...
        return self.processClupcxCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg, returnType=returnType)
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processClupcxCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processClupcxCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True, reportStatus=True, sign=False, dontSend=False, expiration=None, skipSign=False):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.UpcxClientPath, self.upcxClientArgs(), dontSendStr, expirationStr)
        cmdArr=cmd.split()
    ...
    # Get actions mapped to an account (clupcx get actions)
    ...
    def getAccountUpcxBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clupcx get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("upcx.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountUpcxBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountUpcxBalance(self, scope):
        """Returns SYS currency0000 account balance from clupcx get table command. Returned balance is an integer e.g. 980311. """
    ...
    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.UpcxClientPath, self.upcxClientArgs(), account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False, sign=False):
        ...
        cmd="%s %s -v set contract -j %s %s %s" % (Utils.UpcxClientPath, self.upcxClientArgs(), signStr, account.name, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False, signatures=None):
        cmd="%s %s push action -j %s %s" % (Utils.UpcxClientPath, self.upcxClientArgs(), account, action)
        cmdArr=cmd.split()
        # not using __sign_str, since cmdArr messes up the string
    ...
    def pushTransaction(self, trans, opts="--skip-sign", silentErrors=False, permissions=None):
        ...
        cmd="%s %s push transaction -j" % (Utils.UpcxClientPath, self.upcxClientArgs())
        cmdArr=cmd.split()
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False, sign=False):
        ...
        trans=self.processClupcxCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False, sign=False, reportStatus=True):
        ...
        trans=self.processClupcxCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans, reportStatus=reportStatus)
    ...
    def processClupcxCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        assert(isinstance(returnType, ReturnType))
        cmd="%s %s %s" % (Utils.UpcxClientPath, self.upcxClientArgs(), cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processClupcxCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clupcx get info."""
    ...
    # If nodupcxPath is equal to None, it will use the existing nodupcx path
    def relaunch(self, chainArg=None, newChain=False, skipGenesis=True, timeout=Utils.systemWaitTimeout, addSwapFlags=None, cachePopen=False, nodupcxPath=None):
        ...
        if nodupcxPath: splittedCmd[0] = nodupcxPath
    ...
    # Require PREACTIVATE_FEATURE to be activated and require upcx.bios with preactivate_feature
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("upcx", "activate", data, opts)
    ...
    def getLatestBlockHeaderState(self):
        ...
        latestBlockHeaderState = self.processClupcxCmd(cmdDesc, cmdDesc)
    ...
    # kill all exsiting nodupcx in case lingering from previous test
    @staticmethod
    def killAllNodupcx():
