...
#    
#   node 0 : a nodupcx with 21 producers 'defproducera', 'defproducerb', ...., 'defproduceru'
#   node 1 : a nodupcx with one producer 'vltproducera' and configured to connect to a block vault.
...
try:
    ...
    if cluster.launch(onlyBios=False, pnodes=1, totalNodes=totalNodes,
                    ...
                    extraNodupcxArgs=" --blocks-log-stride 20 --max-retained-block-files 3 --logconf %s" % loggingFile,
                    specificExtraNodupcxArgs={
                        1:"--plugin upcxio::blockvault_client_plugin --block-vault-backend postgresql://postgres:password@localhost"},
    ...
    Print("Wait until block 1 is no longer retrievable, this ensures newly joined nodupcx cannot sync from net plugin")
