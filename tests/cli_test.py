...
def cleos_sign_test():
    ...
    assert(b'"account": "upcxio.token"' in output)
    assert(b'"name": "transfer"' in output)
    assert(b'"actor": "upcxio"' in output)
    ...
    cmd = ['./programs/cleos/cleos', '--print-request', 'sign', '-c', chain, '--public-key', 'UPCX8Dq1KosJ9PMn1vKQK3TbiihgfUiDBUsz471xaCE6eYUssPB1KY', packed_trx]
    outs=None
    errs=None
    try:
    ...
    assert(b'"account": "upcxio.token"' in errs)
    assert(b'"name": "transfer"' in errs)
    assert(b'"actor": "upcxio"' in errs)
