...
UPCX_STUFF_DIR=$(mktemp -d)
trap "rm -rf $UPCX_STUFF_DIR" EXIT
NODUPCX_LAUNCH_PARAMS="./programs/nodupcx/nodupcx -d $UPCX_STUFF_DIR --config-dir $UPCX_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -pupcx"
