#!/bin/bash
set -eo pipefail
# The purpose of this test is to ensure that the output of the "nodupcx --full-version" command matches the version string defined by our CMake files
echo '##### Nodupcx Full Version Label Test #####'
# orient ourselves
...
# get nodupcx version
ACTUAL=$($BUILD_ROOT/bin/nodupcx --full-version)
EXIT_CODE=$?
# verify 0 exit code explicitly
if [[ $EXIT_CODE -ne 0 ]]; then
    echo "Nodupcx produced non-zero exit code \"$EXIT_CODE\"."
    exit $EXIT_CODE
fi
# test version
