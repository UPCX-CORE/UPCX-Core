...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = "upcxio.token"_n )
{
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_scope_test, TESTER_T, backing_store_ts) { try {
   ...
   t.create_accounts({ "upcxio.token"_n, "upcxio.ram"_n, "upcxio.ramfee"_n, "upcxio.stake"_n,
      "upcxio.bpay"_n, "upcxio.vpay"_n, "upcxio.saving"_n, "upcxio.names"_n });

   std::vector<account_name> accs{"inita"_n, "initb"_n, "initc"_n, "initd"_n};
   t.create_accounts(accs);
   t.produce_block();

   t.set_code( "upcxio.token"_n, contracts::upcxio_token_wasm() );
   t.set_abi( "upcxio.token"_n, contracts::upcxio_token_abi().data() );
   t.produce_blocks(1);

   // create currency
   auto act = mutable_variant_object()
         ("issuer",       "upcxio")
         ("maximum_supply", upcxio::chain::asset::from_string("1000000000.0000 SYS"));
   t.push_action("upcxio.token"_n, "create"_n, "upcxio.token"_n, act );

   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, upcxio::chain::asset::from_string("999.0000 SYS") );
   }
   t.produce_blocks(1);

   // iterate over scope
   upcxio::chain_apis::read_only plugin(*(t.control), {}, fc::microseconds::maximum());
   upcxio::chain_apis::read_only::get_table_by_scope_params param{"upcxio.token"_n, "accounts"_n, "inita", "", 10};
   upcxio::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param);

   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name("upcxio.token"_n), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name("inita"_n), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name("accounts"_n), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name("upcxio"_n), result.rows[0].payer);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_test, TESTER_T, backing_store_ts) { try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, upcxio::chain::asset::from_string("10000.0000 SYS") );
   }
   t.produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "upcxio")
         ("maximum_supply", upcxio::chain::asset::from_string("1000000000.0000 AAA"));
   t.push_action("upcxio.token"_n, "create"_n, "upcxio.token"_n, act );
   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, upcxio::chain::asset::from_string("9999.0000 AAA") );
   }
   t.produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "upcxio")
         ("maximum_supply", upcxio::chain::asset::from_string("1000000000.0000 CCC"));
   t.push_action("upcxio.token"_n, "create"_n, "upcxio.token"_n, act );
   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, upcxio::chain::asset::from_string("7777.0000 CCC") );
   }
   t.produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "upcxio")
         ("maximum_supply", upcxio::chain::asset::from_string("1000000000.0000 BBB"));
   t.push_action("upcxio.token"_n, "create"_n, "upcxio.token"_n, act );
   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, upcxio::chain::asset::from_string("8888.0000 BBB") );
   }
   t.produce_blocks(1);

   // get table: normal case
   upcxio::chain_apis::read_only plugin(*(t.control), {}, fc::microseconds::maximum());
   upcxio::chain_apis::read_only::get_table_rows_params p;
   p.code = "upcxio.token"_n;
   p.scope = "inita";
   p.table = "accounts"_n;
   p.json = true;
   p.index_position = "primary";
   upcxio::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("upcxio", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("upcxio", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("upcxio", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("upcxio", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_by_seckey_test, TESTER_T, backing_store_ts) { try {
   ...
   t.set_code( config::system_account_name, contracts::upcxio_system_wasm() );
   t.set_abi( config::system_account_name, contracts::upcxio_system_abi().data() );
   ...
   auto bidname = [&t]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return t.push_action( "upcxio"_n, "bidname"_n, bidder, fc::mutable_variant_object()
                           ("bidder",  bidder)
                           ("newname", newname)
                           ("bid", bid)
                            );
   ...
   bidname("inita"_n, "com"_n, upcxio::chain::asset::from_string("10.0000 SYS"));
   bidname("initb"_n, "org"_n, upcxio::chain::asset::from_string("11.0000 SYS"));
   bidname("initc"_n, "io"_n, upcxio::chain::asset::from_string("12.0000 SYS"));
   bidname("initd"_n, "html"_n, upcxio::chain::asset::from_string("14.0000 SYS"));
   t.produce_blocks(1);
   ...
   bidname("inita"_n, name(all_digits_name_1), upcxio::chain::asset::from_string("1.0000 SYS"));
   bidname("inita"_n, name(all_digits_name_2), upcxio::chain::asset::from_string("1.0000 SYS"));
   t.produce_blocks(1);
   ...
   p.key_type = "name";
   p.limit = 10;
   upcxio::chain_apis::read_only::get_table_rows_result all_digits_result = plugin.read_only::get_table_rows(p);
