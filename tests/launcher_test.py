...
# launcher-test
#
# Specifically tests using the bios bootstrap script that is created by upcx-launcher
#
###############################################################
...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.upcxAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.upcxAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.upcxAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.upcxAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.upcxAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.upcxAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.upcxAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
