...
# launcher-test
#
# Specifically tests using the bios bootstrap script that is created by upcxio-launcher
#
###############################################################
...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.upcxioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.upcxioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.upcxioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.upcxioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.upcxioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.upcxioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.upcxioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
