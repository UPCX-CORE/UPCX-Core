...
try:
    ...
    accountsToCreate = [cluster.upcxioAccount]
    ...
    Print("Create new accounts via %s" % (cluster.upcxioAccount.name))
    # create accounts via upcxio as otherwise a bid is needed
    for account in accounts:
        trans = node.createInitializeAccount(account, cluster.upcxioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
    ...
    Print("Transfer funds to new accounts via %s" % (cluster.upcxioAccount.name))
    for account in accounts:
        ...
        trans = node.transferFunds(cluster.upcxioAccount, account, transferAmount, "test transfer", waitForTransBlock=False, reportStatus=False, sign = True)
        checkTransIds.append(Node.getTransId(trans))
