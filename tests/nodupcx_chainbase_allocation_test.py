...
try:
    ...
    # (automatically taken care by the automatically generated upcxio account)
    assert cluster.launch(
        ...
        specificExtraNodupcxArgs={
            1:"--read-mode irreversible --plugin upcxio::producer_api_plugin"})
    ...
    cmd = "create account -j upcxio sample UPCX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\
         UPCX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV --delay-sec 600 -p upcxio"
    ...
    producerNode.createAccount(newProducerAcc, cluster.upcxioAccount)

    setProdsStr = '{"schedule": ['
    setProdsStr += '{"producer_name":' + newProducerAcc.name + ',"block_signing_key":' + newProducerAcc.activePublicKey + '}'
    setProdsStr += ']}'
    cmd="push action -j upcxio setprods '{}' -p upcxio".format(setProdsStr)
    trans = producerNode.processClupcxCmd(cmd, cmd, silentErrors=False)
    assert trans
