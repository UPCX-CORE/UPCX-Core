...
try:
    ...
    specificExtraNodupcxArgs[0] = "--plugin upcx::producer_plugin --plugin upcx::chain_api_plugin --plugin upcx::http_plugin "
    "--plugin upcx::txn_test_gen_plugin --plugin upcx::producer_api_plugin "
    # producer nodes will be mapped to 0 through totalProducerNodes-1, so the number totalProducerNodes will be the non-producing node
    specificExtraNodupcxArgs[totalProducerNodes] = "--plugin upcx::producer_plugin --plugin upcx::chain_api_plugin --plugin upcx::http_plugin "
    "--plugin upcx::txn_test_gen_plugin --plugin upcx::producer_api_plugin "
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.upcxAccount,accounts[0],accounts[1]])
    ...
    for account in accounts:
        ...
        trans=nonProdNode.createInitializeAccount(account, cluster.upcxAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.upcxAccount.name, account.name))
        nonProdNode.transferFunds(cluster.upcxAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
