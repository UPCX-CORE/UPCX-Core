...
try:
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.upcxAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.upcxAccount.name, account.name))
        node.transferFunds(cluster.upcxAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
