...
###############################################################
# nodupcx_run_remote_test
#
#  Tests remote capability of the nodupcx_run_test. Test will setup cluster and pass nodes info to nodupcx_run_test. E.g.
#  nodupcx_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodupcx_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodupcx", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killupcxInstances, False, False, killAll, dumpErrorDetails)
