...
try:
    ...
    for nodeNum in range(txnGenNodeNum, txnGenNodeNum+startedNonProdNodes):
        specificExtraNodupcxArgs[nodeNum]="--plugin upcxio::txn_test_gen_plugin --txn-test-gen-account-prefix txntestacct"
    ...
    txnGenNodes[0].txnGenCreateTestAccounts(cluster.upcxioAccount.name, cluster.upcxioAccount.activePrivateKey)
