...
# upcx should have the same key as defproducera
upcx = copy.copy(defproduceraAccount)
upcx.name = "upcx"
...
wasmFile="upcx.system.wasm"
abiFile="upcx.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(upcx, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish upcx.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, upcx)
