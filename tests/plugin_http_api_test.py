...
class PluginHttpTest(unittest.TestCase):
    ...
    def startEnv(self) :
        ...
        nodupcx_plugins = (" --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s "
                          " --plugin %s --plugin %s --plugin %s --plugin %s ") % ( "upcxio::trace_api_plugin",
                                                                                   ...
                                                                                   "upcxio::net_plugin",
                                                                                   "upcxio::net_api_plugin",
                                                                                   "upcxio::producer_plugin",
                                                                                   "upcxio::producer_api_plugin",
                                                                                   "upcxio::chain_api_plugin",
                                                                                   "upcxio::http_plugin",
    ...
    def test_ChainApi(self) :
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"code\":\"upcxio.token\", \"account\":\"unknown\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"code\":\"upcxio.token\",\"symbol\":\"SYS\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{%s,%s,%s}'") % (default_cmd,
                                             ...
                                             "\"code\":\"upcxio.token\"",
                                             "\"action\":\"issue\"",
                                             "\"args\":{\"to\":\"upcxio.token\", \"quantity\":\"1.0000\%20UPCX\",\"memo\":\"m\"}")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s}'") % \
                    (default_cmd,
                     ...
                     "\"actions\":[{\"account\":\"upcxio.token\",\"name\": \"transfer\",\"authorization\": [{\"actor\": \"han\",\"permission\": \"active\"}],\"data\": \"000000000000a6690000000000ea305501000000000000000453595300000000016d\"}]",
