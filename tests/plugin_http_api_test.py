...
class PluginHttpTest(unittest.TestCase):
    ...
    def startEnv(self) :
        ...
        nodupcx_plugins = (" --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s --plugin %s "
                          " --plugin %s --plugin %s --plugin %s --plugin %s ") % ( "upcx::trace_api_plugin",
                                                                                   "upcx::test_control_api_plugin",
                                                                                   "upcx::test_control_plugin",
                                                                                   "upcx::net_plugin",
                                                                                   "upcx::net_api_plugin",
                                                                                   "upcx::producer_plugin",
                                                                                   "upcx::producer_api_plugin",
                                                                                   "upcx::chain_api_plugin",
                                                                                   "upcx::http_plugin",
                                                                                   "upcx::db_size_api_plugin",
                                                                                   "upcx::history_plugin",
                                                                                   "upcx::history_api_plugin")
        ...
        start_nodupcx_cmd = ("%s -e -p upcx %s %s ") % (Utils.UpcxServerPath, nodupcx_plugins, nodupcx_flags)
    ...
    def test_ChainApi(self) :
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"code\":\"upcx.token\", \"account\":\"unknown\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"code\":\"upcx.token\",\"symbol\":\"SYS\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{%s,%s,%s}'") % (default_cmd,
                                             ...
                                             "\"code\":\"upcx.token\"",
                                             "\"action\":\"issue\"",
                                             "\"args\":{\"to\":\"upcx.token\", \"quantity\":\"1.0000\%20UPCX\",\"memo\":\"m\"}")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{\"transaction\":{%s,%s,%s,%s,%s,%s,%s},\"available_keys\":[%s,%s,%s]}'") % \
                    (default_cmd,
                     ...
                     "\"signatures\": []",
                     "\"authorizations\": []",
                     "\"UPCX4toFS3YXEQCkuuw1aqDLrtHim86Gz9u3hBdcBw5KNPZcursVHq\"",
                     "\"UPCX7d9A3uLe6As66jzN8j44TXJUqJSK3bFjjEEqR4oTvNAB3iM9SA\"",
                     "\"UPCX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\"")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s}'") % \
                    (default_cmd,
                     ...
                     "\"actions\":[{\"account\":\"upcx.token\",\"name\": \"transfer\",\"authorization\": [{\"actor\": \"han\",\"permission\": \"active\"}],\"data\": \"000000000000a6690000000000ea305501000000000000000453595300000000016d\"}]",
    ...
    def test_HistoryApi(self) :
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"public_key\":\"UPCX6FxXbikY5ZUN9qEdeLbEYLKZzJwRYRr2PuC3rqfSu67LvhPARi\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
    ...
    def test_WalletApi(self) :
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'[%s, %s, %s]'") % (signed_transaction,
                                                                             "[\"UPCX696giL6VxeJhtEgKtWPK8aQeT8YXNjw2a7vE5wHunffhfa5QSQ\"]",
