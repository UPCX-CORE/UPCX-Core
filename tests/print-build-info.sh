#!/bin/bash
set -eo pipefail
# The purpose of this test is to ensure that the output of the "nodupcx --print-build-info" command.
# This includes verifying valid output in JSON shape and checking parameters (only boost for now).
echo '##### Nodupcx Print Build Info Test #####'
# orient ourselves
...
PRINT_BUILD_INFO="$BUILD_ROOT/bin/nodupcx --print-build-info 2>&1 | tee >(cat - >&9) || :"
