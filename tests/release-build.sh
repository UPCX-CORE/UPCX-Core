...
echo ''
echo '    The purpose of this test is to ensure that nodupcx was built with compiler'
echo 'optimizations enabled. While there is no way to programmatically determine that'
echo 'given one binary, we do set a debug flag in nodupcx when it is built with'
echo 'asserts. This test checks that debug flag. Anyone intending to build and install'
echo 'nodupcx from source should perform a "release build" which excludes asserts and'
...
# find nodupcx
[[ $(git --version) ]] && cd "$(git rev-parse --show-toplevel)/build/programs/nodupcx" || cd "$(dirname "${BASH_SOURCE[0]}")/../programs/nodupcx"
if [[ ! -f nodupcx ]]; then
    echo 'ERROR: nodupcx binary not found!'
    echo ''
    echo 'I looked here...'
    echo "$ ls -la \"$(pwd)/programs/nodupcx\""
    ls -la "$(pwd)/programs/nodupcx"
...
# run nodupcx to generate state files
./nodupcx --extract-build-info build-info.json 1>/dev/null 2>/dev/null
