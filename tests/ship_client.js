...
const { Serialize } = require("upcxjs");
...
class Connection {
  ...
  get_status_result_v0(response) {
    ...
    var timeSinceBlockNumChange =
      (currentTimeSec - this.blockNumChangeTime) / 1000;
    ...
    response["time_since_last_block"] =
      (currentTimeSec - this.lastResponseTime) / 1000;
    ...
    if (this.numRequests < 0) {
      ...
      status["max_time"] = {
        block_num: this.max["blockNum"],
        time_delta: this.max["time"],
      };
...
const connection = new Connection(
  commander.socketAddress,
  commander.numRequests
);
