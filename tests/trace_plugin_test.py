...
class TraceApiPluginTest(unittest.TestCase):
    ...
    def startEnv(self) :
        ...
        traceNodupcxArgs = " --plugin upcx::trace_api_plugin --trace-no-abis --trace-dir=."
        self.cluster.launch(totalNodes=1, extraNodupcxArgs=traceNodupcxArgs)
        self.walletMgr.launch()
        testWalletName="testwallet"
        testWallet=self.walletMgr.create(testWalletName, [self.cluster.upcxAccount, self.cluster.defproduceraAccount])
        ...
        for account in self.accounts:
            node.createInitializeAccount(account, self.cluster.upcxAccount, buyRAM=1000000, stakedDeposit=5000000, waitForTransBlock=True, exitOnError=True)
