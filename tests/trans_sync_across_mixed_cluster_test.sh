...
killAll()
{
  programs/upcxio-launcher/upcxio-launcher -k 15
}

cleanup()
{
    rm -rf etc/upcxio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clupcx/clupcx get info)"
  verifyErrorCode "clupcx get info"
...
# stand up nodupcx cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodupcx \"--plugin upcxio::wallet_api_plugin\" $launcherOpts
programs/upcxio-launcher/upcxio-launcher --nodupcx "--plugin upcxio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clupcx/clupcx --port $port2 get block 1)"
    verifyErrorCode "clupcx get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clupcx/clupcx create key)"
verifyErrorCode "clupcx create key"
...
PASSWORD_INITA="$(programs/clupcx/clupcx wallet create --name inita)"
verifyErrorCode "clupcx wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clupcx/clupcx wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clupcx wallet import"
programs/clupcx/clupcx wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clupcx wallet import"
programs/clupcx/clupcx wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clupcx wallet import"
programs/clupcx/clupcx wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clupcx wallet import"
...
ACCOUNT_INFO="$(programs/clupcx/clupcx create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clupcx create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clupcx/clupcx get account testera)"
verifyErrorCode "clupcx get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clupcx/clupcx transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clupcx transfer"
   ...
   TRANS_INFO="$(programs/clupcx/clupcx --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clupcx get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
