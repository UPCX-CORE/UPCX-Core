...
fastUnstakeSystem = './fast.refund/upcx.system/upcx.system.wasm'
...
def startNode(nodeIndex, account):
    ...
    cmd = (
        ...
        # (which is defined in .../chain/include/upcx/chain/config.hpp
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.clupcx + 'create account upcx ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.clupcx + 'system newaccount --transfer upcx %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.clupcx + 'transfer upcx %s "%s"' % (a['name'], intToCurrency(unstaked)))
...
def claimRewards():
    table = getJsonOutput(args.clupcx + 'get table upcx upcx producers -l 100')
    times = []
...
def updateAuth(account, permission, parent, controller):
    run(args.clupcx + 'push action upcx updateauth' + jsonArg({
        'account': account,
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'upcx', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'upcx', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.clupcx + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'upcx setcode' + jsonArg(setcode) + ' -p ' + proposer)
...
def msigReplaceSystem():
    run(args.clupcx + 'push action upcx buyrambytes' + jsonArg(['upcx', accounts[0]['name'], 200000]) + '-p upcx')
    sleep(1)
...
def stepStartWallet():
    ...
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'upcx', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(10.0)
def stepInstallSystemContracts():
    run(args.clupcx + 'set contract upcx.token ' + args.contracts_dir + '/upcx.token/')
    run(args.clupcx + 'set contract upcx.msig ' + args.contracts_dir + '/upcx.msig/')
def stepCreateTokens():
    run(args.clupcx + 'push action upcx.token create \'["upcx", "10000000000.0000 %s"]\' -p upcx.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.clupcx + 'push action upcx.token issue \'["upcx", "%s", "memo"]\' -p upcx' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    ...
    # activate PREACTIVATE_FEATURE before installing upcx.boot
    retry('curl -X POST http://127.0.0.1:%d' % args.http_port + 
        '/v1/producer/schedule_protocol_feature_activations ' +
        '-d \'{"protocol_features_to_activate": ["0ec7e080177b2c02b278d5088611686b49d739925a92d9bfcacd7fc6b74053bd"]}\'')
    sleep(3)

    # install upcx.boot which supports the native actions and activate 
    # action that allows activating desired protocol features prior to 
    # deploying a system contract with more features such as upcx.bios 
    # or upcx.system
    retry(args.clupcx + 'set contract upcx ' + args.contracts_dir + '/upcx.boot/')
    sleep(3)

    # activate remaining features
    # KV_DATABASE
    retry(args.clupcx + 'push action upcx activate \'["825ee6288fb1373eab1b5187ec2f04f6eacb39cb3a97f356a07c91622dd61d16"]\' -p upcx@active')
    # ACTION_RETURN_VALUE
    retry(args.clupcx + 'push action upcx activate \'["c3a6138c5061cf291310887c0b5c71fcaffeab90d5deb50d3b9e687cead45071"]\' -p upcx@active')
    # CONFIGURABLE_WASM_LIMITS
    retry(args.clupcx + 'push action upcx activate \'["bf61537fd21c61a60e542a5d66c3f6a78da0589336868307f94a82bccea84e88"]\' -p upcx@active')
    # BLOCKCHAIN_PARAMETERS
    retry(args.clupcx + 'push action upcx activate \'["5443fcf88330c586bc0e5f3dee10e7f63c76c00249c87fe4fbf7f38c082006b4"]\' -p upcx@active')
    # GET_SENDER
    retry(args.clupcx + 'push action upcx activate \'["f0af56d2c5a48d60a4a5b5c903edfb7db3a736a94ed589d0b797df33ff9d3e1d"]\' -p upcx@active')
    # FORWARD_SETCODE
    retry(args.clupcx + 'push action upcx activate \'["2652f5f96006294109b3dd0bbde63693f55324af452b799ee137a81a905eed25"]\' -p upcx@active')
    # ONLY_BILL_FIRST_AUTHORIZER
    retry(args.clupcx + 'push action upcx activate \'["8ba52fe7a3956c5cd3a656a3174b931d3bb2abb45578befc59f283ecd816a405"]\' -p upcx@active')
    # RESTRICT_ACTION_TO_SELF
    retry(args.clupcx + 'push action upcx activate \'["ad9e3d8f650687709fd68f4b90b41f7d825a365b02c23a636cef88ac2ac00c43"]\' -p upcx@active')
    # DISALLOW_EMPTY_PRODUCER_SCHEDULE
    retry(args.clupcx + 'push action upcx activate \'["68dcaa34c0517d19666e6b33add67351d8c5f69e999ca1e37931bc410a297428"]\' -p upcx@active')
     # FIX_LINKAUTH_RESTRICTION
    retry(args.clupcx + 'push action upcx activate \'["e0fb64b1085cc5538970158d05a009c24e276fb94e1a0bf6a528b48fbc4ff526"]\' -p upcx@active')
     # REPLACE_DEFERRED
    retry(args.clupcx + 'push action upcx activate \'["ef43112c6543b88db2283a2e077278c315ae2c84719a8b25f25cc88565fbea99"]\' -p upcx@active')
    # NO_DUPLICATE_DEFERRED_ID
    retry(args.clupcx + 'push action upcx activate \'["4a90c00d55454dc5b059055ca213579c6ea856967712a56017487886a4d4cc0f"]\' -p upcx@active')
    # ONLY_LINK_TO_EXISTING_PERMISSION
    retry(args.clupcx + 'push action upcx activate \'["1a99a59d87e06e09ec5b028a9cbb7749b4a5ad8819004365d02dc4379a8b7241"]\' -p upcx@active')
    # RAM_RESTRICTIONS
    retry(args.clupcx + 'push action upcx activate \'["4e7bf348da00a945489b2a681749eb56f5de00b900014e137ddae39f48f69d67"]\' -p upcx@active')
    # WEBAUTHN_KEY
    retry(args.clupcx + 'push action upcx activate \'["4fca8bd82bbd181e714e283f83e1b45d95ca5af40fb89ad3977b653c448f78c2"]\' -p upcx@active')
    # WTMSIG_BLOCK_SIGNATURES
    retry(args.clupcx + 'push action upcx activate \'["299dcb6af692324b899b39f16d5a530a33062804e41f09dc97e9f156b4476707"]\' -p upcx@active')
    sleep(1)

    # install upcx.system latest version
    retry(args.clupcx + 'set contract upcx ' + args.contracts_dir + '/upcx.system/')
    # setpriv is only available after upcx.system is installed
    run(args.clupcx + 'push action upcx setpriv' + jsonArg(['upcx.msig', 1]) + '-p upcx@active')
    sleep(3)

def stepInitSystemContract():
    run(args.clupcx + 'push action upcx init' + jsonArg(['0', '4,' + args.symbol]) + '-p upcx@active')
    sleep(1)
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-upcx/stderr')
...
commands = [
    ...
    ('s', 'sys',                createSystemAccounts,       True,    "Create system accounts (upcx.*)"),
    ...
    ('q', 'resign',             stepResign,                 True,    "Resign upcx"),
    ('m', 'msg-replace',        msigReplaceSystem,          False,   "Replace system contract using msig"),
    ('X', 'xfer',               stepTransfer,               False,   "Random transfer tokens (infinite loop)"),
    ('l', 'log',                stepLog,                    True,    "Show tail of node's log"),
]

parser.add_argument('--public-key', metavar='', help="UPCX Public Key", default='UPCX8Znrtgwt8TfpmbVpTKvA2oB8Nqey625CLN8bCN3TEbgx86Dsvr', dest="public_key")
parser.add_argument('--private-Key', metavar='', help="UPCX Private Key", default='5K463ynhZoCDDa4RDcr63cUwWLTnKqmdcoTKTHBjqoKfv4u5V7p', dest="private_key")
...
parser.add_argument('--symbol', metavar='', help="The upcx.system symbol", default='SYS')
