...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for upcx reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("upcx"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("upcx"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with upcx reqauth
   chain.unlink_authority(name("alice"), name("upcx"), name("reqauth"));
   ...
   // Link authority for any upcx action with alice's scud key
   chain.link_authority(name("alice"), name("upcx"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("upcx"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with upcx. prefix with privileged account
   chain.create_account(name("upcx.test1"));

   // Creating account with upcx. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("upcx.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'upcx.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("upcx"), name("upcx.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("upcx"), name("upcx.any"), name("reqauth") );

   /// this should succeed because upcx::reqauth is linked to any permission
   chain.push_reqauth(name("alice"), { permission_level{"alice"_n, name("spending")} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = upcx::chain::resource_limits::resource_usage_object;
   using by_owner = upcx::chain::resource_limits::by_owner;
