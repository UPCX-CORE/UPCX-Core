...
public:
   void deploy_contract( bool call_init = true ) {
      set_code( config::system_account_name, contracts::upcxio_system_wasm() );
      set_abi( config::system_account_name, contracts::upcxio_system_abi().data() );
      if( call_init ) {
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "upcxio_global_state", data, abi_serializer::create_yield_function( abi_serializer_max_time ) );
   }
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance("upcxio.token"_n, symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create upcxio.msig and upcxio.token
        create_accounts({"upcxio.msig"_n, "upcxio.token"_n, "upcxio.ram"_n, "upcxio.ramfee"_n, "upcxio.stake"_n, "upcxio.vpay"_n, "upcxio.bpay"_n, "upcxio.saving"_n });
        // Set code for the following accounts:
        //  - upcxio (code: upcxio.bios) (already set by tester constructor)
        //  - upcxio.msig (code: upcxio.msig)
        //  - upcxio.token (code: upcxio.token)
        // set_code_abi("upcxio.msig"_n, contracts::upcxio_msig_wasm(), contracts::upcxio_msig_abi().data());//, &upcxio_active_pk);
        // set_code_abi("upcxio.token"_n, contracts::upcxio_token_wasm(), contracts::upcxio_token_abi().data()); //, &upcxio_active_pk);

        set_code_abi("upcxio.msig"_n,
                     contracts::upcxio_msig_wasm(),
                     contracts::upcxio_msig_abi().data());//, &upcxio_active_pk);
        set_code_abi("upcxio.token"_n,
                     contracts::upcxio_token_wasm(),
                     contracts::upcxio_token_abi().data()); //, &upcxio_active_pk);

        // Set privileged for upcxio.msig and upcxio.token
        set_privileged("upcxio.msig"_n);
        set_privileged("upcxio.token"_n);

        // Verify upcxio.msig and upcxio.token is privileged
        const auto& upcxio_msig_acc = get<account_metadata_object, by_name>("upcxio.msig"_n);
        BOOST_TEST(upcxio_msig_acc.is_privileged() == true);
        const auto& upcxio_token_acc = get<account_metadata_object, by_name>("upcxio.token"_n);
        BOOST_TEST(upcxio_token_acc.is_privileged() == true);


        // Create SYS tokens in upcxio.token, set its manager as upcxio
        ...
        create_currency("upcxio.token"_n, config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to upcxio.system
        issue("upcxio.token"_n, config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth("upcxio.stake"_n, a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("upcxio"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards("runnerup1"_n), upcxio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth("b1"_n, "b1"_n, core_from_string("49999500.0000"), core_from_string("49999500.0000")), upcxio_assert_message_exception);
