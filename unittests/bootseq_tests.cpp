...
public:
   void deploy_contract( bool call_init = true ) {
      set_code( config::system_account_name, contracts::upcx_system_wasm() );
      set_abi( config::system_account_name, contracts::upcx_system_abi().data() );
      if( call_init ) {
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "upcx_global_state", data, abi_serializer::create_yield_function( abi_serializer_max_time ) );
   }
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance("upcx.token"_n, symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create upcx.msig and upcx.token
        create_accounts({"upcx.msig"_n, "upcx.token"_n, "upcx.ram"_n, "upcx.ramfee"_n, "upcx.stake"_n, "upcx.vpay"_n, "upcx.bpay"_n, "upcx.saving"_n });
        // Set code for the following accounts:
        //  - upcx (code: upcx.bios) (already set by tester constructor)
        //  - upcx.msig (code: upcx.msig)
        //  - upcx.token (code: upcx.token)
        // set_code_abi("upcx.msig"_n, contracts::upcx_msig_wasm(), contracts::upcx_msig_abi().data());//, &upcx_active_pk);
        // set_code_abi("upcx.token"_n, contracts::upcx_token_wasm(), contracts::upcx_token_abi().data()); //, &upcx_active_pk);

        set_code_abi("upcx.msig"_n,
                     contracts::upcx_msig_wasm(),
                     contracts::upcx_msig_abi().data());//, &upcx_active_pk);
        set_code_abi("upcx.token"_n,
                     contracts::upcx_token_wasm(),
                     contracts::upcx_token_abi().data()); //, &upcx_active_pk);

        // Set privileged for upcx.msig and upcx.token
        set_privileged("upcx.msig"_n);
        set_privileged("upcx.token"_n);

        // Verify upcx.msig and upcx.token is privileged
        const auto& upcx_msig_acc = get<account_metadata_object, by_name>("upcx.msig"_n);
        BOOST_TEST(upcx_msig_acc.is_privileged() == true);
        const auto& upcx_token_acc = get<account_metadata_object, by_name>("upcx.token"_n);
        BOOST_TEST(upcx_token_acc.is_privileged() == true);


        // Create SYS tokens in upcx.token, set its manager as upcx
        ...
        create_currency("upcx.token"_n, config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to upcx.system
        issue("upcx.token"_n, config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth("upcx.stake"_n, a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("upcx"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards("runnerup1"_n), upcx_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth("b1"_n, "b1"_n, core_from_string("49999500.0000"), core_from_string("49999500.0000")), upcx_assert_message_exception);
