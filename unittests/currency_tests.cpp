...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance("upcx.token"_n, symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action("upcx.token"_n, "issue"_n, mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::upcx_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time ))
      {
         create_account( "upcx.token"_n);
         set_code( "upcx.token"_n, contracts::upcx_token_wasm() );

         auto result = push_action("upcx.token"_n, "create"_n, mutable_variant_object()
                 ("issuer",       upcx_token)
         ...
         result = push_action("upcx.token"_n, "issue"_n, mutable_variant_object()
                 ("to",       upcx_token)
      ...
      static const name upcx_token;
};

const name currency_tester::upcx_token = "upcx.token"_n;
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance("upcx.token"_n, expected.get_symbol(), "upcx.token"_n);
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action("upcx.token"_n, "transfer"_n, mutable_variant_object()
         ("from", upcx_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action("upcx.token"_n, "transfer"_n, mutable_variant_object()
                                    ("from", upcx_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action("alice"_n, "transfer"_n, data),
                             upcx_assert_message_exception, upcx_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(upcx_token, "alice"_n, "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer("alice"_n, "carl"_n, "20.50 USD"), upcx_assert_message_exception);
   }
