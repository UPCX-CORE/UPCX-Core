...
class kv_tester : public tester {
 public:
   ...
   void test_max_iterators() {
      ...
      // individual limits
      BOOST_TEST(iterlimit({{"upcx.kvram"_n, 7, false}}) == "");
      BOOST_TEST(iterlimit({{"upcx.kvram"_n, 2000, false}}) == "Too many iterators");
      // erase iterators and create more
      BOOST_TEST(iterlimit({{"upcx.kvram"_n, 6, false},
                            {"upcx.kvram"_n, 2, true},
                            {"upcx.kvram"_n, 3, false}}) == "");
      BOOST_TEST(iterlimit({{"upcx.kvram"_n, 6, false},
                            {"upcx.kvram"_n, 2, true},
                            {"upcx.kvram"_n, 4, false}}) == "Too many iterators");
      ...
      // BOOST_TEST_REQUIRE(set_kv_limits("upcx.kvram"_n, 1024, 1024, 0xFFFFFFFF) == "");
      // BOOST_TEST_REQUIRE(set_kv_limits("upcx.kvram"_n, 1024, 1024, 0xFFFFFFFF) == "");
      // BOOST_TEST(iterlimit({{"upcx.kvram"_n, 0xFFFFFFFF, false}, {"upcx.kvram"_n, 1, false}}) == "Too many iterators");
   }
   ...
   void test_undo() {
      ...
      set_transaction_headers(trx);
      trx.sign(get_private_key("kvtest"_n, "active"), control->get_chain_id());
      BOOST_CHECK_THROW(push_transaction(trx), upcx_assert_code_exception);
...
constexpr name databases[] = { "upcx.kvram"_n };
...
(module
 ...
 (memory 1)
 (func (export "apply") (param i64 i64 i64)
  (call $upcx_assert (i32.eq (call $kv_it_create (get_local 0) (i32.const 0) (i32.const 0)) (i32.const 1)) (i32.const 80))
  (call $upcx_assert (i32.eq (call $kv_get_data (i32.const 0) (i32.const 0) (i32.const 0)) (i32.const 0)) (i32.const 160))
  (drop (call $kv_set (get_local 0) (i32.const 0) (i32.const 0) (i32.const 1) (i32.const 1) (get_local 0)))
 )
...
BOOST_DATA_TEST_CASE_F(tester, notify, bdata::make(databases), db) {
   ...
   push_action( "upcx"_n, "setpriv"_n, "upcx"_n, mutable_variant_object()("account", "setup"_n)("is_priv", 1));
