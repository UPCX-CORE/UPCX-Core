...
BOOST_AUTO_TEST_CASE(transaction_test) { try {
   ...
   empty_cfd_trx.context_free_actions.push_back({ {}, "upcx"_n, ""_n, bytes() });
   empty_cfd_trx.context_free_data.push_back(bytes());
   test.set_transaction_headers(empty_cfd_trx);
   empty_cfd_trx.sign( test.get_private_key( "upcx"_n, "active" ), test.control->get_chain_id() );
...
BOOST_AUTO_TEST_CASE(pruned_transaction_test) {
   tester t;
   signed_transaction trx;
   trx.context_free_actions.push_back({ {}, "upcx"_n, ""_n, bytes() });
   trx.context_free_data.push_back(bytes());
   t.set_transaction_headers(trx);
   trx.sign( t.get_private_key( "upcx"_n, "active" ), t.control->get_chain_id() );
...
upcx::chain::transaction_trace_ptr push_cfd_transaction(upcx::testing::tester& t) {
    signed_transaction trx;
    trx.actions.push_back({ { permission_level{ "upcx"_n, "active"_n } }, "upcx"_n, ""_n, bytes() } );
    trx.context_free_actions.push_back({ {}, "upcx"_n, ""_n, bytes() });
    trx.context_free_data.push_back(bytes());
    t.set_transaction_headers(trx);
    trx.sign( t.get_private_key( "upcx"_n, "active" ), t.control->get_chain_id() );
...
BOOST_AUTO_TEST_CASE(block_prune_state_test) {
   tester t;
   upcx::chain::transaction_trace_ptr trace;
