...
#include <upcxio/chain/config.hpp>
#include <upcxio/testing/chainbase_fixture.hpp>
#include <upcxio/testing/tester.hpp>
#include "fork_test_utilities.hpp"

#include <boost/test/unit_test.hpp>

using namespace upcxio::chain::resource_limits;
...
// TODO: restore weighted capacity cpu tests
   ...
   BOOST_FIXTURE_TEST_CASE(get_account_net_limit_ex_and_get_account_cpu_limit_ex, resource_limits_fixture) try {
      ...
      constexpr uint32_t net_window = upcxio::chain::config::account_net_usage_average_window_ms / upcxio::chain::config::block_interval_ms;
      constexpr uint32_t cpu_window = upcxio::chain::config::account_cpu_usage_average_window_ms / upcxio::chain::config::block_interval_ms;
   ...
   BOOST_AUTO_TEST_CASE(light_net_validation) try {
      ...
      // Modify NET bill in transaction receipt within trigger block to cause the NET usage of test account to exceed its quota.
      {
         ...
         upcxio::chain::deque<digest_type> trx_digests;
