...
#include <upcx/testing/snapshot_suites.hpp>
...
void exhaustive_snapshot(const upcx::chain::backing_store_type main_store,
                         const upcx::chain::backing_store_type sub_store) {
...
{
   exhaustive_snapshot<SNAPSHOT_SUITE>(upcx::chain::backing_store_type::CHAINBASE,
                                       upcx::chain::backing_store_type::CHAINBASE);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(test_exhaustive_snapshot_cb_to_rdb, SNAPSHOT_SUITE, snapshot_suites)
{
   exhaustive_snapshot<SNAPSHOT_SUITE>(upcx::chain::backing_store_type::CHAINBASE,
                                       upcx::chain::backing_store_type::ROCKSDB);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(test_exhaustive_snapshot_rdb_to_cb, SNAPSHOT_SUITE, snapshot_suites)
{
   exhaustive_snapshot<SNAPSHOT_SUITE>(upcx::chain::backing_store_type::ROCKSDB,
                                       upcx::chain::backing_store_type::CHAINBASE);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_replay_over_snapshot, SNAPSHOT_SUITE, snapshot_suites)
{
   ...
   auto genesis = upcx::chain::block_log::extract_genesis_state(chain.get_config().blog.log_dir);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_compatible_versions, SNAPSHOT_SUITE, snapshot_suites)
{
   ...
   auto genesis = upcx::chain::block_log::extract_genesis_state(source_log_dir);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_kv_snapshot, SNAPSHOT_SUITE, snapshot_suites) {
   for (backing_store_type origin_backing_store : { backing_store_type::CHAINBASE, backing_store_type::ROCKSDB }) {
      for (backing_store_type resulting_backing_store: { backing_store_type::CHAINBASE, backing_store_type::ROCKSDB }) {
         ...
         chain.push_action("upcx"_n, "setpriv"_n, "upcx"_n, mutable_variant_object()("account", "manager")("is_priv", 1));
         ...
         for (int generation = 0; generation < generation_count; generation++) {
            ...
            for (auto contract : contracts) {
               ...
               signed_transaction trx;
               trx.actions.push_back({{{contract, "active"_n}}, contract, "upcx.kvram"_n, {}});
               chain.set_transaction_headers(trx);
