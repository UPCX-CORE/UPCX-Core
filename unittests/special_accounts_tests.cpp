...
using tester = upcx::testing::tester;
...
{ try {
      ...
      const auto& nobody_active_authority = chain1_db.get<permission_object, upcx::chain::by_owner>(boost::make_tuple(config::null_account_name, config::active_name));
      ...
      const auto& nobody_owner_authority = chain1_db.get<permission_object, upcx::chain::by_owner>(boost::make_tuple(config::null_account_name, config::owner_name));
