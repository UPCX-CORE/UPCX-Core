...
[[upcxio::action]]
person kv_addr_book::get(upcxio::name account_name) {
   ...
   if (itr != addresses.account_name_uidx.end()) {
      ...
      upcxio::print_f("Person not found in addressbook.");
...
std::vector<person> kv_addr_book::getbylastname(std::string last_name) {
   ...
   upcxio::name min_account_name{0};
   upcxio::name max_account_name{UINT_MAX};
...
void kv_addr_book::upsert(
   ...
   if (existing_person.get_account_name().value == 0) {
      upcxio::print_f("Person was successfully added to addressbook.");
   }
   else {
      upcxio::print_f("Person was successfully updated in addressbook.");
   }
}

// deletes a person based on primary key account_name
[[upcxio::action]]
void kv_addr_book::del(upcxio::name account_name) {
   address_table addresses{"kvaddrbook"_n};
   ...
   if (itr != addresses.account_name_uidx.end()) {
      ...
      upcxio::print_f("Person was successfully deleted from addressbook.");
   }
   else {
      upcxio::print_f("Person not found in addressbook.");
   }
