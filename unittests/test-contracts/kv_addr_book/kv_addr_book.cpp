...
[[upcx::action]]
person kv_addr_book::get(upcx::name account_name) {
   ...
   if (itr != addresses.account_name_uidx.end()) {
      ...
      upcx::print_f("Person not found in addressbook.");
...
std::vector<person> kv_addr_book::getbylastname(std::string last_name) {
   ...
   upcx::name min_account_name{0};
   upcx::name max_account_name{UINT_MAX};
...
void kv_addr_book::upsert(
   ...
   if (existing_person.get_account_name().value == 0) {
      upcx::print_f("Person was successfully added to addressbook.");
   }
   else {
      upcx::print_f("Person was successfully updated in addressbook.");
   }
}

// deletes a person based on primary key account_name
[[upcx::action]]
void kv_addr_book::del(upcx::name account_name) {
   address_table addresses{"kvaddrbook"_n};
   ...
   if (itr != addresses.account_name_uidx.end()) {
      ...
      upcx::print_f("Person was successfully deleted from addressbook.");
   }
   else {
      upcx::print_f("Person not found in addressbook.");
   }
