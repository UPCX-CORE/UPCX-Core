...
struct person {
   ...
   upcx::non_unique<upcx::name, std::string> first_name;
   upcx::non_unique<upcx::name, std::string> last_name;
   upcx::non_unique<upcx::name, std::string, std::string, std::string, std::string> street_city_state_cntry;
   upcx::non_unique<upcx::name, std::string> personal_id;
   std::pair<std::string, std::string> country_personal_id;

   upcx::name get_account_name() const {
      return account_name;
   }
...
class [[upcx::contract]] kv_addr_book : public upcx::contract {

   struct [[upcx::table]] address_table : upcx::kv::table<person, "kvaddrbook"_n> {
      ...
      index<non_unique<upcx::name, std::string>> first_name_idx {
         name{"firstname"_n},
         &person::first_name};
      index<non_unique<upcx::name, std::string>> last_name_idx {
         name{"lastname"_n},
         &person::last_name};
      index<non_unique<upcx::name, std::string>> personal_id_idx {
         name{"persid"_n},
         &person::personal_id};
      ...
      address_table(upcx::name contract_name) {
         init(contract_name,
            account_name_uidx,
   ...
   public:
      ...
      kv_addr_book(upcx::name receiver, upcx::name code, upcx::datastream<const char*> ds)
         : contract(receiver, code, ds) {}

      // retrieves a person based on primary key account_name
      [[upcx::action]]
      person get(upcx::name account_name);
      ...
      [[upcx::action]]
      void upsert(upcx::name account_name,
         std::string first_name,
         std::string last_name,
         std::string street,
         std::string city,
      ...
      [[upcx::action]]
      void del(upcx::name account_name);
      ...
      using get_action = upcx::action_wrapper<"get"_n, &kv_addr_book::get>;
      using get_by_cntry_pers_id_action = upcx::action_wrapper<"getbycntrpid"_n, &kv_addr_book::getbycntrpid>;
      using get_by_last_name_action = upcx::action_wrapper<"getbylastname"_n, &kv_addr_book::getbylastname>;
      using get_buy_address_action = upcx::action_wrapper<"getbyaddress"_n, &kv_addr_book::getbyaddress>;
      using upsert_action = upcx::action_wrapper<"upsert"_n, &kv_addr_book::upsert>;
      using del_action = upcx::action_wrapper<"del"_n, &kv_addr_book::del>;
      using is_pers_id_in_cntry_action = upcx::action_wrapper<"checkpidcntr"_n, &kv_addr_book::checkpidcntr>;
      using iterate_action = upcx::action_wrapper<"iterate"_n, &kv_addr_book::iterate>;
