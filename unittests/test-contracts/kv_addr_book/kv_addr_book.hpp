...
struct person {
   ...
   upcxio::non_unique<upcxio::name, std::string> first_name;
   upcxio::non_unique<upcxio::name, std::string> last_name;
   upcxio::non_unique<upcxio::name, std::string, std::string, std::string, std::string> street_city_state_cntry;
   upcxio::non_unique<upcxio::name, std::string> personal_id;
   std::pair<std::string, std::string> country_personal_id;

   upcxio::name get_account_name() const {
      return account_name;
   }
...
class [[upcxio::contract]] kv_addr_book : public upcxio::contract {

   struct [[upcxio::table]] address_table : upcxio::kv::table<person, "kvaddrbook"_n> {
      ...
      index<non_unique<upcxio::name, std::string>> first_name_idx {
         name{"firstname"_n},
         &person::first_name};
      index<non_unique<upcxio::name, std::string>> last_name_idx {
         name{"lastname"_n},
         &person::last_name};
      index<non_unique<upcxio::name, std::string>> personal_id_idx {
         name{"persid"_n},
         &person::personal_id};
      ...
      address_table(upcxio::name contract_name) {
         init(contract_name,
            account_name_uidx,
   ...
   public:
      ...
      kv_addr_book(upcxio::name receiver, upcxio::name code, upcxio::datastream<const char*> ds)
         : contract(receiver, code, ds) {}

      // retrieves a person based on primary key account_name
      [[upcxio::action]]
      person get(upcxio::name account_name);
      ...
      [[upcxio::action]]
      void upsert(upcxio::name account_name,
         std::string first_name,
         std::string last_name,
         std::string street,
         std::string city,
      ...
      [[upcxio::action]]
      void del(upcxio::name account_name);
      ...
      using get_action = upcxio::action_wrapper<"get"_n, &kv_addr_book::get>;
      using get_by_cntry_pers_id_action = upcxio::action_wrapper<"getbycntrpid"_n, &kv_addr_book::getbycntrpid>;
      using get_by_last_name_action = upcxio::action_wrapper<"getbylastname"_n, &kv_addr_book::getbylastname>;
      using get_buy_address_action = upcxio::action_wrapper<"getbyaddress"_n, &kv_addr_book::getbyaddress>;
      using upsert_action = upcxio::action_wrapper<"upsert"_n, &kv_addr_book::upsert>;
      using del_action = upcxio::action_wrapper<"del"_n, &kv_addr_book::del>;
      using is_pers_id_in_cntry_action = upcxio::action_wrapper<"checkpidcntr"_n, &kv_addr_book::checkpidcntr>;
      using iterate_action = upcxio::action_wrapper<"iterate"_n, &kv_addr_book::iterate>;
