#include <upcx/contract.hpp>
#include <upcx/name.hpp>
#include <upcx/privileged.hpp>

extern "C" __attribute__((upcx_wasm_import)) void set_resource_limit(int64_t, int64_t, int64_t);
extern "C" __attribute__((upcx_wasm_import)) uint32_t get_kv_parameters_packed(void* params, uint32_t size, uint32_t max_version);
extern "C" __attribute__((upcx_wasm_import)) void set_kv_parameters_packed(const char* params, uint32_t size);
#ifdef USE_UPCX_CDT_1_7_X
extern "C" __attribute__((upcx_wasm_import)) uint32_t read_action_data( void* msg, uint32_t len );
extern "C" __attribute__((upcx_wasm_import))    uint32_t action_data_size();
#endif

using namespace upcx;

// Manages resources used by the kv-store
class [[upcx::contract]] kv_bios : upcx::contract {
 public:
   using contract::contract;
   [[upcx::action]] void setramlimit(name account, int64_t limit) {
      set_resource_limit(account.value, "ram"_n.value, limit);
   }
   [[upcx::action]] void ramkvlimits(uint32_t k, uint32_t v, uint32_t i) {
      kvlimits_impl(k, v, i);
   }
