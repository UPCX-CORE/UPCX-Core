...
class [[upcxio::contract("kv_test")]] kvtest : public upcxio::contract {
 public:
   using upcxio::contract::contract;

   [[upcxio::action]] void itlifetime() {
   ...
   [[upcxio::action]] void itlimit(std::vector<itparam> params) {
   ...
   [[upcxio::action]] void erase(name contract, const std::vector<char>& k) {
      kv_erase(contract.value, k.data(), k.size());
   }

   [[upcxio::action]] void set(name contract, const std::vector<char>& k, const std::vector<char>& v, name payer) {
      kv_set(contract.value, k.data(), k.size(), v.data(), v.size(), payer.value);
   }

   [[upcxio::action]] void get(name contract, const std::vector<char>& k,
   ...
   [[upcxio::action]] void setmany(name contract, const std::vector<kv_t>& kvs) {
      for (auto& kv : kvs) //
         kv_set(contract.value, kv.k.data(), kv.k.size(), kv.v.data(), kv.v.size(), contract.value);
   }

   [[upcxio::action]] void getdata() {
   ...
   [[upcxio::action]] void scan(name contract, const std::vector<char>& prefix,
   ...
   [[upcxio::action]] void scanrev(name contract, const std::vector<char>& prefix,
                                  const std::optional<std::vector<char>>& lower, const std::vector<kv_t>& expected) {
      auto itr = kv_it_create(contract.value, prefix.data(), prefix.size());
      uint32_t stat;
   ...
   [[upcxio::action]] void itstaterased(name contract, const std::vector<char>& prefix,
