...
class [[upcx::contract("kv_test")]] kvtest : public upcx::contract {
 public:
   using upcx::contract::contract;

   [[upcx::action]] void itlifetime() {
   ...
   [[upcx::action]] void itlimit(std::vector<itparam> params) {
   ...
   [[upcx::action]] void erase(name contract, const std::vector<char>& k) {
      kv_erase(contract.value, k.data(), k.size());
   }

   [[upcx::action]] void set(name contract, const std::vector<char>& k, const std::vector<char>& v, name payer) {
      kv_set(contract.value, k.data(), k.size(), v.data(), v.size(), payer.value);
   }

   [[upcx::action]] void get(name contract, const std::vector<char>& k,
   ...
   [[upcx::action]] void setmany(name contract, const std::vector<kv_t>& kvs) {
      for (auto& kv : kvs) //
         kv_set(contract.value, kv.k.data(), kv.k.size(), kv.v.data(), kv.v.size(), contract.value);
   }

   [[upcx::action]] void getdata() {
   ...
   [[upcx::action]] void scan(name contract, const std::vector<char>& prefix,
   ...
   [[upcx::action]] void scanrev(name contract, const std::vector<char>& prefix,
                                  const std::optional<std::vector<char>>& lower, const std::vector<kv_t>& expected) {
      auto itr = kv_it_create(contract.value, prefix.data(), prefix.size());
      uint32_t stat;
   ...
   [[upcx::action]] void itstaterased(name contract, const std::vector<char>& prefix,
