...
proxy::proxy( upcx::name self, upcx::name first_receiver, upcx::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      upcx::token::transfer_action a( "upcx.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, upcx::ignore<std::vector<char>> ) {
