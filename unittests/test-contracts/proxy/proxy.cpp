...
proxy::proxy( upcxio::name self, upcxio::name first_receiver, upcxio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      upcxio::token::transfer_action a( "upcxio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, upcxio::ignore<std::vector<char>> ) {
