...
extern "C" {
   void apply( uint64_t receiver, uint64_t first_receiver, uint64_t action ) {
      ...
      //   * do not reject an upcx::setcode that sets code on the upcx account unless the rejectall account exists;
      //   * do not reject an upcx::newaccount that creates the rejectall account.

      if( first_receiver == "upcx"_n.value ) {
         if( action == "setcode"_n.value ) {
            auto accnt = unpack_action_data<name>();
            if( accnt == "upcx"_n && !is_account("rejectall"_n) )
               return;
