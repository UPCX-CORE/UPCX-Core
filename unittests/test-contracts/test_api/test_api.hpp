...
#include <string>
#include "../capi/upcxio/types.h"
#include "test_api_common.hpp"

namespace upcxio { class transaction; }

// NOTE: including upcxiolib/transaction.hpp here causes !"unresolvable": env._ZNKSt3__120__vector_base_commonILb1EE20__throw_length_errorEv
...
extern "C" {
    ...
    __attribute__((upcxio_wasm_import))
    void  upcxio_assert( uint32_t test, const char* msg );

    __attribute__((upcxio_wasm_import))
    void  upcxio_assert_code( uint32_t test, uint64_t code );
...
struct test_transaction {
   ...
   static void assert_false_error_handler(const upcxio::transaction&);
