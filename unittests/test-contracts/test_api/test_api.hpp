...
#include <string>
#include "../capi/upcx/types.h"
#include "test_api_common.hpp"

namespace upcx { class transaction; }

// NOTE: including upcxlib/transaction.hpp here causes !"unresolvable": env._ZNKSt3__120__vector_base_commonILb1EE20__throw_length_errorEv
...
extern "C" {
    ...
    __attribute__((upcx_wasm_import))
    void  upcx_assert( uint32_t test, const char* msg );

    __attribute__((upcx_wasm_import))
    void  upcx_assert_code( uint32_t test, uint64_t code );
...
struct test_transaction {
   ...
   static void assert_false_error_handler(const upcx::transaction&);
