...
#include <upcxio/permission.hpp>
#include <upcxio/print.hpp>
#include <upcxio/serialize.hpp>
...
struct check_auth_msg {
   ...
   std::vector<upcxio::public_key> pubkeys;
...
void test_permission::check_authorization( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   int64_t res64 = upcxio::check_permission_authorization( params.account,
                                                     params.permission,
...
void test_permission::test_permission_last_used( uint64_t /* receiver */, uint64_t code, uint64_t action ) {
   ...
   upcxio_assert( upcxio::get_permission_last_used(params.account, params.permission) == msec, "unexpected last used permission time" );
}
...
void test_permission::test_account_creation_time( uint64_t /* receiver */, uint64_t code, uint64_t action ) {
   ...
   time_point msec{ microseconds{params.last_used_time}};
   upcxio_assert( upcxio::get_account_creation_time(params.account) == msec, "unexpected account creation time" );
}
