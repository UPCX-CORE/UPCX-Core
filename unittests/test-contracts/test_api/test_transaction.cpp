...
struct test_action_action {
   static upcx::name get_account() {
      return upcx::name{ACCOUNT};
   }

   static upcx::name get_name() {
      return upcx::name{NAME};
   }
...
void test_transaction::send_action_large() {
   ...
   act.send();
   upcx_assert( false, "send_message_large() should've thrown an error" );
}
...
void test_transaction::send_action_recurse() {
   using namespace upcx;
   char buffer[1024];
   upcx::read_action_data( buffer, 1024 );
...
void test_transaction::test_tapos_block_prefix() {
   using namespace upcx;
   int tbp;
   upcx::read_action_data( (char*)&tbp, sizeof(int) );
   upcx_assert( tbp == upcx::tapos_block_prefix(), "tapos_block_prefix does not match" );
}

void test_transaction::test_tapos_block_num() {
   using namespace upcx;
   int tbn;
   upcx::read_action_data( (char*)&tbn, sizeof(int) );
   upcx_assert( tbn == upcx::tapos_block_num(), "tapos_block_num does not match" );
}

void test_transaction::test_read_transaction() {
   using namespace upcx;
   checksum256 h;
   auto size = upcx::transaction_size();
   char buf[size];
   uint32_t read = upcx::read_transaction( buf, size );
   upcx_assert( size == read, "read_transaction failed");
   h = upcx::sha256(buf, read);
   print(h);
}
...
void test_transaction::test_transaction_size() {
   ...
   upcx::read_action_data( (char*)&trans_size, sizeof(uint32_t) );
   print( "size: ", upcx::transaction_size() );
   upcx_assert( trans_size == upcx::transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_action_sender( uint64_t receiver, uint64_t, uint64_t ) {
   using namespace upcx;
   uint64_t cur_send;
   upcx::read_action_data( &cur_send, sizeof(name) );
...
void test_transaction::send_transaction_empty( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   upcx_assert( false, "send_transaction_empty() should've thrown an error" );
}
...
void test_transaction::assert_false_error_handler( const upcx::transaction& dtrx ) {
   upcx_assert( dtrx.actions.size() == 1, "transaction should only have one action" );
   upcx_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   upcx_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   upcx_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   upcx_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   upcx_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::send_transaction_large( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   upcx_assert( false, "send_transaction_large() should've thrown an error" );
}
...
void test_transaction::send_deferred_tx_with_dtt_action() {
   using namespace upcx;
   dtt_action dtt_act;
   upcx::read_action_data( &dtt_act, upcx::action_data_size() );
...
void test_transaction::cancel_deferred_transaction_success() {
   using namespace upcx;
   auto r = upcx::cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   upcx_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace upcx;
   auto r = upcx::cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   upcx_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   upcx_assert( false, "send_cfa_action_fail() should've thrown an error" );
}

void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( upcx::name{"testtrans"}.value, upcx::name{"table"}.value, upcx::name{"testtrans"}.value, 0, buf, 4 );
}

void test_transaction::context_free_api() {
   char buf[128] = {0};
   upcx::get_context_free_data( 0, buf, sizeof(buf) );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   bool res = upcx::cancel_deferred( sender_id );
   ...
   trx.send( sender_id, upcx::name{receiver} );
}
