...
struct test_action_action {
   static upcxio::name get_account() {
      return upcxio::name{ACCOUNT};
   }

   static upcxio::name get_name() {
      return upcxio::name{NAME};
   }
...
void test_transaction::send_action_large() {
   ...
   act.send();
   upcxio_assert( false, "send_message_large() should've thrown an error" );
}
...
void test_transaction::send_action_recurse() {
   using namespace upcxio;
   char buffer[1024];
   upcxio::read_action_data( buffer, 1024 );
...
void test_transaction::test_tapos_block_prefix() {
   using namespace upcxio;
   int tbp;
   upcxio::read_action_data( (char*)&tbp, sizeof(int) );
   upcxio_assert( tbp == upcxio::tapos_block_prefix(), "tapos_block_prefix does not match" );
}

void test_transaction::test_tapos_block_num() {
   using namespace upcxio;
   int tbn;
   upcxio::read_action_data( (char*)&tbn, sizeof(int) );
   upcxio_assert( tbn == upcxio::tapos_block_num(), "tapos_block_num does not match" );
}

void test_transaction::test_read_transaction() {
   using namespace upcxio;
   checksum256 h;
   auto size = upcxio::transaction_size();
   char buf[size];
   uint32_t read = upcxio::read_transaction( buf, size );
   upcxio_assert( size == read, "read_transaction failed");
   h = upcxio::sha256(buf, read);
   print(h);
}
...
void test_transaction::test_transaction_size() {
   ...
   upcxio::read_action_data( (char*)&trans_size, sizeof(uint32_t) );
   print( "size: ", upcxio::transaction_size() );
   upcxio_assert( trans_size == upcxio::transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_action_sender( uint64_t receiver, uint64_t, uint64_t ) {
   using namespace upcxio;
   uint64_t cur_send;
   upcxio::read_action_data( &cur_send, sizeof(name) );
...
void test_transaction::send_transaction_empty( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   upcxio_assert( false, "send_transaction_empty() should've thrown an error" );
}
...
void test_transaction::assert_false_error_handler( const upcxio::transaction& dtrx ) {
   upcxio_assert( dtrx.actions.size() == 1, "transaction should only have one action" );
   upcxio_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   upcxio_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   upcxio_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   upcxio_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   upcxio_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::send_transaction_large( uint64_t receiver, uint64_t, uint64_t ) {
   ...
   upcxio_assert( false, "send_transaction_large() should've thrown an error" );
}
...
void test_transaction::deferred_print() {
   upcxio::print("deferred executed\n");
}
...
void test_transaction::send_deferred_tx_with_dtt_action() {
   using namespace upcxio;
   dtt_action dtt_act;
   upcxio::read_action_data( &dtt_act, upcxio::action_data_size() );
...
void test_transaction::cancel_deferred_transaction_success() {
   using namespace upcxio;
   auto r = upcxio::cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   upcxio_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace upcxio;
   auto r = upcxio::cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   upcxio_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   upcxio_assert( false, "send_cfa_action_fail() should've thrown an error" );
}

void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( upcxio::name{"testtrans"}.value, upcxio::name{"table"}.value, upcxio::name{"testtrans"}.value, 0, buf, 4 );
}

void test_transaction::context_free_api() {
   char buf[128] = {0};
   upcxio::get_context_free_data( 0, buf, sizeof(buf) );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   bool res = upcxio::cancel_deferred( sender_id );
   ...
   trx.send( sender_id, upcxio::name{receiver} );
}
