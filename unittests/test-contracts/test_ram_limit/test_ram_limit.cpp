...
CONTRACT test_ram_limit : public contract {
   ...
   using contract::contract;
      ...
      ACTION setentry( name payer, uint64_t from, uint64_t to, uint64_t size ) {
         ...
         upcx::print("test_ram_limit::setentry ", upcx::name{self}, "\n");
      ...
      ACTION rmentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         upcx::print("test_ram_limit::rmentry ", upcx::name{self}, "\n");
         test_table table( self, self.value );
         for ( int key = from; key <=to; ++key ) {
            auto itr = table.find(key);
            upcx_assert ( itr != table.end(), "could not find test_table entry" );
            table.erase(itr);
         }
      }
      ...
      ACTION printentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         upcx::print("test_ram_limit::printout ", upcx::name{self}, ":");
         test_table table( self, self.value );
         for ( int key = from; key <= to; ++key ) {
            auto itr = table.find(key);
            upcx::print("\nkey=", key);
            upcx_assert ( itr != table.end(), "could not find test_table entry" );
            upcx::print(" size=", itr->data.size());
         }
      }
   ...
   private:
      ...
      typedef upcx::multi_index< "test.table"_n, test> test_table;
};
...
UPCX_DISPATCH( test_ram_limit, (setentry)(rmentry)(printentry) )
