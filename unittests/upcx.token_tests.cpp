...
class upcx_token_tester : public tester {
public:

   upcx_token_tester() {
      produce_blocks( 2 );

      create_accounts( { "alice"_n, "bob"_n, "carol"_n, "upcx.token"_n } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = upcx::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "upcx.token"_n, acc, "accounts"_n, name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( "upcx.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(upcx_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, upcx_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, upcx_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, upcx_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, upcx_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, upcx_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, upcx_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, upcx_token_tester ) try {
