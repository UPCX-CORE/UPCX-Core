...
namespace upcx_system {

class upcx_system_tester : public TESTER {
public:

   upcx_system_tester()
   : upcx_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   upcx_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ "upcx.token"_n, "upcx.ram"_n, "upcx.ramfee"_n, "upcx.stake"_n,
               "upcx.bpay"_n, "upcx.vpay"_n, "upcx.saving"_n, "upcx.names"_n });

      produce_blocks( 100 );

      set_code( "upcx.token"_n, contracts::upcx_token_wasm() );
      set_abi( "upcx.token"_n, contracts::upcx_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( "upcx.token"_n );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( "upcx.token"_n, config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("upcx") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("upcx")) + get_balance(name("upcx.ramfee")) + get_balance(name("upcx.stake")) + get_balance(name("upcx.ram")) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( "upcx.token"_n, act, "accounts"_n, name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "upcx.token"_n, "issue"_n, manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "upcx.token"_n, "transfer"_n, manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = upcx::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "upcx.token"_n, name(symbol_code), "stat"_n, name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( "upcx.msig"_n, config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("upcx"), name("upcx.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( "upcx.msig"_n, contracts::upcx_msig_wasm() );
         set_abi( "upcx.msig"_n, contracts::upcx_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( "upcx.msig"_n );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( name("upcx"), name("alice1111111"), core_from_string("650000000.0000"), name("upcx") );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::upcx_code_name}, 1},
