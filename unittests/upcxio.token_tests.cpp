...
class upcxio_token_tester : public tester {
public:

   upcxio_token_tester() {
      produce_blocks( 2 );

      create_accounts( { "alice"_n, "bob"_n, "carol"_n, "upcxio.token"_n } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = upcxio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "upcxio.token"_n, acc, "accounts"_n, name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( "upcxio.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(upcxio_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, upcxio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, upcxio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, upcxio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, upcxio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, upcxio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, upcxio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, upcxio_token_tester ) try {
