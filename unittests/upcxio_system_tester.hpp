...
namespace upcxio_system {

class upcxio_system_tester : public TESTER {
public:

   upcxio_system_tester()
   : upcxio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   upcxio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ "upcxio.token"_n, "upcxio.ram"_n, "upcxio.ramfee"_n, "upcxio.stake"_n,
               "upcxio.bpay"_n, "upcxio.vpay"_n, "upcxio.saving"_n, "upcxio.names"_n });

      produce_blocks( 100 );

      set_code( "upcxio.token"_n, contracts::upcxio_token_wasm() );
      set_abi( "upcxio.token"_n, contracts::upcxio_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( "upcxio.token"_n );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( "upcxio.token"_n, config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("upcxio") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("upcxio")) + get_balance(name("upcxio.ramfee")) + get_balance(name("upcxio.stake")) + get_balance(name("upcxio.ram")) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( "upcxio.token"_n, act, "accounts"_n, name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "upcxio.token"_n, "issue"_n, manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "upcxio.token"_n, "transfer"_n, manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = upcxio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "upcxio.token"_n, name(symbol_code), "stat"_n, name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( "upcxio.msig"_n, config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("upcxio"), name("upcxio.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( "upcxio.msig"_n, contracts::upcxio_msig_wasm() );
         set_abi( "upcxio.msig"_n, contracts::upcxio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( "upcxio.msig"_n );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::upcxio_code_name}, 1},
...
inline uint64_t M( const string& upcx_str ) {
   return core_from_string( upcx_str ).get_amount();
}
