...
struct wasm_config_tester : TESTER {
   ...
   void set_wasm_params(const wasm_config& params) {
      ...
      trx.actions.emplace_back(vector<permission_level>{{"upcxio"_n,config::active_name}}, "upcxio"_n, "setwparams"_n,
                               bios_abi_ser.variant_to_binary("setwparams", fc::mutable_variant_object()("cfg", params),
                                                              abi_serializer::create_yield_function( abi_serializer_max_time )));
      trx.actions[0].authorization = vector<permission_level>{{"upcxio"_n,config::active_name}};
      set_transaction_headers(trx);
      trx.sign(get_private_key("upcxio"_n, "active"), control->get_chain_id());
...
BOOST_DATA_TEST_CASE_F(wasm_config_tester, max_mutable_global_bytes, data::make({ 4096, 8192 , 16384 }) * data::make({0, 1}), n_globals, oversize) {
   ...
   std::string code = [&] {
      ...
      ss << "(module ";
      ss << " (func $upcxio_assert (import \"env\" \"upcxio_assert\") (param i32 i32))";
      ss << " (memory 1)";
      for(int i = 0; i < n_globals + oversize; i += 4)
         ss << "(global (mut i32) (i32.const " << i << "))";
      ss << " (func (export \"apply\") (param i64 i64 i64)";
      for(int i = 0; i < n_globals + oversize; i += 4)
         ss << "(call $upcxio_assert (i32.eq (get_global " << i/4 << ") (i32.const " << i << ")) (i32.const 0))";
      ss << " )";
      ss << ")";
      return ss.str();
   }();
...
(module
  ...
  (func (export "apply") (param i64 i64 i64)
    (call $$upcxio_assert (i32.eq (i32.load (i32.const ${OFFSET})) (i32.const 0x44332211)) (i32.const 0))
  )
)
)=====";
...
BOOST_FIXTURE_TEST_CASE( max_pages, wasm_config_tester ) try {
   ...
   for(uint64_t max_pages : {600, 400}) { // above and below the default limit
      ...
      set_wasm_params(params);
      produce_block();
      BOOST_CHECK_THROW(pushit(0), upcxio::chain::wasm_exception);
...
// that would prevent it from being set on the upcxio account and executing.
...
BOOST_FIXTURE_TEST_CASE(reset_chain_tests, wasm_config_tester) {
   ...
   // Reset parameters and system contract
   {
      ...
      auto make_setcode = [](const std::vector<uint8_t>& code) {
         return setcode{ "upcxio"_n, 0, 0, bytes(code.begin(), code.end()) };
      };
      trx.actions.push_back({ { { "upcxio"_n, config::active_name} }, make_setcode(wast_to_wasm(min_set_parameters_wast)) });
      trx.actions.push_back({ { { "upcxio"_n, config::active_name} }, "upcxio"_n, ""_n, fc::raw::pack(genesis_state::default_initial_wasm_configuration) });
      trx.actions.push_back({ { { "upcxio"_n, config::active_name} }, make_setcode(contracts::upcxio_bios_wasm()) });
      set_transaction_headers(trx);
      trx.sign(get_private_key("upcxio"_n, "active"), control->get_chain_id());
      push_transaction(trx);
   }
