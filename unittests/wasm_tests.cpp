...
BOOST_FIXTURE_TEST_CASE( basic_test, TESTER ) try {
   ...
   transaction_id_type yes_assert_id;
   {
      ...
      BOOST_CHECK_THROW(push_transaction( trx ), upcxio_assert_message_exception);
   }
...
BOOST_FIXTURE_TEST_CASE( f32_f64_overflow_tests, tester ) try {
   ...
   auto check = [&](const char *wast_template, const char *op, const char *param) -> bool {
      ...
      try {
         ...
         const auto& receipt = get_transaction_receipt(trx.id());
         return true;
      } catch (upcxio::chain::wasm_execution_error &) {
         return false;
      }
   };
   //
...
BOOST_DATA_TEST_CASE_F( old_wasm_tester, big_memory, bdata::make({false, true}), activate_wasm_config ) try {
   ...
   string biggest_memory_wast_f = fc::format_string(biggest_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES", upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024)));
   ...
   string too_big_memory_wast_f = fc::format_string(too_big_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES_PLUS_ONE", upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024)+1));
   BOOST_CHECK_THROW(set_code("bigmem"_n, too_big_memory_wast_f.c_str()), upcxio::chain::wasm_exception);
...
BOOST_DATA_TEST_CASE_F( old_wasm_tester, table_init_tests, bdata::make({false, true}), activate_wasm_config ) try {
   ...
   BOOST_CHECK_THROW(set_code("tableinit"_n, too_big_table), upcxio::chain::wasm_exception);
...
BOOST_FIXTURE_TEST_CASE( table_init_oob, TESTER ) try {
   ...
   trx.actions[0].authorization = vector<permission_level>{{"tableinitoob"_n,config::active_name}};
    ...
    auto pushit_and_expect_fail = [&]() {
      ...
      BOOST_CHECK_THROW(push_transaction(trx), upcxio::chain::wasm_exception);
   };
   ...
   BOOST_CHECK_THROW(set_code("tableinitoob"_n, table_init_oob_no_table_wast), upcxio::chain::wasm_exception);
...
BOOST_FIXTURE_TEST_CASE( memory_init_border, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code("memoryborder"_n, memory_init_toolong), upcxio::chain::wasm_exception);
   BOOST_CHECK_THROW(set_code("memoryborder"_n, memory_init_negative), upcxio::chain::wasm_exception);
...
BOOST_FIXTURE_TEST_CASE( nested_limit_test, TESTER ) try {
   ...
   }
   {
      ...
      for(unsigned int i = 0; i < 1024; ++i)
         ss << ")";
      ss << "))";
      BOOST_CHECK_THROW(set_code("nested"_n, ss.str().c_str()), upcxio::chain::wasm_exception);
   }
...
BOOST_DATA_TEST_CASE_F( old_wasm_tester, lotso_globals, bdata::make({false, true}), activate_wasm_config ) try {
   ...
   .c_str()), upcxio::chain::wasm_exception);
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( offset_check_old, old_wasm_tester ) try {
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << upcxio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0)))";
   ...
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << upcxio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << upcxio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0)))";
      ss << ") (export \"apply\" (func $apply)) )";

      BOOST_CHECK_THROW(set_code("offsets"_n, ss.str().c_str()), upcxio::chain::wasm_exception);
      produce_block();
   }
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << upcxio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
...
BOOST_FIXTURE_TEST_CASE(upcxio_abi, TESTER) try {
   ...
   // verify to_variant works on upcx native contract type: newaccount
...
BOOST_FIXTURE_TEST_CASE( check_table_maximum, TESTER ) try {
   ...
   BOOST_CHECK_THROW(push_transaction(trx), upcxio::chain::wasm_execution_error);
   }
   ...
   signed_transaction trx;
   action act;
   act.name = name(upcxio::chain::wasm_constraints::maximum_table_elements+54334);
   act.account = "tbl"_n;
   act.authorization = vector<permission_level>{{"tbl"_n,config::active_name}};
   trx.actions.push_back(act);
      set_transaction_headers(trx);
   ...
   BOOST_CHECK_EXCEPTION(push_transaction(trx), upcxio::chain::wasm_execution_error, [](const upcxio::chain::wasm_execution_error &e) {return true;});
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE( big_maligned_host_ptr, TESTER ) try {
   ...
   string large_maligned_host_ptr_wast_f = fc::format_string(large_maligned_host_ptr, fc::mutable_variant_object()
                                              ("MAX_WASM_PAGES", upcxio::chain::wasm_constraints::maximum_linear_memory/(64*1024))
                                              ("MAX_NAME_ARRAY", (upcxio::chain::wasm_constraints::maximum_linear_memory-1)/sizeof(chain::account_name)));
...
BOOST_DATA_TEST_CASE_F( old_wasm_tester, depth_tests, bdata::make({false, true}), activate_wasm_config ) try {
   ...
   string wasm_depth_okay = fc::format_string(depth_assert_wasm, fc::mutable_variant_object()
                                              ("MAX_DEPTH", upcxio::chain::wasm_constraints::maximum_call_depth));
   ...
   string wasm_depth_one_over = fc::format_string(depth_assert_wasm, fc::mutable_variant_object()
                                              ("MAX_DEPTH", upcxio::chain::wasm_constraints::maximum_call_depth+1));
...
// In a previous version of upcx-vm, this would leave
...
// TODO: Update to use upcx-vm once merged
...
BOOST_AUTO_TEST_CASE( billed_cpu_test ) try {
   ...
   cpu_limit -= UPCX_PERCENT( cpu_limit, 10 * config::percent_1 ); // transaction_context verifies within 10%, so subtract 10% out
   ...
   uint32_t billed_cpu_time_us = UPCX_PERCENT( (combined_cpu_limit - subjective_cpu_bill_us), 89 *config::percent_1 );
   ...
   billed_cpu_time_us = UPCX_PERCENT( combined_cpu_limit - subjective_cpu_bill_us, 89 *config::percent_1 );
   push_trx( ptrx, fc::time_point::maximum(), billed_cpu_time_us, false, subjective_cpu_bill_us );
   ...
   billed_cpu_time_us = UPCX_PERCENT( combined_cpu_limit - subjective_cpu_bill_us, 90 * config::percent_1 );
   ...
   subjective_cpu_bill_us = 0;
   billed_cpu_time_us = UPCX_PERCENT( combined_cpu_limit - subjective_cpu_bill_us, 91 * config::percent_1 );
   BOOST_CHECK_EXCEPTION(push_trx( ptrx, fc::time_point::maximum(), billed_cpu_time_us, false, subjective_cpu_bill_us ), tx_cpu_usage_exceeded,
                         fc_exception_message_starts_with("estimated") );
   ...
   ptrx = create_trx(0);
   subjective_cpu_bill_us = 0;
   billed_cpu_time_us = UPCX_PERCENT( leeway.count(), 89 *config::percent_1 );
   BOOST_CHECK_EXCEPTION(push_trx( ptrx, fc::time_point::maximum(), billed_cpu_time_us, false, subjective_cpu_bill_us ), tx_cpu_usage_exceeded,
                         fc_exception_message_starts_with("billed") );
...
BOOST_FIXTURE_TEST_CASE(net_usage_tests, tester ) try {
   ...
   (import "env" "upcxio_assert" (func $upcxio_assert (param i32 i32)))
      ...
      produce_blocks(1);
      signed_transaction trx;
      auto wasm = ::upcxio::chain::wast_to_wasm(code);
