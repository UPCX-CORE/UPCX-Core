...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({"upcx.token"_n, "alice"_n, "bob"_n, "charlie"_n});
         chain->set_code("upcx.token"_n, contracts::upcx_token_wasm() );
         chain->set_abi("upcx.token"_n, contracts::upcx_token_abi().data() );
         chain->push_action( "upcx.token"_n, "create"_n, "upcx.token"_n, mvo()
              ( "issuer", "upcx.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( "upcx.token"_n, "issue"_n, "upcx.token"_n, mvo()
              ( "to", "upcx.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( "upcx.token"_n, "transfer"_n, from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, "upcx.token"_n, "alice"_n};
   test.init();

   test.transfer( "upcx.token"_n, "alice"_n, "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, "upcx.token"_n, "bob"_n};
   test.init();

   test.transfer( "upcx.token"_n, "alice"_n, "1000.00 TOK" );

   test.transfer( "alice"_n, "upcx.token"_n );
   ...
   test.chain->set_code("bob"_n, contracts::upcx_token_wasm() );
   test.chain->set_abi("bob"_n, contracts::upcx_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code("charlie"_n, contracts::upcx_token_wasm() );
   test.chain->set_abi("charlie"_n, contracts::upcx_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, "upcx.token"_n, "bob"_n, "charlie"_n};
...
BOOST_AUTO_TEST_CASE( blacklist_upcx ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, contracts::upcx_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'upcx::onerror' is on the action blacklist")
                        );
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(upcx::testing::base_tester::get_public_key(name("alice"), "active"));
   auth.accounts.push_back( permission_level_weight{{"alice"_n, config::upcx_code_name}, 1} );

   tester1.chain->push_action( "upcx"_n, "updateauth"_n, "alice"_n, mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(upcx::testing::base_tester::get_public_key(name("bob"), "active"));
   auth.accounts.push_back( permission_level_weight{{"alice"_n, config::upcx_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{"bob"_n, config::upcx_code_name}, 1} );

   tester1.chain->push_action( "upcx"_n, "updateauth"_n, "bob"_n, mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(upcx::testing::base_tester::get_public_key(name("charlie"), "active"));
   auth.accounts.push_back( permission_level_weight{{"charlie"_n, config::upcx_code_name}, 1} );

   tester1.chain->push_action( "upcx"_n, "updateauth"_n, "charlie"_n, mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const packed_transaction_ptr&> x) {
      ...
      if( act.account == "upcx"_n && act.name == "onblock"_n ) return;
