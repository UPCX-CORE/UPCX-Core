...
from compile_upcx_tests import CompileError

import compile_upcx_tests
import generate_upcx_tests
...
def generate_wasm_and_copy():
    ...
    for d in os.listdir():
        ...
        if wasm_file in altered_wasms:
            ...
            try:
                generate_upcx_tests.main(g_wasm_file, t_wasm_file, o_wast_file, map_file)
                wasm_file_path = os.path.join(d, wasm_file)
                out = subprocess.run(
                    ['upcx-wast2wasm', o_wast_file, '-o', os.path.join(d, wasm_file)],
                    capture_output=True
                )
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("""Please provide:
                ...
                ex:
                python setup_upcx_tests.py ~/code/upcx-vm-test-wasms ~/code/upcx ~/wasm_spec_tests
              """)
        sys.exit(1)
    ...
    generator = os.path.join(GENERATOR_DIR, 'upcx_test_generator')
