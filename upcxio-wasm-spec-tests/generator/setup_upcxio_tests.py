...
from compile_upcxio_tests import CompileError

import compile_upcxio_tests
import generate_upcxio_tests
...
def compile_wasm():
    ...
    if len(fs) > 5:
        with Pool(os.cpu_count() - 2) as p:
            p.map(compile_upcxio, fs_m)
    else:
        for d in fs_m:
            compile_upcxio(d)

def compile_upcxio(f):
    d, name = f
    compile_upcxio_tests.main(
        d,
        f'{name}.{d}.wasm.cpp',
        f'{name}.{d}-int.wasm',
    )
...
def generate_wasm_and_copy():
    ...
    for d in os.listdir():
        ...
        if wasm_file in altered_wasms:
            ...
            try:
                generate_upcxio_tests.main(g_wasm_file, t_wasm_file, o_wast_file, map_file)
                wasm_file_path = os.path.join(d, wasm_file)
                out = subprocess.run(
                    ['upcxio-wast2wasm', o_wast_file, '-o', os.path.join(d, wasm_file)],
                    capture_output=True
                )
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("""Please provide:
                ...
                ex:
                python setup_upcxio_tests.py ~/code/upcx-vm-test-wasms ~/code/upcx ~/wasm_spec_tests
              """)
        sys.exit(1)
    ...
    generator = os.path.join(GENERATOR_DIR, 'upcxio_test_generator')
